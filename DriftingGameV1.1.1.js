  ...
  class DriftGame {
    ...
    normalizeReplayFrame(frame, fallbackLevelIndex = 0) {
      ...
      if (Array.isArray(frame) && frame.length >= 4) {
        const levelIndex = clamp(Math.floor(Number.isFinite(frame[0]) ? frame[0] : safeLevelIndex), 0, LEVELS.length - 1);
        const x = Number(frame[1]);
        const y = Number(frame[2]);
        const angle = Number(frame[3]);
        const elapsedTime = Number(frame[4]);
        const visualData = this.normalizeReplayVisualData(frame[5]);
    ...
    getReplayFrameElapsedTime(replay, frameIndex) {
    ...
    async syncOnlineLeaderboard({ includeLocalSeed = false, force = false, includeReplay = false } = {}) {
      ...
      try {
        ...
        if (includeLocalSeed && !this.onlineLeaderboardSeededLocalRecords && !hasCachedRows) {
