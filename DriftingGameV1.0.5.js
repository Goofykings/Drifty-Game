(() => {
  ...
  class DriftGame {
    ...
    normalizeReplayFrame(frame, fallbackLevelIndex = 0) {
      ...
      if (Array.isArray(frame) && frame.length >= 4) {
        ...
        if (Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(angle)) {
          return [levelIndex, x, y, angle, Number.isFinite(elapsedTime) && elapsedTime >= 0 ? elapsedTime : null];
        }
      }
    ...
    captureCurrentRunReplayFrame(levelIndex = this.levelIndex, car = this.car, force = false) {
      ...
      this.currentRunReplayFrames.push([
        safeLevelIndex,
    ...
    applyAccountProfilePayload(profile) {
      ...
      if (Number.isFinite(profile.highestVisitedLevel)) {
        const nextHighest = clamp(Math.floor(profile.highestVisitedLevel), 0, LEVELS.length - 1);
    ...
    getCommunitySearchLayout() {
      ...
      return {
        x: metrics.x,
        y: metrics.searchTop,
        w: Math.max(180, metrics.cardW - metrics.sortW - metrics.controlGap),
        h: metrics.searchH,
    ...
    getCommunitySortLayout() {
      ...
      return {
        x: metrics.x + metrics.cardW - metrics.sortW,
        y: metrics.searchTop,
        w: metrics.sortW,
        h: metrics.searchH,
    ...
    getMyLevelDetailLayout(level = this.getActiveMyLevel()) {
      const scale = this.getScreenFitScale(760, 700, 0.56);
      const buttonW = Math.min(Math.round(260 * scale), this.width - 56);
      const buttonH = Math.max(42, Math.round(58 * scale));
      const buttonGap = Math.max(12, Math.round(18 * scale));
      const playButton = {
        id: "play_my_level_detail",
        label: "Play",
        x: Math.round(this.width * 0.5 - buttonW * 0.5),
        y: Math.round(this.height * 0.33),
        w: buttonW,
        h: buttonH,
        fontSize: Math.max(17, Math.round(24 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
      ...
      const actionButton = {
        ...
        x: playButton.x,
        y: playButton.y + playButton.h + buttonGap,
        w: buttonW,
        h: Math.max(38, Math.round(50 * scale)),
        fontSize: Math.max(15, Math.round(20 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
      ...
      const editButton = {
        ...
        label: "Edit",
        x: playButton.x,
        y: Math.min(this.height - Math.max(88, Math.round(110 * scale)), actionButton.y + actionButton.h + Math.max(24, Math.round(52 * scale))),
        w: buttonW,
        h: Math.max(40, Math.round(52 * scale)),
        fontSize: Math.max(16, Math.round(21 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
        hover: this.myLevelDetailEditHover,
      };
      const panelW = Math.min(Math.round(720 * scale), this.width - 36);
      const panelH = Math.max(180, Math.round(220 * scale));
      return {
        scale,
        panelX: Math.round(this.width * 0.5 - panelW * 0.5),
        panelY: Math.round(Math.max(122, this.height * 0.14)),
        panelW,
        panelH,
        playButton,
        actionButton,
      ...
      if (this.currentScreen === "leaderboard_replay") {
        if (this.hoveredLeaderboardButton === "back") {
          ...
          if (this.activeLeaderboardReplay.paused && this.activeLeaderboardReplay.ended && this.activeLeaderboardReplay.frameIndex >= lastIndex && lastIndex > 0) {
            this.activeLeaderboardReplay.frameIndex = 0;
            this.activeLeaderboardReplay.frameTimer = 0;
            this.activeLeaderboardReplay.elapsedTime = 0;
            this.activeLeaderboardReplay.paused = false;
            this.activeLeaderboardReplay.ended = false;
            this.activeLeaderboardReplay.finishEffectTriggered = false;
            this.activeLeaderboardReplay.goalTriggered = false;
            this.activeLeaderboardReplay.goalTimer = 0;
            this.activeLeaderboardReplay.explosionParticles = [];
            this.activeLeaderboardReplay.finishDebris = [];
            const firstFrame = this.getReplayFrameState(this.activeLeaderboardReplay.frames[0], this.activeLeaderboardReplay.levelIndex ?? 0);
            if (firstFrame) {
              this.activeLeaderboardReplay.currentLevelIndex = firstFrame.levelIndex;
              this.activeLeaderboardReplay.level = new LevelMap(LEVELS[firstFrame.levelIndex]);
              this.activeLeaderboardReplay.camera.snap(firstFrame.x, firstFrame.y);
            }
    ...
    refreshLeaderboardReplayHover() {
      ...
      const hovered = [this.getLeaderboardReplayBackButton()].find(
        (button) =>
          this.mouse.x >= button.x &&
          this.mouse.x <= button.x + button.w &&
          this.mouse.y >= button.y &&
    ...
    openCreateLevel(returnScreen = "online", options = {}) {
      this.beginScreenWipe(() => {
        ...
        this.canvas.style.cursor = "default";
        if (editLevel) {
          const levelText = Array.isArray(editLevel.map) ? editLevel.map.join("\n") : "";
          this.setActiveMyLevel(editLevel);
          this.publishLevelName = editLevel.name;
          this.syncEditingMyLevelText(levelText);
    ...
    renderLeaderboardReplayScreen(ctx, now) {
      ...
      const statusText = replay.paused
        ? (replay.ended ? "Replay ended. Tap anywhere to watch again." : "Paused. Tap anywhere to resume.")
        : "Tap anywhere to pause.";
      ctx.fillText(fitTextToWidth(ctx, statusText, this.width - 180), this.width * 0.5, this.height - 28);
      ctx.restore();

      this.renderMenuButtons(ctx, [this.getLeaderboardReplayBackButton()]);
    }
    ...
    renderMyLevelDetailScreen(ctx, now) {
      ...
      const statusLabel = getCommunitySubmissionStatusLabel(level.submissionStatus);
      const statusInfo = this.getMyLevelDetailActionInfo(level);

      ctx.save();
      ctx.textAlign = "center";
      ctx.fillStyle = "#f5fbff";
      ctx.font = `700 ${Math.max(24, Math.round(40 * layout.scale))}px Consolas, monospace`;
      ctx.fillText(fitTextToWidth(ctx, level.name, this.width - 44), this.width * 0.5, Math.max(72, this.height * 0.14));

      ctx.fillStyle = "#9affd2";
      ctx.font = `${Math.max(13, Math.round(18 * layout.scale))}px Consolas, monospace`;
      ctx.fillText(`Status: ${statusLabel}`, this.width * 0.5, Math.max(102, this.height * 0.14 + Math.round(32 * layout.scale)));
      ...
      ctx.textAlign = "left";
      ctx.fillStyle = "#dff8ff";
      ctx.font = `700 ${Math.max(15, Math.round(21 * layout.scale))}px Consolas, monospace`;
      ctx.fillText("Your Level", layout.panelX + Math.round(24 * layout.scale), layout.panelY + Math.round(38 * layout.scale));

      ctx.fillStyle = "rgba(226, 240, 250, 0.9)";
      ctx.font = `${Math.max(12, Math.round(16 * layout.scale))}px Consolas, monospace`;
      const noteLines = normalizeCommunitySubmissionStatus(level.submissionStatus || "private") === "declined" && level.reviewMessage
        ? wrapTextToLines(ctx, `Admin note: ${level.reviewMessage}`, layout.panelW - Math.round(64 * layout.scale), 4)
        : [];
      ctx.fillText(
        `Best time ${Number.isFinite(level.myBestTime) ? formatTime(level.myBestTime) : "--:--.---"}`,
        layout.panelX + Math.round(24 * layout.scale),
        layout.panelY + Math.round(76 * layout.scale)
      );
      ctx.fillText(
        fitTextToWidth(ctx, statusInfo.description, layout.panelW - Math.round(48 * layout.scale)),
        layout.panelX + Math.round(24 * layout.scale),
        layout.panelY + Math.round(108 * layout.scale)
      );
      if (noteLines.length > 0) {
        noteLines.forEach((line, index) => {
          ...
          ctx.fillText(
            line,
            layout.panelX + Math.round(24 * layout.scale),
            layout.panelY + Math.round(144 * layout.scale) + index * Math.max(16, Math.round(20 * layout.scale))
          );
        });
      }
      if (this.myLevelDetailStatusText) {
        ...
        ctx.font = `${Math.max(12, Math.round(16 * layout.scale))}px Consolas, monospace`;
        ctx.fillText(
          fitTextToWidth(ctx, this.myLevelDetailStatusText, this.width - 56),
          this.width * 0.5,
          layout.editButton.y - Math.max(18, Math.round(24 * layout.scale))
        );
      }
      ctx.restore();
    ...
    renderCreateLevelScreen(ctx, now) {
      ...
      ctx.fillStyle = "#edf7ff";
      ctx.font = "700 48px Consolas, monospace";
      ctx.fillText(isEditingLevel ? "Edit Level" : "Create a Level", this.width * 0.5, Math.max(70, this.height * 0.14));
      ctx.restore();

      this.renderOnlineBackButton(ctx);

      const instructions = isEditingLevel
        ? [
            "1. Your saved level is already loaded into Test Your Level below.",
            "2. Make any changes you want and keep testing until the level feels right.",
            "3. The publish box stays synced while you edit, so you only need to update one text area.",
            "4. When you are ready, press Publish to submit the updated version for review.",
          ]
        : [
      ...
      this.renderCreateLevelBox(
        ctx,
        layout.frame.x,
        layout.frame.y,
        layout.frame.w,
        layout.frame.h,
        isEditingLevel ? "Editing Existing Level" : "Empty Level Frame",
        isEditingLevel
          ? "Your current level is loaded below.\nThe frame copier is hidden while editing an existing level.\nTest changes, then publish when you are ready."
          : EMPTY_LEVEL_FRAME.join("\n"),
        scale
      );
      if (!isEditingLevel) {
