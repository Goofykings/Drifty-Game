  ...
  class DriftGame {
    ...
    getCampaignCompleteButtons() {
      ...
      return [
        ...
        { id: "next", label: "Next", x: startX + width + gap, y, w: width, h: height },
      ];
    }
    ...
    loadLevel(index) {
      ...
      if (this.gameMode === "speedrun" && this.speedrunTimerStarted) {
    ...
    finishCurrentLevel() {
      ...
      this.levelCompletionSummary = {
        ...
        perfectRun: this.gameMode === "speedrun" ? false : perfectRun,
    ...
    updateCampaignGame(dt) {
      ...
      if (this.gameMode === "speedrun") {
        if (!this.speedrunTimerStarted && this.levelIndex === 0 && speed >= LEVEL_TIMER_START_SPEED) {
          this.speedrunTimerStarted = true;
          this.levelTimerStarted = true;
        }
        if (this.speedrunTimerStarted && !this.goalTriggered) {
          this.totalTimer += dt;
          this.levelTimer += dt;
        }
      } else {
    ...
    updateGoal(dt) {
      ...
      const penetration = this.getCarTilePenetration(this.car, goalTileX, goalTileY);
      if (penetration) {
    ...
    isTileEdgeExposed(tileX, tileY, edge) {
      if (edge === "left") {
        return !this.level.isWall(tileX - 1, tileY);
      }
      if (edge === "right") {
        return !this.level.isWall(tileX + 1, tileY);
      }
      if (edge === "top") {
        return !this.level.isWall(tileX, tileY - 1);
      }
      return !this.level.isWall(tileX, tileY + 1);
    }
    ...
    renderWorld(ctx, camera, now, carState) {
      ...
      const goalBlast = isTagMode ? 0 : this.getGoalExplosionProgress();
      const goalVisibility = isTagMode ? 1 : (this.goalTriggered ? lerp(1, 0.18, easeInOutCubic(goalBlast)) : 1);
      const completionBlackout = this.completed && this.levelCompletionSummary
        ...
        : 0;
      const environmentAlpha = goalVisibility * (1 - completionBlackout);
      ...
      if (environmentAlpha > 0.001) {
        ...
        for (const tile of this.level.wallTiles) {
          ...
          if (headlightGlow) {
            const glowColor = isBumperWall ? "241, 221, 255" : "244, 235, 207";
            const edgeHighlight = isBumperWall ? "255, 241, 255" : "255, 248, 222";
            const glowDepth = 18;
            const blurSize = 18;
    ...
    renderCampaignCompletionOverlay(ctx) {
      ...
      const statusText = summary.speedrun
        ? "All tracks cleared"
      ...
      for (const button of this.getCampaignCompleteButtons()) {
        const hover = button.id === "home" ? this.completionHomeHover : this.completionNextHover;
        ...
        ctx.stroke();
        ctx.shadowBlur = 0;
        ctx.fillStyle = button.id === "next" ? "#fff1c7" : "#eff9ff";
        ctx.font = "700 22px Consolas, monospace";
        ctx.fillText(button.label, 0, 8);
        ctx.restore();
      }
    ...
    renderUI(ctx, now) {
      ...
      ctx.fillStyle = "#84b6d3";
      ctx.font = "16px Consolas, monospace";
      ctx.fillText(fitTextToWidth(ctx, this.level.name, maxTextWidth), panelX + 20, panelY + 56);
