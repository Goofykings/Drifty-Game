(() => {
  ...
  const WALL_BOUNCE = 0.05;
  const WALL_SPEED_RETAIN = 2;
  const WALL_FLASH_DURATION = 0.22;
  const WALL_FLASH_SPEED = 900;
  const WALL_IMPACT_SEPARATION = 11;
  const WALL_IMPACT_TURN = 0.12;
  ...
  const BUMPER_SPEED_RETAIN = 1.02;
  ...
  class DriftGame {
    ...
    getLevelSelectCards() {
      const columns = Math.min(3, LEVELS.length);
      const rows = Math.ceil(LEVELS.length / columns);
      const gapX = 26;
      const gapY = 24;
      const cardWidth = Math.min(240, (this.width - 120 - gapX * (columns - 1)) / columns);
      const cardHeight = rows > 1 ? 118 : 132;
      ...
      const startY = this.height * 0.44 - totalHeight * 0.5;
    ...
    applyWallResponse(car, normalX, normalY, impactSpeed, tileX, tileY, owner = null, wallType = "wall") {
      ...
      const bouncedNormal = isBumperWall
        ...
        : -normalSpeed * bounceScale + (freshImpact ? 24 + impactSpeed * 0.05 : 8);
