(() => {
  ...
  const WALL_TANGENT_RETAIN = 0.72;
  const WALL_BOUNCE = 0.18;
  const WALL_SPEED_RETAIN = 0.8;
  const WALL_FLASH_DURATION = 0.22;
  const WALL_FLASH_SPEED = 900;
  const WALL_IMPACT_SEPARATION = 8;
  const WALL_IMPACT_TURN = 0.24;
  const TILE_LIGHT_RADIUS = 200;
  const GOAL_HOLD_TIME = 0.35;
  const SMOKE_DAMAGE_START = 1000;
  const FIRE_DAMAGE_START = 2500;
  const SMOKE_DAMAGE_SPAN = 1800;
  const FIRE_DAMAGE_SPAN = 1000;
  const FIRE_FAILURE_DELAY = 1.0;
  const EXPLOSION_DURATION = 0.75;
  const TAG_MATCH_DURATION = 120;
  const TAG_TRANSFER_COOLDOWN = 5;
  const TAG_PUSH_VELOCITY = 360;
  ...
  class Car {
    ...
    updateControlState(controlState, dt, allowThrottle = true) {
      ...
      if (throttle !== 0) {
        this.vx += facingX * throttle * ACCELERATION * dt;
        this.vy += facingY * throttle * ACCELERATION * dt;
      }
      ...
      const turnStrength = clamp(speed / MAX_SPEED, 0, 1);
      const accelerationRatio = clamp(ACCELERATION / MAX_SPEED, 0, 1);
      const driftStartRatio = clamp(0.58 - accelerationRatio * 0.55, 0.16, 0.58);
      const driftStartSpeed = MAX_SPEED * driftStartRatio;
      ...
      if (speed > 16 && steer !== 0) {
        ...
        const turnResponse = 3.5 + turnStrength * 11.5;
        const targetTurn = steer * movingForward * TURN_RATE * speedTurnAuthority;
        this.angularVelocity = approachExp(this.angularVelocity, targetTurn, turnResponse, dt);
      } else {
        const angularDrag = 4.8 - driftReadiness * 3.2;
      ...
      if (speed > 1) {
        ...
        const gripRate = (1 - TURN_SLIP) * (steerAmount > 0 ? 2.0 : 5.0);
    ...
    updateControls(input, dt, allowThrottle = true) {
  ...
  class DriftGame {
    ...
    getLevelSelectCards() {
      const cardWidth = Math.min(260, this.width * 0.24);
    ...
    refreshLevelSelectHover() {
      ...
      this.hoveredLevelSelectButton = hoveredBack ? backButton.id : null;
      this.hoveredLevelCard = hoveredCard ? hoveredCard.index : null;
      this.canvas.style.cursor = hoveredBack || hoveredCard ? "pointer" : "default";
    }
    ...
    handleCanvasClick(event) {
      ...
      if (this.currentScreen === "title") {
        ...
        } else if (this.hoveredTitleButton === "tag") {
          return;
      ...
      if (this.currentScreen === "game" && this.hoveredGameButton === "menu") {
        this.returnToTitle();
      }
    }
    ...
    updateCampaignGame(dt) {
      ...
      const throttleLocked = this.fireSequenceTimer > 0 || this.exploding;
      const telemetry = this.car.updateControls(this.input, dt, !throttleLocked);
      if (!this.exploding) {
    ...
    updateTagGame(dt) {
      ...
      for (const player of this.tagCars) {
        ...
        const telemetry = player.car.updateControlState(controlState, dt, !throttleLocked);
        if (!player.exploding) {
    ...
    updateTitleScreen(dt) {
      ...
      for (const button of this.titleButtons) {
        const targetHover = button.id === this.hoveredTitleButton ? 1 : 0;
    ...
    moveCar(car, dt, owner = null) {
      ...
      for (let index = 0; index < steps; index += 1) {
        // Micro-steps keep the circular collider from clipping through narrow wall gaps.
        car.x += car.vx * subDt;
        car.y += car.vy * subDt;

        const collision = this.resolveWallPenetration(car);
        if (collision.hit) {
          this.applyWallResponse(car, collision.normalX, collision.normalY, collision.impactSpeed, collision.tileX, collision.tileY, owner);
        }
      }
    }
    ...
    resolveWallPenetration(car) {
      ...
      for (let pass = 0; pass < 4; pass += 1) {
        let passHit = false;
        const nearbyWalls = this.level.getWallTilesNear(car.x, car.y, CAR_RADIUS);
        ...
        for (const tile of nearbyWalls) {
          ...
          const nearestX = clamp(car.x, left, right);
          const nearestY = clamp(car.y, top, bottom);
          let dx = car.x - nearestX;
          let dy = car.y - nearestY;
          let distanceSq = dx * dx + dy * dy;

          if (distanceSq >= CAR_RADIUS * CAR_RADIUS - 0.0001) {
            continue;
          }
          ...
          if (distanceSq > 0.0001) {
            ...
            const deltaX = car.x - tileCenterX;
            const deltaY = car.y - tileCenterY;
            if (Math.abs(deltaX) > Math.abs(deltaY)) {
              normalX = Math.sign(deltaX) || 1;
              normalY = 0;
              penetration = CAR_RADIUS;
            } else {
              normalX = 0;
              normalY = Math.sign(deltaY) || 1;
              penetration = CAR_RADIUS;
            }
          }

          car.x += normalX * (penetration + 0.05);
          car.y += normalY * (penetration + 0.05);
          passHit = true;
          hit = true;
          sumNormalX += normalX;
          sumNormalY += normalY;
          const impactSpeed = Math.max(0, -dot(car.vx, car.vy, normalX, normalY));
    ...
    applyWallResponse(car, normalX, normalY, impactSpeed, tileX, tileY, owner = null) {
      ...
      const bounceScale = owner
        ? freshImpact ? (WALL_BOUNCE + flashStrength * 0.42) : WALL_BOUNCE * 0.2
        : freshImpact ? (WALL_BOUNCE + flashStrength * 0.58) : WALL_BOUNCE * 0.2;
      const tangentRetain = owner
        ? freshImpact ? lerp(WALL_TANGENT_RETAIN, 0.64, flashStrength) : 0.82
        : freshImpact ? lerp(WALL_TANGENT_RETAIN, 0.6, flashStrength) : 0.82;
      const speedRetain = owner
        ? freshImpact ? lerp(WALL_SPEED_RETAIN, 0.6, flashStrength) : 0.88
        : freshImpact ? lerp(WALL_SPEED_RETAIN, 0.54, flashStrength) : 0.88;
      const bouncedNormal = -normalSpeed * bounceScale + (freshImpact ? 36 + impactSpeed * 0.08 : 10);
      ...
      if (freshImpact && impactSpeed > 0) {
        ...
        if (postImpactSpeed > 20 && glancingImpact > 0.12) {
          ...
          const turnAmount = (WALL_IMPACT_TURN * 0.4 + glancingImpact * 0.22) * glancingImpact;
          car.angle += angleDelta * turnAmount;
          car.angularVelocity += angleDelta * (2.5 + glancingImpact * 4.5) * glancingImpact;
        }
    ...
    updateGoal(dt) {
      ...
      const margin = 10;
      const insideGoal =
        this.car.x > goal.x + margin &&
        this.car.x < goal.x + goal.w - margin &&
        this.car.y > goal.y + margin &&
        this.car.y < goal.y + goal.h - margin;

      if (insideGoal) {
        this.goalTimer += dt;
        if (this.goalTimer >= GOAL_HOLD_TIME) {
          this.nextLevel();
        }
      } else {
        this.goalTimer = 0;
    ...
    getWallFlashIntensity(tileX, tileY) {
      ...
      const fade = clamp(flash.life / WALL_FLASH_DURATION, 0, 1);
    ...
    castHeadlightRay(originX, originY, angle, maxDistance = 540, step = 14) {
      ...
      for (let distance = step; distance <= maxDistance; distance += step) {
        ...
        for (let index = 0; index < 6; index += 1) {
          ...
          if (this.level.isWall(Math.floor(midX / TILE_SIZE), Math.floor(midY / TILE_SIZE))) {
            high = mid;
          } else {
            low = mid;
          }
    ...
    renderHeadlightCone(ctx, camera, state, tint = "255, 244, 214") {
      ...
      const rightX = -facingY;
      const rightY = facingX;
      const baseX = state.x + facingX * (CAR_LENGTH * 0.42);
      const baseY = state.y + facingY * (CAR_LENGTH * 0.42);
      const headlightOffset = CAR_WIDTH * 0.28;
      const spread = 0.44;
      const range = 520;
      const rayCount = 20;
      ...
      for (const sign of [-1, 1]) {
        const originX = baseX + rightX * headlightOffset * sign;
        const originY = baseY + rightY * headlightOffset * sign;
        const originScreen = this.worldToScreen(originX, originY, camera);
        const centerRay = this.castHeadlightRay(originX, originY, state.angle, range);
        const beamLength = Math.max(48, centerRay.distance);
        const rays = [];
    ...
    renderMenuButtons(ctx, buttons) {
      ...
      for (const button of buttons) {
        ...
        const glowAlpha = 0.16 + button.hover * 0.32;

        ctx.save();
        ctx.translate(centerX, centerY);
        ctx.scale(scale, scale);
        ctx.shadowColor = `rgba(128, 214, 255, ${0.22 + button.hover * 0.4})`;
        ctx.shadowBlur = 8 + button.hover * 28;
        ...
        ctx.fillStyle = fill;
        ctx.strokeStyle = `rgba(152, 223, 255, ${0.28 + button.hover * 0.55})`;
        ctx.lineWidth = 2;
        ctx.beginPath();
        ...
        ctx.shadowBlur = 0;
        ctx.fillStyle = button.hover > 0.01 ? "#f4fdff" : "#d6eaf3";
    ...
    renderLevelSelectCards(ctx) {
      ctx.save();
      ctx.textAlign = "center";
      for (const card of this.getLevelSelectCards()) {
        ...
        const border = card.unlocked
          ? card.isCurrent
            ? `rgba(214, 244, 255, ${0.8 + hover * 0.18})`
            : `rgba(125, 196, 232, ${0.38 + hover * 0.24})`
          : "rgba(52, 60, 68, 0.65)";
        const titleColor = card.unlocked ? (card.isCurrent ? "#f4fdff" : "#dcecf6") : "#55606b";
        ...
        ctx.shadowColor = card.unlocked
          ? card.isCurrent
            ? "rgba(166, 233, 255, 0.42)"
            : `rgba(112, 186, 219, ${0.18 + hover * 0.16})`
          : "rgba(0, 0, 0, 0.2)";
        ...
        ctx.fillStyle = card.unlocked
          ? `rgba(186, 240, 255, ${card.isCurrent ? 0.22 : 0.12 + hover * 0.08})`
          : "rgba(32, 38, 43, 0.45)";
        ctx.beginPath();
        ctx.roundRect(-card.w * 0.5 + 8, -card.h * 0.5 + 8, card.w - 16, 28, 12);
        ctx.fill();

        ctx.fillStyle = subColor;
        ctx.font = "700 15px Consolas, monospace";
        ctx.fillText(`LEVEL ${card.index + 1}`, 0, -30);

        ctx.fillStyle = titleColor;
        ctx.font = "700 22px Consolas, monospace";
        ctx.fillText(card.name, 0, 4);

        ctx.fillStyle = card.unlocked ? (card.isCurrent ? "#fff0b8" : "#bdd8e7") : "#404851";
        ctx.font = "16px Consolas, monospace";
        ctx.fillText(card.isCurrent ? "Current Track" : card.unlocked ? "Unlocked" : "Locked", 0, 38);

        if (!card.unlocked) {
          ctx.fillStyle = "rgba(8, 10, 13, 0.56)";
          ctx.beginPath();
          ctx.roundRect(-card.w * 0.5 + 18, card.h * 0.5 - 42, card.w - 36, 24, 10);
          ctx.fill();
        }
    ...
    renderUI(ctx, now) {
      ...
      ctx.fillStyle = "#dceaf3";
      ctx.font = "600 24px Consolas, monospace";
      ctx.fillText(`Level ${this.levelIndex + 1}/${LEVELS.length}`, panelX + 20, panelY + 32);
      ...
      ctx.fillStyle = "#f0f7fb";
      ctx.font = "18px Consolas, monospace";
      ctx.fillText(`Level Time  ${formatTime(this.levelTimer)}`, panelX + 20, panelY + 86);
      ctx.fillText(`Total Time  ${formatTime(this.totalTimer)}`, panelX + 20, panelY + 112);
      ctx.fillText(`Damage      ${Math.round(this.totalDamage)}`, panelX + 20, panelY + 138);
      ctx.fillText(`Status      ${this.getDamageStatus()}`, panelX + 20, panelY + 164);

      if (!this.completed) {
        ctx.fillStyle = this.totalDamage >= FIRE_DAMAGE_START ? "#ff8f66" : this.totalDamage >= SMOKE_DAMAGE_START ? "#c7cfd6" : "#89d0a8";
        ctx.fillText("Drive into the glowing finish cube", panelX + 20, panelY + 190);
      } else {
        ctx.fillStyle = "#ffd166";
        ctx.fillText("All levels cleared", panelX + 20, panelY + 190);
      }

      ctx.textAlign = "right";
      ctx.fillStyle = "rgba(235, 245, 250, 0.9)";
      ctx.fillText("WASD / Arrows drive   R restart   Space after finish", this.width - 30, this.height - 30);
      ctx.textAlign = "left";
      ...
      if (this.completed) {
        ...
        ctx.fillRect(-220, -98, 440, 196);
        ctx.strokeStyle = "rgba(255, 209, 102, 0.7)";
        ctx.lineWidth = 2;
        ctx.strokeRect(-220, -98, 440, 196);
        ctx.fillStyle = "#fff2cc";
        ctx.font = "700 34px Consolas, monospace";
        ctx.textAlign = "center";
        ctx.fillText("TRACKS CLEARED", 0, -16);
        ctx.font = "18px Consolas, monospace";
        ctx.fillText(`Final Time ${formatTime(this.totalTimer)}`, 0, 20);
        ctx.fillText(`Total Damage ${Math.round(this.totalDamage)}`, 0, 48);
        ctx.fillText("Press Space to run all levels again", 0, 76);
        ctx.restore();
      }
      ctx.restore();
    }
