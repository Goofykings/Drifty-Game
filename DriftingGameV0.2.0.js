  ...
  class DriftGame {
    ...
    getTouchJoystickDriveBasis() {
      ...
      return {
        ...
        rightX: -facingY,
        rightY: facingX,
        reverseThreshold: -0.72,
      };
    }

    getSinglePlayerControlState() {
      if (this.touchJoystick.active && this.canUseTouchJoystick()) {
        const desiredX = this.touchJoystick.inputX;
        const desiredY = this.touchJoystick.inputY;
        const pullAmount = clamp(Math.hypot(desiredX, desiredY), 0, 1);
        const throttleMagnitude = pullAmount >= 0.9 ? 1 : pullAmount;
        const basis = this.getTouchJoystickDriveBasis();
        const reverseAlignment = dot(desiredX, desiredY, basis.forwardX, basis.forwardY);
        const reversing = reverseAlignment <= basis.reverseThreshold;
        return {
          throttle: throttleMagnitude <= 0.001 ? 0 : (reversing ? -throttleMagnitude : throttleMagnitude),
          steer: clamp(dot(desiredX, desiredY, basis.rightX, basis.rightY), -1, 1),
        };
      }
      return {
    ...
    recordSpeedrunWorldRecord(time, carVariant = this.playerCarVariant, carColor = this.playerCarColor, targetKey = "speedrunWorldRecord") {
    ...
    finishCurrentLevel() {
      ...
      if (this.gameMode === "speedrun") {
        ...
        this.recordSpeedrunWorldRecord(this.totalTimer, this.playerCarVariant, this.playerCarColor, "speedrunWorldRecord");
        if (speedrunPerfect) {
          this.recordSpeedrunWorldRecord(this.totalTimer, this.playerCarVariant, this.playerCarColor, "perfectSpeedrunWorldRecord");
        }
    ...
    renderLeaderboardScreen(ctx, now) {
      ...
      speedrunRecords.forEach((entry, index) => {
        ...
        ctx.fillText(hasRecord ? formatTime(entry.record.time) : "--:--.---", 10, 48);
    ...
    renderWorld(ctx, camera, now, carState) {
      ...
      const isBlackoutMode = this.gameMode === "blackout";
    ...
    renderTouchJoystick(ctx) {
      ...
      ctx.beginPath();
      ctx.arc(0, 0, outerRadius, Math.PI - reverseHalfAngle, Math.PI + reverseHalfAngle);
      ctx.arc(0, 0, innerRadius, Math.PI + reverseHalfAngle, Math.PI - reverseHalfAngle, true);
      ctx.closePath();
      ctx.fill();
      ctx.restore();
