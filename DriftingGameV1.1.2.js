(() => {
  ...
  const ONLINE_LEADERBOARD_CACHE_MAX_AGE_MS = 3 * 60 * 1000;
  const ONLINE_LEADERBOARD_REPLAY_CACHE_MAX_AGE_MS = 10 * 60 * 1000;
  const ALLOW_LOCAL_LEADERBOARD_SEEDING = false;
  const ONLINE_COMMUNITY_CACHE_MAX_AGE_MS = 5 * 60 * 1000;
  const ONLINE_MY_LEVELS_CACHE_MAX_AGE_MS = 2 * 60 * 1000;
  const MIN_VALID_LEVEL_LEADERBOARD_TIMES_MS = [
    7100,
    0,
    0,
    0,
  ...
  const ONLINE_LEADERBOARD_CONFIG = (() => {
    const defaults = {
      ...
      saveProfileRpcName: "save_drifty_account_profile",
      ...
      incrementCommunityPlayRpcName: "increment_drifty_community_level_play_count",
      unpublishCommunityLevelRpcName: "unpublish_drifty_community_level_submission",
  ...
  const LEVEL_DIFFICULTY_TIERS = {
    1: { dots: 1, label: "Easy", color: "#43c96f" },
    2: { dots: 2, label: "Normal", color: "#f0a941" },
    3: { dots: 3, label: "Hard", color: "#cf6d1e" },
    4: { dots: 4, label: "Insane", color: "#9e2328" },
    5: { dots: 5, label: "Impossible", color: "#7d46f4" },
  };
  ...
  const GHOST_MODE_OPTIONS = [
    { id: "none", label: "No Ghost" },
    { id: "world_record", label: "World Record Ghost" },
    { id: "personal_record", label: "Personal Record Ghost" },
  ];
  const ONLINE_RACING_TEMPLATE_MATCHES = {
    public: [],
    private: [
      { name: "Private Race 01", detail: "Code required - Template lobby" },
      { name: "Friends Only", detail: "Code required - Template lobby" },
      { name: "Code Lobby", detail: "Code required - Template lobby" },
    ],
  };
  const COMMUNITY_LEVEL_SORT_OPTIONS = [
    { id: "most_views", label: "Most Views" },
    { id: "largest_time", label: "Largest Time" },
    { id: "smallest_time", label: "Smallest Time" },
    { id: "alphabetical", label: "Alphabetical" },
    { id: "date_published", label: "Date Published" },
  ];
  const COMMUNITY_LEVEL_VIEW_COUNT_MIN_SECONDS = 8;
  const CREATE_LEVEL_MIN_SIZE = 4;
  const CREATE_LEVEL_MAX_SIZE = 100;
  const CREATE_LEVEL_DEFAULT_SIZE = 10;
  const CREATE_LEVEL_VOID_TILE = " ";
  const CREATE_LEVEL_WALL_PALETTE = [
    { id: "wall", label: "Wall", tile: "#" },
    { id: "hazard", label: "Hazard", tile: "X" },
    { id: "bumper", label: "Bumper", tile: "P" },
    { id: "goal", label: "Goal", tile: "G" },
  ];
  const CREATE_LEVEL_FLOOR_PALETTE = [
    { id: "floor", label: "Floor", tile: "." },
    { id: "start", label: "Start", tile: "S" },
    { id: "ice", label: "Ice", tile: "I" },
  ];
  ...
  function formatModifiedTimestamp(value) {
    ...
    return `Last modified ${dateText} ${timeText}`;
  }
  ...
  function getLevelDifficultyTier(value) {
    const normalized = Math.max(1, Math.min(5, Math.round(Number(value) || 1)));
    return LEVEL_DIFFICULTY_TIERS[normalized] || LEVEL_DIFFICULTY_TIERS[1];
  }
  ...
  class DriftGame {
    constructor() {
      ...
      this.communitySortSelect = document.createElement("select");
      this.communitySortSelect.className = "drift-username-input";
      this.communitySortSelect.setAttribute("aria-label", "Sort community levels");
      this.communitySortSelect.style.position = "absolute";
      this.communitySortSelect.style.zIndex = "6";
      this.communitySortSelect.style.display = "none";
      this.communitySortSelect.innerHTML = COMMUNITY_LEVEL_SORT_OPTIONS
        .map((option) => `<option value="${option.id}">${option.label}</option>`)
        .join("");
      document.body.appendChild(this.communitySortSelect);
      ...
      this.createLevelSizeXInput = document.createElement("input");
      this.createLevelSizeXInput.type = "number";
      this.createLevelSizeXInput.className = "drift-username-input drift-size-input";
      this.createLevelSizeXInput.setAttribute("aria-label", "Build width");
      this.createLevelSizeXInput.min = String(CREATE_LEVEL_MIN_SIZE);
      this.createLevelSizeXInput.max = String(CREATE_LEVEL_MAX_SIZE);
      this.createLevelSizeXInput.style.position = "absolute";
      this.createLevelSizeXInput.style.zIndex = "6";
      this.createLevelSizeXInput.style.display = "none";
      document.body.appendChild(this.createLevelSizeXInput);
      this.createLevelSizeYInput = document.createElement("input");
      this.createLevelSizeYInput.type = "number";
      this.createLevelSizeYInput.className = "drift-username-input drift-size-input";
      this.createLevelSizeYInput.setAttribute("aria-label", "Build height");
      this.createLevelSizeYInput.min = String(CREATE_LEVEL_MIN_SIZE);
      this.createLevelSizeYInput.max = String(CREATE_LEVEL_MAX_SIZE);
      this.createLevelSizeYInput.style.position = "absolute";
      this.createLevelSizeYInput.style.zIndex = "6";
      this.createLevelSizeYInput.style.display = "none";
      document.body.appendChild(this.createLevelSizeYInput);
      this.myLevelDeletePasswordInput = document.createElement("input");
      this.myLevelDeletePasswordInput.type = "password";
      this.myLevelDeletePasswordInput.className = "drift-username-input";
      this.myLevelDeletePasswordInput.setAttribute("aria-label", "Delete level password");
      this.myLevelDeletePasswordInput.placeholder = "Enter password";
      this.myLevelDeletePasswordInput.autocomplete = "current-password";
      this.myLevelDeletePasswordInput.spellcheck = false;
      this.myLevelDeletePasswordInput.style.position = "absolute";
      this.myLevelDeletePasswordInput.style.zIndex = "7";
      this.myLevelDeletePasswordInput.style.display = "none";
      document.body.appendChild(this.myLevelDeletePasswordInput);
      ...
      this.pageScrollTouch = { id: null, startY: 0, lastY: 0, moved: false };
      ...
      this.hoveredLeaderboardReplayId = null;
      ...
      this.leaderboardReplayBackHover = 0;
      this.leaderboardReplayPrevHover = 0;
      this.leaderboardReplayNextHover = 0;
      this.titleSettingsHover = 0;
      this.settingsBackHover = 0;
      this.settingsGhostOptionHovers = Object.fromEntries(GHOST_MODE_OPTIONS.map((option) => [option.id, 0]));
      ...
      this.onlineRacingHover = 0;
      this.onlineRacingJoinHover = 0;
      this.onlineRacingCreateHover = 0;
      this.onlineRacingAccountHover = 0;
      this.onlineRacingFriendsHover = 0;
      this.onlineRacingLogoutHover = 0;
      this.onlineRacingPublicTabHover = 0;
      this.onlineRacingPrivateTabHover = 0;
      this.onlineRacingSelectedTab = "public";
      this.onlineRacingMatchHovers = [];
      this.hoveredOnlineRacingMatch = null;
      this.myLevelsCreateHover = 0;
      this.myLevelMessageCloseHover = 0;
      this.communityDetailPlayHover = 0;
      this.myLevelDetailPlayHover = 0;
      this.myLevelDetailActionHover = 0;
      this.myLevelDetailEditHover = 0;
      this.myLevelDetailDeleteHover = 0;
      ...
      this.nextCreatedLevelNumber = 1;
      this.createLevelEditor = null;
      this.createLevelButtonHovers = Object.create(null);
      this.hoveredCreateLevelButton = null;
      this.hoveredCreateLevelPaletteIndex = null;
      this.hoveredCreateLevelResizeEdge = null;
      this.createLevelBottomPanelCollapsed = false;
      this.createLevelBottomPanelAnim = 1;
      this.createLevelPointerGesture = null;
      this.createLevelSuppressNextClick = false;
      this.createLevelTouchGesture = {
        mode: null,
        touches: new Map(),
        startDistance: 0,
        startZoom: 1,
      ...
      this.activeMyLevel = null;
      this.communityLevelReturnScreen = "community_levels";
      this.createLevelReturnScreen = "online";
      this.myLevelMessageDialog = null;
      this.myLevelActionPrompt = null;
      this.hoveredMyLevelActionPromptButton = null;
      this.myLevelActionPromptCancelHover = 0;
      this.myLevelActionPromptConfirmHover = 0;
      this.myLevelActionRequestInFlight = false;
      this.myLevelDetailStatusText = "";
      this.myLevelDetailStatusTone = "neutral";
      ...
      this.signedInSessionToken = "";
      this.accountProfileSaveTimer = null;
      this.accountProfileSaveQueue = Promise.resolve();
      this.accountProfileSavePending = false;
      ...
      this.communitySortSelect.value = this.communityLevelSort;
      this.publishLevelInput.value = this.publishLevelText;
      this.createLevelSizeXInput.value = String(CREATE_LEVEL_DEFAULT_SIZE);
      this.createLevelSizeYInput.value = String(CREATE_LEVEL_DEFAULT_SIZE);
      ...
      this.personalLevelRecords = LEVELS.map(() => ({ time: null, carVariant: null, carColor: null, replayFrames: [] }));
      this.ghostMode = "none";
      this.activeGhostReplay = null;
      this.ghostReplayRequestId = 0;
      ...
      this.currentRunReplayFrames = [];
      this.pendingReplaySkidEvents = [];
      this.pendingReplayWallFlashEvents = [];
      this.activeLeaderboardReplay = null;
      ...
      this.onlineLeaderboardReplayLastSyncAt = 0;
      ...
      this.onlineMyLevelsLastSyncAt = 0;
      this.onlineMyLevelsLastSyncUser = "";
      ...
      this.customLevelInput.addEventListener("input", (event) => {
        if (this.currentScreen === "create_level" && this.createLevelEditor?.mode === "setup") {
          this.createLevelEditor.importText = String(event.target.value ?? "");
        } else if (this.isEditingMyLevel()) {
          this.syncEditingMyLevelText(event.target.value, "custom");
        } else {
      ...
      this.publishLevelNameInput.addEventListener("input", (event) => {
        if (this.currentScreen === "create_level" && this.createLevelEditor && this.isCreateLevelEditorLocked()) {
          event.target.value = this.publishLevelName;
          return;
        }
        ...
        if (this.currentScreen === "create_level" && this.createLevelEditor) {
          if (this.createLevelEditor.name !== cleaned) {
            this.pushCreateLevelUndoSnapshot(this.createLevelEditor);
          }
          this.createLevelEditor.name = cleaned;
          this.markCreateLevelEditorChanged({ clearValidation: false });
        }
        this.customLevelStatusText = "";
      });
      this.publishLevelNameInput.addEventListener("blur", () => {
        const normalizedName = normalizeCommunityLevelName(this.publishLevelName);
        if (this.currentScreen === "create_level" && this.createLevelEditor && this.createLevelEditor.name !== normalizedName) {
          this.pushCreateLevelUndoSnapshot(this.createLevelEditor);
        }
        this.publishLevelName = normalizedName;
        this.publishLevelNameInput.value = this.publishLevelName;
        if (this.currentScreen === "create_level" && this.createLevelEditor) {
          this.createLevelEditor.name = this.publishLevelName;
          this.markCreateLevelEditorChanged({ clearValidation: false });
        }
      });
      this.publishLevelInput.addEventListener("input", (event) => {
        if (this.isEditingMyLevel()) {
          this.syncEditingMyLevelText(event.target.value, "publish");
        } else {
          this.publishLevelText = event.target.value;
        }
        this.customLevelStatusText = "";
      });
      this.myLevelDeletePasswordInput.addEventListener("input", (event) => {
        if (!this.myLevelActionPrompt || this.myLevelActionPrompt.mode !== "delete") {
          return;
        }
        this.myLevelActionPrompt.password = String(event.target.value ?? "");
        this.myLevelActionPrompt.statusText = "";
        this.myLevelActionPrompt.statusTone = "neutral";
      });
      ...
      this.communitySortSelect.addEventListener("change", (event) => {
        const selected = String(event.target.value || "");
        this.communityLevelSort = COMMUNITY_LEVEL_SORT_OPTIONS.some((option) => option.id === selected)
          ? selected
          : "most_views";
        this.setOnlineLevelListScrollY(0, "community_levels");
        this.refreshOnlineHover();
      ...
      for (const eventName of ["keydown", "keyup", "keypress"]) {
        ...
        this.communitySortSelect.addEventListener(eventName, (event) => event.stopPropagation());
        this.createLevelSizeXInput.addEventListener(eventName, (event) => event.stopPropagation());
        this.createLevelSizeYInput.addEventListener(eventName, (event) => event.stopPropagation());
        this.myLevelDeletePasswordInput.addEventListener(eventName, (event) => event.stopPropagation());
      }
      this.createLevelSizeXInput.addEventListener("input", (event) => {
        if (this.currentScreen !== "create_level" || !this.createLevelEditor) {
          return;
        }
      ...
      this.createLevelSizeYInput.addEventListener("input", (event) => {
        if (this.currentScreen !== "create_level" || !this.createLevelEditor) {
          return;
        }
        const cleaned = String(event.target.value ?? "").replace(/[^\d]/g, "");
      ...
      this.createLevelSizeXInput.addEventListener("blur", () => {
        if (this.currentScreen === "create_level" && this.createLevelEditor) {
          this.applyCreateLevelSizeInput("width");
        }
      });
      this.createLevelSizeYInput.addEventListener("blur", () => {
        if (this.currentScreen === "create_level" && this.createLevelEditor) {
          this.applyCreateLevelSizeInput("height");
        }
      });
      this.createLevelSizeXInput.addEventListener("keydown", (event) => {
        if (event.key === "Enter" && this.currentScreen === "create_level" && this.createLevelEditor) {
          this.applyCreateLevelSizeInput("width");
          this.createLevelSizeXInput.blur();
        }
      });
      this.createLevelSizeYInput.addEventListener("keydown", (event) => {
        if (event.key === "Enter" && this.currentScreen === "create_level" && this.createLevelEditor) {
          this.applyCreateLevelSizeInput("height");
          this.createLevelSizeYInput.blur();
        }
      });
    ...
    installShell() {
      ...
      document.documentElement.style.background = "#020409";
      ...
      if (!colorInputStyle) {
        ...
        colorInputStyle.textContent = `
          ...
          .drift-size-input::-webkit-outer-spin-button,
          .drift-size-input::-webkit-inner-spin-button {
            -webkit-appearance: none;
            margin: 0;
          }
    ...
    resize() {
      ...
      const viewportWidth = Math.max(1, Math.round(visualViewport?.width || window.innerWidth || 1));
      const viewportHeight = Math.max(1, Math.round(visualViewport?.height || window.innerHeight || 1));
      const desiredHeight = this.getDesiredCanvasHeightForScreen(this.currentScreen, viewportHeight);
      const pageScrollEnabled = desiredHeight > viewportHeight + 1;
      ...
      this.canvas.style.touchAction = pageScrollEnabled ? "pan-y" : "none";
      document.body.style.overflowY = pageScrollEnabled ? "auto" : "hidden";
      document.documentElement.style.overflowY = pageScrollEnabled ? "auto" : "hidden";
      if (this.pageScrollEnabled && !pageScrollEnabled) {
        window.scrollTo(0, 0);
      }
      this.pageScrollEnabled = pageScrollEnabled;
      ...
      this.updateMyLevelDeletePasswordInputVisibility();
      this.updateCommunitySearchInputVisibility();
      this.updateCommunitySortSelectVisibility();
      this.updateCustomizationPickerVisibility();
    }

    getDesiredCanvasHeightForScreen(screen = this.currentScreen, viewportHeight = this.viewportHeight || window.innerHeight || 1) {
      const minViewport = Math.max(1, Math.round(viewportHeight));
      if (screen === "leaderboard") {
        return Math.max(minViewport, 980);
      }
      if (screen === "create_level") {
        return minViewport;
      }
      if (screen === "my_level_detail") {
        return Math.max(minViewport, 820);
      }
      return minViewport;
    }

    shouldAllowNativePageScroll() {
      return this.pageScrollEnabled && this.currentScreen !== "game" && !this.isScrollableOnlineListScreen();
    }
    ...
    normalizeWorldRecordEntry(entry) {
      ...
      const replayFrames = this.normalizeReplayFrames(entry?.replayFrames ?? entry?.runFrames ?? entry?.run_frames, Number.isInteger(entry?.levelIndex) ? entry.levelIndex : 0);
      return {
        time,
        ...
        replayFrames: time == null ? [] : replayFrames,
      };
    }

    normalizeReplaySkidEvent(event) {
      if (Array.isArray(event) && event.length >= 6) {
        const x1 = Number(event[0]);
        const y1 = Number(event[1]);
        const x2 = Number(event[2]);
        const y2 = Number(event[3]);
        const width = Number(event[4]);
        const alpha = Number(event[5]);
        const life = Number(event[6]);
        const maxLife = Number(event[7]);
        if ([x1, y1, x2, y2, width, alpha].every(Number.isFinite)) {
          const safeLife = Number.isFinite(life) && life > 0 ? life : 7.5;
          return {
            x1,
            y1,
            x2,
            ...
            alpha,
            life: safeLife,
            maxLife: Number.isFinite(maxLife) && maxLife > 0 ? maxLife : safeLife,
          };
        }
      }
      const x1 = Number(event?.x1);
      const y1 = Number(event?.y1);
      const x2 = Number(event?.x2);
      const y2 = Number(event?.y2);
      const width = Number(event?.width);
      const alpha = Number(event?.alpha);
      const life = Number(event?.life);
      const maxLife = Number(event?.maxLife);
      if ([x1, y1, x2, y2, width, alpha].every(Number.isFinite)) {
        const safeLife = Number.isFinite(life) && life > 0 ? life : 7.5;
        return {
          x1,
    ...
    normalizeReplayWallFlashEvent(event) {
      if (Array.isArray(event) && event.length >= 3) {
        const tileX = Math.floor(Number(event[0]));
        const tileY = Math.floor(Number(event[1]));
        const strength = Number(event[2]);
        const life = Number(event[3]);
        const maxLife = Number(event[4]);
        if (Number.isFinite(tileX) && Number.isFinite(tileY) && Number.isFinite(strength)) {
          const safeLife = Number.isFinite(life) && life > 0 ? life : WALL_FLASH_DURATION;
          return {
            tileX,
            tileY,
            strength,
      ...
      const tileX = Math.floor(Number(event?.tileX));
      const tileY = Math.floor(Number(event?.tileY));
    ...
    normalizeReplayVisualData(raw) {
      if (!raw || typeof raw !== "object") {
        return { skidEvents: [], wallFlashEvents: [] };
      }
      const skidSource = Array.isArray(raw.skidEvents)
        ? raw.skidEvents
        : Array.isArray(raw.skids)
          ? raw.skids
          : [];
      const wallFlashSource = Array.isArray(raw.wallFlashEvents)
        ? raw.wallFlashEvents
        : Array.isArray(raw.wallFlashes)
          ? raw.wallFlashes
          : [];
      return {
        skidEvents: skidSource.map((event) => this.normalizeReplaySkidEvent(event)).filter(Boolean),
        wallFlashEvents: wallFlashSource.map((event) => this.normalizeReplayWallFlashEvent(event)).filter(Boolean),
      };
    }

    hasReplayVisualData(visualData) {
      return Boolean(visualData) && (
        (Array.isArray(visualData.skidEvents) && visualData.skidEvents.length > 0) ||
        (Array.isArray(visualData.wallFlashEvents) && visualData.wallFlashEvents.length > 0)
      );
    }

    normalizeReplayFrame(frame, fallbackLevelIndex = 0) {
      const safeLevelIndex = clamp(Math.floor(Number.isFinite(fallbackLevelIndex) ? fallbackLevelIndex : 0), 0, LEVELS.length - 1);
      if (Array.isArray(frame) && frame.length >= 4) {
        const firstValue = Number(frame[0]);
        const firstLooksLikeLevelIndex = Number.isInteger(firstValue) && firstValue >= 0 && firstValue < LEVELS.length;
        const levelIndex = firstLooksLikeLevelIndex ? firstValue : safeLevelIndex;
        const offset = firstLooksLikeLevelIndex ? 1 : 0;
        const x = Number(frame[offset]);
        const y = Number(frame[offset + 1]);
        const angle = Number(frame[offset + 2]);
        const elapsedTime = Number(frame[offset + 3]);
        const visualData = this.normalizeReplayVisualData(frame[offset + 4]);
        if (Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(angle)) {
          return [
            ...
            this.hasReplayVisualData(visualData) ? visualData : null,
          ];
        }
      }
      const levelIndex = clamp(Math.floor(Number.isFinite(frame?.levelIndex) ? frame.levelIndex : safeLevelIndex), 0, LEVELS.length - 1);
      const x = Number(frame?.x);
      const y = Number(frame?.y);
      const angle = Number(frame?.angle);
      const elapsedTime = Number(frame?.elapsedTime ?? frame?.time);
      const visualData = this.normalizeReplayVisualData(frame?.visualData ?? frame?.visuals ?? frame?.replayVisuals);
      if (Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(angle)) {
        return [
          levelIndex,
          x,
    ...
    normalizeReplayFrames(frames, fallbackLevelIndex = 0) {
      if (!Array.isArray(frames)) {
        return [];
      }
      return frames
        .map((frame) => this.normalizeReplayFrame(frame, fallbackLevelIndex))
        .filter(Boolean);
    }

    normalizeReplayTimeline(frames, finalTime = null) {
      if (!Array.isArray(frames) || frames.length === 0) {
        return [];
      }
      const safeFinalTime = Number.isFinite(finalTime) && finalTime >= 0 ? finalTime : 0;
      const lastIndex = Math.max(0, frames.length - 1);
      let previousTime = -1;
      return frames.map((frame, index) => {
        const next = [...frame];
        const explicitTime = Number.isFinite(next[4]) && next[4] >= 0 ? next[4] : null;
        const inferredTime = lastIndex <= 0 ? 0 : safeFinalTime * (index / lastIndex);
        const nextTime = explicitTime != null && explicitTime > previousTime
          ? explicitTime
          : inferredTime;
        next[4] = Math.max(previousTime + 0.0001, nextTime);
        if (index === 0) {
          next[4] = explicitTime === 0 ? 0 : Math.max(0, Math.min(next[4], inferredTime));
        }
        previousTime = next[4];
        return next;
      });
    ...
    getReplayFrameState(frame, fallbackLevelIndex = 0) {
      const normalized = this.normalizeReplayFrame(frame, fallbackLevelIndex);
      if (!normalized) {
        return null;
      }
      return {
        levelIndex: normalized[0],
        x: normalized[1],
        y: normalized[2],
        angle: normalized[3],
        elapsedTime: normalized[4],
        visualData: normalized[5] || { skidEvents: [], wallFlashEvents: [] },
      };
    }

    beginRunReplayCapture() {
      this.currentRunReplayFrames = [];
      this.pendingReplaySkidEvents = [];
      this.pendingReplayWallFlashEvents = [];
    }

    queueReplaySkidEvent(mark) {
      if (!mark) {
        return;
      }
      this.pendingReplaySkidEvents.push({
        x1: Math.round(mark.x1 * 10) / 10,
        y1: Math.round(mark.y1 * 10) / 10,
        x2: Math.round(mark.x2 * 10) / 10,
        y2: Math.round(mark.y2 * 10) / 10,
        width: Math.round(mark.width * 100) / 100,
        alpha: Math.round(mark.alpha * 1000) / 1000,
        life: Math.round(mark.life * 1000) / 1000,
        maxLife: Math.round(mark.maxLife * 1000) / 1000,
      });
    }

    queueReplayWallFlashEvent(tileX, tileY, strength, life = WALL_FLASH_DURATION, maxLife = WALL_FLASH_DURATION) {
      const safeTileX = Math.floor(Number(tileX));
      const safeTileY = Math.floor(Number(tileY));
      const safeStrength = Number(strength);
      if (!Number.isFinite(safeTileX) || !Number.isFinite(safeTileY) || !Number.isFinite(safeStrength)) {
        return;
      }
      const existing = this.pendingReplayWallFlashEvents.find((event) => event.tileX === safeTileX && event.tileY === safeTileY);
      if (existing) {
        existing.strength = Math.max(existing.strength, safeStrength);
        existing.life = Math.max(existing.life, life);
        existing.maxLife = Math.max(existing.maxLife, maxLife);
        return;
      }
      this.pendingReplayWallFlashEvents.push({
        tileX: safeTileX,
        tileY: safeTileY,
        strength: safeStrength,
        life: Number.isFinite(life) && life > 0 ? life : WALL_FLASH_DURATION,
        maxLife: Number.isFinite(maxLife) && maxLife > 0 ? maxLife : WALL_FLASH_DURATION,
      });
    }

    consumeReplayCaptureVisualData() {
      const visualData = {
        skidEvents: this.pendingReplaySkidEvents.map((event) => ({ ...event })),
        wallFlashEvents: this.pendingReplayWallFlashEvents.map((event) => ({ ...event })),
      };
      this.pendingReplaySkidEvents = [];
      this.pendingReplayWallFlashEvents = [];
      return this.hasReplayVisualData(visualData) ? visualData : null;
    }

    getCurrentReplayCaptureElapsedTime() {
      if (this.isSpeedrunMode()) {
        return Number.isFinite(this.totalTimer) && this.totalTimer >= 0 ? this.totalTimer : 0;
      }
      return Number.isFinite(this.levelTimer) && this.levelTimer >= 0 ? this.levelTimer : 0;
    }

    shouldCaptureCurrentRunReplayFrame(force = false) {
      if (force) {
        return true;
      }
      ...
      return this.isSpeedrunMode() ? this.speedrunTimerStarted : this.levelTimerStarted;
    }

    captureCurrentRunReplayFrame(levelIndex = this.levelIndex, car = this.car, force = false) {
      if (!this.shouldCaptureCurrentRunReplayFrame(force)) {
        return;
      }
      if (!car || !Number.isFinite(car.x) || !Number.isFinite(car.y) || !Number.isFinite(car.angle)) {
        return;
      }
      const safeLevelIndex = clamp(Math.floor(Number.isFinite(levelIndex) ? levelIndex : 0), 0, LEVELS.length - 1);
      const elapsedTime = Math.round(this.getCurrentReplayCaptureElapsedTime() * 1000) / 1000;
      const visualData = this.consumeReplayCaptureVisualData();
      const frame = [
        safeLevelIndex,
        Math.round(car.x * 10) / 10,
        Math.round(car.y * 10) / 10,
        Math.round(car.angle * 10000) / 10000,
        elapsedTime,
      ];
      if (visualData) {
        frame.push(visualData);
      }
      this.currentRunReplayFrames.push(frame);
    }

    getCurrentRunReplayFrames() {
      return this.normalizeReplayFrames(this.currentRunReplayFrames, this.levelIndex);
    }

    getFastestReplayRecord(candidates, requireReplay = true) {
      return candidates
        .map((candidate) => ({
          ...candidate,
          record: this.normalizeWorldRecordEntry(candidate?.record ?? candidate),
        }))
        .filter((candidate) =>
          candidate.record.time != null &&
          (!requireReplay || (Array.isArray(candidate.record.replayFrames) && candidate.record.replayFrames.length > 0))
        )
        .sort((a, b) => a.record.time - b.record.time)[0] || null;
    }

    getAllTimeLeaderboardEntry(scope, levelIndex = null, fallbackEntry = null) {
      const recordKey = this.getOnlineLeaderboardRecordKey(scope, levelIndex, "all_time", "all_time");
      const row = this.onlineLeaderboardRowsByKey.get(recordKey);
      const fallback = this.normalizeWorldRecordEntry(fallbackEntry);
      const online = this.normalizeWorldRecordEntry(row?.entry);
      if (this.isOnlineLeaderboardConfigured() && this.onlineLeaderboardLastSyncAt > 0 && online.time == null) {
        return {
          record: online,
          recordKey,
          scope,
      ...
      const record = online.time != null ? online : fallback;
      if (
        record.time != null &&
        (!Array.isArray(record.replayFrames) || record.replayFrames.length === 0) &&
        Array.isArray(fallback.replayFrames) &&
        fallback.replayFrames.length > 0 &&
        fallback.time === record.time
      ) {
        record.replayFrames = fallback.replayFrames;
      }
      return {
        record,
        recordKey,
    ...
    getGhostReplayRecordForCurrentLevel() {
      if (
        this.ghostMode === "none" ||
        this.isCustomLevelMode() ||
        this.isRaceMode() ||
        this.isSpeedrunMode() ||
        !Number.isInteger(this.levelIndex) ||
        this.levelIndex < 0 ||
        this.levelIndex >= LEVELS.length
      ) {
        return null;
      }
      if (this.ghostMode === "personal_record") {
        const record = this.normalizeWorldRecordEntry(this.personalLevelRecords[this.levelIndex]);
        return record.time != null && record.replayFrames.length > 0
          ? { record, label: "Personal Best" }
          : null;
      }
      if (this.ghostMode === "world_record") {
        const candidates = this.gameMode === "blackout"
          ? [
              this.getAllTimeLeaderboardEntry(
                this.getVersionedLevelScope("level_blackout", this.levelIndex),
                this.levelIndex,
                this.blackoutWorldRecords[this.levelIndex]
              ),
            ]
          : [
              this.getAllTimeLeaderboardEntry(
                this.getVersionedLevelScope("level_normal", this.levelIndex),
                this.levelIndex,
                this.worldRecords[this.levelIndex]
              ),
              this.getAllTimeLeaderboardEntry(
                this.getVersionedLevelScope("level_perfect", this.levelIndex),
                this.levelIndex,
                this.perfectWorldRecords[this.levelIndex]
              ),
            ];
        const candidate = this.getFastestReplayRecord(candidates, false);
        return candidate ? { ...candidate, label: "World Record" } : null;
      }
      return null;
    }

    prepareActiveGhostReplay() {
      const requestId = ++this.ghostReplayRequestId;
      const ghost = this.getGhostReplayRecordForCurrentLevel();
      if (!ghost) {
        this.activeGhostReplay = null;
        return;
      }
      const hasReplayFrames = Array.isArray(ghost.record.replayFrames) && ghost.record.replayFrames.length > 0;
      const canRefreshOnlineReplay = Boolean(ghost.recordKey && this.isOnlineLeaderboardConfigured());
      if (hasReplayFrames) {
        this.setActiveGhostReplayFromRecord(ghost);
      } else {
        this.activeGhostReplay = null;
      }
      if (canRefreshOnlineReplay) {
        this.fetchOnlineLeaderboardReplayRow(ghost.recordKey)
          .then((row) => {
            if (requestId !== this.ghostReplayRequestId || this.currentScreen !== "game") {
              return;
            }
            if (row && typeof row === "object") {
              this.cacheOnlineLeaderboardRows([row]);
              this.applyOnlineLeaderboardRows([row]);
              const refreshed = this.getGhostReplayRecordForCurrentLevel();
              if (refreshed && Array.isArray(refreshed.record.replayFrames) && refreshed.record.replayFrames.length > 0) {
                this.setActiveGhostReplayFromRecord(refreshed);
              }
            }
          })
          .catch(() => undefined);
    ...
    setActiveGhostReplayFromRecord(ghost) {
      const frames = this.normalizeReplayTimeline(
        this.normalizeReplayFrames(ghost.record.replayFrames, this.levelIndex),
        ghost.record.time
      );
      this.activeGhostReplay = {
        label: ghost.label,
        record: ghost.record,
        frames,
        levelIndex: this.levelIndex,
      };
      if (this.activeGhostReplay.frames.length === 0) {
        this.activeGhostReplay = null;
      }
    }

    getReplayFrameInferredElapsedTime(replay, frameIndex) {
      const lastIndex = Math.max(0, replay?.frames?.length - 1);
      const safeIndex = clamp(Math.floor(Number.isFinite(frameIndex) ? frameIndex : 0), 0, lastIndex);
      const finalTime = Number.isFinite(replay?.record?.time) && replay.record.time >= 0 ? replay.record.time : 0;
      return lastIndex <= 0 ? 0 : finalTime * (safeIndex / lastIndex);
    }

    getReplayFrameElapsedTime(replay, frameIndex, previousTime = null) {
      const lastIndex = Math.max(0, replay?.frames?.length - 1);
      const safeIndex = clamp(Math.floor(Number.isFinite(frameIndex) ? frameIndex : 0), 0, lastIndex);
      const frame = this.getReplayFrameState(replay?.frames?.[safeIndex], replay?.levelIndex ?? 0);
      const explicitTime = Number.isFinite(frame?.elapsedTime) && frame.elapsedTime >= 0 ? frame.elapsedTime : null;
      const inferredTime = this.getReplayFrameInferredElapsedTime(replay, safeIndex);
      if (
        explicitTime == null ||
        (safeIndex > 0 && Number.isFinite(previousTime) && explicitTime <= previousTime)
      ) {
        return inferredTime;
      }
      return explicitTime;
    ...
    getGhostReplayFrameAtElapsed(replay, elapsedTime) {
      ...
      let previous = this.getReplayFrameState(replay.frames[0], replay.levelIndex);
      if (!previous) {
        return null;
      }
      let previousTime = this.getReplayFrameElapsedTime(replay, 0);
      if (elapsed <= previousTime || replay.frames.length === 1) {
        return previous;
      }
      for (let index = 1; index < replay.frames.length; index += 1) {
        const next = this.getReplayFrameState(replay.frames[index], replay.levelIndex);
        if (!next) {
          continue;
        }
        const nextTime = this.getReplayFrameElapsedTime(replay, index, previousTime);
        if (elapsed <= nextTime || index === replay.frames.length - 1) {
          const span = Math.max(0.0001, nextTime - previousTime);
          const t = clamp((elapsed - previousTime) / span, 0, 1);
          const angleDelta = Math.atan2(Math.sin(next.angle - previous.angle), Math.cos(next.angle - previous.angle));
          return {
            ...
            elapsedTime: elapsed,
            visualData: next.visualData || { skidEvents: [], wallFlashEvents: [] },
          };
        }
        previous = next;
    ...
    getReplayDisplayElapsedTime(replay) {
      if (!replay) {
        return 0;
      }
      const finalTime = Number.isFinite(replay.record?.time) && replay.record.time >= 0 ? replay.record.time : 0;
      const lastIndex = Math.max(0, replay.frames.length - 1);
      const currentTime = this.getReplayFrameElapsedTime(replay, replay.frameIndex);
      if (replay.paused || replay.frameIndex >= lastIndex || FIXED_DT <= 0) {
        return clamp(currentTime, 0, finalTime);
      }
      const nextTime = this.getReplayFrameElapsedTime(replay, replay.frameIndex + 1);
      const blend = clamp(replay.frameTimer / FIXED_DT, 0, 1);
      return clamp(lerp(currentTime, nextTime, blend), 0, finalTime);
    }

    resetLeaderboardReplayFrameVisuals(replay) {
      if (!replay) {
        return;
      }
      replay.skidMarks = [];
      replay.wallFlashes = new Map();
    }

    resetLeaderboardReplayPlaybackState(replay, { preserveFrame = true } = {}) {
      if (!replay) {
        return;
      }
      if (!preserveFrame) {
      ...
      replay.elapsedTime = preserveFrame ? replay.elapsedTime : 0;
      replay.paused = false;
      replay.ended = false;
      replay.finishEffectTriggered = false;
      replay.goalTriggered = false;
      replay.goalTimer = 0;
      replay.explosionParticles = [];
      replay.finishDebris = [];
      this.resetLeaderboardReplayFrameVisuals(replay);
      const anchorFrame = this.getReplayFrameState(replay.frames[replay.frameIndex], replay.levelIndex ?? 0)
        || this.getReplayFrameState(replay.frames[0], replay.levelIndex ?? 0);
      if (anchorFrame) {
        replay.currentLevelIndex = anchorFrame.levelIndex;
        replay.level = new LevelMap(LEVELS[anchorFrame.levelIndex]);
        replay.camera.snap(anchorFrame.x, anchorFrame.y);
      }
      replay.elapsedTime = this.getReplayDisplayElapsedTime(replay);
    }

    ageLeaderboardReplayFrameVisuals(replay, dt) {
      if (!replay || dt <= 0) {
        return;
      }
      for (const mark of replay.skidMarks) {
        mark.life -= dt;
      }
      replay.skidMarks = replay.skidMarks.filter((mark) => mark.life > 0);
      for (const flash of replay.wallFlashes.values()) {
        flash.life -= dt;
      }
      for (const [key, flash] of replay.wallFlashes.entries()) {
        if (flash.life <= 0) {
          replay.wallFlashes.delete(key);
        }
      }
    }

    applyLeaderboardReplayFrameVisuals(replay, frame) {
      if (!replay || !frame?.visualData) {
        return;
      }
      for (const skidEvent of frame.visualData.skidEvents || []) {
        replay.skidMarks.push({
          x1: skidEvent.x1,
          y1: skidEvent.y1,
          x2: skidEvent.x2,
          y2: skidEvent.y2,
          width: skidEvent.width,
          alpha: skidEvent.alpha,
          life: skidEvent.life,
          maxLife: skidEvent.maxLife,
        });
      }
      for (const flashEvent of frame.visualData.wallFlashEvents || []) {
        const key = `${flashEvent.tileX},${flashEvent.tileY}`;
        const existing = replay.wallFlashes.get(key);
        replay.wallFlashes.set(key, {
          strength: existing ? Math.max(existing.strength, flashEvent.strength) : flashEvent.strength,
          life: flashEvent.life,
          maxLife: flashEvent.maxLife,
        });
      }
    }

    rebuildLeaderboardReplayFrameVisuals(replay, frameIndex = replay?.frameIndex ?? 0) {
      if (!replay) {
        return;
      }
      this.resetLeaderboardReplayFrameVisuals(replay);
      const targetIndex = clamp(Math.floor(Number.isFinite(frameIndex) ? frameIndex : 0), 0, Math.max(0, replay.frames.length - 1));
      let previousFrame = this.getReplayFrameState(replay.frames[0], replay.levelIndex ?? 0);
      for (let index = 1; index <= targetIndex; index += 1) {
        const frame = this.getReplayFrameState(replay.frames[index], replay.levelIndex ?? 0);
        if (!frame) {
          continue;
        }
        if (previousFrame && frame.levelIndex !== previousFrame.levelIndex) {
          this.resetLeaderboardReplayFrameVisuals(replay);
        }
        this.applyLeaderboardReplayFrameVisuals(replay, frame);
        this.ageLeaderboardReplayFrameVisuals(replay, FIXED_DT);
        previousFrame = frame;
      }
    }

    stepLeaderboardReplayFrame(replay, direction) {
      if (!replay) {
        return false;
      }
      const lastIndex = Math.max(0, replay.frames.length - 1);
      const nextIndex = clamp(replay.frameIndex + direction, 0, lastIndex);
      if (nextIndex === replay.frameIndex) {
        return false;
      }
      replay.frameIndex = nextIndex;
      replay.frameTimer = 0;
      replay.ended = false;
      replay.finishEffectTriggered = false;
      replay.goalTriggered = false;
      ...
      replay.explosionParticles = [];
      replay.finishDebris = [];
      this.rebuildLeaderboardReplayFrameVisuals(replay, nextIndex);
      const frame = this.getReplayFrameState(replay.frames[nextIndex], replay.levelIndex ?? 0);
      if (frame) {
        replay.currentLevelIndex = frame.levelIndex;
        replay.level = new LevelMap(LEVELS[frame.levelIndex]);
        replay.camera.snap(frame.x, frame.y);
      }
      replay.elapsedTime = this.getReplayDisplayElapsedTime(replay);
      return true;
    }

    getReplayFinishSourceCar(replay, frame) {
      const previousFrame = this.getReplayFrameState(
        replay?.frames?.[Math.max(0, (replay?.frameIndex || 0) - 1)],
        replay?.levelIndex ?? 0
      );
      const velocityScale = FIXED_DT > 0 ? 1 / FIXED_DT : 0;
      return {
        x: frame?.x ?? 0,
        y: frame?.y ?? 0,
        vx: previousFrame && frame ? (frame.x - previousFrame.x) * velocityScale : 0,
        vy: previousFrame && frame ? (frame.y - previousFrame.y) * velocityScale : 0,
      };
    }

    triggerLeaderboardReplayGoalFinish(replay, frame) {
      if (!replay || replay.finishEffectTriggered || !frame || !replay.level?.goal) {
        return;
      }
      replay.finishEffectTriggered = true;
      replay.goalTriggered = true;
      replay.goalTimer = GOAL_ADVANCE_DELAY;
      replay.explosionParticles.length = 0;
      replay.finishDebris.length = 0;
      const goalCenterX = replay.level.goal.x + replay.level.goal.w * 0.5;
      const goalCenterY = replay.level.goal.y + replay.level.goal.h * 0.5;
      const sourceCar = this.getReplayFinishSourceCar(replay, frame);
      const previousLevel = this.level;
      const previousExplosionParticles = this.explosionParticles;
      const previousFinishDebris = this.finishDebris;
      this.level = replay.level;
      this.explosionParticles = replay.explosionParticles;
      this.finishDebris = replay.finishDebris;
      this.spawnGoalExplosionBurst(goalCenterX, goalCenterY, sourceCar, isValidHexColor(replay.record?.carColor) ? replay.record.carColor : null);
      this.spawnFinishDebris(goalCenterX, goalCenterY, sourceCar);
      this.level = previousLevel;
      this.explosionParticles = previousExplosionParticles;
      this.finishDebris = previousFinishDebris;
    }

    updateLeaderboardReplayGoalFinish(replay, dt) {
      if (!replay) {
        return;
      }
      if (replay.goalTriggered) {
        replay.goalTimer = Math.max(0, replay.goalTimer - dt);
      }
      for (const particle of replay.explosionParticles) {
        particle.life -= dt;
      ...
      replay.explosionParticles = replay.explosionParticles.filter((particle) => particle.life > 0);
      for (const piece of replay.finishDebris) {
        piece.life -= dt;
        piece.x += piece.vx * dt;
        piece.y += piece.vy * dt;
      ...
      replay.finishDebris = replay.finishDebris.filter((piece) => piece.life > 0);
    }
    ...
    getOnlineLeaderboardCacheMaxAgeMs() {
      return ONLINE_LEADERBOARD_CACHE_MAX_AGE_MS;
    }

    getOnlineLeaderboardReplayCacheMaxAgeMs() {
      return ONLINE_LEADERBOARD_REPLAY_CACHE_MAX_AGE_MS;
    }

    getOnlineCommunityCacheMaxAgeMs() {
      return ONLINE_COMMUNITY_CACHE_MAX_AGE_MS;
    }

    getOnlineMyLevelsCacheMaxAgeMs() {
      return ONLINE_MY_LEVELS_CACHE_MAX_AGE_MS;
    }

    isCacheFresh(lastSyncAt, maxAgeMs) {
      return Number.isFinite(lastSyncAt) &&
        lastSyncAt > 0 &&
        Number.isFinite(maxAgeMs) &&
        maxAgeMs > 0 &&
        (Date.now() - lastSyncAt) < maxAgeMs;
    }
    ...
    getOnlineLeaderboardActivePeriodFilter() {
      const dailyKey = this.getLeaderboardPeriodKey("daily");
      const weeklyKey = this.getLeaderboardPeriodKey("weekly");
      return [
        "leaderboard_period.eq.all_time",
        `and(leaderboard_period.eq.daily,period_key.eq.${dailyKey})`,
        `and(leaderboard_period.eq.weekly,period_key.eq.${weeklyKey})`,
      ].join(",");
    }

    addOnlineLeaderboardActivePeriodFilter(params) {
      params.set("or", `(${this.getOnlineLeaderboardActivePeriodFilter()})`);
      return params;
    }
    ...
    getOnlineLeaderboardFetchUrl(includePlayerName = true) {
      const params = new URLSearchParams({
        select: includePlayerName
          ? "record_key,leaderboard_period,period_key,scope,level_index,time_ms,car_variant,car_color,player_name,deaths,run_frames,updated_at"
          : "record_key,leaderboard_period,period_key,scope,level_index,time_ms,car_variant,car_color,deaths,run_frames,updated_at",
        order: "leaderboard_period.asc,period_key.desc,scope.asc,level_index.asc.nullslast",
      });
      this.addOnlineLeaderboardActivePeriodFilter(params);
      return `${this.getOnlineLeaderboardBaseUrl()}/rest/v1/${ONLINE_LEADERBOARD_CONFIG.tableName}?${params.toString()}`;
    }

    getOnlineLeaderboardFetchUrlWithoutReplay(includePlayerName = true) {
      const params = new URLSearchParams({
        select: includePlayerName
    ...
    getOnlineLeaderboardReplayFetchUrl(recordKey, includePlayerName = true) {
      const params = new URLSearchParams({
        select: includePlayerName
          ? "record_key,leaderboard_period,period_key,scope,level_index,time_ms,car_variant,car_color,player_name,deaths,run_frames,updated_at"
          : "record_key,leaderboard_period,period_key,scope,level_index,time_ms,car_variant,car_color,deaths,run_frames,updated_at",
        record_key: `eq.${String(recordKey || "")}`,
        limit: "1",
      });
      return `${this.getOnlineLeaderboardBaseUrl()}/rest/v1/${ONLINE_LEADERBOARD_CONFIG.tableName}?${params.toString()}`;
    }
    ...
    isOnlineAccountProfileConfigured() {
      return this.isOnlineLeaderboardConfigured() &&
        typeof ONLINE_LEADERBOARD_CONFIG.saveProfileRpcName === "string" &&
        ONLINE_LEADERBOARD_CONFIG.saveProfileRpcName.trim().length > 0;
    }

    hasSignedInAccountSession() {
      return Boolean(this.signedInUsername) &&
        typeof this.signedInSessionToken === "string" &&
        this.signedInSessionToken.trim().length > 0;
    }

    async callOnlineRpc(rpcName, payload) {
    ...
    async callOnlineAccountRpc(rpcName, username, password) {
      return this.callOnlineRpc(rpcName, {
        p_username: username,
        p_password: password,
      });
    }

    getSavedCampaignLevelCount(source) {
      const explicitCount = Number(source?.campaignLevelCount ?? source?.campaign_level_count);
      if (Number.isFinite(explicitCount) && explicitCount >= 1) {
        return Math.floor(explicitCount);
      }
      const fallbackCollections = [
        source?.levelBestTimes,
        source?.levelCleanFinishes,
        source?.worldRecords,
        source?.perfectWorldRecords,
        source?.blackoutWorldRecords,
      ];
    ...
    normalizeHighestVisitedLevelForCurrentCampaign(savedHighestVisitedLevel, savedCampaignLevelCount = LEVELS.length) {
      const currentLevelCount = LEVELS.length;
      const clampedHighest = clamp(
        Math.floor(Number.isFinite(savedHighestVisitedLevel) ? savedHighestVisitedLevel : 0),
        0,
        Math.max(0, currentLevelCount - 1)
      );
      const previousLevelCount = Math.max(1, Math.floor(Number.isFinite(savedCampaignLevelCount) ? savedCampaignLevelCount : currentLevelCount));
      if (currentLevelCount <= previousLevelCount) {
        return clampedHighest;
      }
      if (clampedHighest >= previousLevelCount - 1) {
        return clamp(previousLevelCount, 0, currentLevelCount - 1);
      }
      return clampedHighest;
    }

    getAccountProfilePayload() {
      return {
        campaignLevelCount: LEVELS.length,
        highestVisitedLevel: clamp(Math.floor(Number.isFinite(this.highestVisitedLevel) ? this.highestVisitedLevel : 0), 0, LEVELS.length - 1),
        levelBestTimes: LEVELS.map((_, index) => {
          const value = this.levelBestTimes[index];
          return Number.isFinite(value) && value >= 0 ? value : null;
        }),
        levelCleanFinishes: LEVELS.map((_, index) => Boolean(this.levelCleanFinishes[index])),
        playerCarColor: isValidHexColor(this.playerCarColor) ? this.playerCarColor : "#909090",
        playerCarVariant: this.getCarVariantDefinition(this.playerCarVariant).id,
        ghostMode: GHOST_MODE_OPTIONS.some((option) => option.id === this.ghostMode) ? this.ghostMode : "none",
        personalLevelRecords: this.personalLevelRecords.map((record, index) => this.normalizeWorldRecordEntry(record, index)),
        myCreatedLevels: this.getAccountProfileCreatedLevels(),
      };
    }

    serializeMyCreatedLevelForAccountProfile(level) {
      const normalized = this.normalizeMyCreatedLevelEntry(level, level?.submissionStatus || "private");
      if (!normalized) {
        return null;
      }
      return {
        ...
        creatorTime: Number.isFinite(normalized.creatorTime) ? normalized.creatorTime : null,
        myBestTime: Number.isFinite(normalized.myBestTime) ? normalized.myBestTime : null,
        startAngle: Number.isFinite(normalized.startAngle) ? normalized.startAngle : 0,
        leaderboard: normalizeCommunityLeaderboard(normalized.leaderboard, normalized.creator, normalized.creatorTime),
        map: normalized.map,
        hiddenMap: Array.isArray(normalized.hiddenMap) ? normalized.hiddenMap : normalized.map,
        hiddenOriginX: Number.isFinite(normalized.hiddenOriginX) ? normalized.hiddenOriginX : 0,
        hiddenOriginY: Number.isFinite(normalized.hiddenOriginY) ? normalized.hiddenOriginY : 0,
        local: Boolean(normalized.local),
        submissionStatus: normalized.submissionStatus || "private",
        reviewMessage: normalized.reviewMessage || "",
        playCount: Number.isFinite(normalized.playCount) ? Math.max(0, Math.floor(normalized.playCount)) : 0,
        createdAt: Number.isFinite(normalized.createdAt) ? normalized.createdAt : 0,
        updatedAt: Number.isFinite(normalized.updatedAt) ? normalized.updatedAt : 0,
      };
    }

    getAccountProfileCreatedLevels() {
      return this.getMyCreatedLevels()
        .map((level) => this.serializeMyCreatedLevelForAccountProfile(level))
        .filter(Boolean);
    }

    applyAccountProfilePayload(profile) {
      if (!profile || typeof profile !== "object") {
        return false;
      }
      let changed = false;

      if (Number.isFinite(profile.highestVisitedLevel)) {
        const nextHighest = this.normalizeHighestVisitedLevelForCurrentCampaign(
          profile.highestVisitedLevel,
          this.getSavedCampaignLevelCount(profile)
        );
        if (nextHighest !== this.highestVisitedLevel) {
          this.highestVisitedLevel = nextHighest;
          changed = true;
        }
      }

      if (Array.isArray(profile.levelBestTimes)) {
        const nextLevelBestTimes = LEVELS.map((_, index) => {
          const value = profile.levelBestTimes[index];
          return Number.isFinite(value) && value >= 0 ? value : null;
        });
        if (nextLevelBestTimes.some((value, index) => value !== this.levelBestTimes[index])) {
          this.levelBestTimes = nextLevelBestTimes;
          changed = true;
        }
      }

      if (Array.isArray(profile.levelCleanFinishes)) {
        const nextLevelCleanFinishes = LEVELS.map((_, index) => Boolean(profile.levelCleanFinishes[index]));
        if (nextLevelCleanFinishes.some((value, index) => value !== this.levelCleanFinishes[index])) {
          this.levelCleanFinishes = nextLevelCleanFinishes;
          changed = true;
        }
      }

      if (Array.isArray(profile.personalLevelRecords)) {
        const nextPersonalLevelRecords = LEVELS.map((_, index) => {
          const incoming = this.normalizeWorldRecordEntry(profile.personalLevelRecords[index], index);
          const current = this.normalizeWorldRecordEntry(this.personalLevelRecords[index], index);
          if (incoming.time != null && (current.time == null || incoming.time < current.time)) {
            return incoming;
          }
          return current;
        });
        if (nextPersonalLevelRecords.some((record, index) => record.time !== this.normalizeWorldRecordEntry(this.personalLevelRecords[index], index).time)) {
          this.personalLevelRecords = nextPersonalLevelRecords;
          changed = true;
        }
      }

      if (isValidHexColor(profile.playerCarColor) && profile.playerCarColor !== this.playerCarColor) {
        this.playerCarColor = profile.playerCarColor;
        changed = true;
      }

      if (typeof profile.playerCarVariant === "string") {
        const nextVariant = this.getCarVariantDefinition(profile.playerCarVariant).id;
        if (nextVariant !== this.playerCarVariant) {
          this.playerCarVariant = nextVariant;
          changed = true;
        }
      }

      if (typeof profile.ghostMode === "string" && GHOST_MODE_OPTIONS.some((option) => option.id === profile.ghostMode) && profile.ghostMode !== this.ghostMode) {
        this.ghostMode = profile.ghostMode;
        changed = true;
      }

      if (Array.isArray(profile.myCreatedLevels)) {
        const accountName = normalizeUsername(this.signedInUsername || this.playerUsername);
        for (const level of profile.myCreatedLevels) {
          const normalized = this.normalizeMyCreatedLevelEntry(
            {
              ...level,
              creator: normalizeUsername(level?.creator) || accountName || "Player",
            },
            level?.submissionStatus || "private"
          );
          if (normalized && (!accountName || normalized.creator === accountName)) {
            const existingIndex = this.getMyCreatedLevelMatchIndex(normalized);
            const existing = existingIndex >= 0
              ? this.normalizeMyCreatedLevelEntry(this.myCreatedLevels[existingIndex], this.myCreatedLevels[existingIndex]?.submissionStatus || "private")
              : null;
            if (
              existing &&
            ...
            const saved = this.upsertMyCreatedLevel(normalized, { save: false, fallbackStatus: normalized.submissionStatus || "private" });
            if (saved) {
              changed = true;
            }
          }
    ...
    scheduleSignedInAccountProfileSave(delayMs = 300) {
      if (!this.hasSignedInAccountSession() || !this.isOnlineAccountProfileConfigured()) {
        return;
      }
      this.accountProfileSavePending = true;
      if (this.accountProfileSaveTimer != null) {
        clearTimeout(this.accountProfileSaveTimer);
      }
      this.accountProfileSaveTimer = window.setTimeout(() => {
        this.accountProfileSaveTimer = null;
        this.flushSignedInAccountProfileSave();
      }, Math.max(0, delayMs));
    }

    async flushSignedInAccountProfileSave() {
      if (!this.accountProfileSavePending || !this.hasSignedInAccountSession() || !this.isOnlineAccountProfileConfigured()) {
        return;
      }
      const username = this.signedInUsername;
      const sessionToken = this.signedInSessionToken;
      const profile = this.getAccountProfilePayload();
      this.accountProfileSavePending = false;
      this.accountProfileSaveQueue = this.accountProfileSaveQueue
        .catch(() => undefined)
        .then(async () => {
          const result = await this.callOnlineRpc(ONLINE_LEADERBOARD_CONFIG.saveProfileRpcName, {
            p_username: username,
          ...
          if (status !== "ok") {
            throw new Error(status || "profile_sync_failed");
          }
        })
        .catch((error) => {
          const message = String(error?.message || "");
          if (message === "invalid_session") {
            this.setAuthStatus("Account session expired. Log out and log back in to sync progress.", "error");
            return;
          }
          this.setAuthStatus("Account profile sync failed. Local progress is still saved on this device.", "error");
        });
      return this.accountProfileSaveQueue;
    }
    ...
    cacheOnlineLeaderboardRows(rows, replace = false) {
      const previousRowsByKey = replace ? this.onlineLeaderboardRowsByKey : null;
      if (replace) {
        this.onlineLeaderboardRowsByKey = new Map();
      }
      for (const row of rows) {
        const normalized = this.normalizeOnlineLeaderboardRow(row);
        const previous = normalized.recordKey
          ? (replace ? previousRowsByKey?.get(normalized.recordKey) : this.onlineLeaderboardRowsByKey.get(normalized.recordKey))
          : null;
        if (
          (!Array.isArray(normalized.entry.replayFrames) || normalized.entry.replayFrames.length === 0) &&
          Array.isArray(previous?.entry?.replayFrames) &&
          previous.entry.replayFrames.length > 0
        ) {
          normalized.entry = {
            ...normalized.entry,
            replayFrames: previous.entry.replayFrames,
          };
        }
    ...
    async fetchOnlineLeaderboardRows({ includeReplay = false } = {}) {
      const requestUrls = includeReplay
        ? [
            this.getOnlineLeaderboardFetchUrl(true),
            this.getOnlineLeaderboardFetchUrlWithoutReplay(true),
            this.getOnlineLeaderboardFetchUrl(false),
            this.getOnlineLeaderboardFetchUrlWithoutReplay(false),
            this.getLegacyOnlineLeaderboardFetchUrl(true),
            this.getLegacyOnlineLeaderboardFetchUrl(false),
          ]
        : [
            this.getOnlineLeaderboardFetchUrlWithoutReplay(true),
            this.getOnlineLeaderboardFetchUrlWithoutReplay(false),
    ...
    async fetchOnlineLeaderboardReplayRow(recordKey) {
      const requestUrls = [
        this.getOnlineLeaderboardReplayFetchUrl(recordKey, true),
        this.getOnlineLeaderboardReplayFetchUrl(recordKey, false),
      ];
      let response = null;
      for (const url of requestUrls) {
        response = await fetch(url, {
    ...
    getMinValidLevelLeaderboardTimeMs(scope, levelIndex) {
      if (typeof scope !== "string" || !scope.startsWith("level_") || !Number.isInteger(levelIndex)) {
        return null;
      }
      const minimum = MIN_VALID_LEVEL_LEADERBOARD_TIMES_MS[levelIndex];
      return Number.isFinite(minimum) && minimum > 0 ? minimum : null;
    }

    isOnlineLeaderboardTimeAllowed(scope, levelIndex, timeMs) {
      const minimum = this.getMinValidLevelLeaderboardTimeMs(scope, levelIndex);
      return minimum == null || (Number.isFinite(timeMs) && timeMs >= minimum);
    }
    ...
    async submitOnlineLeaderboardRecord(scope, levelIndex, entry, period = "all_time", periodKey = this.getLeaderboardPeriodKey(period)) {
      ...
      const minimumTime = this.getMinValidLevelLeaderboardTimeMs(scope, levelIndex);
      if (minimumTime != null && !this.isOnlineLeaderboardTimeAllowed(scope, levelIndex, normalized.time)) {
        this.setOnlineLeaderboardStatus(`Ignored impossible online record below ${(minimumTime / 1000).toFixed(2)}s.`, "warn");
        return null;
      }
      ...
      const payload = {
        ...
        p_run_frames: this.normalizeReplayFrames(normalized.replayFrames, levelIndex),
      ...
      if (!response.ok && (response.status === 400 || response.status === 404)) {
        ...
        delete fallbackPayload.p_run_frames;
    ...
    applySignedInUsername(username, sessionToken = "") {
      ...
      this.signedInSessionToken = typeof sessionToken === "string" ? sessionToken : "";
      this.signOutConfirmOpen = false;
    ...
    logOutAccount() {
      ...
      if (this.accountProfileSavePending) {
        this.flushSignedInAccountProfileSave();
      }
      if (this.accountProfileSaveTimer != null) {
        clearTimeout(this.accountProfileSaveTimer);
    ...
    async submitAuthDialog() {
      ...
      try {
        ...
        if (status === "ok") {
          ...
          const sessionToken = typeof result?.session_token === "string" ? result.session_token : "";
          this.applySignedInUsername(accountUsername, sessionToken);
          if (result?.profile && typeof result.profile === "object") {
            this.applyAccountProfilePayload(result.profile);
          }
          this.savePersistentProgress();
          if (this.authMode === "signup") {
            this.scheduleSignedInAccountProfileSave(0);
          }
    ...
    async syncOnlineLeaderboard({ includeLocalSeed = false, force = false, includeReplay = false } = {}) {
      if (!this.isOnlineLeaderboardConfigured()) {
      ...
      const hasSyncedBefore = this.onlineLeaderboardLastSyncAt > 0;
      const hasCachedRows = this.onlineLeaderboardRowsByKey.size > 0;
      const leaderboardCacheFresh = this.isCacheFresh(this.onlineLeaderboardLastSyncAt, this.getOnlineLeaderboardCacheMaxAgeMs());
      const replayCacheFresh = !includeReplay || this.isCacheFresh(this.onlineLeaderboardReplayLastSyncAt, this.getOnlineLeaderboardReplayCacheMaxAgeMs());
      if (!force && hasSyncedBefore && leaderboardCacheFresh && replayCacheFresh) {
        this.setOnlineLeaderboardStatus("Online WR live", "online");
        return;
      }
      ...
      try {
        const rows = await this.fetchOnlineLeaderboardRows({ includeReplay });
        this.cacheOnlineLeaderboardRows(rows, true);
        this.applyOnlineLeaderboardRows(rows);
        if (includeLocalSeed && ALLOW_LOCAL_LEADERBOARD_SEEDING && !this.onlineLeaderboardSeededLocalRecords && !hasCachedRows) {
          await this.seedOnlineLeaderboardFromLocalRecords();
          this.onlineLeaderboardSeededLocalRecords = true;
          const refreshedRows = await this.fetchOnlineLeaderboardRows({ includeReplay });
          this.cacheOnlineLeaderboardRows(refreshedRows, true);
          this.applyOnlineLeaderboardRows(refreshedRows);
        } else if (includeLocalSeed) {
          this.onlineLeaderboardSeededLocalRecords = true;
        }
        this.onlineLeaderboardLastSyncAt = Date.now();
        if (includeReplay) {
          this.onlineLeaderboardReplayLastSyncAt = this.onlineLeaderboardLastSyncAt;
        }
        if (this.pruneOwnedUnnamedLeaderboardRecordKeys()) {
          this.savePersistentProgress();
        }
    ...
    canTrackOnlineCommunityLevelViews() {
      return this.isOnlineCommunityConfigured() &&
        typeof ONLINE_LEADERBOARD_CONFIG.incrementCommunityPlayRpcName === "string" &&
        ONLINE_LEADERBOARD_CONFIG.incrementCommunityPlayRpcName.trim().length > 0;
    }

    canUnpublishOnlineCommunityLevel() {
      return this.isOnlineCommunityConfigured() &&
        typeof ONLINE_LEADERBOARD_CONFIG.unpublishCommunityLevelRpcName === "string" &&
        ONLINE_LEADERBOARD_CONFIG.unpublishCommunityLevelRpcName.trim().length > 0;
    }

    getOnlineCommunityLevelsFetchUrl() {
      const params = new URLSearchParams({
        select: "level_id,level_name,creator_name,creator_time_ms,level_text,start_angle,leaderboard_json,approved_at,play_count",
        order: "play_count.desc,approved_at.desc",
      });
      return `${this.getOnlineLeaderboardBaseUrl()}/rest/v1/${ONLINE_LEADERBOARD_CONFIG.communityLevelsTableName}?${params.toString()}`;
    }

    getLegacyOnlineCommunityLevelsFetchUrl() {
      const params = new URLSearchParams({
    ...
    getOnlineCommunityPlayCountIncrementUrl() {
      return `${this.getOnlineLeaderboardBaseUrl()}/rest/v1/rpc/${ONLINE_LEADERBOARD_CONFIG.incrementCommunityPlayRpcName}`;
    }
    ...
    normalizeOnlineCommunityLevelRow(row) {
      ...
      return {
        ...
        reviewMessage: "",
        playCount: Number.isFinite(row?.play_count) ? Math.max(0, Math.floor(row.play_count)) : 0,
    ...
    normalizeMyCreatedLevelEntry(entry, fallbackStatus = "private") {
      ...
      const storedMap = this.parseStoredCreateLevelMapText(levelText);
      if (storedMap.error) {
        return null;
      }
      const hiddenMapSource = typeof entry?.hidden_level_text === "string"
        ? entry.hidden_level_text
        : typeof entry?.hiddenLevelText === "string"
          ? entry.hiddenLevelText
          : Array.isArray(entry?.hiddenMap) ? entry.hiddenMap.join("\n") : "";
      const hiddenMapParsed = hiddenMapSource
        ? this.parseStoredCreateLevelMapText(hiddenMapSource)
        : null;
      const parsed = this.parseCustomLevelText(levelText);
      ...
      const localIdCandidate = entry?.local_id ?? entry?.localId;
      const localId = typeof localIdCandidate === "string" && localIdCandidate.trim()
        ? localIdCandidate.trim()
        : (!remoteIdCandidate && !submissionIdCandidate
          ? `legacy-${Math.max(createdAt, updatedAt, 0) || `${creator}\n${name}\n${levelText}`}`
          : "");
      return {
        localId,
        ...
        creatorTime,
        myBestTime,
        startAngle: Number.isFinite(entry?.start_angle)
          ? entry.start_angle
          : Number.isFinite(entry?.startAngle)
            ? entry.startAngle
            : parsed.error ? 0 : parsed.definition.startAngle,
        leaderboard: normalizeCommunityLeaderboard(entry?.leaderboard_json ?? entry?.leaderboard, creator, creatorTime),
        map: storedMap.rows,
        hiddenMap: (hiddenMapParsed && !hiddenMapParsed.error ? hiddenMapParsed.rows : storedMap.rows),
        hiddenOriginX: Math.max(0, Math.floor(Number.isFinite(entry?.hidden_origin_x) ? entry.hidden_origin_x : (Number.isFinite(entry?.hiddenOriginX) ? entry.hiddenOriginX : 0))),
        hiddenOriginY: Math.max(0, Math.floor(Number.isFinite(entry?.hidden_origin_y) ? entry.hidden_origin_y : (Number.isFinite(entry?.hiddenOriginY) ? entry.hiddenOriginY : 0))),
        local: Boolean(entry?.local),
        submissionStatus,
        reviewMessage,
        playCount: Number.isFinite(entry?.play_count)
          ? Math.max(0, Math.floor(entry.play_count))
          : Number.isFinite(entry?.playCount) ? Math.max(0, Math.floor(entry.playCount)) : 0,
        createdAt,
        updatedAt,
      };
    }
    ...
    mergeMyCreatedLevelEntries(current, incoming) {
      ...
      return {
        ...current,
        ...incoming,
        localId: incoming.localId || current.localId || "",
        ...
        hiddenMap: Array.isArray(incoming.hiddenMap) && incoming.hiddenMap.length > 0 ? incoming.hiddenMap : current.hiddenMap,
        hiddenOriginX: Number.isFinite(incoming.hiddenOriginX) ? incoming.hiddenOriginX : current.hiddenOriginX,
        hiddenOriginY: Number.isFinite(incoming.hiddenOriginY) ? incoming.hiddenOriginY : current.hiddenOriginY,
        ...
        playCount: Math.max(
          Number.isFinite(current.playCount) ? Math.floor(current.playCount) : 0,
          Number.isFinite(incoming.playCount) ? Math.floor(incoming.playCount) : 0
        ),
        createdAt,
    ...
    upsertMyCreatedLevel(entry, { save = true, fallbackStatus = "private" } = {}) {
      ...
      const existingIndex = nextLevels.findIndex((candidate) => {
        ...
        return (
          (normalized.localId && normalizedCandidate.localId === normalized.localId) ||
    ...
    getMyCreatedLevelMatchIndex(level, source = this.myCreatedLevels) {
      if (!level || !Array.isArray(source)) {
        return -1;
      }
      ...
      return source.findIndex((candidate) => {
        ...
        return (
          (level.localId && normalizedCandidate.localId === level.localId) ||
          (level.submissionId && normalizedCandidate.submissionId === level.submissionId) ||
          (level.remoteId && normalizedCandidate.remoteId === level.remoteId) ||
          (signature && this.getCommunityLevelSignature(normalizedCandidate) === signature)
        );
      });
    }

    setActiveMyLevel(level) {
      const normalized = this.normalizeMyCreatedLevelEntry(level, level?.submissionStatus || "private");
      this.activeMyLevel = normalized ? { ...normalized } : null;
    }

    getActiveMyLevel() {
      if (!this.activeMyLevel) {
        return null;
      }
      const currentLevels = this.getMyCreatedLevels();
      const matchIndex = this.getMyCreatedLevelMatchIndex(this.activeMyLevel, currentLevels);
      if (matchIndex >= 0) {
        const match = currentLevels[matchIndex];
        this.activeMyLevel = { ...match };
        return match;
      }
      return this.activeMyLevel;
    }

    isEditingMyLevel() {
      return Boolean(this.editingMyLevel);
    }

    syncEditingMyLevelText(value, source = "custom") {
      const nextValue = String(value ?? "");
      this.customLevelTestText = nextValue;
      this.publishLevelText = nextValue;
      if (source !== "custom" && this.customLevelInput && document.activeElement !== this.customLevelInput) {
        this.customLevelInput.value = nextValue;
      }
      if (source !== "publish" && this.publishLevelInput && document.activeElement !== this.publishLevelInput) {
        this.publishLevelInput.value = nextValue;
      }
    }

    generateMyCreatedLevelLocalId() {
      return `local-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
    }

    parseStoredCreateLevelMapText(text) {
      const rows = String(text || "").replace(/\r/g, "").split("\n");
      if (rows.length < CREATE_LEVEL_MIN_SIZE) {
        return { error: `Add at least ${CREATE_LEVEL_MIN_SIZE} rows.` };
      }
      const width = rows[0]?.length || 0;
      if (width < CREATE_LEVEL_MIN_SIZE) {
        return { error: `Make the level at least ${CREATE_LEVEL_MIN_SIZE} columns wide.` };
      }
      if (!rows.every((row) => row.length === width)) {
        return { error: "Every row must be the same length." };
      }
      if (rows.length > CREATE_LEVEL_MAX_SIZE || width > CREATE_LEVEL_MAX_SIZE) {
        return { error: `Keep levels ${CREATE_LEVEL_MAX_SIZE} by ${CREATE_LEVEL_MAX_SIZE} tiles or smaller.` };
      }
      for (const row of rows) {
        for (const tile of row) {
          if (!"#.ISGXP ".includes(tile)) {
            return { error: `Invalid tile "${tile}".` };
          }
        }
      }
    ...
    applyImportedCreateLevelMapText(text, editor = this.createLevelEditor) {
      if (!editor) {
        return { error: "Open the level creator first." };
      }
      const rawText = String(text || "");
      if (!rawText.trim()) {
        return { error: "Paste a level map first." };
      }
      const parsed = this.parseStoredCreateLevelMapText(rawText);
      if (parsed.error) {
        return parsed;
      }
      const nextRows = this.mapRowsToCreateLevelRows(parsed.rows);
      editor.width = parsed.width;
      editor.height = parsed.height;
      editor.rows = nextRows;
      editor.storedRows = this.normalizeCreateLevelHiddenRows(nextRows, nextRows);
      editor.hiddenOriginX = 0;
      editor.hiddenOriginY = 0;
      editor.floorPatternOffsetX = 0;
      editor.floorPatternOffsetY = 0;
      editor.undoStack = [];
      editor.redoStack = [];
      editor.setupSource = "import";
      editor.importText = rawText.replace(/\r/g, "");
      editor.startAngle = this.snapCreateLevelAngle(editor.startAngle);
      editor.validatedTime = null;
      editor.validatedSignature = "";
      editor.angleEditActive = false;
      editor.lastSavedAt = 0;
      editor.lastChangeAt = Date.now();
      this.syncCreateLevelStoredRows(editor);
      this.customLevelTestText = parsed.rows.join("\n");
      this.publishLevelText = this.customLevelTestText;
      return parsed;
    }

    getNextCreateLevelDefaultName() {
      const usedNumbers = new Set();
      const pattern = /^new level (\d+)$/i;
      for (const level of this.myCreatedLevels || []) {
        const rawName = typeof level?.level_name === "string"
          ? level.level_name
      ...
      this.nextCreatedLevelNumber = number + 1;
      return `New Level ${number}`;
    }

    sanitizeCreateLevelSizeValue(value) {
      const text = String(value ?? "").replace(/[^\d]/g, "");
      if (!text) {
        return { raw: "", value: null };
      }
      const numeric = clamp(Number.parseInt(text, 10) || CREATE_LEVEL_DEFAULT_SIZE, CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      return {
        raw: String(numeric),
        value: numeric,
      };
    ...
    buildCreateLevelRows(width = CREATE_LEVEL_DEFAULT_SIZE, height = CREATE_LEVEL_DEFAULT_SIZE) {
      const safeWidth = clamp(Math.floor(Number.isFinite(width) ? width : CREATE_LEVEL_DEFAULT_SIZE), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const safeHeight = clamp(Math.floor(Number.isFinite(height) ? height : CREATE_LEVEL_DEFAULT_SIZE), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      return Array.from({ length: safeHeight }, (_, y) => (
        Array.from({ length: safeWidth }, (_, x) => (
          x === 0 || y === 0 || x === safeWidth - 1 || y === safeHeight - 1 ? "#" : "."
        ))
      ));
    }

    cloneCreateLevelRows(rows) {
      return Array.isArray(rows) ? rows.map((row) => Array.isArray(row) ? [...row] : []) : [];
    }

    mapRowsToCreateLevelRows(mapRows) {
      if (!Array.isArray(mapRows) || mapRows.length === 0) {
        return this.buildCreateLevelRows();
      }
      return mapRows.map((row) => String(row || "").split(""));
    }

    normalizeCreateLevelHiddenRows(sourceRows, visibleRows = null) {
      const fallbackRows = Array.isArray(visibleRows) && visibleRows.length > 0
        ? this.cloneCreateLevelRows(visibleRows)
        : this.buildCreateLevelRows();
      const parsedRows = Array.isArray(sourceRows) && sourceRows.length > 0
        ? sourceRows.map((row) => Array.isArray(row) ? row.map((tile) => String(tile || "").charAt(0) || ".") : String(row || "").split(""))
        : fallbackRows;
      let width = parsedRows.reduce((max, row) => Math.max(max, Array.isArray(row) ? row.length : 0), 0);
      let height = parsedRows.length;
      if (Array.isArray(visibleRows) && visibleRows.length > 0) {
        width = Math.max(width, visibleRows.reduce((max, row) => Math.max(max, Array.isArray(row) ? row.length : 0), 0));
        height = Math.max(height, visibleRows.length);
      }
      width = clamp(width || CREATE_LEVEL_DEFAULT_SIZE, CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      height = clamp(height || CREATE_LEVEL_DEFAULT_SIZE, CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const allowedTiles = new Set(["#", ".", "I", "S", "G", "X", "P", " "]);
      const normalized = Array.from({ length: height }, (_, y) => (
        Array.from({ length: width }, (_, x) => {
          const tile = parsedRows[y]?.[x];
          if (allowedTiles.has(tile)) {
            return tile;
          }
          const fallbackTile = fallbackRows[y]?.[x];
          return allowedTiles.has(fallbackTile) ? fallbackTile : ".";
        })
      ));
      if (Array.isArray(visibleRows) && visibleRows.length > 0) {
        for (let y = 0; y < Math.min(visibleRows.length, normalized.length); y += 1) {
          for (let x = 0; x < Math.min(visibleRows[y]?.length || 0, normalized[y].length); x += 1) {
            const tile = visibleRows[y]?.[x];
            if (allowedTiles.has(tile)) {
              normalized[y][x] = tile;
            }
          }
    ...
    getCreateLevelHiddenOrigin(editor = this.createLevelEditor) {
      const width = clamp(Math.floor(Number.isFinite(editor?.width) ? editor.width : CREATE_LEVEL_DEFAULT_SIZE), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const height = clamp(Math.floor(Number.isFinite(editor?.height) ? editor.height : CREATE_LEVEL_DEFAULT_SIZE), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      return {
        x: clamp(Math.floor(Number.isFinite(editor?.hiddenOriginX) ? editor.hiddenOriginX : 0), 0, Math.max(0, CREATE_LEVEL_MAX_SIZE - width)),
        y: clamp(Math.floor(Number.isFinite(editor?.hiddenOriginY) ? editor.hiddenOriginY : 0), 0, Math.max(0, CREATE_LEVEL_MAX_SIZE - height)),
      };
    }

    rebuildCreateLevelVisibleRowsFromStored(editor = this.createLevelEditor) {
      if (!editor) {
        return [];
      }
      const origin = this.getCreateLevelHiddenOrigin(editor);
      return Array.from({ length: editor.height }, (_, y) => (
        Array.from({ length: editor.width }, (_, x) => editor.storedRows?.[origin.y + y]?.[origin.x + x] ?? ".")
      ));
    }

    syncCreateLevelStoredRows(editor = this.createLevelEditor, { minStoredWidth = null, minStoredHeight = null } = {}) {
      if (!editor) {
        return;
      }
      const origin = this.getCreateLevelHiddenOrigin(editor);
      editor.hiddenOriginX = origin.x;
      editor.hiddenOriginY = origin.y;
      editor.storedRows = this.normalizeCreateLevelHiddenRows(editor.storedRows);
      const requiredWidth = Math.max(
        editor.hiddenOriginX + editor.width,
        Number.isFinite(minStoredWidth) ? Math.floor(minStoredWidth) : 0
      );
      const requiredHeight = Math.max(
        editor.hiddenOriginY + editor.height,
        Number.isFinite(minStoredHeight) ? Math.floor(minStoredHeight) : 0
      );
      const storedWidth = clamp(Math.max(editor.storedRows[0]?.length || 0, requiredWidth), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const storedHeight = clamp(Math.max(editor.storedRows.length || 0, requiredHeight), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      while (editor.storedRows.length < storedHeight) {
        editor.storedRows.push(Array.from({ length: storedWidth }, () => "."));
      }
      for (const row of editor.storedRows) {
        while (row.length < storedWidth) {
          row.push(".");
        }
      }
      for (let y = 0; y < editor.height; y += 1) {
        for (let x = 0; x < editor.width; x += 1) {
          const tile = editor.rows?.[y]?.[x];
          if (tile != null && editor.storedRows[editor.hiddenOriginY + y]?.[editor.hiddenOriginX + x] != null) {
            editor.storedRows[editor.hiddenOriginY + y][editor.hiddenOriginX + x] = tile;
          }
        }
      }
      editor.rows = this.rebuildCreateLevelVisibleRowsFromStored(editor);
    }

    getCreateLevelEditorAngleStep() {
      return Math.PI / 4;
    }

    snapCreateLevelAngle(angle) {
      const step = this.getCreateLevelEditorAngleStep();
      const normalized = Math.round((Number.isFinite(angle) ? angle : 0) / step) * step;
      ...
      return Math.round(wrapped * 100000) / 100000;
    }

    buildCreateLevelEditorSignature(editor = this.createLevelEditor) {
      if (!editor) {
        return "";
      }
      const rows = this.getCreateLevelRowsAsStrings(editor);
      return `${editor.width}x${editor.height}\n${this.snapCreateLevelAngle(editor.startAngle)}\n${rows.join("\n")}`;
    }

    createLevelEditorHasPendingChangesSinceValidation(editor = this.createLevelEditor) {
      if (!editor || !editor.validatedSignature) {
        return true;
      }
      return this.buildCreateLevelEditorSignature(editor) !== editor.validatedSignature;
    }

    clearCreateLevelValidation(editor = this.createLevelEditor) {
      if (!editor) {
        return;
      }
      editor.validatedTime = null;
      editor.validatedSignature = "";
      if (this.pendingCommunityLevel && this.currentScreen === "create_level") {
        this.pendingCommunityLevel.bestTime = null;
      }
    }

    getCreateLevelRowsAsStrings(editor = this.createLevelEditor) {
      if (!editor) {
        return [];
      }
      return editor.rows.map((row) => row.slice(0, editor.width).join(""));
    }

    getCreateLevelStartTile(editor = this.createLevelEditor) {
      if (!editor) {
        return null;
      }
      for (let y = 0; y < editor.height; y += 1) {
        for (let x = 0; x < editor.width; x += 1) {
          if (editor.rows[y]?.[x] === "S") {
            return { x, y };
          }
        }
      }
    ...
    getCreateLevelGoalTile(editor = this.createLevelEditor) {
      if (!editor) {
        return null;
      }
      for (let y = 0; y < editor.height; y += 1) {
        for (let x = 0; x < editor.width; x += 1) {
          if (editor.rows[y]?.[x] === "G") {
            return { x, y };
          }
        }
      }
    ...
    createCreateLevelEditorState({ name = "", mapRows = null, hiddenMapRows = null, hiddenOriginX = 0, hiddenOriginY = 0, startAngle = 0, submissionStatus = "private", validatedTime = null, setupSource = "blank", importText = "" } = {}) {
      const rows = mapRows ? this.mapRowsToCreateLevelRows(mapRows) : this.buildCreateLevelRows();
      const width = clamp(rows[0]?.length || CREATE_LEVEL_DEFAULT_SIZE, CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const height = clamp(rows.length || CREATE_LEVEL_DEFAULT_SIZE, CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const safeRows = rows.map((row) => {
        const nextRow = row.slice(0, width);
        while (nextRow.length < width) {
          nextRow.push(nextRow.length === 0 || nextRow.length === width - 1 ? "#" : ".");
        }
        return nextRow;
      }).slice(0, height);
      while (safeRows.length < height) {
        safeRows.push(Array.from({ length: width }, (_, x) => (
          x === 0 || x === width - 1 || safeRows.length === 0 || safeRows.length === height - 1 ? "#" : "."
        )));
      }
      const editor = {
        mode: mapRows ? "editor" : "setup",
        name: normalizeCommunityLevelName(name) || this.getNextCreateLevelDefaultName(),
        width,
        height,
        rows: safeRows,
        startAngle: this.snapCreateLevelAngle(startAngle),
        activeTab: "walls",
        selectedPaletteId: "wall",
        paletteScrollByTab: { walls: 0, floors: 0 },
        mouseMode: "move",
        storedRows: this.normalizeCreateLevelHiddenRows(hiddenMapRows, safeRows),
        hiddenOriginX: Math.max(0, Math.floor(Number.isFinite(hiddenOriginX) ? hiddenOriginX : 0)),
        hiddenOriginY: Math.max(0, Math.floor(Number.isFinite(hiddenOriginY) ? hiddenOriginY : 0)),
        floorPatternOffsetX: 0,
        floorPatternOffsetY: 0,
        cameraX: width * TILE_SIZE * 0.5,
        cameraY: height * TILE_SIZE * 0.5,
        zoom: 1,
        status: normalizeCommunitySubmissionStatus(submissionStatus || "private"),
        validatedTime: Number.isFinite(validatedTime) ? validatedTime : null,
        validatedSignature: "",
        messageText: "",
        messageTone: "neutral",
        setupSource: setupSource === "import" ? "import" : "blank",
        importText: String(importText || ""),
        menuOpen: false,
        exitConfirmOpen: false,
        publishPanelOpen: false,
        deleteConfirmOpen: false,
        angleEditActive: false,
        savedDraftSignature: "",
        lastSavedAt: 0,
        lastChangeAt: 0,
        undoStack: [],
      ...
      if (Number.isFinite(editor.validatedTime)) {
        editor.validatedSignature = this.buildCreateLevelEditorSignature(editor);
      }
      this.syncCreateLevelStoredRows(editor);
      return editor;
    }

    isCreateLevelEditorLocked(editor = this.createLevelEditor) {
      return Boolean(editor && editor.status === "pending");
    }

    captureCreateLevelEditorSnapshot(editor = this.createLevelEditor) {
      if (!editor) {
        return null;
      }
      this.syncCreateLevelStoredRows(editor);
      return {
        name: normalizeCommunityLevelName(editor.name) || "",
        width: editor.width,
        height: editor.height,
        rows: this.cloneCreateLevelRows(editor.rows),
        storedRows: this.cloneCreateLevelRows(editor.storedRows),
        hiddenOriginX: this.getCreateLevelHiddenOrigin(editor).x,
        hiddenOriginY: this.getCreateLevelHiddenOrigin(editor).y,
        floorPatternOffsetX: Math.floor(Number.isFinite(editor.floorPatternOffsetX) ? editor.floorPatternOffsetX : 0),
        floorPatternOffsetY: Math.floor(Number.isFinite(editor.floorPatternOffsetY) ? editor.floorPatternOffsetY : 0),
        startAngle: this.snapCreateLevelAngle(editor.startAngle),
        activeTab: editor.activeTab === "floors" || editor.activeTab === "erase" ? editor.activeTab : "walls",
        selectedPaletteId: editor.selectedPaletteId || "",
        mouseMode: editor.mouseMode === "drag" ? "drag" : "move",
        validatedTime: Number.isFinite(editor.validatedTime) ? editor.validatedTime : null,
        validatedSignature: String(editor.validatedSignature || ""),
      };
    }

    restoreCreateLevelEditorSnapshot(snapshot, editor = this.createLevelEditor) {
      if (!editor || !snapshot) {
        return false;
      }
      editor.name = normalizeCommunityLevelName(snapshot.name) || this.getNextCreateLevelDefaultName();
      editor.width = clamp(Math.floor(Number.isFinite(snapshot.width) ? snapshot.width : editor.width), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      editor.height = clamp(Math.floor(Number.isFinite(snapshot.height) ? snapshot.height : editor.height), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      editor.rows = this.cloneCreateLevelRows(snapshot.rows).slice(0, editor.height).map((row) => row.slice(0, editor.width));
      while (editor.rows.length < editor.height) {
        editor.rows.push(Array.from({ length: editor.width }, () => "."));
      }
      for (const row of editor.rows) {
        while (row.length < editor.width) {
          row.push(".");
        }
      }
      editor.storedRows = this.normalizeCreateLevelHiddenRows(snapshot.storedRows);
      editor.hiddenOriginX = Math.max(0, Math.floor(Number.isFinite(snapshot.hiddenOriginX) ? snapshot.hiddenOriginX : 0));
      editor.hiddenOriginY = Math.max(0, Math.floor(Number.isFinite(snapshot.hiddenOriginY) ? snapshot.hiddenOriginY : 0));
      editor.floorPatternOffsetX = Math.floor(Number.isFinite(snapshot.floorPatternOffsetX) ? snapshot.floorPatternOffsetX : 0);
      editor.floorPatternOffsetY = Math.floor(Number.isFinite(snapshot.floorPatternOffsetY) ? snapshot.floorPatternOffsetY : 0);
      this.syncCreateLevelStoredRows(editor);
      editor.startAngle = this.snapCreateLevelAngle(snapshot.startAngle);
      editor.activeTab = snapshot.activeTab === "floors" || snapshot.activeTab === "erase" ? snapshot.activeTab : "walls";
      editor.selectedPaletteId = snapshot.selectedPaletteId || (editor.activeTab === "walls" ? "wall" : "floor");
      editor.mouseMode = snapshot.mouseMode === "drag" ? "drag" : "move";
      editor.validatedTime = Number.isFinite(snapshot.validatedTime) ? snapshot.validatedTime : null;
      editor.validatedSignature = editor.validatedTime != null
        ? (String(snapshot.validatedSignature || "") || this.buildCreateLevelEditorSignature(editor))
        : "";
      editor.angleEditActive = false;
      editor.lastSavedAt = 0;
      editor.lastChangeAt = Date.now();
      this.clampCreateLevelCamera(editor);
      this.clampCreateLevelPaletteScroll();
      this.syncCreateLevelEditorMetaInputs();
      this.refreshCreateLevelHover();
      return true;
    }

    pushCreateLevelUndoSnapshot(editor = this.createLevelEditor) {
      if (!editor) {
        return false;
      }
      const snapshot = this.captureCreateLevelEditorSnapshot(editor);
      if (!snapshot) {
        return false;
      }
    ...
    undoCreateLevelEditor(editor = this.createLevelEditor) {
      if (!editor || this.isCreateLevelEditorLocked(editor) || editor.undoStack.length <= 0) {
        return false;
      }
      const snapshot = editor.undoStack.pop();
      const current = this.captureCreateLevelEditorSnapshot(editor);
      if (current) {
        editor.redoStack.push(current);
      }
      return this.restoreCreateLevelEditorSnapshot(snapshot, editor);
    }

    redoCreateLevelEditor(editor = this.createLevelEditor) {
      if (!editor || this.isCreateLevelEditorLocked(editor) || editor.redoStack.length <= 0) {
        return false;
      }
    ...
    getCreateLevelSelectedPaletteItem(editor = this.createLevelEditor) {
      if (!editor) {
        return null;
      }
      if (editor.activeTab === "erase") {
        return { id: "erase", label: "Erase", tile: CREATE_LEVEL_VOID_TILE };
      }
      const palette = editor.activeTab === "walls" ? CREATE_LEVEL_WALL_PALETTE : CREATE_LEVEL_FLOOR_PALETTE;
      return palette.find((item) => item.id === editor.selectedPaletteId) || palette[0] || null;
    }

    getCreateLevelPaletteItems(tab = this.createLevelEditor?.activeTab) {
      if (tab === "walls") {
        return CREATE_LEVEL_WALL_PALETTE;
      }
      if (tab === "floors") {
        return CREATE_LEVEL_FLOOR_PALETTE;
      }
      return [];
    }

    setCreateLevelEditorMessage(text, tone = "neutral") {
      if (!this.createLevelEditor) {
        return;
      }
      this.createLevelEditor.messageText = String(text || "");
      this.createLevelEditor.messageTone = tone;
    }

    markCreateLevelEditorChanged({ clearValidation = true } = {}) {
      if (!this.createLevelEditor) {
        return;
      }
      if (clearValidation) {
        this.clearCreateLevelValidation(this.createLevelEditor);
      }
      this.createLevelEditor.lastSavedAt = 0;
      this.createLevelEditor.lastChangeAt = Date.now();
    }

    getCreateLevelEditorTile(editor, gridX, gridY) {
      if (!editor || gridX < 0 || gridY < 0 || gridX >= editor.width || gridY >= editor.height) {
        return null;
      }
      return editor.rows[gridY]?.[gridX] ?? null;
    }

    setCreateLevelEditorTile(editor, gridX, gridY, nextTile) {
      if (!editor || gridX < 0 || gridY < 0 || gridX >= editor.width || gridY >= editor.height) {
        return false;
      }
      const origin = this.getCreateLevelHiddenOrigin(editor);
      const currentTile = editor.rows[gridY]?.[gridX];
      if (currentTile === nextTile) {
        return false;
      }
      if (nextTile === "S" || nextTile === "G") {
        const storedRows = Array.isArray(editor.storedRows) && editor.storedRows.length > 0 ? editor.storedRows : editor.rows;
        for (let y = 0; y < storedRows.length; y += 1) {
          for (let x = 0; x < (storedRows[y]?.length || 0); x += 1) {
            if (storedRows[y]?.[x] === nextTile) {
              storedRows[y][x] = ".";
              const visibleX = x - origin.x;
              const visibleY = y - origin.y;
              if (editor.rows?.[visibleY]?.[visibleX] != null) {
                editor.rows[visibleY][visibleX] = ".";
              }
            }
          }
        }
      }
      editor.rows[gridY][gridX] = nextTile;
      if (editor.storedRows?.[origin.y + gridY]?.[origin.x + gridX] != null) {
        editor.storedRows[origin.y + gridY][origin.x + gridX] = nextTile;
      }
      return true;
    }

    resizeCreateLevelEditor(editor, nextWidth, nextHeight) {
      if (!editor) {
        return;
      }
      const width = clamp(Math.floor(Number.isFinite(nextWidth) ? nextWidth : editor.width), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const height = clamp(Math.floor(Number.isFinite(nextHeight) ? nextHeight : editor.height), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      ...
      editor.hiddenOriginX = clamp(editor.hiddenOriginX, 0, Math.max(0, CREATE_LEVEL_MAX_SIZE - editor.width));
      editor.hiddenOriginY = clamp(editor.hiddenOriginY, 0, Math.max(0, CREATE_LEVEL_MAX_SIZE - editor.height));
      this.syncCreateLevelStoredRows(editor);
      editor.cameraX = clamp(editor.cameraX, 0, width * TILE_SIZE);
      editor.cameraY = clamp(editor.cameraY, 0, height * TILE_SIZE);
      editor.paletteScrollByTab.walls = 0;
      editor.paletteScrollByTab.floors = 0;
      this.markCreateLevelEditorChanged();
      this.syncCreateLevelEditorMetaInputs();
    }

    captureCreateLevelResizeSnapshot(editor = this.createLevelEditor) {
      if (!editor) {
        return null;
      }
      this.syncCreateLevelStoredRows(editor);
      ...
      return {
        ...
        rows: this.cloneCreateLevelRows(editor.rows),
        storedRows: this.cloneCreateLevelRows(editor.storedRows),
        hiddenOriginX: origin.x,
        hiddenOriginY: origin.y,
        floorPatternOffsetX: Math.floor(Number.isFinite(editor.floorPatternOffsetX) ? editor.floorPatternOffsetX : 0),
        floorPatternOffsetY: Math.floor(Number.isFinite(editor.floorPatternOffsetY) ? editor.floorPatternOffsetY : 0),
    ...
    resizeCreateLevelEditorFromSnapshot(editor, snapshot, { left = 0, top = 0, right = snapshot?.width || 0, bottom = snapshot?.height || 0 } = {}) {
      if (!editor || !snapshot) {
        return false;
      }
      const snapshotOriginX = clamp(Math.floor(Number.isFinite(snapshot.hiddenOriginX) ? snapshot.hiddenOriginX : 0), 0, Math.max(0, CREATE_LEVEL_MAX_SIZE - snapshot.width));
      const snapshotOriginY = clamp(Math.floor(Number.isFinite(snapshot.hiddenOriginY) ? snapshot.hiddenOriginY : 0), 0, Math.max(0, CREATE_LEVEL_MAX_SIZE - snapshot.height));
      const nextWidth = clamp(Math.floor(right - left), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const nextHeight = clamp(Math.floor(bottom - top), CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      if (
        editor.width === snapshot.width &&
        editor.height === snapshot.height &&
        nextWidth === snapshot.width &&
        nextHeight === snapshot.height &&
        left === 0 &&
        top === 0 &&
        snapshotOriginX === (editor.hiddenOriginX || 0) &&
        snapshotOriginY === (editor.hiddenOriginY || 0)
      ) {
        return false;
      }
      const snapshotStoredRows = this.normalizeCreateLevelHiddenRows(snapshot.storedRows);
      const snapshotStoredWidth = Math.max(snapshotStoredRows[0]?.length || 0, snapshotOriginX + snapshot.width);
      const snapshotStoredHeight = Math.max(snapshotStoredRows.length || 0, snapshotOriginY + snapshot.height);
      const nextOriginXRaw = snapshotOriginX + left;
      const nextOriginYRaw = snapshotOriginY + top;
      const nextVisibleRightRaw = snapshotOriginX + right;
      const nextVisibleBottomRaw = snapshotOriginY + bottom;
      const padLeft = Math.max(0, -nextOriginXRaw);
      const padTop = Math.max(0, -nextOriginYRaw);
      const padRight = Math.max(0, nextVisibleRightRaw - snapshotStoredWidth);
      const padBottom = Math.max(0, nextVisibleBottomRaw - snapshotStoredHeight);
      const nextStoredWidth = clamp(snapshotStoredWidth + padLeft + padRight, CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const nextStoredHeight = clamp(snapshotStoredHeight + padTop + padBottom, CREATE_LEVEL_MIN_SIZE, CREATE_LEVEL_MAX_SIZE);
      const nextStoredRows = Array.from({ length: nextStoredHeight }, (_, y) => (
        Array.from({ length: nextStoredWidth }, (_, x) => {
          const sourceX = x - padLeft;
          const sourceY = y - padTop;
          if (
            sourceX >= 0 &&
            sourceY >= 0 &&
            sourceX < snapshotStoredWidth &&
            sourceY < snapshotStoredHeight
          ) {
            return snapshotStoredRows[sourceY]?.[sourceX] ?? ".";
          }
          return ".";
        })
      ));
      ...
      editor.storedRows = nextStoredRows;
      editor.hiddenOriginX = clamp(nextOriginXRaw + padLeft, 0, Math.max(0, nextStoredWidth - nextWidth));
      editor.hiddenOriginY = clamp(nextOriginYRaw + padTop, 0, Math.max(0, nextStoredHeight - nextHeight));
      editor.floorPatternOffsetX = Math.floor(Number.isFinite(snapshot.floorPatternOffsetX) ? snapshot.floorPatternOffsetX : 0) + left;
      editor.floorPatternOffsetY = Math.floor(Number.isFinite(snapshot.floorPatternOffsetY) ? snapshot.floorPatternOffsetY : 0) + top;
      editor.rows = this.rebuildCreateLevelVisibleRowsFromStored(editor);
      editor.cameraX = snapshot.cameraX - left * TILE_SIZE;
      editor.cameraY = snapshot.cameraY - top * TILE_SIZE;
      editor.zoom = snapshot.zoom;
      this.markCreateLevelEditorChanged();
      this.clampCreateLevelCamera(editor);
      this.syncCreateLevelEditorMetaInputs();
      return true;
    ...
    getCreateLevelResizeEdgeHotspots(layout = this.getCreateLevelLayout(), editor = this.createLevelEditor) {
      if (!editor || !layout || layout.mode !== "editor" || editor.menuOpen || editor.publishPanelOpen || editor.exitConfirmOpen || editor.deleteConfirmOpen) {
        return [];
      }
      const bounds = this.getCreateLevelEditorWorldBounds(editor);
      const topLeft = this.createLevelWorldToScreen(bounds.x, bounds.y, layout.viewport, editor);
      const bottomRight = this.createLevelWorldToScreen(bounds.x + bounds.w, bounds.y + bounds.h, layout.viewport, editor);
      const threshold = Math.max(10, Math.round(12 * layout.scale));
      const verticalInset = Math.min(Math.max(12, threshold), Math.max(0, (bottomRight.y - topLeft.y) * 0.18));
      const horizontalInset = Math.min(Math.max(12, threshold), Math.max(0, (bottomRight.x - topLeft.x) * 0.18));
      return [
        {
          edge: "left",
          cursor: "ew-resize",
          ...
          y2: bottomRight.y,
          rect: {
            x: topLeft.x - threshold,
            y: topLeft.y + verticalInset,
            w: threshold * 2,
            h: Math.max(0, bottomRight.y - topLeft.y - verticalInset * 2),
          },
        },
        {
          edge: "right",
        ...
        },
        {
          ...
          y2: topLeft.y,
          rect: {
            x: topLeft.x + horizontalInset,
            y: topLeft.y - threshold,
            w: Math.max(0, bottomRight.x - topLeft.x - horizontalInset * 2),
            h: threshold * 2,
          },
        },
        {
          ...
          rect: {
            x: topLeft.x + horizontalInset,
            y: bottomRight.y - threshold,
            w: Math.max(0, bottomRight.x - topLeft.x - horizontalInset * 2),
            h: threshold * 2,
          },
        },
      ].filter((hotspot) => hotspot.rect.w > 0 && hotspot.rect.h > 0);
    }

    getCreateLevelResizeEdgeTarget(x, y, layout = this.getCreateLevelLayout(), editor = this.createLevelEditor) {
      const candidates = this.getCreateLevelResizeEdgeHotspots(layout, editor).filter((hotspot) => this.isPointInsideRect(x, y, hotspot.rect));
      if (candidates.length <= 0) {
        return null;
      }
      candidates.sort((a, b) => {
        const distanceA = a.edge === "left" || a.edge === "right" ? Math.abs(x - a.x1) : Math.abs(y - a.y1);
        const distanceB = b.edge === "left" || b.edge === "right" ? Math.abs(x - b.x1) : Math.abs(y - b.y1);
        return distanceA - distanceB;
      });
      return { type: "resize_edge", edge: candidates[0].edge, cursor: candidates[0].cursor };
    }

    applyCreateLevelResizeEdgeDrag(gesture, screenX, screenY, layout = this.getCreateLevelLayout(), editor = this.createLevelEditor) {
      if (!gesture || !editor || !layout || layout.mode !== "editor" || gesture.mode !== "resize_edge" || !gesture.resizeSnapshot) {
        return false;
      }
      const resizeViewport = gesture.resizeViewport || layout.viewport;
      const referenceEditor = {
        cameraX: gesture.resizeSnapshot.cameraX,
        cameraY: gesture.resizeSnapshot.cameraY,
        zoom: gesture.resizeSnapshot.zoom,
      };
      const world = this.createLevelScreenToWorld(screenX, screenY, resizeViewport, referenceEditor);
      let left = 0;
      let top = 0;
      let right = gesture.resizeSnapshot.width;
      let bottom = gesture.resizeSnapshot.height;
      if (gesture.edge === "left") {
        left = clamp(
          Math.round(world.x / TILE_SIZE),
          right - CREATE_LEVEL_MAX_SIZE,
          gesture.resizeSnapshot.width - CREATE_LEVEL_MIN_SIZE
        );
      } else if (gesture.edge === "right") {
        right = clamp(
          Math.round(world.x / TILE_SIZE),
          CREATE_LEVEL_MIN_SIZE,
          left + CREATE_LEVEL_MAX_SIZE
        );
      } else if (gesture.edge === "top") {
        top = clamp(
          Math.round(world.y / TILE_SIZE),
          bottom - CREATE_LEVEL_MAX_SIZE,
          gesture.resizeSnapshot.height - CREATE_LEVEL_MIN_SIZE
        );
      } else if (gesture.edge === "bottom") {
        bottom = clamp(
          Math.round(world.y / TILE_SIZE),
          CREATE_LEVEL_MIN_SIZE,
          top + CREATE_LEVEL_MAX_SIZE
        );
      }
      const changed = this.resizeCreateLevelEditorFromSnapshot(editor, gesture.resizeSnapshot, { left, top, right, bottom });
      if (changed) {
        gesture.moved = true;
      }
      return changed;
    ...
    syncCreateLevelEditorMetaInputs() {
      if (!this.createLevelEditor) {
        return;
      }
      this.publishLevelName = normalizeCommunityLevelName(this.createLevelEditor.name);
      if (document.activeElement !== this.publishLevelNameInput) {
        this.publishLevelNameInput.value = this.publishLevelName;
      }
      if (document.activeElement !== this.createLevelSizeXInput) {
        this.createLevelSizeXInput.value = String(this.createLevelEditor.width);
      }
      if (document.activeElement !== this.createLevelSizeYInput) {
        this.createLevelSizeYInput.value = String(this.createLevelEditor.height);
      }
    }

    applyCreateLevelSizeInput(axis) {
      if (!this.createLevelEditor || this.isCreateLevelEditorLocked()) {
        this.syncCreateLevelEditorMetaInputs();
        return;
      }
      const input = axis === "height" ? this.createLevelSizeYInput : this.createLevelSizeXInput;
      const sanitized = this.sanitizeCreateLevelSizeValue(input.value);
      const fallback = axis === "height" ? this.createLevelEditor.height : this.createLevelEditor.width;
      const nextValue = sanitized.value == null ? fallback : sanitized.value;
      input.value = String(nextValue);
      this.pushCreateLevelUndoSnapshot(this.createLevelEditor);
      this.resizeCreateLevelEditor(
        this.createLevelEditor,
        axis === "width" ? nextValue : this.createLevelEditor.width,
        axis === "height" ? nextValue : this.createLevelEditor.height
      );
    }

    getCreateLevelEditorWorldBounds(editor = this.createLevelEditor) {
      if (!editor) {
      ...
      return {
        x: 0,
        y: 0,
        w: editor.width * TILE_SIZE,
        h: editor.height * TILE_SIZE,
      };
    }

    centerCreateLevelCamera(editor = this.createLevelEditor) {
      if (!editor) {
        return;
      }
      const bounds = this.getCreateLevelEditorWorldBounds(editor);
      editor.cameraX = bounds.w * 0.5;
      editor.cameraY = bounds.h * 0.5;
    }

    clampCreateLevelCamera(editor = this.createLevelEditor, viewport = null) {
      if (!editor) {
        return;
      }
      const layout = viewport || this.getCreateLevelLayout();
      const view = layout?.mode === "editor" ? layout.viewport : viewport;
      if (!view) {
        return;
      }
      const bounds = this.getCreateLevelEditorWorldBounds(editor);
      const halfW = view.w * 0.5 / Math.max(0.001, editor.zoom);
      const halfH = view.h * 0.5 / Math.max(0.001, editor.zoom);
      editor.cameraX = clamp(editor.cameraX, -halfW, bounds.w + halfW);
      editor.cameraY = clamp(editor.cameraY, -halfH, bounds.h + halfH);
    }

    zoomCreateLevelCamera(deltaSteps, anchorX = null, anchorY = null) {
      const editor = this.createLevelEditor;
      if (!editor || editor.mode !== "editor") {
        return;
      }
      const layout = this.getCreateLevelLayout();
      if (!layout || layout.mode !== "editor") {
        return;
      }
      const before = editor.zoom;
      const target = clamp(before * Math.pow(1.12, deltaSteps), 0.28, 2.6);
      if (Math.abs(target - before) < 0.0001) {
        return;
      }
      if (Number.isFinite(anchorX) && Number.isFinite(anchorY) && this.isPointInsideRect(anchorX, anchorY, layout.viewport)) {
        const world = this.createLevelScreenToWorld(anchorX, anchorY, layout.viewport, editor);
        editor.zoom = target;
        editor.cameraX = world.x - (anchorX - (layout.viewport.x + layout.viewport.w * 0.5)) / editor.zoom;
        editor.cameraY = world.y - (anchorY - (layout.viewport.y + layout.viewport.h * 0.5)) / editor.zoom;
      } else {
        editor.zoom = target;
      }
      this.clampCreateLevelCamera(editor, layout);
    }

    createLevelScreenToWorld(screenX, screenY, viewport, editor = this.createLevelEditor) {
      const localX = screenX - (viewport.x + viewport.w * 0.5);
      const localY = screenY - (viewport.y + viewport.h * 0.5);
      return {
        x: editor.cameraX + localX / Math.max(0.001, editor.zoom),
        y: editor.cameraY + localY / Math.max(0.001, editor.zoom),
      };
    }

    createLevelWorldToScreen(worldX, worldY, viewport, editor = this.createLevelEditor) {
      return {
        x: viewport.x + viewport.w * 0.5 + (worldX - editor.cameraX) * editor.zoom,
        y: viewport.y + viewport.h * 0.5 + (worldY - editor.cameraY) * editor.zoom,
      };
    }

    getCreateLevelGridAtScreen(screenX, screenY, layout = this.getCreateLevelLayout(), editor = this.createLevelEditor) {
      if (!editor || !layout || layout.mode !== "editor" || !this.isPointInsideRect(screenX, screenY, layout.viewport)) {
        return null;
      }
      const world = this.createLevelScreenToWorld(screenX, screenY, layout.viewport, editor);
      return {
        worldX: world.x,
        worldY: world.y,
        gridX: Math.floor(world.x / TILE_SIZE),
        gridY: Math.floor(world.y / TILE_SIZE),
      };
    }

    canPlaceCreateLevelAt(gridX, gridY, editor = this.createLevelEditor) {
      return Boolean(editor && gridX >= 0 && gridY >= 0 && gridX < editor.width && gridY < editor.height);
    }

    continueCreateLevelPaintGesture(gesture, gridX, gridY) {
      if (!gesture || !Number.isInteger(gridX) || !Number.isInteger(gridY)) {
        return false;
      }
      const startX = Number.isInteger(gesture.lastGridX) ? gesture.lastGridX : gridX;
      const startY = Number.isInteger(gesture.lastGridY) ? gesture.lastGridY : gridY;
      const dx = gridX - startX;
      const dy = gridY - startY;
      const steps = Math.max(Math.abs(dx), Math.abs(dy), 1);
      let previousX = Number.NaN;
      let previousY = Number.NaN;
      let changed = false;
      if (!gesture.historyCommitted) {
        this.pushCreateLevelUndoSnapshot();
        gesture.historyCommitted = true;
      }
      for (let step = 0; step <= steps; step += 1) {
        const nextX = Math.round(startX + (dx * step) / steps);
        const nextY = Math.round(startY + (dy * step) / steps);
        if (nextX === previousX && nextY === previousY) {
          continue;
        }
        previousX = nextX;
        previousY = nextY;
        changed = this.tryApplyCreateLevelToolAt(nextX, nextY) || changed;
      }
      gesture.lastGridX = gridX;
      gesture.lastGridY = gridY;
      gesture.applied = gesture.applied || changed;
      return changed;
    }

    tryApplyCreateLevelToolAt(gridX, gridY) {
      const editor = this.createLevelEditor;
      if (!editor || editor.mode !== "editor") {
        return false;
      }
      if (!this.canPlaceCreateLevelAt(gridX, gridY, editor)) {
        this.setCreateLevelEditorMessage("Increase the build area to place here.", "error");
        return false;
      }
      if (this.isCreateLevelEditorLocked(editor)) {
        this.setCreateLevelEditorMessage("Pending levels cannot be edited until you unpublish them.", "error");
        return false;
      }
      const paletteItem = this.getCreateLevelSelectedPaletteItem(editor);
      const nextTile = paletteItem?.tile ?? ".";
      if (!this.setCreateLevelEditorTile(editor, gridX, gridY, nextTile)) {
        return false;
      }
      editor.angleEditActive = nextTile === "S" || editor.angleEditActive;
      this.markCreateLevelEditorChanged();
      this.setCreateLevelEditorMessage("", "neutral");
      return true;
    }

    buildCreateLevelDefinitionFromEditor(editor = this.createLevelEditor) {
      if (!editor) {
        return { error: "Open the level editor first." };
      }
      const rows = this.getCreateLevelRowsAsStrings(editor);
      let startCount = 0;
      let goalCount = 0;
      ...
      for (let y = 0; y < editor.height; y += 1) {
        for (let x = 0; x < editor.width; x += 1) {
          const tile = editor.rows[y]?.[x];
          if (tile === CREATE_LEVEL_VOID_TILE) {
            hasVoid = true;
          } else if (!"#.ISGXP".includes(tile)) {
            invalidTile = tile;
          }
          if (tile === "S") {
            startCount += 1;
      ...
      if (invalidTile) {
        return { error: `Invalid tile "${invalidTile}".` };
      }
      if (hasVoid) {
        return { error: "Fill every tile inside the build area before testing." };
      }
      if (startCount !== 1) {
        return { error: "There must be exactly one start tile." };
      }
      if (goalCount !== 1) {
        return { error: "There must be exactly one goal tile." };
      }
      for (let x = 0; x < editor.width; x += 1) {
        const top = editor.rows[0]?.[x];
        const bottom = editor.rows[editor.height - 1]?.[x];
          if (!(top === "#" || top === "P" || top === "G" || top === "X") || !(bottom === "#" || bottom === "P" || bottom === "G" || bottom === "X")) {
            return { error: "The border must be fully sealed with wall blocks." };
          }
        }
      for (let y = 0; y < editor.height; y += 1) {
        const left = editor.rows[y]?.[0];
        const right = editor.rows[y]?.[editor.width - 1];
        if (!(left === "#" || left === "P" || left === "G" || left === "X") || !(right === "#" || right === "P" || right === "G" || right === "X")) {
          return { error: "The border must be fully sealed with wall blocks." };
        }
      }
      const definition = {
        name: normalizeCommunityLevelName(editor.name) || "Custom Level",
        startAngle: this.snapCreateLevelAngle(editor.startAngle),
        map: rows,
      };
      return {
    ...
    preparePendingCommunityLevelFromEditor() {
      const built = this.buildCreateLevelDefinitionFromEditor();
      if (built.error) {
        this.setCreateLevelEditorMessage(built.error, "error");
        return null;
      }
      this.customLevelTestText = built.text;
      this.publishLevelText = built.text;
      this.publishLevelName = built.definition.name;
      this.pendingCommunityLevel = {
        name: built.definition.name,
        creator: this.playerUsername,
        startAngle: built.definition.startAngle,
        map: built.definition.map,
        bestTime: null,
        submissionId: this.isEditingMyLevel() ? this.editingMyLevel?.submissionId || "" : "",
        remoteId: this.isEditingMyLevel() ? this.editingMyLevel?.remoteId || "" : "",
        local: Boolean(this.isEditingMyLevel() && this.editingMyLevel?.local),
        sessionToken: this.hasSignedInAccountSession() ? this.signedInSessionToken : "",
      };
      return built;
    }

    buildCreateLevelDraftFromEditor(editor = this.createLevelEditor) {
      if (!editor) {
        return null;
      }
      this.syncCreateLevelStoredRows(editor);
      return {
        name: normalizeCommunityLevelName(editor.name) || "Custom Level",
        startAngle: this.snapCreateLevelAngle(editor.startAngle),
        map: this.getCreateLevelRowsAsStrings(editor),
        hiddenMap: editor.storedRows.map((row) => row.join("")),
        hiddenOriginX: this.getCreateLevelHiddenOrigin(editor).x,
        hiddenOriginY: this.getCreateLevelHiddenOrigin(editor).y,
      };
    }

    buildCreateLevelEditorSaveSignature(editor = this.createLevelEditor) {
      const draft = this.buildCreateLevelDraftFromEditor(editor);
      if (!draft) {
        return "";
      }
      return [
        normalizeCommunityLevelName(draft.name) || "",
        this.snapCreateLevelAngle(draft.startAngle),
        draft.map.join("\n"),
        draft.hiddenMap.join("\n"),
        draft.hiddenOriginX,
        draft.hiddenOriginY,
      ].join("\n---\n");
    }

    createLevelEditorHasUnsavedChanges(editor = this.createLevelEditor) {
      if (!editor) {
        return false;
      }
      if (!editor.savedDraftSignature) {
        return true;
      }
      return this.buildCreateLevelEditorSaveSignature(editor) !== editor.savedDraftSignature;
    }

    saveCreateLevelEditor({ exitAfterSave = false, isAutoSave = false } = {}) {
      const editor = this.createLevelEditor;
      if (!editor) {
        return;
      }
      ...
      if (!draft) {
        if (!isAutoSave) {
          this.setCreateLevelEditorMessage("Save failed.", "error");
        }
        return;
      }
      const now = Date.now();
      const baseLevel = this.isEditingMyLevel() ? this.editingMyLevel : null;
      const savedLevel = this.upsertMyCreatedLevel(
        {
          ...(baseLevel || {}),
          localId: baseLevel?.localId || this.generateMyCreatedLevelLocalId(),
          name: draft.name,
          creator: this.playerUsername,
          startAngle: draft.startAngle,
          map: draft.map,
          hiddenMap: draft.hiddenMap,
          hiddenOriginX: draft.hiddenOriginX,
          hiddenOriginY: draft.hiddenOriginY,
          myBestTime: baseLevel?.myBestTime ?? null,
          creatorTime: baseLevel?.creatorTime ?? null,
          leaderboard: Array.isArray(baseLevel?.leaderboard) ? baseLevel.leaderboard : [],
          local: baseLevel?.submissionStatus === "published" ? Boolean(baseLevel.local) : true,
          submissionStatus: baseLevel?.submissionStatus || "private",
          reviewMessage: baseLevel?.reviewMessage || "",
          remoteId: baseLevel?.remoteId || "",
          submissionId: baseLevel?.submissionId || "",
          createdAt: baseLevel?.createdAt || now,
          updatedAt: now,
        },
        { save: false, fallbackStatus: baseLevel?.submissionStatus || "private" }
      );
      if (!savedLevel) {
        this.setCreateLevelEditorMessage("Save failed.", "error");
        return;
      }
      this.editingMyLevel = { ...savedLevel };
      this.setActiveMyLevel(savedLevel);
      editor.name = savedLevel.name;
      editor.status = savedLevel.submissionStatus || "private";
      editor.savedDraftSignature = this.buildCreateLevelEditorSaveSignature(editor);
      editor.lastSavedAt = now;
      editor.lastChangeAt = now;
      this.savePersistentProgress();
      if (!isAutoSave) {
        this.setCreateLevelEditorMessage(exitAfterSave ? "Level saved." : "Level saved to My Levels.", "ok");
      }
      if (exitAfterSave) {
        this.reopenActiveMyLevelDetail();
      }
    }

    deleteEditingMyLevel() {
      const target = this.isEditingMyLevel() ? this.editingMyLevel : this.getActiveMyLevel();
      if (!target) {
        return;
      }
      const matchIndex = this.getMyCreatedLevelMatchIndex(target);
      if (matchIndex >= 0) {
        this.myCreatedLevels.splice(matchIndex, 1);
      }
      this.activeMyLevel = null;
      this.editingMyLevel = null;
      this.savePersistentProgress();
    ...
    copyTextToClipboard(text, onSuccess, onFailure) {
      const success = typeof onSuccess === "function" ? onSuccess : () => {};
      const failure = typeof onFailure === "function" ? onFailure : () => {};
      const nextText = String(text ?? "");
      if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
        navigator.clipboard.writeText(nextText).then(success).catch(failure);
        return;
      }
      try {
    ...
    applyMyCreatedLevelPatch(level, patch, { save = true } = {}) {
      const matchIndex = this.getMyCreatedLevelMatchIndex(level);
      const baseLevel = matchIndex >= 0
        ? (this.normalizeMyCreatedLevelEntry(this.myCreatedLevels[matchIndex], this.myCreatedLevels[matchIndex]?.submissionStatus || "private") || null)
        : this.normalizeMyCreatedLevelEntry(level, level?.submissionStatus || "private");
      if (!baseLevel) {
        return null;
      }
      const merged = {
      ...
      if (!Object.prototype.hasOwnProperty.call(patch, "remoteId")) {
        merged.remoteId = baseLevel.remoteId || "";
      }
      if (!Object.prototype.hasOwnProperty.call(patch, "submissionId")) {
        merged.submissionId = baseLevel.submissionId || "";
      }
      const normalized = this.normalizeMyCreatedLevelEntry(merged, merged.submissionStatus || baseLevel.submissionStatus || "private");
      if (!normalized) {
        return null;
      }
      if (matchIndex >= 0) {
        this.myCreatedLevels[matchIndex] = normalized;
      } else {
        this.myCreatedLevels.unshift(normalized);
      }
      this.setActiveMyLevel(normalized);
      this.rebuildCommunityLevels();
      if (save) {
        this.savePersistentProgress();
      }
    ...
    getMyCreatedLevels() {
      ...
      const registerLevel = (entry, fallbackStatus) => {
        ...
        const signature = this.getCommunityLevelSignature(normalized);
        let target = null;
        if (normalized.localId && byIdentity.has(`local:${normalized.localId}`)) {
          target = byIdentity.get(`local:${normalized.localId}`);
        } else if (normalized.remoteId && byIdentity.has(`remote:${normalized.remoteId}`)) {
        ...
        if (target.localId) {
          byIdentity.set(`local:${target.localId}`, target);
        }
        if (target.remoteId) {
    ...
    removeOnlineCommunityLevel(level) {
      if (!level?.remoteId) {
        return false;
      }
      const nextLevels = this.onlineCommunityLevels.filter((entry) => entry.remoteId !== level.remoteId);
      if (nextLevels.length === this.onlineCommunityLevels.length) {
        return false;
      }
      this.onlineCommunityLevels = nextLevels;
      this.rebuildCommunityLevels();
      return true;
    }
    ...
    async syncOnlineMyLevels({ force = false } = {}) {
      ...
      if (
        !force &&
        creatorName &&
        this.onlineMyLevelsLastSyncUser === creatorName &&
        this.isCacheFresh(this.onlineMyLevelsLastSyncAt, this.getOnlineMyLevelsCacheMaxAgeMs())
      ) {
        return;
      }
      try {
        ...
        this.onlineMyLevelsLastSyncAt = Date.now();
        this.onlineMyLevelsLastSyncUser = creatorName;
        this.savePersistentProgress();
      } catch {
    ...
    applyOnlineCommunityLevelRow(row) {
      ...
      this.rebuildCommunityLevels();
      if (this.activeCommunityLevel?.remoteId === normalized.remoteId) {
        this.activeCommunityLevel = {
          ...this.activeCommunityLevel,
          ...normalized,
          map: Array.isArray(this.activeCommunityLevel.map) && this.activeCommunityLevel.map.length > 0
            ? this.activeCommunityLevel.map
            : normalized.map,
        };
      }
    }
    ...
    async fetchOnlineCommunityLevelRows() {
      let response = await fetch(this.getOnlineCommunityLevelsFetchUrl(), {
        method: "GET",
        headers: this.getOnlineLeaderboardHeaders(false),
      });
      if (!response.ok && response.status === 400) {
        response = await fetch(this.getLegacyOnlineCommunityLevelsFetchUrl(), {
          method: "GET",
          headers: this.getOnlineLeaderboardHeaders(false),
        });
      }
    ...
    async syncOnlineCommunityLevels() {
      ...
      if (this.isCacheFresh(this.onlineCommunityLastSyncAt, this.getOnlineCommunityCacheMaxAgeMs())) {
        return;
      }
    ...
    async submitCommunityLevelForReview(level) {
      ...
      if (level.submissionId) {
        payload.p_submission_id = level.submissionId;
      }
      if (level.sessionToken) {
        payload.p_session_token = level.sessionToken;
      }
    ...
    async unpublishOnlineMyLevel(level) {
      if (!this.canUnpublishOnlineCommunityLevel() || !level?.submissionId) {
        return null;
      }
      const response = await fetch(`${this.getOnlineLeaderboardBaseUrl()}/rest/v1/rpc/${ONLINE_LEADERBOARD_CONFIG.unpublishCommunityLevelRpcName}`, {
        method: "POST",
        headers: this.getOnlineLeaderboardHeaders(true),
        body: JSON.stringify({
          p_submission_id: level.submissionId,
          p_creator_name: level.creator,
          p_session_token: this.signedInSessionToken,
        }),
      });
      if (!response.ok) {
        throw new Error(`Community unpublish failed with ${response.status}`);
      }
      const responsePayload = await response.json();
      return Array.isArray(responsePayload) ? responsePayload[0] : responsePayload;
    }
    ...
    async incrementOnlineCommunityLevelPlayCount(levelId) {
      if (!this.canTrackOnlineCommunityLevelViews() || !levelId) {
        return null;
      }
      const response = await fetch(this.getOnlineCommunityPlayCountIncrementUrl(), {
        method: "POST",
        headers: this.getOnlineLeaderboardHeaders(true),
        body: JSON.stringify({
          p_level_id: levelId,
      ...
      if (!response.ok) {
        throw new Error(`Community play count increment failed with ${response.status}`);
      }
      const payload = await response.json();
      const row = Array.isArray(payload) ? payload[0] : payload;
      if (row && typeof row === "object") {
    ...
    beginPendingCommunityLevelView(level) {
      if (!this.canTrackOnlineCommunityLevelViews() || !level?.remoteId) {
        this.pendingCommunityLevelView = null;
        return;
      }
      this.pendingCommunityLevelView = {
        levelId: level.remoteId,
        elapsed: 0,
        sent: false,
      };
    }

    clearPendingCommunityLevelView() {
      this.pendingCommunityLevelView = null;
    }

    updatePendingCommunityLevelView(dt) {
      const pendingView = this.pendingCommunityLevelView;
      if (!pendingView) {
        return;
      }
      const activeRemoteId = this.activeCommunityLevel?.remoteId || "";
      if (this.currentScreen !== "game" || this.gameMode !== "community" || !activeRemoteId || activeRemoteId !== pendingView.levelId) {
        this.clearPendingCommunityLevelView();
        return;
      }
      if (pendingView.sent) {
        return;
      }
      pendingView.elapsed += Math.max(0, dt);
      if (
        pendingView.elapsed < COMMUNITY_LEVEL_VIEW_COUNT_MIN_SECONDS &&
        !this.goalTriggered &&
        !this.completed
      ) {
        return;
      }
      pendingView.sent = true;
      this.incrementOnlineCommunityLevelPlayCount(pendingView.levelId).catch(() => undefined);
    }
    ...
    getTitleSettingsButton() {
      const scale = this.getScreenFitScale(760, 540, 0.58);
      const size = Math.max(42, Math.round(54 * scale));
      const margin = Math.max(16, Math.round(24 * scale));
      return {
        id: "settings",
        label: "Settings",
        x: margin,
        y: margin,
        w: Math.max(116, Math.round(142 * scale)),
        h: size,
        fontSize: Math.max(14, Math.round(18 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
        hover: this.titleSettingsHover,
      };
    }

    getSettingsBackButton() {
      return {
        id: "back",
        label: "Back",
        x: 24,
        ...
        fontSize: 18,
        radius: 12,
        hover: this.settingsBackHover,
      };
    }

    getSettingsGhostOptionButtons() {
      const scale = this.getScreenFitScale(760, 560, 0.58);
      const gap = Math.max(14, Math.round(20 * scale));
      const width = Math.min(Math.round(360 * scale), this.width - 48);
      const height = Math.max(48, Math.round(64 * scale));
      const x = Math.round(this.width * 0.5 - width * 0.5);
      const startY = Math.round(this.height * 0.38);
      return GHOST_MODE_OPTIONS.map((option, index) => ({
        id: `ghost_${option.id}`,
        ghostMode: option.id,
        label: option.label,
        x,
        ...
        radius: Math.max(10, Math.round(14 * scale)),
        hover: this.settingsGhostOptionHovers[option.id] || 0,
        accent: this.ghostMode === option.id ? "green" : "blue",
      }));
    }
    ...
    getDisplayedLeaderboardEntry(scope, levelIndex = null, fallbackEntry = null) {
      if (this.selectedLeaderboardPeriod === "all_time") {
        if (this.isOnlineLeaderboardConfigured() && this.onlineLeaderboardLastSyncAt > 0) {
          const recordKey = this.getOnlineLeaderboardRecordKey(scope, levelIndex, "all_time", "all_time");
          const row = this.onlineLeaderboardRowsByKey.get(recordKey);
    ...
    getLeaderboardReplayRecordKey(scope, levelIndex = null, period = this.selectedLeaderboardPeriod) {
      const safePeriod = typeof period === "string" ? period : "all_time";
      return this.getOnlineLeaderboardRecordKey(scope, levelIndex, safePeriod, this.getLeaderboardPeriodKey(safePeriod));
    }

    canOpenLeaderboardReplayTarget(record, scope, levelIndex = null, period = this.selectedLeaderboardPeriod) {
      const normalized = this.normalizeWorldRecordEntry(record);
      if (!Number.isFinite(normalized.time)) {
        return false;
      }
      if (Array.isArray(normalized.replayFrames) && normalized.replayFrames.length > 0) {
        return true;
      }
      return this.onlineLeaderboardRowsByKey.has(this.getLeaderboardReplayRecordKey(scope, levelIndex, period));
    }

    getLeaderboardReplayTargetId(scope, levelIndex = null) {
      return Number.isInteger(levelIndex) ? `${scope}:${levelIndex}` : scope;
    }

    getLeaderboardReplayModeFromScope(scope) {
      if (typeof scope !== "string") {
        return "campaign";
      }
      if (scope.startsWith("speedrun_blackout")) {
        return "blackout_speedrun";
      }
      if (scope.startsWith("speedrun_")) {
        return "speedrun";
      }
      if (scope.startsWith("level_blackout")) {
        return "blackout";
      }
      return "campaign";
    }

    getLeaderboardReplayTargets() {
      ...
      const displayedColumns = [
        { scopeBase: "level_normal", title: "Normal", records: this.getDisplayedLevelLeaderboardRecords("level_normal", this.worldRecords), x: startX },
        { scopeBase: "level_perfect", title: "Perfect", records: this.getDisplayedLevelLeaderboardRecords("level_perfect", this.perfectWorldRecords), x: startX + panelW + panelGap },
        { scopeBase: "level_blackout", title: "Blackout", records: this.getDisplayedLevelLeaderboardRecords("level_blackout", this.blackoutWorldRecords), x: startX + (panelW + panelGap) * 2 },
      ];
      const targets = [];
        for (const column of displayedColumns) {
          for (let index = 0; index < LEVELS.length; index += 1) {
            const record = this.normalizeWorldRecordEntry(column.records[index]);
            const scope = this.getVersionedLevelScope(column.scopeBase, index);
            if (!this.canOpenLeaderboardReplayTarget(record, scope, index)) {
              continue;
            }
            const rowY = 48 + index * rowH;
            targets.push({
              id: this.getLeaderboardReplayTargetId(scope, index),
              scope,
              levelIndex: index,
              recordKey: this.getLeaderboardReplayRecordKey(scope, index),
              playbackMode: this.getLeaderboardReplayModeFromScope(scope),
              label: `${column.title} L${index + 1}`,
              x: column.x + 10,
              y: panelsTop + rowY + 2,
            w: panelW - 20,
            h: rowH - 4,
      ...
      const speedrunEntries = [
        { scopeBase: "speedrun_normal", title: "Normal", record: this.getDisplayedSpeedrunLeaderboardRecord("speedrun_normal", this.speedrunWorldRecord) },
        { scopeBase: "speedrun_perfect", title: "Perfect", record: this.getDisplayedSpeedrunLeaderboardRecord("speedrun_perfect", this.perfectSpeedrunWorldRecord) },
        { scopeBase: "speedrun_blackout", title: "Blackout", record: this.getDisplayedSpeedrunLeaderboardRecord("speedrun_blackout", this.blackoutSpeedrunWorldRecord) },
      ];
      speedrunEntries.forEach((entry, index) => {
        const record = this.normalizeWorldRecordEntry(entry.record);
        const scope = this.getVersionedSpeedrunScope(entry.scopeBase);
        if (!this.canOpenLeaderboardReplayTarget(record, scope, null)) {
          return;
        }
        targets.push({
          id: this.getLeaderboardReplayTargetId(scope),
          scope,
          levelIndex: null,
          recordKey: this.getLeaderboardReplayRecordKey(scope, null),
          playbackMode: this.getLeaderboardReplayModeFromScope(scope),
          label: `Speedrun ${entry.title}`,
          x: speedrunX + 12 + index * (speedrunSectionW + speedrunGap),
          y: speedrunY + speedrunSectionY,
          w: speedrunSectionW,
          h: speedrunSectionH,
          record,
        });
    ...
    getLeaderboardReplayBackButton() {
      return {
        id: "back",
        label: "Back",
        x: 24,
        ...
        fontSize: 18,
        radius: 12,
        hover: this.leaderboardReplayBackHover,
      };
    }

    getLeaderboardReplayStepButtons() {
      if (!this.activeLeaderboardReplay?.paused) {
        return [];
      }
      const scale = this.getScreenFitScale(1280, 720, 0.55);
      const size = Math.max(34, Math.round(42 * scale));
      ...
      return [
        {
          id: "replay_prev",
          label: "<",
          x: Math.round(centerX - size - gap * 0.5),
          y,
          w: size,
          h: size,
          fontSize: Math.max(18, Math.round(24 * scale)),
          radius: 12,
          hover: this.leaderboardReplayPrevHover,
        },
        {
          id: "replay_next",
          label: ">",
          x: Math.round(centerX + gap * 0.5),
          y,
          w: size,
          h: size,
          fontSize: Math.max(18, Math.round(24 * scale)),
          radius: 12,
          hover: this.leaderboardReplayNextHover,
        },
      ];
    }
    ...
    getTitleUsernameLayout() {
      ...
      const headerInset = Math.round(42 * scale);
      const statusGap = Math.max(8, Math.round(10 * scale));
      const inputY = headerInset;
      let cursorY = inputY + inputH + buttonGap;
      const buttonY = cursorY;
      if (showUpdateButton) {
        cursorY += buttonH + buttonGap;
      }
      let authY = cursorY;
      let statusY = cursorY;
      let logoutY = cursorY;
      ...
      const panelH = Math.max(108, cursorY + inputPad);
    ...
    isOnlineRacingScreen(screen = this.currentScreen) {
      return screen === "online_racing" ||
        screen === "online_racing_join" ||
        screen === "online_racing_host" ||
        screen === "online_racing_account";
    }

    getOnlineRacingMenuButtons() {
      const scale = this.getScreenFitScale(760, 540, 0.56);
      const gap = Math.max(14, Math.round(22 * scale));
      const width = Math.min(Math.round(340 * scale), this.width - 48);
      const height = Math.max(52, Math.round(74 * scale));
      const startY = Math.round(this.height * 0.36);
      const x = Math.round(this.width * 0.5 - width * 0.5);
      return [
        { id: "online_racing_join", label: "Join Match", x, y: startY, w: width, h: height, fontSize: Math.max(17, Math.round(25 * scale)), radius: 14, hover: this.onlineRacingJoinHover, accent: "green" },
        { id: "online_racing_create", label: "Create Match", x, y: startY + height + gap, w: width, h: height, fontSize: Math.max(17, Math.round(25 * scale)), radius: 14, hover: this.onlineRacingCreateHover },
        { id: "online_racing_account", label: "Account", x, y: startY + (height + gap) * 2, w: width, h: height, fontSize: Math.max(17, Math.round(25 * scale)), radius: 14, hover: this.onlineRacingAccountHover },
      ];
    }

    getOnlineRacingJoinTabs() {
      const scale = this.getScreenFitScale(820, 620, 0.62);
      const gap = Math.max(10, Math.round(12 * scale));
      const width = Math.max(120, Math.round(180 * scale));
      const height = Math.max(38, Math.round(48 * scale));
      const startX = Math.round(this.width * 0.5 - (width * 2 + gap) * 0.5);
      const y = Math.round(this.height * 0.24);
      return [
        { id: "online_racing_tab_public", label: "Public", x: startX, y, w: width, h: height, fontSize: Math.max(15, Math.round(18 * scale)), hover: this.onlineRacingPublicTabHover },
        { id: "online_racing_tab_private", label: "Private", x: startX + width + gap, y, w: width, h: height, fontSize: Math.max(15, Math.round(18 * scale)), hover: this.onlineRacingPrivateTabHover },
      ];
    }

    getOnlineRacingMatchCards() {
      const matches = ONLINE_RACING_TEMPLATE_MATCHES[this.onlineRacingSelectedTab] || [];
      const scale = this.getScreenFitScale(840, 660, 0.62);
      const cardW = Math.min(Math.round(520 * scale), this.width - 48);
      const cardH = Math.max(62, Math.round(78 * scale));
      const gap = Math.max(12, Math.round(16 * scale));
      const startY = Math.round(this.height * 0.34);
      ...
      return matches.map((match, index) => ({
        ...
        w: cardW,
        h: cardH,
        private: this.onlineRacingSelectedTab === "private",
      }));
    }

    getOnlineRacingAccountButtons() {
      const scale = this.getScreenFitScale(760, 560, 0.58);
      const width = Math.min(Math.round(300 * scale), this.width - 48);
      const height = Math.max(48, Math.round(64 * scale));
      const gap = Math.max(14, Math.round(20 * scale));
      const x = Math.round(this.width * 0.5 - width * 0.5);
      const startY = Math.round(this.height * 0.56);
      return [
        { id: "online_racing_friends", label: "Friends", x, y: startY, w: width, h: height, fontSize: Math.max(16, Math.round(23 * scale)), radius: 14, hover: this.onlineRacingFriendsHover },
        { id: "online_racing_logout", label: "Log Out", x, y: startY + height + gap, w: width, h: height, fontSize: Math.max(16, Math.round(23 * scale)), radius: 14, hover: this.onlineRacingLogoutHover, accent: "red" },
      ];
    }
    ...
    getOnlineLevelListMetrics(screen = this.currentScreen) {
      if (screen === "community_levels") {
        ...
        const margin = Math.max(6, Math.round(10 * scale));
        ...
        const sortW = Math.max(158, Math.round(198 * scale));
        const controlGap = Math.max(10, Math.round(12 * scale));
        const cardW = Math.min(Math.round(930 * scale), this.width - margin * 2);
        const stackedControls = cardW - sortW - controlGap < 180;
        const cardH = Math.max(58, Math.round(78 * scale));
        const gap = Math.max(8, Math.round(12 * scale));
        const controlsBottom = stackedControls
          ? searchTop + searchH * 2 + controlGap
          : searchTop + searchH;
        const startY = controlsBottom + searchGap;
        const x = Math.round(this.width * 0.5 - cardW * 0.5);
        const viewportTop = startY - Math.max(16, Math.round(18 * scale));
        const viewportBottom = this.height - Math.max(8, Math.round(12 * scale));
        const hoverOverflowPadX = Math.max(34, Math.round(40 * scale));
      ...
      if (screen === "my_levels") {
        ...
        const cardW = Math.min(Math.round(900 * scale), this.width - margin * 2);
        ...
        const hoverOverflowPadX = Math.max(30, Math.round(36 * scale));
        const hoverOverflowPadY = Math.max(16, Math.round(18 * scale));
        const bottomContentPad = Math.max(hoverOverflowPadY, Math.round(cardH * 0.28));
        return {
          screen,
    ...
    getCommunitySearchLayout() {
      ...
      return {
        x: metrics.x,
        y: metrics.searchTop,
        w: metrics.stackedControls
          ? metrics.cardW
          : Math.max(180, metrics.cardW - metrics.sortW - metrics.controlGap),
        h: metrics.searchH,
        fontSize: Math.max(13, Math.round(metrics.searchH * 0.42)),
      };
    }

    getCommunitySortLayout() {
      const metrics = this.getOnlineLevelListMetrics("community_levels");
      if (!metrics) {
        return null;
      }
      return {
        x: metrics.stackedControls ? metrics.x : metrics.x + metrics.cardW - metrics.sortW,
        y: metrics.stackedControls ? metrics.searchTop + metrics.searchH + metrics.controlGap : metrics.searchTop,
        w: metrics.stackedControls ? metrics.cardW : metrics.sortW,
        h: metrics.searchH,
        fontSize: Math.max(12, Math.round(metrics.searchH * 0.38)),
      };
    }
    ...
    getCommunityLevelCards() {
      ...
      return this.getRankedCommunityLevels().map((entry, index) => ({
        ...
        playCountValue: Number.isFinite(entry.level?.playCount) ? Math.max(0, Math.floor(entry.level.playCount)) : 0,
        x,
    ...
    compareCommunityLevelsBySort(leftLevel, rightLevel) {
      const leftName = normalizeCommunityLevelName(leftLevel?.name).toLowerCase();
      const rightName = normalizeCommunityLevelName(rightLevel?.name).toLowerCase();
      const leftViews = Number.isFinite(leftLevel?.playCount) ? Math.max(0, Math.floor(leftLevel.playCount)) : 0;
      const rightViews = Number.isFinite(rightLevel?.playCount) ? Math.max(0, Math.floor(rightLevel.playCount)) : 0;
      const leftTime = Number.isFinite(leftLevel?.creatorTime) ? leftLevel.creatorTime : null;
      const rightTime = Number.isFinite(rightLevel?.creatorTime) ? rightLevel.creatorTime : null;
      const leftPublished = Number.isFinite(leftLevel?.createdAt) ? leftLevel.createdAt : 0;
      const rightPublished = Number.isFinite(rightLevel?.createdAt) ? rightLevel.createdAt : 0;

      if (this.communityLevelSort === "largest_time") {
      ...
      } else if (this.communityLevelSort === "smallest_time") {
        if (leftTime == null && rightTime != null) {
          return 1;
        }
        if (leftTime != null && rightTime == null) {
      ...
      } else if (this.communityLevelSort === "alphabetical") {
        if (leftName !== rightName) {
          return leftName.localeCompare(rightName);
        }
      } else if (this.communityLevelSort === "date_published") {
    ...
    getRankedCommunityLevels() {
      ...
      if (!query) {
        return levels.sort((a, b) => this.compareCommunityLevelsBySort(a.level, b.level));
      }
      return levels.sort((a, b) => {
        if (b.score !== a.score) {
          return b.score - a.score;
        }
        const sorted = this.compareCommunityLevelsBySort(a.level, b.level);
        return sorted !== 0 ? sorted : a.index - b.index;
      });
    }
    ...
    getMyLevelDetailActionInfo(level = this.getActiveMyLevel()) {
      if (this.myLevelActionRequestInFlight) {
        return {
          label: "Working...",
          mode: "busy",
          enabled: false,
          description: "Saving your level changes.",
        };
      }
      const status = normalizeCommunitySubmissionStatus(level?.submissionStatus || "private");
      if (status === "published") {
        return {
          label: "Unpublish",
          mode: "unpublish",
          enabled: true,
          description: "Remove this level from Community Levels and move it back to Private.",
        };
      }
      if (status === "pending") {
        return {
          ...
          mode: "pending",
          enabled: false,
          description: "This level is waiting for admin review.",
        };
      }
      return {
        label: "Publish",
        ...
        description: status === "declined"
          ? "Send this updated level back in for review."
          : "Send this level in for admin review.",
      };
    }

    getMyLevelDetailLayout(level = this.getActiveMyLevel()) {
      const scale = this.getScreenFitScale(860, 640, 0.58);
      const panelW = Math.min(Math.round(740 * scale), this.width - 34);
      const panelH = Math.max(244, Math.round(292 * scale));
      const panelX = Math.round(this.width * 0.5 - panelW * 0.5);
      const panelY = Math.round(Math.max(138, this.height * 0.28));
      const innerPad = Math.max(18, Math.round(24 * scale));
      const buttonGap = Math.max(10, Math.round(14 * scale));
      const buttonH = Math.max(42, Math.round(52 * scale));
      const buttonW = Math.max(86, Math.floor((panelW - innerPad * 2 - buttonGap * 2) / 3));
      const buttonsRowW = buttonW * 3 + buttonGap * 2;
      const buttonsY = panelY + panelH - innerPad - buttonH - Math.max(54, Math.round(62 * scale));
      const firstButtonX = Math.round(panelX + panelW * 0.5 - buttonsRowW * 0.5);
      const actionInfo = this.getMyLevelDetailActionInfo(level);
      const playButton = {
        id: "play_my_level_detail",
        label: "Play",
        x: firstButtonX,
        y: buttonsY,
        w: buttonW,
        h: buttonH,
        fontSize: Math.max(15, Math.round(21 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
        hover: this.myLevelDetailPlayHover,
      };
      const actionButton = {
        id: "my_level_detail_action",
        ...
        fontSize: Math.max(15, Math.round(21 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
        hover: this.myLevelDetailActionHover,
      };
      const editButton = {
        id: "edit_my_level",
        label: "Edit",
        ...
        fontSize: Math.max(15, Math.round(21 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
        hover: this.myLevelDetailEditHover,
      };
      const deleteButton = {
        id: "delete_my_level",
        label: "Delete",
        enabled: this.isOnlineAccountConfigured() && Boolean(this.signedInUsername),
        x: Math.round(panelX + panelW * 0.5 - Math.max(120, Math.round(170 * scale)) * 0.5),
        y: panelY + panelH - innerPad - buttonH,
        w: Math.max(120, Math.round(170 * scale)),
        h: buttonH,
        fontSize: Math.max(14, Math.round(19 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
        hover: this.myLevelDetailDeleteHover,
      };
      return {
        scale,
        titleY: Math.max(72, Math.round(this.height * 0.14)),
        subtitleY: Math.max(104, Math.round(this.height * 0.14) + Math.round(30 * scale)),
        panelX,
        panelY,
        panelW,
        panelH,
        innerPad,
        headerY: panelY + innerPad + Math.round(4 * scale),
        statusValueY: panelY + innerPad + Math.round(32 * scale),
        messageY: panelY + innerPad + Math.round(64 * scale),
        noteY: panelY + innerPad + Math.round(92 * scale),
        playButton,
        actionButton,
        editButton,
        deleteButton,
    ...
    getMyLevelActionPromptButtons() {
      const scale = this.getScreenFitScale(560, 420, 0.58);
      const buttonW = Math.max(106, Math.round(140 * scale));
      const buttonH = Math.max(38, Math.round(46 * scale));
      const gap = Math.max(12, Math.round(18 * scale));
      const startX = Math.round(this.width * 0.5 - buttonW - gap * 0.5);
      const y = Math.round(this.height * 0.5 + Math.round(48 * scale));
      const confirmLabel = this.myLevelActionPrompt?.mode === "unpublish"
        ? "Unpublish"
        : this.myLevelActionPrompt?.mode === "delete"
          ? "Delete"
          : "Publish";
      return [
        {
          ...
          h: buttonH,
          fontSize: Math.max(13, Math.round(17 * scale)),
          radius: Math.max(9, Math.round(12 * scale)),
          hover: this.myLevelActionPromptCancelHover,
        },
        {
          id: "confirm",
          label: confirmLabel,
          ...
          fontSize: Math.max(13, Math.round(17 * scale)),
          radius: Math.max(9, Math.round(12 * scale)),
          hover: this.myLevelActionPromptConfirmHover,
        },
      ];
    }

    getMyLevelActionPromptLayout() {
      const scale = this.getScreenFitScale(560, 420, 0.58);
      const isDelete = this.myLevelActionPrompt?.mode === "delete";
      const panelW = Math.min(Math.round(520 * scale), this.width - 24);
      const panelH = Math.min(Math.round((isDelete ? 290 : 228) * scale), this.height - 24);
      const panelX = Math.round(this.width * 0.5 - panelW * 0.5);
      const panelY = Math.round(this.height * 0.5 - panelH * 0.5);
      const passwordInput = isDelete
        ? {
            x: panelX + Math.round(28 * scale),
            y: panelY + Math.round(132 * scale),
            w: panelW - Math.round(56 * scale),
            h: Math.max(38, Math.round(44 * scale)),
          }
        : null;
      return {
        scale,
        ...
        panelY,
        passwordInput,
        buttons: this.getMyLevelActionPromptButtons(),
      };
    }

    getCreateLevelLayout() {
      const editor = this.createLevelEditor || this.createCreateLevelEditorState();
      const scale = this.getScreenFitScale(1280, 900, 0.48);
      const margin = Math.max(12, Math.round(18 * scale));
      if (editor.mode === "setup") {
        const panelW = Math.min(Math.round(620 * scale), this.width - margin * 2);
        const importSelected = editor.setupSource === "import";
        const panelH = Math.min(
          Math.max(importSelected ? 396 : 282, Math.round((importSelected ? 450 : 322) * scale)),
          this.height - margin * 2
        );
        const panelX = this.width * 0.5 - panelW * 0.5;
        const panelY = Math.max(88, this.height * 0.5 - panelH * 0.5);
        const nameH = Math.max(40, Math.round(50 * scale));
        const optionButtonH = Math.max(34, Math.round(42 * scale));
        const buttonW = Math.max(110, Math.round(154 * scale));
        const buttonH = Math.max(38, Math.round(48 * scale));
        const gap = Math.max(12, Math.round(18 * scale));
        const buttonsY = panelY + panelH - buttonH - Math.max(10, Math.round(14 * scale));
        const totalW = buttonW * 2 + gap;
        const startX = this.width * 0.5 - totalW * 0.5;
        const optionY = panelY + Math.max(176, Math.round(194 * scale));
        const optionW = Math.floor((panelW - Math.max(44, Math.round(52 * scale)) - gap) / 2);
        const optionX = panelX + Math.max(22, Math.round(26 * scale));
        const importInputY = optionY + optionButtonH + Math.max(16, Math.round(20 * scale));
        const importInputH = Math.max(118, buttonsY - importInputY - Math.max(18, Math.round(22 * scale)));
        return {
          mode: "setup",
          scale,
          panel: { x: panelX, y: panelY, w: panelW, h: panelH, radius: Math.max(18, Math.round(24 * scale)) },
          titleY: panelY + Math.max(46, Math.round(56 * scale)),
          copyY: panelY + Math.max(84, Math.round(98 * scale)),
          nameInput: {
            x: panelX + Math.max(22, Math.round(26 * scale)),
            y: panelY + Math.max(112, Math.round(126 * scale)),
            w: panelW - Math.max(44, Math.round(52 * scale)),
            h: nameH,
          },
          sourceButtons: [
            { id: "setup_source_blank", label: "Blank Level", x: optionX, y: optionY, w: optionW, h: optionButtonH, fontSize: Math.max(13, Math.round(17 * scale)) },
            { id: "setup_source_import", label: "Import Level", x: optionX + optionW + gap, y: optionY, w: optionW, h: optionButtonH, fontSize: Math.max(13, Math.round(17 * scale)) },
          ],
          importInput: {
            x: optionX,
            y: importInputY,
          ...
          buttons: [
            { id: "setup_back", label: "Back", x: startX, y: buttonsY, w: buttonW, h: buttonH, fontSize: Math.max(15, Math.round(22 * scale)) },
            { id: "setup_done", label: "Done", x: startX + buttonW + gap, y: buttonsY, w: buttonW, h: buttonH, fontSize: Math.max(15, Math.round(22 * scale)) },
          ],
        };
      }

      const compactEditorLayout = this.width < 560;
      const bottomPanelCollapsed = Boolean(this.createLevelBottomPanelCollapsed);
      const bottomPanelOpen = clamp(
        Number.isFinite(this.createLevelBottomPanelAnim)
          ? this.createLevelBottomPanelAnim
          : (bottomPanelCollapsed ? 0 : 1),
        0,
        1
      );
      const bottomPanelScale = clamp(
        Math.min(this.width / 980, this.height / 820),
        compactEditorLayout ? 0.62 : 0.78,
        1
      );
      const quickButtonH = Math.max(34, Math.round(40 * scale));
      const quickGap = compactEditorLayout ? Math.max(6, Math.round(8 * scale)) : Math.max(8, Math.round(10 * scale));
      const quickButtonW = compactEditorLayout ? Math.max(64, Math.round(88 * scale)) : Math.max(88, Math.round(118 * scale));
      const iconButtonW = quickButtonH;
      const topBarH = compactEditorLayout
        ? Math.max(96, Math.round(118 * scale))
        : Math.max(52, Math.round(62 * scale));
      const sideButtonSize = Math.max(34, Math.round(42 * scale));
      const expandedBottomPanelH = compactEditorLayout
        ? Math.max(188, Math.round(316 * scale * bottomPanelScale))
        : Math.max(136, Math.round(194 * scale * bottomPanelScale));
      const collapsedBottomPanelH = Math.max(40, Math.round(48 * scale));
      const bottomPanelH = Math.round(
        collapsedBottomPanelH + (expandedBottomPanelH - collapsedBottomPanelH) * bottomPanelOpen
      );
      const paletteTabsH = Math.max(28, Math.round(40 * scale * bottomPanelScale));
      const paletteViewportH = Math.max(66, Math.round(96 * scale * bottomPanelScale));
      const topY = margin;
      const viewport = {
        x: margin + sideButtonSize + Math.max(12, Math.round(14 * scale)),
        y: topY + topBarH + Math.max(10, Math.round(12 * scale)),
        w: this.width - margin * 2 - sideButtonSize - Math.max(12, Math.round(14 * scale)),
        h: this.height - topBarH - bottomPanelH - margin * 2 - Math.max(20, Math.round(24 * scale)),
      };
      const topBar = {
        x: margin,
        y: topY,
      ...
      const compactPad = Math.max(10, Math.round(12 * scale));
      const topBarButtonPad = compactPad;
      const editorTitleX = topBar.x + Math.round(18 * scale);
      const titleGap = quickGap;
      const editorNameInputX = editorTitleX;
      const topRowY = topBar.y + compactPad;
      const compactActionRowY = topRowY + quickButtonH + quickGap;
      const quickRightX = compactEditorLayout
        ? topBar.x + topBar.w - compactPad - quickButtonW
        : topBar.x + topBar.w - topBarButtonPad - quickButtonW;
      const quickPublishX = quickRightX - quickGap - quickButtonW;
      const quickPlayX = quickPublishX - quickGap - quickButtonW;
      const compactActionTotalW = iconButtonW * 2 + quickButtonW * 3 + quickGap * 4;
      const compactActionStartX = Math.max(
        topBar.x + Math.max(8, Math.round(10 * scale)),
        Math.round(topBar.x + topBar.w * 0.5 - compactActionTotalW * 0.5)
      );
      const compactNameMaxW = Math.max(
        112,
        topBar.w - (topBarButtonPad * 2 + quickButtonW + quickGap + compactActionStartX - topBar.x)
      );
      const maxEditorNameInputW = compactEditorLayout
        ? compactNameMaxW
        : Math.max(
            180,
            quickPlayX - quickGap - editorNameInputX - (quickButtonW + iconButtonW * 2 + quickGap * 3)
          );
      const targetEditorNameInputW = Math.round(
        COMMUNITY_LEVEL_NAME_MAX_LENGTH * Math.max(6.8, 7.6 * scale) + Math.max(28, Math.round(34 * scale))
      );
      const editorNameInputW = Math.min(maxEditorNameInputW, Math.max(compactEditorLayout ? 112 : 180, targetEditorNameInputW));
      const saveButtonX = compactEditorLayout
        ? topBar.x + topBar.w - compactPad - quickButtonW
        : editorNameInputX + editorNameInputW + titleGap;
      const undoButtonX = compactEditorLayout
        ? compactActionStartX
        : saveButtonX + quickButtonW + quickGap;
      const redoButtonX = compactEditorLayout
        ? compactActionStartX + iconButtonW + quickGap
        : undoButtonX + iconButtonW + quickGap;
      const editorTitleMaxW = editorNameInputW;
      const bottomPanel = {
        x: margin,
        y: this.height - bottomPanelH - margin,
        w: this.width - margin * 2,
        h: bottomPanelH,
      };
      const bottomInnerPad = Math.max(10, Math.round(14 * scale * bottomPanelScale));
      const bottomToggleButtonSize = Math.max(28, Math.round(34 * scale * bottomPanelScale));
      const bottomToggleButton = {
        id: "editor_bottom_toggle",
        label: bottomPanelCollapsed ? "^" : "v",
        x: bottomPanel.x + bottomPanel.w - bottomInnerPad - bottomToggleButtonSize,
        y: bottomPanel.y + Math.max(6, Math.round(8 * scale)),
        w: bottomToggleButtonSize,
        h: bottomToggleButtonSize,
        fontSize: Math.max(14, Math.round(18 * scale * bottomPanelScale)),
        textOffsetY: -1,
      };
      let sizePanelW = Math.max(148, Math.round(206 * scale * bottomPanelScale));
      let mouseModePanelW = Math.max(110, Math.round(150 * scale * bottomPanelScale));
      let panelColumnGap = compactEditorLayout ? Math.max(16, Math.round(22 * scale * bottomPanelScale)) : Math.max(18, Math.round(30 * scale * bottomPanelScale));
      let paletteTopY = bottomPanel.y + bottomInnerPad;
      let paletteTopH = paletteTabsH + Math.max(14, Math.round(18 * scale * bottomPanelScale)) + paletteViewportH;
      let mouseModePanel = { x: bottomPanel.x, y: bottomPanel.y, w: 0, h: 0 };
      let palettePanel = { x: bottomPanel.x, y: bottomPanel.y, w: 0, h: 0 };
      let sizePanel = { x: bottomPanel.x, y: bottomPanel.y, w: 0, h: 0 };
      let tabGap = quickGap;
      let tabW = Math.max(74, Math.round(98 * scale * bottomPanelScale));
      let paletteViewport = { x: bottomPanel.x, y: bottomPanel.y, w: 0, h: 0 };
      let sizeInputW = Math.max(50, Math.round(64 * scale * bottomPanelScale));
      let sizeInputH = Math.max(30, Math.round(38 * scale * bottomPanelScale));
      let sizeArrowW = sizeInputW;
      let sizeArrowH = Math.max(20, Math.round(24 * scale * bottomPanelScale));
      let sizeGap = Math.max(10, Math.round(18 * scale * bottomPanelScale));
      let sizeStartX = bottomPanel.x + bottomInnerPad;
      let sizeMidY = bottomPanel.y + bottomInnerPad;
      let mouseModeButtonH = Math.max(32, Math.round(42 * scale * bottomPanelScale));
      let mouseModeButtonW = Math.max(84, Math.round(120 * scale * bottomPanelScale));
      let mouseModeButtonX = bottomPanel.x + bottomInnerPad;
      let mouseModeButtonsTop = bottomPanel.y + bottomInnerPad;
      let mouseModeButtonGap = Math.max(8, Math.round(12 * scale * bottomPanelScale));
      if (!bottomPanelCollapsed) {
        sizePanelW = compactEditorLayout
          ? bottomPanel.w - Math.max(24, Math.round(28 * scale * bottomPanelScale))
          : Math.max(148, Math.round(206 * scale * bottomPanelScale));
        mouseModePanelW = compactEditorLayout
          ? Math.max(92, Math.round(bottomPanel.w * 0.25))
          : Math.max(110, Math.round(150 * scale * bottomPanelScale));
        panelColumnGap = compactEditorLayout ? Math.max(16, Math.round(22 * scale * bottomPanelScale)) : Math.max(18, Math.round(30 * scale * bottomPanelScale));
        paletteTopY = bottomPanel.y + bottomInnerPad;
        paletteTopH = paletteTabsH + Math.max(14, Math.round(18 * scale * bottomPanelScale)) + paletteViewportH;
        mouseModePanel = compactEditorLayout
          ? {
              x: bottomPanel.x + bottomInnerPad,
              y: paletteTopY,
              w: mouseModePanelW,
              h: paletteTopH,
            }
          : {
              x: bottomPanel.x + Math.max(12, Math.round(16 * scale * bottomPanelScale)),
              y: bottomPanel.y + Math.max(10, Math.round(14 * scale * bottomPanelScale)),
              w: mouseModePanelW,
              h: bottomPanel.h - Math.max(20, Math.round(28 * scale * bottomPanelScale)),
            };
        palettePanel = compactEditorLayout
          ? {
              x: mouseModePanel.x + mouseModePanel.w + panelColumnGap,
              y: paletteTopY,
              w: bottomPanel.x + bottomPanel.w - bottomInnerPad - (mouseModePanel.x + mouseModePanel.w + panelColumnGap),
              h: paletteTopH,
            }
          : {
              x: mouseModePanel.x + mouseModePanel.w + panelColumnGap,
              y: mouseModePanel.y,
              w: bottomPanel.w - mouseModePanel.w - sizePanelW - Math.max(44, Math.round(60 * scale * bottomPanelScale)),
              h: mouseModePanel.h,
            };
        sizePanel = compactEditorLayout
          ? {
              x: bottomPanel.x + bottomInnerPad,
              y: palettePanel.y + palettePanel.h + panelColumnGap,
              w: bottomPanel.w - bottomInnerPad * 2,
              h: bottomPanel.y + bottomPanel.h - bottomInnerPad - (palettePanel.y + palettePanel.h + panelColumnGap),
            }
          : {
              x: palettePanel.x + palettePanel.w + panelColumnGap,
              y: palettePanel.y,
              w: sizePanelW,
              h: palettePanel.h,
            };
        tabGap = Math.max(6, Math.round(quickGap * bottomPanelScale));
        tabW = compactEditorLayout
          ? Math.max(62, Math.floor((palettePanel.w - tabGap * 2) / 3))
          : Math.max(78, Math.round(110 * scale * bottomPanelScale));
        paletteViewport = {
          x: palettePanel.x,
          y: palettePanel.y + paletteTabsH + Math.max(12, Math.round(18 * scale * bottomPanelScale)),
          w: palettePanel.w,
          h: paletteViewportH,
        };
        sizeInputW = Math.max(50, Math.round(64 * scale * bottomPanelScale));
        sizeInputH = Math.max(30, Math.round(38 * scale * bottomPanelScale));
        sizeArrowW = sizeInputW;
        sizeArrowH = Math.max(20, Math.round(24 * scale * bottomPanelScale));
        sizeGap = Math.max(10, Math.round(18 * scale * bottomPanelScale));
        sizeStartX = sizePanel.x + sizePanel.w * 0.5 - (sizeInputW * 2 + sizeGap) * 0.5;
        sizeMidY = compactEditorLayout
          ? sizePanel.y + Math.max(38, Math.round(52 * scale * bottomPanelScale))
          : sizePanel.y + Math.round(78 * scale * bottomPanelScale);
        mouseModeButtonH = Math.max(32, Math.round(42 * scale * bottomPanelScale));
        mouseModeButtonW = mouseModePanel.w - Math.max(18, Math.round(24 * scale * bottomPanelScale));
        mouseModeButtonX = mouseModePanel.x + mouseModePanel.w * 0.5 - mouseModeButtonW * 0.5;
        mouseModeButtonsTop = mouseModePanel.y + Math.max(26, Math.round(36 * scale * bottomPanelScale));
        mouseModeButtonGap = Math.max(8, Math.round(12 * scale * bottomPanelScale));
      }
      const publishPanelW = Math.min(Math.round(520 * scale), this.width - margin * 2);
      const publishPanelH = Math.min(Math.round(318 * scale), this.height - margin * 2);
      const publishPanelX = this.width * 0.5 - publishPanelW * 0.5;
      const publishPanelY = this.height * 0.5 - publishPanelH * 0.5;
      const overlayButtonW = Math.max(118, Math.round(154 * scale));
      const overlayButtonH = Math.max(38, Math.round(46 * scale));
      const overlayGap = Math.max(12, Math.round(16 * scale));
      const menuPanelW = Math.min(Math.round(420 * scale), this.width - margin * 2);
      const menuPanelH = Math.min(Math.round(320 * scale), this.height - margin * 2);
      const menuPanelX = this.width * 0.5 - menuPanelW * 0.5;
      const menuPanelY = this.height * 0.5 - menuPanelH * 0.5;
      const menuButtonsTop = menuPanelY + Math.max(82, Math.round(96 * scale));
      const menuButtonsBottom = menuPanelY + menuPanelH - Math.max(26, Math.round(30 * scale));
      const menuButtonCount = 4;
      const menuButtonsAreaH = Math.max(0, menuButtonsBottom - menuButtonsTop);
      const menuButtonGap = Math.max(8, Math.round(10 * scale));
      const menuButtonH = Math.max(
        38,
        Math.min(
          Math.round(54 * scale),
          Math.floor((menuButtonsAreaH - menuButtonGap * Math.max(0, menuButtonCount - 1)) / menuButtonCount)
        )
      );
      const menuButtonStackH = menuButtonH * menuButtonCount + menuButtonGap * Math.max(0, menuButtonCount - 1);
      const menuButtonStartY = menuButtonsTop + Math.max(0, (menuButtonsBottom - menuButtonsTop - menuButtonStackH) * 0.5);
      const confirmPanelW = Math.min(Math.round(450 * scale), this.width - margin * 2);
      const confirmPanelH = Math.min(Math.round(230 * scale), this.height - margin * 2);
      const confirmPanelX = this.width * 0.5 - confirmPanelW * 0.5;
      const confirmPanelY = this.height * 0.5 - confirmPanelH * 0.5;
      return {
        mode: "editor",
        scale,
        margin,
        ...
        paletteViewport,
        sizePanel,
        tabs: bottomPanelCollapsed ? [] : [
          { id: "walls", label: "Walls", x: palettePanel.x, y: palettePanel.y, w: tabW, h: paletteTabsH, fontSize: Math.max(12, Math.round(16 * scale * bottomPanelScale)) },
          { id: "floors", label: "Floors", x: palettePanel.x + tabW + tabGap, y: palettePanel.y, w: tabW, h: paletteTabsH, fontSize: Math.max(12, Math.round(16 * scale * bottomPanelScale)) },
          { id: "erase", label: "Erase", x: palettePanel.x + (tabW + tabGap) * 2, y: palettePanel.y, w: tabW, h: paletteTabsH, fontSize: Math.max(12, Math.round(16 * scale * bottomPanelScale)) },
        ],
        sideButtons: [
          { id: "editor_home", label: "\u2302", x: margin, y: viewport.y + Math.max(18, Math.round(20 * scale)), w: sideButtonSize, h: sideButtonSize, fontSize: Math.max(15, Math.round(18 * scale)), textOffsetY: -1 },
          { id: "editor_zoom_in", label: "+", x: margin, y: viewport.y + Math.max(18, Math.round(20 * scale)) + sideButtonSize + quickGap, w: sideButtonSize, h: sideButtonSize, fontSize: Math.max(18, Math.round(21 * scale)), textOffsetY: -1 },
          { id: "editor_zoom_out", label: "-", x: margin, y: viewport.y + Math.max(18, Math.round(20 * scale)) + (sideButtonSize + quickGap) * 2, w: sideButtonSize, h: sideButtonSize, fontSize: Math.max(18, Math.round(21 * scale)), textOffsetY: -1 },
        ],
        saveButton: {
          id: "editor_save",
          label: "Save",
          x: saveButtonX,
          y: compactEditorLayout ? topRowY : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5,
          w: quickButtonW,
          h: quickButtonH,
        ...
        historyButtons: [
          { id: "editor_undo", label: "\u21B6", x: undoButtonX, y: compactEditorLayout ? compactActionRowY : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: iconButtonW, h: quickButtonH, fontSize: Math.max(16, Math.round(20 * scale)) },
          { id: "editor_redo", label: "\u21B7", x: redoButtonX, y: compactEditorLayout ? compactActionRowY : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: iconButtonW, h: quickButtonH, fontSize: Math.max(16, Math.round(20 * scale)) },
        ],
        quickButtons: [
          { id: "editor_play", label: "Play", x: compactEditorLayout ? redoButtonX + iconButtonW + quickGap : quickPlayX, y: compactEditorLayout ? compactActionRowY : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: quickButtonW, h: quickButtonH, fontSize: Math.max(13, Math.round(18 * scale)) },
          { id: "editor_publish", label: editor.status === "published" ? "Delete" : "Publish", x: compactEditorLayout ? redoButtonX + iconButtonW + quickGap + quickButtonW + quickGap : quickPublishX, y: compactEditorLayout ? compactActionRowY : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: quickButtonW, h: quickButtonH, fontSize: Math.max(13, Math.round(18 * scale)) },
          { id: "editor_menu", label: "Menu", x: compactEditorLayout ? redoButtonX + iconButtonW + quickGap + (quickButtonW + quickGap) * 2 : quickRightX, y: compactEditorLayout ? compactActionRowY : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: quickButtonW, h: quickButtonH, fontSize: Math.max(14, Math.round(18 * scale)) },
        ],
        mouseModeButtons: bottomPanelCollapsed ? [] : [
          { id: "editor_mode_move", label: "Move", x: mouseModeButtonX, y: mouseModeButtonsTop, w: mouseModeButtonW, h: mouseModeButtonH, fontSize: Math.max(12, Math.round(17 * scale * bottomPanelScale)) },
          { id: "editor_mode_drag", label: "Drag", x: mouseModeButtonX, y: mouseModeButtonsTop + mouseModeButtonH + mouseModeButtonGap, w: mouseModeButtonW, h: mouseModeButtonH, fontSize: Math.max(12, Math.round(17 * scale * bottomPanelScale)) },
        ],
        sizeControls: {
          xInput: { x: sizeStartX, y: sizeMidY, w: sizeInputW, h: sizeInputH },
          yInput: { x: sizeStartX + sizeInputW + sizeGap, y: sizeMidY, w: sizeInputW, h: sizeInputH },
          buttons: bottomPanelCollapsed ? [] : [
            { id: "editor_width_up", label: "\u25B2", x: sizeStartX, y: sizeMidY - sizeArrowH - 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(11, Math.round(17 * scale * bottomPanelScale)) },
            { id: "editor_width_down", label: "\u25BC", x: sizeStartX, y: sizeMidY + sizeInputH + 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(11, Math.round(17 * scale * bottomPanelScale)) },
            { id: "editor_height_up", label: "\u25B2", x: sizeStartX + sizeInputW + sizeGap, y: sizeMidY - sizeArrowH - 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(11, Math.round(17 * scale * bottomPanelScale)) },
            { id: "editor_height_down", label: "\u25BC", x: sizeStartX + sizeInputW + sizeGap, y: sizeMidY + sizeInputH + 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(11, Math.round(17 * scale * bottomPanelScale)) },
          ],
        },
        menuOverlay: {
          panel: { x: menuPanelX, y: menuPanelY, w: menuPanelW, h: menuPanelH, radius: Math.max(18, Math.round(24 * scale)) },
          buttons: Array.from({ length: menuButtonCount }, (_, index) => ({
            id: ["editor_menu_back", "editor_menu_export", "editor_menu_save_exit", "editor_menu_exit"][index],
            label: ["Back", "Export File", "Save and Exit", "Exit"][index],
            x: menuPanelX + Math.max(24, Math.round(28 * scale)),
            y: menuButtonStartY + index * (menuButtonH + menuButtonGap),
            w: menuPanelW - Math.max(48, Math.round(56 * scale)),
            h: menuButtonH,
            fontSize: Math.max(15, Math.round(21 * scale)),
          })),
        },
        publishOverlay: {
          panel: { x: publishPanelX, y: publishPanelY, w: publishPanelW, h: publishPanelH, radius: Math.max(18, Math.round(24 * scale)) },
          nameInput: {
            x: publishPanelX + Math.max(26, Math.round(30 * scale)),
            y: publishPanelY + Math.max(64, Math.round(72 * scale)),
            w: publishPanelW - Math.max(52, Math.round(60 * scale)),
            h: Math.max(38, Math.round(44 * scale)),
          },
          testButton: {
            id: "editor_publish_test",
            label: "Test Level",
            x: publishPanelX + publishPanelW * 0.5 - overlayButtonW * 0.5,
            y: publishPanelY + Math.max(132, Math.round(148 * scale)),
            w: overlayButtonW,
            h: overlayButtonH,
            fontSize: Math.max(14, Math.round(19 * scale)),
          },
          actionButton: {
            id: "editor_publish_confirm",
            label: editor.status === "pending" || editor.status === "published" ? "Unpublish" : "Publish",
            x: publishPanelX + publishPanelW * 0.5 - overlayButtonW * 0.5,
            y: publishPanelY + Math.max(208, Math.round(226 * scale)),
            w: overlayButtonW,
            h: overlayButtonH,
            fontSize: Math.max(14, Math.round(19 * scale)),
          },
          backButton: {
            id: "editor_publish_back",
            label: "Back",
            x: publishPanelX + Math.max(24, Math.round(28 * scale)),
            y: publishPanelY + publishPanelH - overlayButtonH - Math.max(18, Math.round(22 * scale)),
            w: Math.max(88, Math.round(108 * scale)),
            h: overlayButtonH,
            fontSize: Math.max(13, Math.round(18 * scale)),
          },
        },
        confirmOverlay: {
          panel: { x: confirmPanelX, y: confirmPanelY, w: confirmPanelW, h: confirmPanelH, radius: Math.max(18, Math.round(24 * scale)) },
          buttons: [
            {
              id: "editor_confirm_cancel",
              label: "Cancel",
              x: this.width * 0.5 - overlayGap * 0.5 - overlayButtonW,
              y: confirmPanelY + confirmPanelH - overlayButtonH - Math.max(18, Math.round(22 * scale)),
              w: overlayButtonW,
              h: overlayButtonH,
              fontSize: Math.max(14, Math.round(19 * scale)),
            },
            {
              id: "editor_confirm_accept",
              label: "Confirm",
              x: this.width * 0.5 + overlayGap * 0.5,
              y: confirmPanelY + confirmPanelH - overlayButtonH - Math.max(18, Math.round(22 * scale)),
              w: overlayButtonW,
              h: overlayButtonH,
              fontSize: Math.max(14, Math.round(19 * scale)),
    ...
    getCreateLevelTestLayout() {
      return this.getCreateLevelLayout();
    }

    getCreateLevelPaletteCards(layout = this.getCreateLevelLayout()) {
      const editor = this.createLevelEditor;
      if (!editor || !layout || layout.mode !== "editor" || layout.bottomPanelCollapsed) {
        return [];
      }
      const items = this.getCreateLevelPaletteItems(editor.activeTab);
      const uiScale = layout.scale * (layout.bottomPanelScale || 1);
      const cardW = Math.max(72, Math.round(98 * uiScale));
      const gap = Math.max(8, Math.round(12 * uiScale));
      const scrollX = editor.paletteScrollByTab[editor.activeTab] || 0;
      return items.map((item, index) => ({
        ...item,
        index,
        x: layout.paletteViewport.x + index * (cardW + gap) - scrollX,
        y: layout.paletteViewport.y,
        w: cardW,
        h: layout.paletteViewport.h,
      }));
    }

    getCreateLevelPaletteMaxScroll(layout = this.getCreateLevelLayout()) {
      const editor = this.createLevelEditor;
      if (!editor || !layout || layout.mode !== "editor" || layout.bottomPanelCollapsed) {
        return 0;
      }
      ...
      const cardW = Math.max(72, Math.round(98 * uiScale));
      const gap = Math.max(8, Math.round(12 * uiScale));
      const contentW = items.length * cardW + Math.max(0, items.length - 1) * gap;
      return Math.max(0, contentW - layout.paletteViewport.w);
    }

    clampCreateLevelPaletteScroll(layout = this.getCreateLevelLayout()) {
      const editor = this.createLevelEditor;
      if (!editor || !layout || layout.mode !== "editor") {
        return;
      }
      ...
      editor.paletteScrollByTab[tab] = clamp(editor.paletteScrollByTab[tab] || 0, 0, this.getCreateLevelPaletteMaxScroll(layout));
    }
    ...
    getCampaignCompleteButtons() {
      const scale = this.getScreenFitScale(560, 520, 0.55);
      const panelW = Math.min(Math.round(520 * scale), this.width - 24);
      const panelH = Math.min(Math.round(300 * scale), this.height - 24);
      const panelX = this.width * 0.5 - panelW * 0.5;
      const panelY = this.height * 0.5 - panelH * 0.5;
      if (this.isCustomLevelMode()) {
        if (this.gameMode === "publish_test") {
          const row = this.getPanelButtonRowLayout(panelX, panelY, panelW, panelH, 3, scale, { preferredWidth: 144, preferredHeight: 54, gap: 14, sidePad: 24, bottomPad: 20 });
          const exitLabel = this.isGameBackButtonMode() ? "Back" : "Home";
          return [
            { id: "home", label: exitLabel, x: row.startX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
            { id: "replay", label: "Retry", x: row.startX + row.stepX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
            { id: "publish_complete", label: "Publish", x: row.startX + row.stepX * 2, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(12, Math.round(19 * scale)), radius: row.radius },
          ];
        }
        const row = this.getPanelButtonRowLayout(panelX, panelY, panelW, panelH, 2, scale, { preferredWidth: 154, preferredHeight: 54, gap: 14, sidePad: 24, bottomPad: 20 });
        const exitLabel = this.isGameBackButtonMode() ? "Back" : "Home";
        return [
          { id: "home", label: exitLabel, x: row.startX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
          { id: "replay", label: "Restart", x: row.startX + row.stepX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
        ];
      }
      const row = this.getPanelButtonRowLayout(panelX, panelY, panelW, panelH, 3, scale, { preferredWidth: 144, preferredHeight: 54, gap: 14, sidePad: 24, bottomPad: 20 });
      return [
        { id: "home", label: "Home", x: row.startX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
        { id: "replay", label: "Play Again", x: row.startX + row.stepX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(12, Math.round(19 * scale)), radius: row.radius },
        { id: "next", label: "Next", x: row.startX + row.stepX * 2, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
      ];
    }

    getPanelButtonRowLayout(panelX, panelY, panelW, panelH, count, scale, options = {}) {
      const sidePad = Math.max(12, Math.round((options.sidePad ?? 24) * scale));
      const bottomPad = Math.max(12, Math.round((options.bottomPad ?? 24) * scale));
      const gap = Math.max(8, Math.round((options.gap ?? 18) * scale));
      const preferredWidth = Math.round((options.preferredWidth ?? 148) * scale);
      const preferredHeight = Math.round((options.preferredHeight ?? 50) * scale);
      const minWidth = options.minWidth ?? 70;
      const minHeight = options.minHeight ?? 36;
      const availableWidth = Math.max(minWidth, panelW - sidePad * 2 - gap * Math.max(0, count - 1));
      const buttonW = Math.max(minWidth, Math.min(preferredWidth, Math.floor(availableWidth / Math.max(1, count))));
      const buttonH = Math.max(minHeight, Math.min(preferredHeight, Math.floor(panelH * 0.24)));
      const totalWidth = buttonW * count + gap * Math.max(0, count - 1);
      return {
        buttonW,
        buttonH,
        gap,
        stepX: buttonW + gap,
        startX: Math.round(panelX + panelW * 0.5 - totalWidth * 0.5),
        y: Math.round(panelY + panelH - bottomPad - buttonH),
        radius: Math.max(12, Math.round(buttonH * 0.24)),
      };
    }

    getHomeConfirmButtons() {
      const scale = this.getScreenFitScale(520, 420, 0.58);
      const panelW = Math.min(Math.round(460 * scale), this.width - 24);
      const panelH = Math.min(Math.round(204 * scale), this.height - 24);
      const panelX = this.width * 0.5 - panelW * 0.5;
      const panelY = this.height * 0.5 - panelH * 0.5;
      const row = this.getPanelButtonRowLayout(panelX, panelY, panelW, panelH, 2, scale, { preferredWidth: 148, preferredHeight: 50, gap: 18, sidePad: 24, bottomPad: 22 });
      return [
        { id: "cancel", label: "Cancel", x: row.startX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
        { id: "leave", label: "Leave", x: row.startX + row.stepX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
      ];
    }
    ...
    getSignOutConfirmButtons() {
      ...
      return [
        { id: "cancel", label: "Cancel", x: row.startX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
        { id: "signout", label: "Sign Out", x: row.startX + row.stepX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
      ];
    }
    ...
    getPublishConfirmButtons() {
      ...
      return [
        { id: "cancel", label: "Cancel", x: row.startX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
        { id: "publish", label: "Publish", x: row.startX + row.stepX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
      ];
    }
    ...
    getCreatePublishPromptButtons() {
      const scale = this.getScreenFitScale(520, 420, 0.58);
      const panelW = Math.min(Math.round(520 * scale), this.width - 24);
      const panelH = Math.min(Math.round(244 * scale), this.height - 24);
      const panelX = this.width * 0.5 - panelW * 0.5;
      const panelY = this.height * 0.5 - panelH * 0.5;
      const row = this.getPanelButtonRowLayout(panelX, panelY, panelW, panelH, 2, scale, { preferredWidth: 148, preferredHeight: 50, gap: 18, sidePad: 24, bottomPad: 22 });
      return [
        { id: "back", label: "Back", x: row.startX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
        { id: "continue", label: "Continue", x: row.startX + row.stepX, y: row.y, w: row.buttonW, h: row.buttonH, fontSize: Math.max(13, Math.round(22 * scale)), radius: row.radius },
      ];
    }
    ...
    getLevelSelectGroups() {
      ...
      return groupDefinitions.flatMap((group) => {
        ...
        for (let offset = 0; offset < count; offset += 1) {
          ...
          const difficulty = getLevelDifficultyTier(LEVELS[index]?.difficulty);
          cards.push({
            index,
            worldTitle: group.title,
            themeColor: group.themeColor,
            difficultyDots: difficulty.dots,
    ...
    setPointerPosition(x, y, inside = true) {
      ...
      } else if (this.currentScreen === "leaderboard_replay") {
        this.refreshLeaderboardReplayHover();
      } else if (this.currentScreen === "online" || this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level" || this.currentScreen === "my_levels" || this.currentScreen === "my_level_detail" || this.isOnlineRacingScreen()) {
        this.refreshOnlineHover();
    ...
    onCanvasPointerMove(event) {
      ...
      if (this.currentScreen !== "create_level" || !this.createLevelEditor || !this.createLevelPointerGesture) {
        return;
      }
      const gesture = this.createLevelPointerGesture;
      const editor = this.createLevelEditor;
      const layout = this.getCreateLevelLayout();
      if (gesture.mode === "pan" && layout.mode === "editor") {
        const deltaX = point.x - gesture.startX;
        const deltaY = point.y - gesture.startY;
        if (Math.hypot(deltaX, deltaY) > 4) {
          gesture.moved = true;
        }
        editor.cameraX = gesture.startCameraX - deltaX / Math.max(0.001, editor.zoom);
        editor.cameraY = gesture.startCameraY - deltaY / Math.max(0.001, editor.zoom);
        this.clampCreateLevelCamera(editor, layout);
        this.canvas.style.cursor = "grabbing";
        return;
      }
      if (gesture.mode === "palette_scroll" && layout.mode === "editor") {
        const deltaX = point.x - gesture.startX;
        if (Math.abs(deltaX) > 4) {
          gesture.moved = true;
        }
        editor.paletteScrollByTab[gesture.tab] = gesture.startScroll - deltaX;
        this.clampCreateLevelPaletteScroll(layout);
        this.canvas.style.cursor = "grabbing";
        return;
      }
      if (gesture.mode === "paint" && layout.mode === "editor") {
        if (Math.hypot(point.x - gesture.startX, point.y - gesture.startY) > 4) {
          gesture.moved = true;
        }
        const grid = this.getCreateLevelGridAtScreen(point.x, point.y, layout, editor);
        if (grid) {
          this.continueCreateLevelPaintGesture(gesture, grid.gridX, grid.gridY);
        }
        this.canvas.style.cursor = "crosshair";
        return;
      }
      if (gesture.mode === "resize_edge" && layout.mode === "editor") {
        this.applyCreateLevelResizeEdgeDrag(gesture, point.x, point.y, layout, editor);
        this.canvas.style.cursor = gesture.edge === "left" || gesture.edge === "right" ? "ew-resize" : "ns-resize";
        return;
      }
      if (gesture.mode === "angle") {
        if (Math.hypot(point.x - gesture.startX, point.y - gesture.startY) > 2) {
          gesture.moved = true;
        }
        this.setCreateLevelAngleFromPoint(point.x, point.y);
        this.canvas.style.cursor = "grabbing";
      }
    }
    ...
    onCanvasPointerDown(event) {
      ...
      if (this.currentScreen !== "create_level" || !this.createLevelEditor || event.button !== 0) {
        return;
      }
      const point = this.getCanvasPointFromClient(event.clientX, event.clientY);
      const editor = this.createLevelEditor;
      const layout = this.getCreateLevelLayout();
      const target = this.getCreateLevelHoverTarget(point.x, point.y);
      this.createLevelSuppressNextClick = false;
      this.createLevelPointerGesture = null;
      if (target?.type === "angle") {
        ...
        this.pushCreateLevelUndoSnapshot(editor);
        this.createLevelPointerGesture = {
          mode: "angle",
          startX: point.x,
          startY: point.y,
          moved: false,
      ...
      if (target?.type === "resize_edge") {
        ...
        this.createLevelPointerGesture = {
          ...
          startY: point.y,
          moved: false,
          resizeSnapshot: this.captureCreateLevelResizeSnapshot(editor),
          resizeViewport: { ...layout.viewport },
        };
        return;
      }
      if (editor.mode !== "editor" || editor.menuOpen || editor.publishPanelOpen || editor.exitConfirmOpen || editor.deleteConfirmOpen) {
        return;
      }
      if (target?.type === "viewport") {
        if (editor.mouseMode === "drag" && !this.isCreateLevelEditorLocked(editor)) {
          this.createLevelPointerGesture = {
            mode: "paint",
            startX: point.x,
            startY: point.y,
          ...
          const grid = this.getCreateLevelGridAtScreen(point.x, point.y);
          if (grid) {
            this.continueCreateLevelPaintGesture(this.createLevelPointerGesture, grid.gridX, grid.gridY);
          }
          return;
        }
        this.createLevelPointerGesture = {
          ...
          startX: point.x,
          startY: point.y,
          startCameraX: editor.cameraX,
          startCameraY: editor.cameraY,
          moved: false,
        };
        return;
      }
      if (target?.type === "palette" && editor.activeTab !== "erase") {
        this.createLevelPointerGesture = {
          mode: "palette_scroll",
          tab: editor.activeTab,
          startX: point.x,
          startY: point.y,
          startScroll: editor.paletteScrollByTab[editor.activeTab] || 0,
          moved: false,
        };
      }
    }
    ...
    onCanvasPointerUp() {
      if (this.currentScreen === "create_level" && this.createLevelPointerGesture) {
        this.createLevelSuppressNextClick = Boolean(
          this.createLevelPointerGesture.moved ||
          this.createLevelPointerGesture.mode === "paint" ||
          this.createLevelPointerGesture.mode === "angle" ||
          this.createLevelPointerGesture.mode === "resize_edge"
        );
        this.createLevelPointerGesture = null;
        this.refreshCreateLevelHover();
      }
    ...
    onCanvasPointerLeave() {
      ...
      this.mouse.down = false;
      this.createLevelPointerGesture = null;
      this.resetCreateLevelTouchGesture();
      this.hoveredCreateLevelButton = null;
      this.hoveredCreateLevelPaletteIndex = null;
      this.hoveredCreateLevelResizeEdge = null;
      this.hoveredTitleButton = null;
    ...
    onCanvasWheel(event) {
      if (this.currentScreen === "create_level" && this.createLevelEditor?.mode === "editor") {
        const point = this.getCanvasPointFromClient(event.clientX, event.clientY);
        const layout = this.getCreateLevelLayout();
        if (layout.mode === "editor" && this.isPointInsideRect(point.x, point.y, layout.viewport)) {
          this.zoomCreateLevelCamera(event.deltaY < 0 ? 1 : -1, point.x, point.y);
          if (event.cancelable) {
            event.preventDefault();
          }
          return;
        }
        if (layout.mode === "editor" && this.isPointInsideRect(point.x, point.y, layout.paletteViewport)) {
          const tab = this.createLevelEditor.activeTab;
          if (tab !== "erase") {
            this.createLevelEditor.paletteScrollByTab[tab] += event.deltaY;
            this.clampCreateLevelPaletteScroll(layout);
            if (event.cancelable) {
              event.preventDefault();
            }
    ...
    handleCanvasTapAt(x, y) {
      ...
      if (this.currentScreen === "create_level") {
        this.handleCreateLevelTap(x, y);
        return;
      }
      if (this.currentScreen === "title") {
        ...
        } else if (this.hoveredTitleButton === "settings") {
          this.openSettings();
      ...
      if (this.currentScreen === "settings") {
        if (this.hoveredSettingsButton === "back") {
          this.returnToTitle();
        } else if (typeof this.hoveredSettingsButton === "string" && this.hoveredSettingsButton.startsWith("ghost_")) {
          const nextMode = this.hoveredSettingsButton.slice("ghost_".length);
          if (GHOST_MODE_OPTIONS.some((option) => option.id === nextMode)) {
            this.ghostMode = nextMode;
            this.savePersistentProgress();
            this.refreshSettingsHover();
          }
        }
      ...
      if (this.currentScreen === "online") {
        ...
        } else if (this.hoveredOnlineButton === "online_racing") {
          this.openOnlineRacingMenu();
        }
        return;
      }
      if (this.isOnlineRacingScreen()) {
        if (this.hoveredOnlineButton === "back") {
          if (this.currentScreen === "online_racing") {
            this.openOnlineMenu();
          } else {
            this.openOnlineRacingMenu();
          }
        } else if (this.hoveredOnlineButton === "online_racing_join") {
          this.openOnlineRacingJoin();
        } else if (this.hoveredOnlineButton === "online_racing_create") {
          this.openOnlineRacingHost();
        } else if (this.hoveredOnlineButton === "online_racing_account") {
          this.openOnlineRacingAccount();
        } else if (this.hoveredOnlineButton === "online_racing_tab_public") {
          this.onlineRacingSelectedTab = "public";
          this.hoveredOnlineRacingMatch = null;
          this.onlineRacingMatchHovers = this.getOnlineRacingMatchCards().map(() => 0);
          this.refreshOnlineHover();
        } else if (this.hoveredOnlineButton === "online_racing_tab_private") {
          this.onlineRacingSelectedTab = "private";
          this.hoveredOnlineRacingMatch = null;
          this.onlineRacingMatchHovers = this.getOnlineRacingMatchCards().map(() => 0);
          this.refreshOnlineHover();
        }
      ...
      if (this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level" || this.currentScreen === "my_levels" || this.currentScreen === "my_level_detail") {
        ...
        if (this.currentScreen === "my_level_detail" && this.myLevelActionPrompt) {
          if (this.hoveredMyLevelActionPromptButton === "cancel") {
            this.closeMyLevelActionPrompt();
          } else if (this.hoveredMyLevelActionPromptButton === "confirm") {
            this.confirmMyLevelActionPrompt();
          }
          return;
        }
        ...
        if (this.hoveredOnlineButton === "back") {
          if (this.currentScreen === "community_detail") {
            ...
            if (this.createLevelReturnScreen === "my_level_detail" && this.getActiveMyLevel()) {
              this.reopenActiveMyLevelDetail();
            } else if (this.createLevelReturnScreen === "my_levels") {
              this.openMyLevels();
            } else {
              this.openOnlineMenu();
            }
          } else if (this.currentScreen === "my_level_detail") {
            this.openMyLevels();
        ...
        } else if (this.hoveredOnlineButton === "play_my_level_detail") {
          this.playActiveMyLevelFromDetail();
        } else if (this.hoveredOnlineButton === "my_level_detail_action") {
          const actionInfo = this.getMyLevelDetailActionInfo();
          if (actionInfo.enabled && (actionInfo.mode === "publish" || actionInfo.mode === "unpublish")) {
            this.openMyLevelActionPrompt(actionInfo.mode);
          }
        } else if (this.hoveredOnlineButton === "edit_my_level") {
          const activeLevel = this.getActiveMyLevel();
          if (activeLevel) {
            this.openCreateLevel("my_level_detail", { editLevel: activeLevel });
          }
        } else if (this.hoveredOnlineButton === "delete_my_level") {
          this.openMyLevelActionPrompt("delete");
          ...
          this.openMyLevelDetail(this.hoveredCommunityLevel);
        }
        return;
      }
      ...
      if (this.currentScreen === "leaderboard") {
        if (this.hoveredLeaderboardButton === "back") {
          ...
          this.refreshLeaderboardHover();
        } else if (this.hoveredLeaderboardReplayId) {
          const replayTarget = this.getLeaderboardReplayTargets().find((target) => target.id === this.hoveredLeaderboardReplayId);
          if (replayTarget) {
            this.openLeaderboardReplay(replayTarget);
          }
        }
        return;
      }
      if (this.currentScreen === "leaderboard_replay") {
        if (this.hoveredLeaderboardButton === "back") {
          this.closeLeaderboardReplay();
        } else if (this.hoveredLeaderboardButton === "replay_prev") {
          this.stepLeaderboardReplayFrame(this.activeLeaderboardReplay, -1);
        } else if (this.hoveredLeaderboardButton === "replay_next") {
          this.stepLeaderboardReplayFrame(this.activeLeaderboardReplay, 1);
        } else if (this.activeLeaderboardReplay) {
          const lastIndex = Math.max(0, this.activeLeaderboardReplay.frames.length - 1);
          if (this.activeLeaderboardReplay.paused && this.activeLeaderboardReplay.ended && this.activeLeaderboardReplay.frameIndex >= lastIndex && lastIndex > 0) {
            this.resetLeaderboardReplayPlaybackState(this.activeLeaderboardReplay, { preserveFrame: false });
          } else {
            this.activeLeaderboardReplay.paused = !this.activeLeaderboardReplay.paused;
          }
        }
        return;
      }
    ...
    getCreateLevelAngleDial(layout = this.getCreateLevelLayout()) {
      const editor = this.createLevelEditor;
      if (!editor || !layout || layout.mode !== "editor") {
        return null;
      }
      const startTile = this.getCreateLevelStartTile(editor);
      if (!startTile) {
        return null;
      }
      const center = this.createLevelWorldToScreen((startTile.x + 0.5) * TILE_SIZE, (startTile.y + 0.5) * TILE_SIZE, layout.viewport, editor);
      const radius = Math.max(16, TILE_SIZE * editor.zoom * 0.42);
      return {
        centerX: center.x,
        centerY: center.y,
        radius,
        dotX: center.x + Math.cos(editor.startAngle) * radius,
        dotY: center.y + Math.sin(editor.startAngle) * radius,
      };
    }

    resetCreateLevelEditorCameraHome() {
      const editor = this.createLevelEditor;
      if (!editor || editor.mode !== "editor") {
        return;
      }
      ...
      const bounds = this.getCreateLevelEditorWorldBounds(editor);
      const fitScale = Math.min(layout.viewport.w / Math.max(1, bounds.w), layout.viewport.h / Math.max(1, bounds.h)) * 0.92;
      editor.zoom = clamp(fitScale, 0.28, 2.2);
      this.centerCreateLevelCamera(editor);
      this.clampCreateLevelCamera(editor, layout);
    }

    setCreateLevelAngleFromPoint(screenX, screenY) {
      const layout = this.getCreateLevelLayout();
      const dial = this.getCreateLevelAngleDial(layout);
      if (!dial || !this.createLevelEditor || this.isCreateLevelEditorLocked(this.createLevelEditor)) {
        return false;
      }
      const angle = Math.atan2(screenY - dial.centerY, screenX - dial.centerX);
      this.createLevelEditor.startAngle = this.snapCreateLevelAngle(angle);
      this.createLevelEditor.angleEditActive = true;
      this.markCreateLevelEditorChanged();
      return true;
    }

    getCreateLevelHoverTarget(x, y) {
      const layout = this.getCreateLevelLayout();
      if (!layout) {
        return null;
      }
      if (layout.mode === "setup") {
        const hoveredButton = [...layout.sourceButtons, ...layout.buttons].find((button) => this.isPointInsideRect(x, y, button));
        return hoveredButton ? { type: "button", id: hoveredButton.id } : null;
      }
      if (this.createLevelEditor?.exitConfirmOpen || this.createLevelEditor?.deleteConfirmOpen) {
        const hoveredConfirm = layout.confirmOverlay.buttons.find((button) => this.isPointInsideRect(x, y, button));
        return hoveredConfirm ? { type: "button", id: hoveredConfirm.id } : null;
      }
      if (this.createLevelEditor?.publishPanelOpen) {
        const publishButtons = [layout.publishOverlay.testButton, layout.publishOverlay.actionButton, layout.publishOverlay.backButton];
        const hoveredPublish = publishButtons.find((button) => this.isPointInsideRect(x, y, button));
        return hoveredPublish ? { type: "button", id: hoveredPublish.id } : null;
      }
      if (this.createLevelEditor?.menuOpen) {
        const hoveredMenuButton = layout.menuOverlay.buttons.find((button) => this.isPointInsideRect(x, y, button));
        return hoveredMenuButton ? { type: "button", id: hoveredMenuButton.id } : null;
      }
      const allButtons = [
        layout.saveButton,
        layout.bottomToggleButton,
        ...layout.historyButtons,
        ...layout.quickButtons,
        ...layout.sideButtons,
        ...layout.mouseModeButtons,
        ...layout.tabs,
        ...layout.sizeControls.buttons,
      ];
      const hoveredButton = allButtons.find((button) => this.isPointInsideRect(x, y, button));
      if (hoveredButton) {
        return { type: "button", id: hoveredButton.id };
      }
      const hoveredPalette = this.getCreateLevelPaletteCards(layout).find((card) => this.isPointInsideRect(x, y, card));
      if (hoveredPalette) {
        return { type: "palette", index: hoveredPalette.index };
      }
      const dial = this.getCreateLevelAngleDial(layout);
      if (dial) {
        const dotDistance = Math.hypot(x - dial.dotX, y - dial.dotY);
        const ringDistance = Math.abs(Math.hypot(x - dial.centerX, y - dial.centerY) - dial.radius);
        if (dotDistance <= Math.max(12, dial.radius * 0.26) || ringDistance <= Math.max(8, dial.radius * 0.16)) {
          return { type: "angle" };
        }
      }
      const resizeEdge = this.getCreateLevelResizeEdgeTarget(x, y, layout, this.createLevelEditor);
      if (resizeEdge) {
        return resizeEdge;
      }
      if (this.isPointInsideRect(x, y, layout.viewport)) {
        return { type: "viewport" };
      }
      return null;
    }

    refreshCreateLevelHover() {
      if (this.currentScreen !== "create_level" || !this.mouse.inside || !this.createLevelEditor) {
        this.hoveredCreateLevelButton = null;
        this.hoveredCreateLevelPaletteIndex = null;
        this.hoveredCreateLevelResizeEdge = null;
        this.canvas.style.cursor = "default";
      ...
      const target = this.getCreateLevelHoverTarget(this.mouse.x, this.mouse.y);
      this.hoveredCreateLevelButton = target?.type === "button" ? target.id : null;
      this.hoveredCreateLevelPaletteIndex = target?.type === "palette" ? target.index : null;
      this.hoveredCreateLevelResizeEdge = target?.type === "resize_edge" ? target.edge : null;
      this.canvas.style.cursor = target
        ? (target.type === "resize_edge"
          ? target.cursor
          : target.type === "viewport"
          ? (this.createLevelEditor?.mouseMode === "drag" ? "crosshair" : "grab")
          : "pointer")
        : "default";
    }

    updateCreateLevelButtonHovers(layout, dt) {
      if (!layout) {
        return;
      }
      const editor = this.createLevelEditor;
      const hoveredId = this.hoveredCreateLevelButton;
      const visibleIds = new Set();
      const updateHover = (button, active = false) => {
        if (!button?.id) {
          return;
        }
        visibleIds.add(button.id);
        const target = active || hoveredId === button.id ? 1 : 0;
        this.createLevelButtonHovers[button.id] = approachExp(this.createLevelButtonHovers[button.id] || 0, target, 16, dt);
      };
      const updateHoverList = (buttons, activePredicate = null) => {
        for (const button of buttons || []) {
          updateHover(button, activePredicate ? activePredicate(button) : false);
        }
      };

      if (layout.mode === "setup") {
        updateHoverList(layout.sourceButtons, (button) => (button.id === "setup_source_blank" && editor?.setupSource !== "import")
          || (button.id === "setup_source_import" && editor?.setupSource === "import"));
        updateHoverList(layout.buttons);
      } else {
        updateHover(layout.saveButton);
        updateHover(layout.bottomToggleButton);
        updateHoverList(layout.historyButtons);
        updateHoverList(layout.quickButtons);
        updateHoverList(layout.sideButtons);
        updateHoverList(layout.mouseModeButtons);
        updateHoverList(layout.tabs, (button) => editor?.activeTab === button.id);
        updateHoverList(layout.sizeControls?.buttons);
        if (editor?.menuOpen) {
          updateHoverList(layout.menuOverlay?.buttons);
        }
        if (editor?.publishPanelOpen) {
          updateHover(layout.publishOverlay?.testButton);
          updateHover(layout.publishOverlay?.actionButton);
          updateHover(layout.publishOverlay?.backButton);
        }
        if (editor?.exitConfirmOpen || editor?.deleteConfirmOpen) {
          updateHoverList(layout.confirmOverlay?.buttons);
        }
      }

      for (const id of Object.keys(this.createLevelButtonHovers)) {
        if (visibleIds.has(id)) {
          continue;
        }
        const next = approachExp(this.createLevelButtonHovers[id] || 0, 0, 16, dt);
        if (next <= 0.001) {
          delete this.createLevelButtonHovers[id];
        } else {
          this.createLevelButtonHovers[id] = next;
        }
      }
    }

    getCreateLevelButtonHover(buttonId, active = false) {
      const hover = this.createLevelButtonHovers[buttonId] || 0;
      return active ? Math.max(hover, 1) : hover;
    }

    openCreateLevelExitConfirm() {
      if (!this.createLevelEditor) {
        return;
      }
      if (!this.createLevelEditorHasUnsavedChanges(this.createLevelEditor)) {
        this.exitCreateLevelEditor();
        return;
      }
      this.createLevelEditor.exitConfirmOpen = true;
      this.createLevelEditor.deleteConfirmOpen = false;
      this.createLevelEditor.menuOpen = false;
      this.createLevelEditor.publishPanelOpen = false;
      this.refreshCreateLevelHover();
    }

    exitCreateLevelEditor() {
      if (this.createLevelReturnScreen === "my_level_detail" && this.getActiveMyLevel()) {
        this.reopenActiveMyLevelDetail();
      } else if (this.createLevelReturnScreen === "my_levels") {
        this.openMyLevels();
    ...
    closeCreateLevelConfirm() {
      if (!this.createLevelEditor) {
        return;
      }
      this.createLevelEditor.exitConfirmOpen = false;
      this.createLevelEditor.deleteConfirmOpen = false;
      this.refreshCreateLevelHover();
    }

    openCreateLevelPublishPanel() {
      if (!this.createLevelEditor) {
        return;
      }
      this.createLevelEditor.publishPanelOpen = true;
      this.createLevelEditor.menuOpen = false;
      this.createLevelEditor.exitConfirmOpen = false;
      this.createLevelEditor.deleteConfirmOpen = false;
      this.syncCreateLevelEditorMetaInputs();
    ...
    async publishCreateLevelEditor() {
      const editor = this.createLevelEditor;
      if (!editor) {
        return;
      }
      if (editor.status === "pending" || editor.status === "published") {
        await this.unpublishCreateLevelEditor();
        return;
      }
      const built = this.preparePendingCommunityLevelFromEditor();
      if (!built || !this.pendingCommunityLevel) {
        return;
      }
      if (this.createLevelEditorHasPendingChangesSinceValidation(editor) || !Number.isFinite(editor.validatedTime)) {
        this.setCreateLevelEditorMessage("Beat the current version first to publish it.", "error");
        return;
      }
      this.pendingCommunityLevel.bestTime = editor.validatedTime;
      ...
      try {
        if (this.isOnlineCommunityConfigured()) {
          ...
          const saved = this.upsertMyCreatedLevel(
            submittedRow || {
              ...this.pendingCommunityLevel,
              creatorTime: editor.validatedTime,
              myBestTime: editor.validatedTime,
              leaderboard: normalizeCommunityLeaderboard([{ playerName: this.playerUsername, time: editor.validatedTime }], this.playerUsername, editor.validatedTime),
          ...
          if (saved) {
            this.editingMyLevel = { ...saved };
            this.setActiveMyLevel(saved);
          }
          this.syncOnlineMyLevels({ force: true });
        } else {
          const saved = this.upsertMyCreatedLevel(
            {
              ...(this.editingMyLevel || {}),
              ...
              submissionStatus: "pending",
              reviewMessage: "",
              createdAt: this.editingMyLevel?.createdAt || Date.now(),
              updatedAt: Date.now(),
            },
            { fallbackStatus: "pending" }
          );
        ...
        editor.publishPanelOpen = false;
        this.savePersistentProgress();
        this.setCreateLevelEditorMessage("Level sent for review.", "ok");
      } catch {
        this.setCreateLevelEditorMessage("Publish request failed. Check the Supabase community SQL setup.", "error");
      } finally {
        this.communityPublishRequestInFlight = false;
      }
    }

    async unpublishCreateLevelEditor() {
      const editor = this.createLevelEditor;
      const active = this.isEditingMyLevel() ? this.editingMyLevel : this.getActiveMyLevel();
      if (!editor || !active) {
        return;
      }
      if (active.submissionId && this.hasSignedInAccountSession() && this.canUnpublishOnlineCommunityLevel()) {
        try {
          await this.unpublishOnlineMyLevel(active);
        } catch {
          this.setCreateLevelEditorMessage("Unpublish failed. Apply the updated community SQL first.", "error");
          return;
        }
      }
      const saved = this.applyMyCreatedLevelPatch(active, {
        submissionStatus: "private",
        remoteId: "",
        reviewMessage: "",
        updatedAt: Date.now(),
      ...
      editor.status = "private";
      editor.publishPanelOpen = false;
      this.setCreateLevelEditorMessage("Level moved back to private.", "ok");
    }

    runCreateLevelEditorPlaytest(publishValidation = false) {
      const built = publishValidation ? this.preparePendingCommunityLevelFromEditor() : this.buildCreateLevelDefinitionFromEditor();
      if (!built || built.error) {
        this.setCreateLevelEditorMessage(built?.error || "Finish the level first.", "error");
        return;
      }
      ...
      this.beginScreenWipe(() => {
        ...
        this.gameMode = publishValidation ? "publish_test" : "custom";
        this.hoveredCreateLevelButton = null;
        this.canvas.style.cursor = "default";
        this.loadCustomLevel(built.definition);
      });
    }

    performCreateLevelAction(actionId) {
      const editor = this.createLevelEditor;
      if (!editor) {
        return;
      }
      if (actionId === "setup_back") {
        this.exitCreateLevelEditor();
        return;
      }
      if (actionId === "setup_source_blank" || actionId === "setup_source_import") {
        editor.setupSource = actionId === "setup_source_import" ? "import" : "blank";
        editor.messageText = "";
        editor.messageTone = "neutral";
        this.updateCustomLevelInputVisibility();
        this.refreshCreateLevelHover();
        return;
      }
      if (actionId === "setup_done") {
        if (editor.setupSource === "import") {
          const imported = this.applyImportedCreateLevelMapText(editor.importText, editor);
          if (imported.error) {
            this.setCreateLevelEditorMessage(imported.error, "error");
            this.updateCustomLevelInputVisibility();
            return;
          }
        }
        ...
        editor.messageText = "";
        editor.messageTone = "neutral";
        this.resetCreateLevelEditorCameraHome();
        this.refreshCreateLevelHover();
        this.updateCustomLevelInputVisibility();
        return;
      }
      if (actionId === "editor_save") {
        if (!this.createLevelEditorHasUnsavedChanges(editor)) {
          return;
        }
        this.saveCreateLevelEditor();
        return;
      }
      if (actionId === "editor_bottom_toggle") {
        this.createLevelBottomPanelCollapsed = !this.createLevelBottomPanelCollapsed;
        this.updateCustomLevelInputVisibility();
        this.refreshCreateLevelHover();
        return;
      }
      if (actionId === "editor_undo") {
        this.undoCreateLevelEditor(editor);
        return;
      }
      if (actionId === "editor_redo") {
        this.redoCreateLevelEditor(editor);
        return;
      }
      if (actionId === "editor_play") {
        this.runCreateLevelEditorPlaytest(false);
        return;
      }
      if (actionId === "editor_publish") {
        if (editor.status === "published") {
          editor.deleteConfirmOpen = true;
          editor.exitConfirmOpen = false;
          editor.menuOpen = false;
          editor.publishPanelOpen = false;
        } else {
          this.openCreateLevelPublishPanel();
        }
        return;
      }
      if (actionId === "editor_menu") {
        editor.menuOpen = true;
        editor.publishPanelOpen = false;
        editor.exitConfirmOpen = false;
        editor.deleteConfirmOpen = false;
        this.refreshCreateLevelHover();
        return;
      }
      if (actionId === "editor_home") {
        this.resetCreateLevelEditorCameraHome();
        return;
      }
      if (actionId === "editor_zoom_in") {
        this.zoomCreateLevelCamera(1);
        return;
      }
      if (actionId === "editor_zoom_out") {
        this.zoomCreateLevelCamera(-1);
        return;
      }
      if (actionId === "editor_menu_back" || actionId === "editor_publish_back" || actionId === "editor_confirm_cancel") {
        editor.menuOpen = false;
        editor.publishPanelOpen = false;
        this.closeCreateLevelConfirm();
        return;
      }
      if (actionId === "editor_menu_exit") {
        this.openCreateLevelExitConfirm();
        return;
      }
      if (actionId === "editor_menu_save_exit") {
        this.saveCreateLevelEditor({ exitAfterSave: true });
        return;
      }
      if (actionId === "editor_menu_export") {
        this.exportCreateLevelEditorFile();
        editor.menuOpen = false;
        return;
      }
      if (actionId === "editor_publish_test") {
        this.runCreateLevelEditorPlaytest(true);
        return;
      }
      if (actionId === "editor_publish_confirm") {
        this.publishCreateLevelEditor();
        return;
      }
      if (actionId === "editor_confirm_accept") {
        if (editor.deleteConfirmOpen) {
          this.deleteEditingMyLevel();
        } else {
          this.exitCreateLevelEditor();
        }
        return;
      }
      if (actionId === "editor_width_up" || actionId === "editor_width_down" || actionId === "editor_height_up" || actionId === "editor_height_down") {
        if (this.isCreateLevelEditorLocked()) {
          this.setCreateLevelEditorMessage("Pending levels cannot be edited until you unpublish them.", "error");
          return;
        }
        this.pushCreateLevelUndoSnapshot(editor);
        const delta = actionId.endsWith("_up") ? 1 : -1;
        if (actionId.startsWith("editor_width")) {
          this.resizeCreateLevelEditor(editor, editor.width + delta, editor.height);
        } else {
          this.resizeCreateLevelEditor(editor, editor.width, editor.height + delta);
        }
        return;
      }
      if (actionId === "walls" || actionId === "floors" || actionId === "erase") {
        editor.activeTab = actionId;
        if (actionId === "walls" && !CREATE_LEVEL_WALL_PALETTE.some((item) => item.id === editor.selectedPaletteId)) {
          editor.selectedPaletteId = CREATE_LEVEL_WALL_PALETTE[0].id;
        }
        if (actionId === "floors" && !CREATE_LEVEL_FLOOR_PALETTE.some((item) => item.id === editor.selectedPaletteId)) {
          editor.selectedPaletteId = CREATE_LEVEL_FLOOR_PALETTE[0].id;
        }
        this.clampCreateLevelPaletteScroll();
        return;
      }
      if (actionId === "editor_mode_move" || actionId === "editor_mode_drag") {
        editor.mouseMode = actionId === "editor_mode_drag" ? "drag" : "move";
        this.refreshCreateLevelHover();
      }
    }

    handleCreateLevelTap(x, y) {
      if (!this.createLevelEditor) {
        return;
      }
      const target = this.getCreateLevelHoverTarget(x, y);
      if (target?.type === "button") {
        this.performCreateLevelAction(target.id);
        return;
      }
      if (target?.type === "palette") {
        const items = this.getCreateLevelPaletteItems(this.createLevelEditor.activeTab);
        const item = items[target.index];
        if (item) {
          this.createLevelEditor.selectedPaletteId = item.id;
        }
        return;
      }
      if (target?.type === "angle") {
        if (this.isCreateLevelEditorLocked(this.createLevelEditor)) {
          this.setCreateLevelEditorMessage("Pending levels cannot be edited until you unpublish them.", "error");
          return;
        }
        this.pushCreateLevelUndoSnapshot(this.createLevelEditor);
        this.setCreateLevelAngleFromPoint(x, y);
        return;
      }
      if (this.createLevelEditor.mode !== "editor" || this.createLevelEditor.menuOpen || this.createLevelEditor.publishPanelOpen || this.createLevelEditor.exitConfirmOpen || this.createLevelEditor.deleteConfirmOpen) {
        return;
      }
      if (target?.type === "viewport") {
        const grid = this.getCreateLevelGridAtScreen(x, y);
        if (grid) {
          if (this.isCreateLevelEditorLocked(this.createLevelEditor)) {
            this.setCreateLevelEditorMessage("Pending levels cannot be edited until you unpublish them.", "error");
            return;
          ...
          this.pushCreateLevelUndoSnapshot(this.createLevelEditor);
          this.tryApplyCreateLevelToolAt(grid.gridX, grid.gridY);
        }
      }
    }
    ...
    refreshTitleHover() {
      ...
      const settingsButton = this.getTitleSettingsButton();
      const hoveredSettings = this.mouse.x >= settingsButton.x &&
        this.mouse.x <= settingsButton.x + settingsButton.w &&
        this.mouse.y >= settingsButton.y &&
        this.mouse.y <= settingsButton.y + settingsButton.h;
      this.hoveredTitleOnlineButton = false;
      ...
      this.hoveredTitleButton = this.hoveredTitleUsernameButton ? null : (hoveredSettings ? "settings" : (hovered ? hovered.id : null));
      this.hoveredSettingsButton = hoveredSettings ? "settings" : null;
      this.hoveredSignOutConfirmButton = null;
      this.canvas.style.cursor = hovered || hoveredSettings || this.hoveredTitleUsernameButton ? "pointer" : "default";
    }

    refreshSettingsHover() {
      if (this.currentScreen !== "settings" || !this.mouse.inside) {
        this.hoveredSettingsButton = null;
        this.canvas.style.cursor = "default";
        return;
      }
      const buttons = [this.getSettingsBackButton(), ...this.getSettingsGhostOptionButtons()];
      const hovered = buttons.find((button) =>
        this.mouse.x >= button.x &&
        this.mouse.x <= button.x + button.w &&
        this.mouse.y >= button.y &&
        this.mouse.y <= button.y + button.h
      );
      this.hoveredSettingsButton = hovered ? hovered.id : null;
      this.canvas.style.cursor = hovered ? "pointer" : "default";
    }

    refreshOnlineHover() {
      if ((!["online", "community_levels", "community_detail", "create_level", "my_levels", "my_level_detail"].includes(this.currentScreen) && !this.isOnlineRacingScreen()) || !this.mouse.inside) {
        this.hoveredOnlineButton = null;
        this.hoveredCommunityLevel = null;
        this.hoveredOnlineRacingMatch = null;
        this.hoveredCreatePublishPromptButton = null;
      ...
      if (this.currentScreen === "my_level_detail" && this.myLevelActionPrompt) {
        const hoveredPromptButton = this.getMyLevelActionPromptButtons().find(
          (button) =>
            this.mouse.x >= button.x &&
            this.mouse.x <= button.x + button.w &&
            this.mouse.y >= button.y &&
        ...
        this.hoveredCommunityLevel = null;
        this.hoveredCreatePublishPromptButton = null;
        this.hoveredMyLevelActionPromptButton = hoveredPromptButton ? hoveredPromptButton.id : null;
        this.canvas.style.cursor = hoveredPromptButton ? "pointer" : "default";
        return;
      }
      ...
      const buttons = this.currentScreen === "online"
        ...
        : this.currentScreen === "online_racing"
          ? [this.getOnlineBackButton(), ...this.getOnlineRacingMenuButtons()]
        : this.currentScreen === "online_racing_join"
          ? [this.getOnlineBackButton(), ...this.getOnlineRacingJoinTabs()]
        : this.currentScreen === "online_racing_account"
          ? [this.getOnlineBackButton(), ...this.getOnlineRacingAccountButtons()]
        ...
        : this.currentScreen === "my_level_detail"
          ? [this.getOnlineBackButton(), ...(() => {
              const activeLevel = this.getActiveMyLevel();
              if (!activeLevel) {
                return [];
              }
              const layout = this.getMyLevelDetailLayout(activeLevel);
              return [layout.playButton, layout.actionButton, layout.editButton, layout.deleteButton];
            })()]
        : this.currentScreen === "create_level"
          ? [
              this.getOnlineBackButton(),
              ...(this.isEditingMyLevel() ? [] : [this.getCreateLevelLayout().copyButton]),
      ...
      const hoveredRacingMatch = this.currentScreen === "online_racing_join"
        ? this.getOnlineRacingMatchCards().find(
            (card) =>
              this.mouse.x >= card.x &&
              this.mouse.x <= card.x + card.w &&
              this.mouse.y >= card.y &&
      ...
      this.hoveredOnlineRacingMatch = hoveredRacingMatch ? hoveredRacingMatch.index : null;
      this.hoveredCreatePublishPromptButton = null;
      this.hoveredMyLevelActionPromptButton = null;
      this.canvas.style.cursor = hoveredButton || hoveredCard || hoveredRacingMatch ? "pointer" : "default";
    }
    ...
    refreshLeaderboardHover() {
      ...
      const hoveredReplay = this.getLeaderboardReplayTargets().find(
        (target) =>
          this.mouse.x >= target.x &&
          this.mouse.x <= target.x + target.w &&
          this.mouse.y >= target.y &&
          this.mouse.y <= target.y + target.h
      );
      this.hoveredLeaderboardButton = hovered ? hovered.id : null;
      this.hoveredLeaderboardReplayId = hoveredReplay ? hoveredReplay.id : null;
      this.hoveredLeaderboardPeriodTab = hoveredTab ? hoveredTab.id : null;
      this.canvas.style.cursor = hovered || hoveredTab || hoveredReplay ? "pointer" : "default";
    }

    refreshLeaderboardReplayHover() {
      if (this.currentScreen !== "leaderboard_replay" || !this.mouse.inside) {
        this.hoveredLeaderboardButton = null;
        this.canvas.style.cursor = "default";
        return;
      }
      const hovered = [this.getLeaderboardReplayBackButton(), ...this.getLeaderboardReplayStepButtons()].find(
        (button) =>
          this.mouse.x >= button.x &&
          this.mouse.x <= button.x + button.w &&
          this.mouse.y >= button.y &&
    ...
    handleCanvasClick(event) {
      const point = this.getCanvasPointFromClient(event.clientX, event.clientY);
      if (this.currentScreen === "create_level" && this.createLevelSuppressNextClick) {
        this.createLevelSuppressNextClick = false;
        this.setPointerPosition(point.x, point.y, true);
        return;
      }
    ...
    startCreateLevelSingleTouchGesture(touch, point, target = this.getCreateLevelHoverTarget(point.x, point.y)) {
      const editor = this.createLevelEditor;
      const layout = this.getCreateLevelLayout();
      this.resetCreateLevelTouchGesture();
      this.createLevelTouchGesture.touchId = touch.identifier;
      this.createLevelTouchGesture.startX = point.x;
      this.createLevelTouchGesture.startY = point.y;
      this.createLevelTouchGesture.lastX = point.x;
      this.createLevelTouchGesture.lastY = point.y;
      this.createLevelTouchGesture.targetType = target?.type || "tap";
      this.createLevelTouchGesture.targetId = target?.id || "";
      if (target?.type === "angle") {
        if (this.isCreateLevelEditorLocked(editor)) {
          this.setCreateLevelEditorMessage("Pending levels cannot be edited until you unpublish them.", "error");
          this.createLevelTouchGesture.mode = "tap";
          return;
        }
        this.pushCreateLevelUndoSnapshot(editor);
        this.createLevelTouchGesture.mode = "angle";
        this.setCreateLevelAngleFromPoint(point.x, point.y);
        return;
      }
      if (target?.type === "resize_edge") {
        ...
        this.pushCreateLevelUndoSnapshot(editor);
        this.createLevelTouchGesture.mode = "resize_edge";
        this.createLevelTouchGesture.edge = target.edge;
        this.createLevelTouchGesture.resizeSnapshot = this.captureCreateLevelResizeSnapshot(editor);
        this.createLevelTouchGesture.resizeViewport = { ...layout.viewport };
        return;
      }
      if (
        editor &&
        editor.mode === "editor" &&
        !editor.menuOpen &&
        !editor.publishPanelOpen &&
        !editor.exitConfirmOpen &&
        !editor.deleteConfirmOpen
      ) {
        if (target?.type === "viewport") {
          if (editor.mouseMode === "drag" && !this.isCreateLevelEditorLocked(editor)) {
            this.createLevelTouchGesture.mode = "paint";
            this.createLevelTouchGesture.historyCommitted = false;
            this.createLevelTouchGesture.applied = false;
            this.createLevelTouchGesture.lastGridX = null;
            this.createLevelTouchGesture.lastGridY = null;
            const grid = this.getCreateLevelGridAtScreen(point.x, point.y);
            if (grid) {
              this.continueCreateLevelPaintGesture(this.createLevelTouchGesture, grid.gridX, grid.gridY);
            }
          } else {
            this.createLevelTouchGesture.mode = "pan";
            this.createLevelTouchGesture.startCameraX = editor.cameraX;
            this.createLevelTouchGesture.startCameraY = editor.cameraY;
          }
          return;
        }
        if (target?.type === "palette" && editor.activeTab !== "erase") {
          this.createLevelTouchGesture.mode = "palette_scroll";
          this.createLevelTouchGesture.startScroll = editor.paletteScrollByTab[editor.activeTab] || 0;
          this.createLevelTouchGesture.tab = editor.activeTab;
          return;
        }
      }
      this.createLevelTouchGesture.mode = "tap";
    ...
    startCreateLevelPinchGesture(touches, layout = this.getCreateLevelLayout()) {
      if (!this.createLevelEditor || !Array.isArray(touches) || touches.length < 2 || layout?.mode !== "editor") {
        return false;
      }
      const first = this.getCanvasPointFromClient(touches[0].clientX, touches[0].clientY);
      const second = this.getCanvasPointFromClient(touches[1].clientX, touches[1].clientY);
      const centerX = (first.x + second.x) * 0.5;
      const centerY = (first.y + second.y) * 0.5;
      if (!this.isPointInsideRect(centerX, centerY, layout.viewport)) {
        return false;
      }
      const world = this.createLevelScreenToWorld(centerX, centerY, layout.viewport, this.createLevelEditor);
      this.resetCreateLevelTouchGesture();
      this.createLevelTouchGesture.mode = "pinch";
      this.createLevelTouchGesture.touches = [touches[0].identifier, touches[1].identifier];
      this.createLevelTouchGesture.startDistance = Math.max(1, Math.hypot(second.x - first.x, second.y - first.y));
      this.createLevelTouchGesture.startZoom = this.createLevelEditor.zoom;
      this.createLevelTouchGesture.anchorWorldX = world.x;
      this.createLevelTouchGesture.anchorWorldY = world.y;
      this.createLevelTouchGesture.startCameraX = this.createLevelEditor.cameraX;
      this.createLevelTouchGesture.startCameraY = this.createLevelEditor.cameraY;
      this.createLevelTouchGesture.startCenterX = centerX;
      this.createLevelTouchGesture.startCenterY = centerY;
      this.createLevelTouchGesture.lastCenterX = centerX;
      this.createLevelTouchGesture.lastCenterY = centerY;
      return true;
    }
    ...
    onCanvasTouchStart(event) {
      ...
      if (this.currentScreen === "create_level" && this.createLevelEditor) {
        this.createLevelSuppressNextClick = false;
        if (
          this.createLevelEditor.mode === "editor" &&
          !this.createLevelEditor.menuOpen &&
          !this.createLevelEditor.publishPanelOpen &&
          !this.createLevelEditor.exitConfirmOpen &&
          !this.createLevelEditor.deleteConfirmOpen &&
          event.touches.length >= 2 &&
          this.startCreateLevelPinchGesture(Array.from(event.touches))
        ) {
          if (event.cancelable) {
            event.preventDefault();
          }
        ...
        this.startCreateLevelSingleTouchGesture(
          firstTouch,
          firstPoint,
          this.getCreateLevelHoverTarget(firstPoint.x, firstPoint.y)
        );
        if (event.cancelable) {
          event.preventDefault();
        }
      ...
      if (this.shouldAllowNativePageScroll()) {
        this.pageScrollTouch = {
          id: firstTouch.identifier,
          startY: firstPoint.y,
          lastY: firstPoint.y,
          moved: false,
      ...
      if (event.cancelable && !this.shouldAllowNativePageScroll()) {
        event.preventDefault();
      }
    }
    ...
    onCanvasTouchMove(event) {
      ...
      if (this.currentScreen === "create_level" && this.createLevelEditor) {
        ...
        if (
          ...
          if (this.createLevelTouchGesture.mode !== "pinch" || this.createLevelTouchGesture.touches.length < 2) {
            this.startCreateLevelPinchGesture(touches, layout);
          }
          const trackedTouches = this.createLevelTouchGesture.touches
            .map((id) => touches.find((touch) => touch.identifier === id))
            .filter(Boolean);
          if (trackedTouches.length >= 2 && layout.mode === "editor") {
            const first = this.getCanvasPointFromClient(trackedTouches[0].clientX, trackedTouches[0].clientY);
            const second = this.getCanvasPointFromClient(trackedTouches[1].clientX, trackedTouches[1].clientY);
            const centerX = (first.x + second.x) * 0.5;
            const centerY = (first.y + second.y) * 0.5;
            const distance = Math.max(1, Math.hypot(second.x - first.x, second.y - first.y));
            const nextZoom = clamp(
              this.createLevelTouchGesture.startZoom * (distance / Math.max(1, this.createLevelTouchGesture.startDistance)),
              0.28,
              2.6
            );
            if (
              Math.abs(centerX - this.createLevelTouchGesture.lastCenterX) > 2 ||
              Math.abs(centerY - this.createLevelTouchGesture.lastCenterY) > 2 ||
              Math.abs(distance - this.createLevelTouchGesture.startDistance) > 2
            ) {
              this.createLevelTouchGesture.moved = true;
            }
            editor.zoom = nextZoom;
            editor.cameraX = this.createLevelTouchGesture.anchorWorldX - (centerX - (layout.viewport.x + layout.viewport.w * 0.5)) / editor.zoom;
            editor.cameraY = this.createLevelTouchGesture.anchorWorldY - (centerY - (layout.viewport.y + layout.viewport.h * 0.5)) / editor.zoom;
            this.clampCreateLevelCamera(editor, layout);
            this.createLevelTouchGesture.lastCenterX = centerX;
            this.createLevelTouchGesture.lastCenterY = centerY;
          }
        ...
        const gesture = this.createLevelTouchGesture;
        const activeTouch = touches.find((touch) => touch.identifier === gesture.touchId);
        if (activeTouch) {
          const point = this.getCanvasPointFromClient(activeTouch.clientX, activeTouch.clientY);
          if (gesture.mode === "pan" && layout.mode === "editor") {
            const deltaX = point.x - gesture.startX;
            const deltaY = point.y - gesture.startY;
            ...
            editor.cameraY = gesture.startCameraY - deltaY / Math.max(0.001, editor.zoom);
            this.clampCreateLevelCamera(editor, layout);
          } else if (gesture.mode === "palette_scroll" && layout.mode === "editor") {
            const deltaX = point.x - gesture.startX;
            ...
            editor.paletteScrollByTab[gesture.tab] = gesture.startScroll - deltaX;
            this.clampCreateLevelPaletteScroll(layout);
          } else if (gesture.mode === "angle") {
            if (Math.hypot(point.x - gesture.startX, point.y - gesture.startY) > 2) {
              gesture.moved = true;
            }
            this.setCreateLevelAngleFromPoint(point.x, point.y);
          } else if (gesture.mode === "paint") {
            if (Math.hypot(point.x - gesture.startX, point.y - gesture.startY) > 6) {
              gesture.moved = true;
            }
            const grid = this.getCreateLevelGridAtScreen(point.x, point.y, layout, editor);
            if (grid) {
          ...
          } else if (gesture.mode === "resize_edge") {
            this.applyCreateLevelResizeEdgeDrag(gesture, point.x, point.y, layout, editor);
          } else if (gesture.mode === "tap") {
            if (Math.hypot(point.x - gesture.startX, point.y - gesture.startY) > 8) {
              gesture.moved = true;
            }
          }
      ...
      if (this.shouldAllowNativePageScroll() && this.pageScrollTouch.id != null) {
        const nativeScrollTouch = Array.from(event.touches).find((touch) => touch.identifier === this.pageScrollTouch.id);
        if (nativeScrollTouch) {
          const point = this.getCanvasPointFromClient(nativeScrollTouch.clientX, nativeScrollTouch.clientY);
          this.pageScrollTouch.moved = this.pageScrollTouch.moved || Math.abs(point.y - this.pageScrollTouch.startY) > 8;
          this.pageScrollTouch.lastY = point.y;
        }
      }
    ...
    onCanvasTouchEnd(event) {
      if (this.currentScreen === "create_level" && this.createLevelEditor) {
        ...
        const primaryEnded = endedTouches.find((touch) => touch.identifier === gesture.touchId) || endedTouches[0] || null;
        if (wasPinching) {
          if (gesture.moved) {
            this.createLevelSuppressNextClick = true;
          }
          if (
            this.createLevelEditor.mode === "editor" &&
            !this.createLevelEditor.menuOpen &&
            ...
            !this.createLevelEditor.deleteConfirmOpen &&
            touches.length >= 2 &&
            this.startCreateLevelPinchGesture(touches)
          ) {
            if (event.cancelable) {
              event.preventDefault();
            }
          ...
          if (touches.length === 1) {
            ...
            const point = this.getCanvasPointFromClient(remainingTouch.clientX, remainingTouch.clientY);
            this.startCreateLevelSingleTouchGesture(
              remainingTouch,
              point,
              this.getCreateLevelHoverTarget(point.x, point.y)
            );
            this.createLevelTouchGesture.moved = true;
          } else {
            this.resetCreateLevelTouchGesture();
          }
        } else if (gesture.touchId != null && primaryEnded) {
          const point = this.getCanvasPointFromClient(primaryEnded.clientX, primaryEnded.clientY);
          if (gesture.mode === "paint" || gesture.mode === "resize_edge") {
            this.createLevelSuppressNextClick = true;
        ...
        if (touches.length > 0) {
          const point = this.getCanvasPointFromClient(touches[0].clientX, touches[0].clientY);
          this.setPointerPosition(point.x, point.y, true);
        } else {
          this.mouse.down = false;
        }
      ...
      for (const touch of event.changedTouches) {
        ...
        if (this.pageScrollTouch.id === touch.identifier) {
          const moved = this.pageScrollTouch.moved;
          this.pageScrollTouch = { id: null, startY: 0, lastY: 0, moved: false };
          if (moved) {
            continue;
          }
    ...
    openSettings() {
      this.beginScreenWipe(() => {
        ...
        this.titleSettingsHover = 0;
        this.settingsBackHover = 0;
        for (const option of GHOST_MODE_OPTIONS) {
          this.settingsGhostOptionHovers[option.id] = this.ghostMode === option.id ? 1 : 0;
        }
        this.canvas.style.cursor = "default";
        this.refreshSettingsHover();
      });
    ...
    openLeaderboardReplay(target) {
      if (!target) {
        return;
      }
      const open = async () => {
        let record = this.normalizeWorldRecordEntry(target.record);
        if ((!Array.isArray(record.replayFrames) || record.replayFrames.length === 0) && this.isOnlineLeaderboardConfigured()) {
          const recordKey = target.recordKey || this.getLeaderboardReplayRecordKey(target.scope, target.levelIndex);
          try {
            this.setOnlineLeaderboardStatus("Loading replay...", "neutral");
            const row = await this.fetchOnlineLeaderboardReplayRow(recordKey);
            if (row && typeof row === "object") {
              this.cacheOnlineLeaderboardRows([row]);
              this.applyOnlineLeaderboardRows([row]);
              this.onlineLeaderboardReplayLastSyncAt = Date.now();
              this.savePersistentProgress();
              record = this.normalizeWorldRecordEntry(this.onlineLeaderboardRowsByKey.get(recordKey)?.entry);
            }
          } catch {
            this.setOnlineLeaderboardStatus("Replay unavailable for this run", "offline");
            return;
          }
        }
        if (!Array.isArray(record.replayFrames) || record.replayFrames.length === 0) {
          this.setOnlineLeaderboardStatus("Replay unavailable for this run", "offline");
          return;
        }
        if (this.currentScreen !== "leaderboard") {
        ...
        this.setOnlineLeaderboardStatus("Online WR live", "online");
        this.beginScreenWipe(() => {
          const replayFrames = this.normalizeReplayFrames(record.replayFrames, Number.isInteger(target.levelIndex) ? target.levelIndex : 0);
          const firstFrame = this.getReplayFrameState(replayFrames[0], Number.isInteger(target.levelIndex) ? target.levelIndex : 0);
          if (!firstFrame) {
            this.activeLeaderboardReplay = null;
            this.currentScreen = "leaderboard";
            return;
          }
          const replayCamera = new Camera();
          replayCamera.snap(firstFrame.x, firstFrame.y);
          this.currentScreen = "leaderboard_replay";
          this.hoveredLeaderboardButton = null;
          this.hoveredLeaderboardReplayId = null;
          this.hoveredLeaderboardPeriodTab = null;
          this.leaderboardBackHover = 0;
          ...
          this.leaderboardReplayPrevHover = 0;
          this.leaderboardReplayNextHover = 0;
          this.activeLeaderboardReplay = {
            id: target.id,
            scope: target.scope,
            levelIndex: target.levelIndex,
            playbackMode: target.playbackMode || this.getLeaderboardReplayModeFromScope(target.scope),
            label: target.label || "Replay",
            record,
            frames: replayFrames,
            frameIndex: 0,
            frameTimer: 0,
            ...
            paused: false,
            ended: false,
            finishEffectTriggered: false,
            goalTriggered: false,
            goalTimer: 0,
            explosionParticles: [],
            finishDebris: [],
            currentLevelIndex: firstFrame.levelIndex,
            level: new LevelMap(LEVELS[firstFrame.levelIndex]),
            camera: replayCamera,
            skidMarks: [],
            wallFlashes: new Map(),
          };
          this.resetLeaderboardReplayFrameVisuals(this.activeLeaderboardReplay);
          this.activeLeaderboardReplay.elapsedTime = this.getReplayDisplayElapsedTime(this.activeLeaderboardReplay);
          this.canvas.style.cursor = "default";
          this.refreshLeaderboardReplayHover();
        });
      };
    ...
    openOnlineRacingJoin() {
      this.beginScreenWipe(() => {
        this.currentScreen = "online_racing_join";
        this.hoveredOnlineButton = null;
        this.hoveredOnlineRacingMatch = null;
        this.onlineBackHover = 0;
        this.onlineRacingSelectedTab = this.onlineRacingSelectedTab === "private" ? "private" : "public";
        this.onlineRacingMatchHovers = this.getOnlineRacingMatchCards().map(() => 0);
        this.canvas.style.cursor = "default";
        this.refreshOnlineHover();
      });
    ...
    openOnlineRacingHost() {
      this.beginScreenWipe(() => {
        this.currentScreen = "online_racing_host";
        this.hoveredOnlineButton = null;
        this.hoveredOnlineRacingMatch = null;
        this.onlineBackHover = 0;
        this.canvas.style.cursor = "default";
    ...
    openMyLevelDetail(index, options = {}) {
      ...
      this.beginScreenWipe(() => {
        ...
        this.setActiveMyLevel(level);
        this.canvas.style.cursor = "default";
        this.refreshOnlineHover();
      });
    }

    reopenActiveMyLevelDetail() {
      const activeLevel = this.getActiveMyLevel();
      const activeIndex = activeLevel ? this.getMyCreatedLevelMatchIndex(activeLevel, this.getMyCreatedLevels()) : -1;
      if (activeIndex >= 0) {
        this.openMyLevelDetail(activeIndex, { preserveStatus: true });
        return;
      }
      this.openMyLevels();
    }
    ...
    openCreateLevel(returnScreen = "online", options = {}) {
      this.beginScreenWipe(() => {
        this.currentScreen = "create_level";
        this.createLevelReturnScreen = returnScreen === "my_levels" || returnScreen === "my_level_detail"
          ? returnScreen
          : "online";
        const editLevel = options?.editLevel
          ? this.normalizeMyCreatedLevelEntry(options.editLevel, options.editLevel?.submissionStatus || "private")
          : null;
        this.editingMyLevel = editLevel ? { ...editLevel } : null;
        this.hoveredOnlineButton = null;
        this.hoveredCommunityLevel = null;
        this.onlineBackHover = 0;
        this.customLevelRunHover = 0;
        ...
        this.canvas.style.cursor = "default";
        if (editLevel) {
          this.setActiveMyLevel(editLevel);
          this.createLevelEditor = this.createCreateLevelEditorState({
            name: editLevel.name,
            mapRows: editLevel.map,
            hiddenMapRows: editLevel.hiddenMap,
            hiddenOriginX: editLevel.hiddenOriginX,
            hiddenOriginY: editLevel.hiddenOriginY,
            startAngle: Number.isFinite(editLevel.startAngle) ? editLevel.startAngle : 0,
            submissionStatus: editLevel.submissionStatus || "private",
            validatedTime: Number.isFinite(editLevel.myBestTime) ? editLevel.myBestTime : null,
          });
          this.createLevelEditor.savedDraftSignature = this.buildCreateLevelEditorSaveSignature(this.createLevelEditor);
          this.createLevelEditor.lastSavedAt = Date.now();
          this.createLevelEditor.lastChangeAt = this.createLevelEditor.lastSavedAt;
          this.publishLevelName = editLevel.name;
          this.customLevelStatusText = "Edit your level, test it, then publish when ready.";
          this.customLevelStatusTone = "neutral";
        } else {
          this.createLevelEditor = this.createCreateLevelEditorState();
          this.publishLevelName = this.createLevelEditor.name;
          this.customLevelStatusText = "";
          this.customLevelStatusTone = "neutral";
        }
        this.syncCreateLevelEditorMetaInputs();
    ...
    playCommunityLevelEntry(level, selectedIndex = null, returnScreen = "community_levels") {
      ...
      this.beginScreenWipe(() => {
        ...
        this.beginPendingCommunityLevelView(level);
    ...
    playActiveMyLevelFromDetail() {
      const level = this.getActiveMyLevel();
      if (!level) {
        return;
      }
      const draftEditor = {
        name: level.name,
        width: level.map[0]?.length || CREATE_LEVEL_DEFAULT_SIZE,
        height: level.map.length || CREATE_LEVEL_DEFAULT_SIZE,
        rows: this.mapRowsToCreateLevelRows(level.map),
        startAngle: Number.isFinite(level.startAngle) ? level.startAngle : 0,
      };
      const built = this.buildCreateLevelDefinitionFromEditor(draftEditor);
      if (built?.error) {
        this.myLevelDetailStatusText = built.error;
        this.myLevelDetailStatusTone = "error";
        return;
      }
      this.playCommunityLevelEntry(level, null, "my_level_detail");
    }

    openMyLevelActionPrompt(mode) {
      if (mode !== "publish" && mode !== "unpublish" && mode !== "delete") {
        return;
      }
      const level = this.getActiveMyLevel();
      if (!level) {
      ...
      this.myLevelDeletePasswordInput.value = "";
      this.hoveredMyLevelActionPromptButton = null;
      this.myLevelActionPromptCancelHover = 0;
      this.myLevelActionPromptConfirmHover = 0;
      this.hoveredOnlineButton = null;
    ...
    closeMyLevelActionPrompt() {
      if (this.myLevelDeletePasswordInput && document.activeElement === this.myLevelDeletePasswordInput) {
        this.myLevelDeletePasswordInput.blur();
      }
      if (this.myLevelDeletePasswordInput) {
        this.myLevelDeletePasswordInput.value = "";
      }
      this.myLevelActionPrompt = null;
      this.hoveredMyLevelActionPromptButton = null;
    ...
    async confirmMyLevelActionPrompt() {
      if (!this.myLevelActionPrompt || this.myLevelActionRequestInFlight) {
        return;
      }
      const level = this.getActiveMyLevel();
      if (!level) {
      ...
      if (this.myLevelActionPrompt.mode === "publish") {
        await this.publishMyLevelFromDetail(level);
      } else if (this.myLevelActionPrompt.mode === "unpublish") {
        await this.unpublishMyLevelFromDetail(level);
      } else if (this.myLevelActionPrompt.mode === "delete") {
        await this.deleteMyLevelFromDetail(level, this.myLevelActionPrompt.password || "");
      }
    }

    async deleteMyLevelFromDetail(level, password) {
      if (!this.myLevelActionPrompt) {
        return;
      }
      if (!this.signedInUsername || !this.isOnlineAccountConfigured()) {
        this.myLevelActionPrompt.statusText = "Log in to an account before deleting a level.";
        this.myLevelActionPrompt.statusTone = "error";
        return;
      }
      if (!password) {
        this.myLevelActionPrompt.statusText = "Enter your password.";
        this.myLevelActionPrompt.statusTone = "error";
        return;
      }
      this.myLevelActionRequestInFlight = true;
      this.myLevelActionPrompt.statusText = "Verifying password...";
      this.myLevelActionPrompt.statusTone = "neutral";
      try {
        const result = await this.callOnlineAccountRpc(ONLINE_LEADERBOARD_CONFIG.logInRpcName, this.signedInUsername, password);
        if (String(result?.status || "") !== "ok") {
          this.myLevelActionPrompt.statusText = String(result?.message || "Password incorrect.");
          this.myLevelActionPrompt.statusTone = "error";
          return;
        }
        if (typeof result?.session_token === "string" && result.session_token.trim()) {
          this.applySignedInUsername(this.signedInUsername, result.session_token.trim());
        }
        if (level.submissionId && this.hasSignedInAccountSession() && this.canUnpublishOnlineCommunityLevel()) {
          try {
            await this.unpublishOnlineMyLevel(level);
          } catch {
            this.myLevelActionPrompt.statusText = "Could not remove the online copy first.";
            this.myLevelActionPrompt.statusTone = "error";
            return;
          }
        }
        ...
        this.myLevelActionPrompt.statusText = "Delete failed. Check your password and connection.";
        this.myLevelActionPrompt.statusTone = "error";
      } finally {
        this.myLevelActionRequestInFlight = false;
      }
    ...
    async publishMyLevelFromDetail(level) {
      const actionPrompt = this.myLevelActionPrompt;
      this.closeMyLevelActionPrompt();
      if (!level) {
        return;
      }
      const bestTime = Number.isFinite(level.myBestTime) ? level.myBestTime : level.creatorTime;
      if (!Number.isFinite(bestTime)) {
        this.myLevelDetailStatusText = "Play the level once first so it has a valid time before publishing.";
        this.myLevelDetailStatusTone = "error";
        return;
      }
      if (level.submissionId && !this.hasSignedInAccountSession()) {
        this.myLevelDetailStatusText = "Log in to manage an existing level submission.";
        this.myLevelDetailStatusTone = "error";
        return;
      }
      this.myLevelActionRequestInFlight = true;
      this.myLevelDetailStatusText = actionPrompt?.mode === "publish" ? "Sending level for review..." : "";
      this.myLevelDetailStatusTone = "neutral";
      try {
        if (this.isOnlineCommunityConfigured()) {
          const submittedRow = await this.submitCommunityLevelForReview({
            ...level,
            bestTime,
            sessionToken: this.hasSignedInAccountSession() ? this.signedInSessionToken : "",
          });
          ...
          const savedLevel = this.applyMyCreatedLevelPatch(level, {
            creatorTime: bestTime,
            myBestTime: bestTime,
            submissionStatus: "pending",
            updatedAt: Date.now(),
        ...
        this.myLevelDetailStatusText = "Level sent for review.";
        this.myLevelDetailStatusTone = "ok";
      } catch {
        this.myLevelDetailStatusText = "Publish request failed. Check the Supabase community SQL setup.";
        this.myLevelDetailStatusTone = "error";
      } finally {
        this.myLevelActionRequestInFlight = false;
      }
    ...
    async unpublishMyLevelFromDetail(level) {
      this.closeMyLevelActionPrompt();
      if (!level) {
        return;
      }
      if (!level.submissionId) {
        ...
        this.removeOnlineCommunityLevel(level);
        this.myLevelDetailStatusText = "Level moved back to private.";
        this.myLevelDetailStatusTone = "ok";
        return;
      }
      if (!this.hasSignedInAccountSession()) {
        this.myLevelDetailStatusText = "Log in to unpublish a level that is already online.";
        this.myLevelDetailStatusTone = "error";
        return;
      }
      this.myLevelActionRequestInFlight = true;
      this.myLevelDetailStatusText = "Removing level from Community Levels...";
      this.myLevelDetailStatusTone = "neutral";
      try {
        const row = this.canUnpublishOnlineCommunityLevel()
          ? await this.unpublishOnlineMyLevel(level)
          : null;
        this.removeOnlineCommunityLevel(level);
        const savedLevel = this.applyMyCreatedLevelPatch(level, {
          ...
          reviewMessage: "",
          updatedAt: Date.now(),
          ...(row ? { submissionId: row.submission_id || level.submissionId } : {}),
        });
        if (savedLevel) {
          this.setActiveMyLevel(savedLevel);
        }
        ...
        this.myLevelDetailStatusText = "Unpublish failed. Apply the updated community SQL first.";
        this.myLevelDetailStatusTone = "error";
      } finally {
        this.myLevelActionRequestInFlight = false;
      }
    ...
    beginPublishValidationRun() {
      ...
      this.createPublishPromptContinueHover = 0;
      this.pendingPublishValidationDefinition = null;
      this.customLevelStatusText = "Beat the level once to publish it.";
    ...
    returnToCommunityLevelBrowser() {
      ...
      this.beginScreenWipe(() => {
        ...
        this.currentScreen = this.communityLevelReturnScreen === "my_level_detail" && this.getActiveMyLevel()
          ? "my_level_detail"
          : this.communityLevelReturnScreen === "my_levels"
          ? "my_levels"
    ...
    loadLevel(index) {
      const preserveReplay = this.isSpeedrunMode() && index > 0 && this.currentRunReplayFrames.length > 0;
      if (!preserveReplay) {
        this.beginRunReplayCapture();
      }
      ...
      this.captureCurrentRunReplayFrame(index, this.car, true);
      this.prepareActiveGhostReplay();
    }
    ...
    loadCustomLevel(definition) {
      ...
      this.car.reset(this.level.start.x, this.level.start.y, this.level.startAngle);
      this.camera.snap(this.level.start.x, this.level.start.y);
      this.captureCurrentRunReplayFrame(0, this.car, true);
    }
    ...
    parseCustomLevelText(text) {
      ...
      if (rows.length > CREATE_LEVEL_MAX_SIZE || width > CREATE_LEVEL_MAX_SIZE) {
        return { error: `Keep test levels ${CREATE_LEVEL_MAX_SIZE} by ${CREATE_LEVEL_MAX_SIZE} tiles or smaller.` };
      }
    ...
    publishCustomLevel() {
      ...
      if (this.isEditingMyLevel()) {
        this.syncEditingMyLevelText(this.publishLevelText, "publish");
      }
      this.publishLevelNameInput.value = this.publishLevelName;
      ...
      const editingLevel = this.isEditingMyLevel() ? this.editingMyLevel : null;
      this.pendingCommunityLevel = {
        ...
        map: parsed.definition.map,
        bestTime: null,
        submissionId: editingLevel?.submissionId || "",
        remoteId: editingLevel?.remoteId || "",
        local: Boolean(editingLevel?.local),
        sessionToken: this.hasSignedInAccountSession() ? this.signedInSessionToken : "",
      };
    ...
    async finishCommunityLevelPublish() {
        ...
        try {
          ...
          this.upsertMyCreatedLevel(
            submittedRow || {
              ...
              submissionStatus: "pending",
              submissionId: this.pendingCommunityLevel.submissionId || "",
              remoteId: "",
              createdAt: Date.now(),
              updatedAt: Date.now(),
            },
          ...
          const isEditingLevel = this.isEditingMyLevel();
          const refreshedLevel = this.applyMyCreatedLevelPatch(
            submittedRow || this.pendingCommunityLevel,
            {
              name: this.pendingCommunityLevel.name,
              creator: this.pendingCommunityLevel.creator,
              creatorTime: this.pendingCommunityLevel.bestTime,
          ...
          this.removeOnlineCommunityLevel(this.pendingCommunityLevel);
          if (isEditingLevel) {
            if (refreshedLevel) {
              this.setActiveMyLevel(refreshedLevel);
            }
            this.pendingCommunityLevel = null;
            this.communityPublishRequestInFlight = false;
            this.myLevelDetailStatusText = "Level sent for review.";
      ...
      if (isEditingLevel) {
        ...
        this.pendingCommunityLevel = null;
        this.savePersistentProgress();
        this.myLevelDetailStatusText = "Level saved as private.";
        this.myLevelDetailStatusTone = "ok";
        this.reopenActiveMyLevelDetail();
        return;
      }
    ...
    copyEmptyLevelFrame() {
      ...
      this.copyTextToClipboard(frameText, markCopied, markFailed);
    }

    exportCreateLevelEditorFile() {
      const editor = this.createLevelEditor;
      if (!editor) {
        return;
      }
      const levelText = this.getCreateLevelRowsAsStrings(editor).join("\n");
      this.copyTextToClipboard(
        levelText,
        () => this.setCreateLevelEditorMessage("Level file copied to clipboard.", "ok"),
        () => this.setCreateLevelEditorMessage("Copy failed. Select the level text manually.", "error")
      );
    }
    ...
    loadPersistentProgress() {
      ...
      try {
        ...
        if (Number.isFinite(parsed.highestVisitedLevel)) {
          this.highestVisitedLevel = this.normalizeHighestVisitedLevelForCurrentCampaign(
            parsed.highestVisitedLevel,
            this.getSavedCampaignLevelCount(parsed)
          );
        }
        ...
        if (Array.isArray(parsed.personalLevelRecords)) {
          this.personalLevelRecords = LEVELS.map((_, index) => this.normalizeWorldRecordEntry(parsed.personalLevelRecords[index]));
        }

        if (typeof parsed.ghostMode === "string" && GHOST_MODE_OPTIONS.some((option) => option.id === parsed.ghostMode)) {
          this.ghostMode = parsed.ghostMode;
        }
        ...
        if (Array.isArray(parsed.onlineMyLevels)) {
          this.onlineMyLevels = parsed.onlineMyLevels
            .map((level) => this.normalizeOnlineMyCreatedLevelRow(level))
            .filter(Boolean);
        }

        if (Array.isArray(parsed.onlineLeaderboardCacheRows)) {
          this.cacheOnlineLeaderboardRows(parsed.onlineLeaderboardCacheRows, true);
          this.applyOnlineLeaderboardRows(parsed.onlineLeaderboardCacheRows);
        }
        if (Number.isFinite(parsed.onlineLeaderboardLastSyncAt)) {
          this.onlineLeaderboardLastSyncAt = Math.max(0, Math.floor(parsed.onlineLeaderboardLastSyncAt));
        }
        if (Array.isArray(parsed.onlineCommunityLevels)) {
          this.onlineCommunityLevels = parsed.onlineCommunityLevels
            .map((level) => this.normalizeOnlineCommunityLevelRow(level))
            .filter(Boolean);
          this.rebuildCommunityLevels();
        }
        if (Number.isFinite(parsed.onlineCommunityLastSyncAt)) {
          this.onlineCommunityLastSyncAt = Math.max(0, Math.floor(parsed.onlineCommunityLastSyncAt));
        }
        if (Number.isFinite(parsed.onlineMyLevelsLastSyncAt)) {
          this.onlineMyLevelsLastSyncAt = Math.max(0, Math.floor(parsed.onlineMyLevelsLastSyncAt));
        }
        if (typeof parsed.onlineMyLevelsLastSyncUser === "string") {
          this.onlineMyLevelsLastSyncUser = normalizeUsername(parsed.onlineMyLevelsLastSyncUser) || "";
        }
        ...
        if (Number.isFinite(parsed.nextCreatedLevelNumber) && parsed.nextCreatedLevelNumber >= 1) {
          this.nextCreatedLevelNumber = Math.floor(parsed.nextCreatedLevelNumber);
        }
        ...
        const savedSignedInSessionToken = typeof parsed.signedInSessionToken === "string" ? parsed.signedInSessionToken.trim() : "";
        if (savedSignedInUsername && savedSignedInSessionToken && savedSignedInUsername === this.playerUsername) {
          this.signedInUsername = savedSignedInUsername;
          this.signedInSessionToken = savedSignedInSessionToken;
          this.authStatusText = `Signed in as ${savedSignedInUsername}`;
          this.authStatusTone = "ok";
        } else if (savedSignedInUsername && !savedSignedInSessionToken) {
          this.signedInUsername = null;
          this.signedInSessionToken = "";
          this.authStatusText = "Log in again to sync account progress.";
          this.authStatusTone = "neutral";
        } else {
          this.signedInUsername = null;
          this.signedInSessionToken = "";
    ...
    savePersistentProgress() {
      const storage = this.getPersistentStorage();
      if (!storage) {
        this.scheduleSignedInAccountProfileSave();
        return;
      }
      ...
      try {
        storage.setItem(
          ...
          JSON.stringify({
            ...
            personalLevelRecords: this.personalLevelRecords.map((record) => this.normalizeWorldRecordEntry(record)),
            ...
            onlineLeaderboardCacheRows: Array.from(this.onlineLeaderboardRowsByKey.values()).map((row) => ({
              record_key: row.recordKey,
              leaderboard_period: row.leaderboardPeriod,
              period_key: row.periodKey,
              scope: row.scope,
              level_index: Number.isInteger(row.levelIndex) ? row.levelIndex : null,
              time_ms: Number.isFinite(row.entry?.time) ? row.entry.time : null,
              car_variant: row.entry?.carVariant || null,
              car_color: row.entry?.carColor || null,
              player_name: row.entry?.playerName || null,
              deaths: Number.isFinite(row.entry?.deaths) ? row.entry.deaths : 0,
              updated_at: null,
            })),
            onlineLeaderboardLastSyncAt: this.onlineLeaderboardLastSyncAt,
            onlineCommunityLevels: this.onlineCommunityLevels.map((level) => ({
              level_id: level.remoteId || "",
              level_name: level.name,
              creator_name: level.creator,
              creator_time_ms: Number.isFinite(level.creatorTime) ? level.creatorTime : null,
              level_text: Array.isArray(level.map) ? level.map.join("\n") : "",
              start_angle: Number.isFinite(level.startAngle) ? level.startAngle : 0,
              leaderboard_json: normalizeCommunityLeaderboard(level.leaderboard, level.creator, level.creatorTime),
              approved_at: Number.isFinite(level.updatedAt) && level.updatedAt > 0 ? new Date(level.updatedAt).toISOString() : null,
              play_count: Number.isFinite(level.playCount) ? Math.max(0, Math.floor(level.playCount)) : 0,
            })),
            onlineCommunityLastSyncAt: this.onlineCommunityLastSyncAt,
            onlineMyLevels: this.onlineMyLevels.map((level) => ({
              local_id: level.localId || "",
              level_id: level.remoteId || "",
              submission_id: level.submissionId || "",
              level_name: level.name,
              creator_name: level.creator,
              creator_time_ms: Number.isFinite(level.creatorTime) ? level.creatorTime : null,
              my_best_time_ms: Number.isFinite(level.myBestTime) ? level.myBestTime : null,
              start_angle: Number.isFinite(level.startAngle) ? level.startAngle : 0,
              leaderboard_json: normalizeCommunityLeaderboard(level.leaderboard, level.creator, level.creatorTime),
              level_text: Array.isArray(level.map) ? level.map.join("\n") : "",
              hidden_level_text: Array.isArray(level.hiddenMap) ? level.hiddenMap.join("\n") : "",
              hidden_origin_x: Number.isFinite(level.hiddenOriginX) ? level.hiddenOriginX : 0,
              hidden_origin_y: Number.isFinite(level.hiddenOriginY) ? level.hiddenOriginY : 0,
              review_status: level.submissionStatus || "private",
              review_message: level.reviewMessage || "",
              play_count: Number.isFinite(level.playCount) ? Math.max(0, Math.floor(level.playCount)) : 0,
              created_at: Number.isFinite(level.createdAt) && level.createdAt > 0 ? new Date(level.createdAt).toISOString() : null,
              updated_at: Number.isFinite(level.updatedAt) && level.updatedAt > 0 ? new Date(level.updatedAt).toISOString() : null,
            })),
            onlineMyLevelsLastSyncAt: this.onlineMyLevelsLastSyncAt,
            onlineMyLevelsLastSyncUser: this.onlineMyLevelsLastSyncUser,
            ...
            myCreatedLevels: this.myCreatedLevels
              .map((level) => ({
                ...
                leaderboard: normalizeCommunityLeaderboard(level.leaderboard, level.creator, level.creatorTime),
                map: level.map,
                hiddenMap: Array.isArray(level.hiddenMap) ? level.hiddenMap : level.map,
    ...
    recordPersonalLevelRecord(index, time, carVariant = this.playerCarVariant, carColor = this.playerCarColor) {
      if (!Number.isInteger(index) || index < 0 || index >= LEVELS.length || !Number.isFinite(time) || time < 0) {
        return false;
      }
      const previous = this.normalizeWorldRecordEntry(this.personalLevelRecords[index]);
      const savedBestTime = Number.isFinite(this.levelBestTimes[index]) ? this.levelBestTimes[index] : null;
      const previousBestTime = [previous.time, savedBestTime]
        .filter((value) => Number.isFinite(value))
        .sort((a, b) => a - b)[0] ?? null;
      if (previousBestTime != null && time >= previousBestTime) {
        return false;
      }
      this.personalLevelRecords[index] = {
        time,
        carVariant: this.getCarVariantDefinition(carVariant).id,
        carColor: isValidHexColor(carColor) ? carColor : "#909090",
        playerName: this.playerUsername,
        replayFrames: this.getCurrentRunReplayFrames(),
      };
      this.savePersistentProgress();
      return true;
    }
    ...
    recordWorldRecord(index, time, carVariant = this.playerCarVariant, carColor = this.playerCarColor, records = this.worldRecords) {
      ...
      const replayFrames = this.getCurrentRunReplayFrames();
      records[index] = {
        time,
        carVariant: this.getCarVariantDefinition(carVariant).id,
        carColor: isValidHexColor(carColor) ? carColor : "#909090",
    ...
    getResponsiveWorldRenderScale() {
      const referenceWidth = this.viewportWidth || this.width;
      const referenceHeight = this.viewportHeight || this.height;
      const viewportFit = Math.min(referenceWidth / 1280, referenceHeight / 720);
      const fitProgress = clamp((viewportFit - 0.32) / 0.68, 0, 1);
      const minDimension = Math.min(referenceWidth, referenceHeight);
      const smallScreenProgress = clamp((minDimension - 320) / 520, 0, 1);
      const smallScreenZoom = lerp(0.92, 1, smallScreenProgress);
      return lerp(0.42, 1, fitProgress) * smallScreenZoom;
    }
    ...
    finishCurrentLevel() {
      ...
      if (this.isCustomLevelMode()) {
        ...
        if (this.gameMode === "publish_test" && this.pendingCommunityLevel) {
          ...
          if (this.createLevelEditor) {
            this.createLevelEditor.validatedTime = Number.isFinite(this.pendingCommunityLevel.bestTime)
              ? this.pendingCommunityLevel.bestTime
              : this.levelTimer;
            this.createLevelEditor.validatedSignature = this.buildCreateLevelEditorSignature(this.createLevelEditor);
          }
      ...
      this.unlockNextLevel();
      this.recordPersonalLevelRecord(this.levelIndex, this.levelTimer, this.playerCarVariant, this.playerCarColor);
      this.recordLevelBestTime(this.levelIndex, this.levelTimer, perfectRun);
    ...
    fixedUpdate(dt) {
      const desiredHeight = this.getDesiredCanvasHeightForScreen(this.currentScreen, this.viewportHeight || window.innerHeight || 1);
      if (this.width !== (this.viewportWidth || window.innerWidth) || this.height !== desiredHeight) {
        this.resize();
      }
      this.updateUsernameInputVisibility();
      this.updateCustomLevelInputVisibility();
      ...
      if (this.currentScreen === "leaderboard_replay") {
        this.updateLeaderboardReplayScreen(dt);
        return;
      }
      if (this.currentScreen === "online" || this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level" || this.currentScreen === "my_levels" || this.currentScreen === "my_level_detail" || this.isOnlineRacingScreen()) {
    ...
    updateCampaignGame(dt) {
      ...
      this.updatePendingCommunityLevelView(dt);
      ...
      this.updateCamera(dt);
      this.updateShake(dt);
      this.captureCurrentRunReplayFrame(this.levelIndex, this.car);
    }
    ...
    updateTitleScreen(dt) {
      ...
      this.titleOnlineHover = 0;
      this.titleSettingsHover = approachExp(this.titleSettingsHover, this.hoveredTitleButton === "settings" ? 1 : 0, 15, dt);
    ...
    updateSettingsScreen(dt) {
      this.updateMenuBackdrop(dt);
      this.refreshSettingsHover();
      this.settingsBackHover = approachExp(this.settingsBackHover, this.hoveredSettingsButton === "back" ? 1 : 0, 15, dt);
      for (const option of GHOST_MODE_OPTIONS) {
        const target = this.ghostMode === option.id || this.hoveredSettingsButton === `ghost_${option.id}` ? 1 : 0;
        this.settingsGhostOptionHovers[option.id] = approachExp(this.settingsGhostOptionHovers[option.id] || 0, target, 15, dt);
      }
      if (this.input.wasPressed("Escape")) {
        this.returnToTitle();
      }
    ...
    updateLeaderboardReplayScreen(dt) {
      const replay = this.activeLeaderboardReplay;
      if (!replay) {
        this.openLeaderboard();
        return;
      }
      this.refreshLeaderboardReplayHover();
      this.leaderboardReplayBackHover = approachExp(this.leaderboardReplayBackHover, this.hoveredLeaderboardButton === "back" ? 1 : 0, 15, dt);
      this.leaderboardReplayPrevHover = approachExp(this.leaderboardReplayPrevHover, this.hoveredLeaderboardButton === "replay_prev" ? 1 : 0, 15, dt);
      this.leaderboardReplayNextHover = approachExp(this.leaderboardReplayNextHover, this.hoveredLeaderboardButton === "replay_next" ? 1 : 0, 15, dt);
      if (this.input.wasPressed("Escape")) {
        this.closeLeaderboardReplay();
        return;
      }
      if (replay.paused) {
        if (this.input.wasPressed("ArrowLeft")) {
          this.stepLeaderboardReplayFrame(replay, -1);
        } else if (this.input.wasPressed("ArrowRight")) {
          this.stepLeaderboardReplayFrame(replay, 1);
        }
      }

      const currentFrame = this.getReplayFrameState(replay.frames[replay.frameIndex], replay.levelIndex ?? 0);
      if (!currentFrame) {
        return;
      }

      if (replay.currentLevelIndex !== currentFrame.levelIndex || !replay.level) {
        replay.currentLevelIndex = currentFrame.levelIndex;
        replay.level = new LevelMap(LEVELS[currentFrame.levelIndex]);
        replay.camera.snap(currentFrame.x, currentFrame.y);
      } else {
        replay.camera.savePrevious();
      }

      if (!replay.paused && replay.frameIndex < replay.frames.length - 1) {
        replay.frameTimer += dt;
        while (replay.frameTimer >= FIXED_DT && replay.frameIndex < replay.frames.length - 1) {
          const previousFrame = this.getReplayFrameState(replay.frames[replay.frameIndex], replay.currentLevelIndex);
          replay.frameTimer -= FIXED_DT;
          replay.frameIndex += 1;
          const advancedFrame = this.getReplayFrameState(replay.frames[replay.frameIndex], replay.currentLevelIndex);
          if (previousFrame && advancedFrame && previousFrame.levelIndex !== advancedFrame.levelIndex) {
            this.resetLeaderboardReplayFrameVisuals(replay);
          }
          this.applyLeaderboardReplayFrameVisuals(replay, advancedFrame);
          this.ageLeaderboardReplayFrameVisuals(replay, FIXED_DT);
        }
      } else if (replay.frameIndex >= replay.frames.length - 1) {
        replay.frameTimer = 0;
      }

      const nextFrame = this.getReplayFrameState(replay.frames[replay.frameIndex], replay.currentLevelIndex);
      if (!nextFrame) {
        return;
      }
      if (replay.currentLevelIndex !== nextFrame.levelIndex) {
        replay.currentLevelIndex = nextFrame.levelIndex;
        replay.level = new LevelMap(LEVELS[nextFrame.levelIndex]);
        replay.camera.snap(nextFrame.x, nextFrame.y);
      } else {
        replay.camera.update(nextFrame.x, nextFrame.y, dt);
      }

      replay.elapsedTime = this.getReplayDisplayElapsedTime(replay);
      if (replay.frameIndex >= replay.frames.length - 1) {
        replay.elapsedTime = Number.isFinite(replay.record?.time) && replay.record.time >= 0
          ? replay.record.time
          : replay.elapsedTime;
        replay.frameTimer = 0;
        if (!replay.finishEffectTriggered) {
          this.triggerLeaderboardReplayGoalFinish(replay, nextFrame);
        }
      }
      if (!replay.paused) {
        this.updateLeaderboardReplayGoalFinish(replay, dt);
      }
      if (
        replay.frameIndex >= replay.frames.length - 1 &&
        replay.finishEffectTriggered &&
        replay.goalTimer <= 0 &&
        replay.explosionParticles.length === 0 &&
        replay.finishDebris.length === 0
      ) {
    ...
    updateOnlineScreen(dt) {
      this.updateMenuBackdrop(dt);
      if (this.currentScreen === "create_level") {
        this.updateCreateLevelScreen(dt);
        return;
      }
      if (this.isScrollableOnlineListScreen()) {
      ...
      this.onlineRacingHover = approachExp(this.onlineRacingHover, this.hoveredOnlineButton === "online_racing" ? 1 : 0, 15, dt);
      this.onlineRacingJoinHover = approachExp(this.onlineRacingJoinHover, this.hoveredOnlineButton === "online_racing_join" ? 1 : 0, 15, dt);
      this.onlineRacingCreateHover = approachExp(this.onlineRacingCreateHover, this.hoveredOnlineButton === "online_racing_create" ? 1 : 0, 15, dt);
      this.onlineRacingAccountHover = approachExp(this.onlineRacingAccountHover, this.hoveredOnlineButton === "online_racing_account" ? 1 : 0, 15, dt);
      this.onlineRacingFriendsHover = approachExp(this.onlineRacingFriendsHover, this.hoveredOnlineButton === "online_racing_friends" ? 1 : 0, 15, dt);
      this.onlineRacingLogoutHover = approachExp(this.onlineRacingLogoutHover, this.hoveredOnlineButton === "online_racing_logout" ? 1 : 0, 15, dt);
      this.onlineRacingPublicTabHover = approachExp(this.onlineRacingPublicTabHover, this.onlineRacingSelectedTab === "public" || this.hoveredOnlineButton === "online_racing_tab_public" ? 1 : 0, 15, dt);
      this.onlineRacingPrivateTabHover = approachExp(this.onlineRacingPrivateTabHover, this.onlineRacingSelectedTab === "private" || this.hoveredOnlineButton === "online_racing_tab_private" ? 1 : 0, 15, dt);
      ...
      this.myLevelDetailPlayHover = approachExp(this.myLevelDetailPlayHover, this.hoveredOnlineButton === "play_my_level_detail" ? 1 : 0, 15, dt);
      this.myLevelDetailActionHover = approachExp(this.myLevelDetailActionHover, this.hoveredOnlineButton === "my_level_detail_action" ? 1 : 0, 15, dt);
      this.myLevelDetailEditHover = approachExp(this.myLevelDetailEditHover, this.hoveredOnlineButton === "edit_my_level" ? 1 : 0, 15, dt);
      this.myLevelDetailDeleteHover = approachExp(this.myLevelDetailDeleteHover, this.hoveredOnlineButton === "delete_my_level" ? 1 : 0, 15, dt);
      ...
      this.myLevelActionPromptCancelHover = approachExp(this.myLevelActionPromptCancelHover, this.hoveredMyLevelActionPromptButton === "cancel" ? 1 : 0, 15, dt);
      this.myLevelActionPromptConfirmHover = approachExp(this.myLevelActionPromptConfirmHover, this.hoveredMyLevelActionPromptButton === "confirm" ? 1 : 0, 15, dt);
      if (this.currentScreen === "create_level" && this.createPublishPromptOpen) {
        if (this.input.wasPressed("Escape")) {
          this.closeCreatePublishPrompt();
        } else if (this.input.wasPressed("Enter") || this.input.wasPressed("Space")) {
      ...
      const expectedRacingMatchCount = this.currentScreen === "online_racing_join" ? this.getOnlineRacingMatchCards().length : 0;
      if (expectedRacingMatchCount !== this.onlineRacingMatchHovers.length) {
        this.onlineRacingMatchHovers = Array.from({ length: expectedRacingMatchCount }, (_, index) => (this.hoveredOnlineRacingMatch === index ? 1 : 0));
      }
      for (let index = 0; index < this.onlineRacingMatchHovers.length; index += 1) {
        this.onlineRacingMatchHovers[index] = approachExp(this.onlineRacingMatchHovers[index], this.hoveredOnlineRacingMatch === index ? 1 : 0, 15, dt);
      }
    }

    updateCreateLevelScreen(dt) {
      const editor = this.createLevelEditor;
      if (!editor) {
        return;
      }
      this.updateMenuBackdrop(dt);
      this.createLevelBottomPanelAnim = approachExp(
        Number.isFinite(this.createLevelBottomPanelAnim) ? this.createLevelBottomPanelAnim : 1,
        this.createLevelBottomPanelCollapsed ? 0 : 1,
        18,
        dt
      );
      this.refreshCreateLevelHover();
      this.updateCreateLevelButtonHovers(this.getCreateLevelLayout(), dt);
      this.updateCustomLevelInputVisibility();
      if (editor.mode !== "editor") {
        if (this.input.wasPressed("Escape")) {
          this.performCreateLevelAction("setup_back");
        } else if (this.input.wasPressed("Enter") || this.input.wasPressed("Space")) {
          this.performCreateLevelAction("setup_done");
        }
        return;
      }
      if (editor.exitConfirmOpen || editor.deleteConfirmOpen) {
        if (this.input.wasPressed("Escape")) {
          ...
          this.performCreateLevelAction("editor_confirm_accept");
        }
        return;
      }
      if (editor.menuOpen) {
      ...
      if (editor.publishPanelOpen) {
        if (this.input.wasPressed("Escape")) {
          this.closeCreateLevelPublishPanel();
        }
        return;
      }
      if (editor.angleEditActive) {
        if (this.isCreateLevelEditorLocked(editor)) {
          editor.angleEditActive = false;
        } else if (this.input.wasPressed("ArrowLeft")) {
          this.pushCreateLevelUndoSnapshot(editor);
          editor.startAngle = this.snapCreateLevelAngle(editor.startAngle - this.getCreateLevelEditorAngleStep());
          this.markCreateLevelEditorChanged();
        } else if (this.input.wasPressed("ArrowRight")) {
          this.pushCreateLevelUndoSnapshot(editor);
          editor.startAngle = this.snapCreateLevelAngle(editor.startAngle + this.getCreateLevelEditorAngleStep());
          ...
          this.pushCreateLevelUndoSnapshot(editor);
          editor.startAngle = this.snapCreateLevelAngle(-Math.PI / 2);
          this.markCreateLevelEditorChanged();
      ...
      const focusedEditorInput = document.activeElement === this.publishLevelNameInput || document.activeElement === this.createLevelSizeXInput || document.activeElement === this.createLevelSizeYInput;
      if (!focusedEditorInput) {
        const panSpeed = 620 / Math.max(0.36, editor.zoom);
        let moveX = 0;
        let moveY = 0;
        ...
        if (moveX !== 0 || moveY !== 0) {
          ...
          editor.cameraX += direction.x * panSpeed * dt;
          editor.cameraY += direction.y * panSpeed * dt;
          this.clampCreateLevelCamera(editor);
        }
        if (this.input.wasPressed("Equal")) {
          this.zoomCreateLevelCamera(1);
        } else if (this.input.wasPressed("Minus")) {
          this.zoomCreateLevelCamera(-1);
        } else if (this.input.wasPressed("KeyZ")) {
          this.undoCreateLevelEditor(editor);
        } else if (this.input.wasPressed("KeyY")) {
          this.redoCreateLevelEditor(editor);
        } else if (this.input.wasPressed("Escape")) {
          editor.menuOpen = true;
        }
      }
      const autoSaveDue = editor.status !== "pending" &&
        this.createLevelEditorHasUnsavedChanges(editor) &&
        Number.isFinite(editor.lastChangeAt) &&
        editor.lastChangeAt > (editor.lastSavedAt || 0) &&
        Date.now() - editor.lastChangeAt >= 60000;
      if (autoSaveDue) {
        this.saveCreateLevelEditor({ isAutoSave: true });
      }
      this.clampCreateLevelPaletteScroll();
    }
    ...
    updateCustomLevelInputVisibility() {
      if (!this.customLevelInput || this.currentScreen !== "create_level" || !this.createLevelEditor) {
        ...
        if (this.createLevelSizeXInput) {
          this.createLevelSizeXInput.style.display = "none";
        }
        if (this.createLevelSizeYInput) {
          this.createLevelSizeYInput.style.display = "none";
        }
        return;
      }
      ...
      const locked = this.isCreateLevelEditorLocked();
      this.customLevelInput.style.display = "none";
      this.publishLevelInput.style.display = "none";
      this.syncCreateLevelEditorMetaInputs();

      let nameRect = null;
      let showSizeInputs = false;
      if (createLayout.mode === "setup") {
        nameRect = createLayout.nameInput;
        if (this.createLevelEditor.setupSource === "import") {
          this.customLevelInput.style.display = "block";
          this.customLevelInput.style.left = `${createLayout.importInput.x}px`;
          this.customLevelInput.style.top = `${createLayout.importInput.y}px`;
          this.customLevelInput.style.width = `${createLayout.importInput.w}px`;
          this.customLevelInput.style.height = `${createLayout.importInput.h}px`;
          this.customLevelInput.style.fontSize = `${Math.max(12, Math.round(createLayout.scale * 13))}px`;
          this.customLevelInput.placeholder = "Paste map text like # . I S G X P here";
          if (document.activeElement !== this.customLevelInput && this.customLevelInput.value !== this.createLevelEditor.importText) {
            this.customLevelInput.value = this.createLevelEditor.importText;
          }
        }
      } else if (createLayout.mode === "editor") {
        if (this.createLevelEditor.publishPanelOpen) {
          nameRect = createLayout.publishOverlay.nameInput;
        } else if (!this.createLevelEditor.menuOpen && !this.createLevelEditor.exitConfirmOpen && !this.createLevelEditor.deleteConfirmOpen) {
          nameRect = createLayout.nameInput;
          showSizeInputs = !createLayout.bottomPanelCollapsed;
        }
      }

      if (nameRect) {
        this.publishLevelNameInput.style.display = "block";
        this.publishLevelNameInput.style.left = `${nameRect.x}px`;
        this.publishLevelNameInput.style.top = `${nameRect.y}px`;
        this.publishLevelNameInput.style.width = `${nameRect.w}px`;
        this.publishLevelNameInput.style.height = `${nameRect.h}px`;
        this.publishLevelNameInput.style.fontSize = `${Math.max(12, Math.round(createLayout.scale * 13))}px`;
        this.publishLevelNameInput.readOnly = locked;
        this.publishLevelNameInput.style.borderColor = locked ? "rgba(128, 146, 160, 0.34)" : "rgba(174, 222, 255, 0.46)";
        this.publishLevelNameInput.style.color = locked ? "#b6c6d1" : "#edf7ff";
      } else {
        this.publishLevelNameInput.style.display = "none";
      }
      ...
      if (showSizeInputs) {
        const xRect = createLayout.sizeControls.xInput;
        const yRect = createLayout.sizeControls.yInput;
        this.createLevelSizeXInput.style.display = "block";
        this.createLevelSizeXInput.style.left = `${xRect.x}px`;
        this.createLevelSizeXInput.style.top = `${xRect.y}px`;
        this.createLevelSizeXInput.style.width = `${xRect.w}px`;
        this.createLevelSizeXInput.style.height = `${xRect.h}px`;
        this.createLevelSizeXInput.style.fontSize = `${Math.max(12, Math.round(createLayout.scale * 13))}px`;
        this.createLevelSizeXInput.readOnly = locked;
        this.createLevelSizeXInput.style.borderColor = locked ? "rgba(128, 146, 160, 0.34)" : "rgba(174, 222, 255, 0.46)";
        this.createLevelSizeXInput.style.color = locked ? "#b6c6d1" : "#edf7ff";
        this.createLevelSizeYInput.style.display = "block";
        this.createLevelSizeYInput.style.left = `${yRect.x}px`;
        this.createLevelSizeYInput.style.top = `${yRect.y}px`;
        this.createLevelSizeYInput.style.width = `${yRect.w}px`;
        this.createLevelSizeYInput.style.height = `${yRect.h}px`;
        this.createLevelSizeYInput.style.fontSize = `${Math.max(12, Math.round(createLayout.scale * 13))}px`;
        this.createLevelSizeYInput.readOnly = locked;
        this.createLevelSizeYInput.style.borderColor = locked ? "rgba(128, 146, 160, 0.34)" : "rgba(174, 222, 255, 0.46)";
        this.createLevelSizeYInput.style.color = locked ? "#b6c6d1" : "#edf7ff";
      } else {
        if (document.activeElement === this.createLevelSizeXInput) {
          this.createLevelSizeXInput.blur();
        }
    ...
    updateMyLevelDeletePasswordInputVisibility() {
      if (!this.myLevelDeletePasswordInput || this.currentScreen !== "my_level_detail" || !this.myLevelActionPrompt || this.myLevelActionPrompt.mode !== "delete") {
        if (this.myLevelDeletePasswordInput) {
          if (document.activeElement === this.myLevelDeletePasswordInput) {
            this.myLevelDeletePasswordInput.blur();
          }
          this.myLevelDeletePasswordInput.style.display = "none";
        }
      ...
      const layout = this.getMyLevelActionPromptLayout();
      if (!layout.passwordInput) {
        if (document.activeElement === this.myLevelDeletePasswordInput) {
          this.myLevelDeletePasswordInput.blur();
        }
        this.myLevelDeletePasswordInput.style.display = "none";
      ...
      this.myLevelDeletePasswordInput.style.display = "block";
      this.myLevelDeletePasswordInput.style.left = `${layout.passwordInput.x}px`;
      this.myLevelDeletePasswordInput.style.top = `${layout.passwordInput.y}px`;
      this.myLevelDeletePasswordInput.style.width = `${layout.passwordInput.w}px`;
      this.myLevelDeletePasswordInput.style.height = `${layout.passwordInput.h}px`;
      this.myLevelDeletePasswordInput.style.fontSize = `${Math.max(12, Math.round(layout.scale * 13))}px`;
      if (document.activeElement !== this.myLevelDeletePasswordInput && this.myLevelDeletePasswordInput.value !== (this.myLevelActionPrompt.password || "")) {
        this.myLevelDeletePasswordInput.value = this.myLevelActionPrompt.password || "";
      }
    }
    ...
    updateCommunitySortSelectVisibility() {
      if (!this.communitySortSelect || this.currentScreen !== "community_levels") {
        if (this.communitySortSelect) {
          if (document.activeElement === this.communitySortSelect) {
            this.communitySortSelect.blur();
          }
          this.communitySortSelect.style.display = "none";
        }
        return;
      ...
      const layout = this.getCommunitySortLayout();
      if (!layout) {
        this.communitySortSelect.style.display = "none";
        return;
      }

      this.communitySortSelect.style.display = "block";
      this.communitySortSelect.style.left = `${layout.x}px`;
      this.communitySortSelect.style.top = `${layout.y}px`;
      this.communitySortSelect.style.width = `${layout.w}px`;
      this.communitySortSelect.style.height = `${layout.h}px`;
      this.communitySortSelect.style.fontSize = `${layout.fontSize}px`;
      this.communitySortSelect.style.padding = "0 12px";
      this.communitySortSelect.style.textAlign = "left";
      this.communitySortSelect.style.pointerEvents = "auto";
      this.communitySortSelect.style.cursor = "pointer";
      this.communitySortSelect.style.borderColor = "rgba(174, 222, 255, 0.46)";
      this.communitySortSelect.style.color = "#edf7ff";
      if (this.communitySortSelect.value !== this.communityLevelSort) {
        this.communitySortSelect.value = this.communityLevelSort;
      }
    }
    ...
    triggerWallFlash(tileX, tileY, strength) {
      ...
      this.wallFlashes.set(key, flash);
      this.queueReplayWallFlashEvent(tileX, tileY, flash.strength, flash.life, flash.maxLife);
    }
    ...
    respawnAtLevelStart() {
      const resetReplayCapture = !this.isSpeedrunMode();
      this.totalDamage = 0;
      this.damageSmoke.length = 0;
      this.damageFire.length = 0;
      this.explosionParticles.length = 0;
      ...
      if (resetReplayCapture) {
        this.beginRunReplayCapture();
      }
      this.car.reset(this.level.start.x, this.level.start.y, this.level.startAngle);
      this.camera.snap(this.level.start.x, this.level.start.y);
      if (resetReplayCapture) {
        this.captureCurrentRunReplayFrame(this.levelIndex, this.car, true);
      }
      this.prepareActiveGhostReplay();
    }
    ...
    appendSkidSegment(side, point, driftAmount, skidMarks = this.skidMarks, owner = null) {
      ...
      skidMarks.push(mark);
      if (!owner) {
        this.queueReplaySkidEvent(mark);
      }
    }
    ...
    render(alpha, now) {
      ...
      if (this.currentScreen === "title") {
        ...
        this.renderSettingsScreen(this.ctx, now);
      } else if (this.currentScreen === "two_player_menu") {
        ...
        this.renderLeaderboardReplayScreen(this.ctx, now);
      } else if (this.currentScreen === "online") {
        this.renderOnlineScreen(this.ctx, now);
      } else if (this.currentScreen === "online_racing") {
        this.renderOnlineRacingScreen(this.ctx, now);
      } else if (this.currentScreen === "online_racing_join") {
        this.renderOnlineRacingJoinScreen(this.ctx, now);
      } else if (this.currentScreen === "online_racing_host") {
        this.renderOnlineRacingHostScreen(this.ctx, now);
      } else if (this.currentScreen === "online_racing_account") {
        this.renderOnlineRacingAccountScreen(this.ctx, now);
        ...
        this.renderMyLevelDetailScreen(this.ctx, now);
    ...
    renderTitleScreen(ctx, now) {
      ...
      this.renderMenuButtons(ctx, [this.getTitleSettingsButton()]);
      this.renderTitleUsernameBox(ctx);
      this.renderSignOutConfirmOverlay(ctx);
    }

    renderSettingsScreen(ctx, now) {
      this.renderMenuBackdrop(ctx, now);
      ctx.save();
      ctx.textAlign = "center";
      ctx.fillStyle = "#edf7ff";
      ctx.font = "700 56px Consolas, monospace";
      ctx.fillText("Settings", this.width * 0.5, Math.max(74, this.height * 0.17));
      ctx.fillStyle = "rgba(178, 213, 240, 0.88)";
      ctx.font = "17px Consolas, monospace";
      ctx.fillText(fitTextToWidth(ctx, "Ghost", this.width - 48), this.width * 0.5, Math.max(116, this.height * 0.17 + 36));
      ctx.restore();

      this.renderMenuButtons(ctx, [this.getSettingsBackButton()]);
      const buttons = this.getSettingsGhostOptionButtons();
      this.renderMenuButtons(ctx, buttons);

      ctx.save();
      ctx.textAlign = "center";
      ctx.fillStyle = "rgba(213, 237, 255, 0.62)";
      ctx.font = "13px Consolas, monospace";
      ctx.fillText(
        fitTextToWidth(ctx, "Ghost cars appear only when that level has a saved replay for the selected record.", this.width - 48),
        this.width * 0.5,
        Math.min(this.height - 38, buttons[buttons.length - 1].y + buttons[buttons.length - 1].h + 42)
      );
      ctx.restore();
    }
    ...
    renderLeaderboardScreen(ctx, now) {
      ...
      const renderColumn = (panelX, title, titleColor, timeColor, borderColor, fillStyle, shimmerStyle, records, baseScope) => {
        ctx.save();
        ...
        for (let index = 0; index < LEVELS.length; index += 1) {
          ...
          const replayTargetId = this.getLeaderboardReplayTargetId(this.getVersionedLevelScope(baseScope, index), index);
          const replayHovered = this.hoveredLeaderboardReplayId === replayTargetId;
          ctx.fillStyle = hasRecord ? "rgba(255, 255, 255, 0.04)" : "rgba(255, 255, 255, 0.02)";
          ctx.beginPath();
          ctx.roundRect(10, rowY + 2, panelW - 20, rowH - 4, 10);
          ctx.fill();
          if (replayHovered) {
            ctx.strokeStyle = "rgba(154, 255, 210, 0.75)";
            ctx.lineWidth = 1.6;
            ctx.stroke();
          }
      ...
      speedrunRecords.forEach((entry, index) => {
        ...
        const replayTargetId = this.getLeaderboardReplayTargetId(this.getVersionedSpeedrunScope(entry.scopeBase));
        const replayHovered = this.hoveredLeaderboardReplayId === replayTargetId;
        ctx.save();
        ...
        ctx.stroke();
        if (replayHovered) {
          ctx.strokeStyle = "rgba(154, 255, 210, 0.78)";
          ctx.lineWidth = 1.8;
          ctx.stroke();
        }
        ...
        const previewScale = clamp((speedrunSectionW - 86) / 54, 0.62, 0.78);
        const previewCenterX = speedrunSectionW - Math.max(28, Math.round(34 * previewScale));
        const previewReservedHalfW = Math.round(CAR_LENGTH * 0.72 * previewScale + 14);
        const previewSafeLeftX = Math.max(24, previewCenterX - previewReservedHalfW - 8);
        ctx.font = "700 19px Consolas, monospace";
        const timeTextWidth = ctx.measureText(timeText).width;
        ctx.fillText(timeText, 10, 48);
        ctx.font = "12px Consolas, monospace";
        const deathsWidth = ctx.measureText(deathsText).width;
        const deathsX = Math.min(
          10 + timeTextWidth + 14,
          Math.max(10, previewSafeLeftX - deathsWidth - 8)
        );
        ctx.fillStyle = hasRecord ? "#c3d0dc" : "#778391";
        ctx.fillText(deathsText, deathsX, 47);
        if (hasRecord) {
          ctx.fillStyle = "rgba(220, 239, 250, 0.76)";
          ctx.font = `700 ${Math.max(9, Math.min(10, Math.round(speedrunSectionW * 0.07)))}px Consolas, monospace`;
          ctx.textAlign = "right";
          const displayName = entry.record.playerName || "";
          const nameRightX = previewSafeLeftX;
          const nameMaxWidth = Math.max(24, nameRightX - 10);
          ctx.fillText(fitTextToWidth(ctx, displayName, nameMaxWidth), nameRightX, speedrunSectionH * 0.64);
          ctx.textAlign = "left";
        }
        renderPreview(entry.record, previewCenterX, speedrunSectionH * 0.64, previewScale, "rgba(120, 132, 143, 0.5)");
        ctx.restore();
      });
      ctx.restore();
    ...
    renderLeaderboardReplayScreen(ctx, now) {
      const replay = this.activeLeaderboardReplay;
      if (!replay) {
        this.renderMenuBackdrop(ctx, now);
        return;
      ...
      const frame = this.getReplayFrameState(replay.frames[replay.frameIndex], replay.levelIndex ?? 0);
      if (!frame || !replay.level) {
        this.renderMenuBackdrop(ctx, now);
        return;
      }
      ...
      const previousGoalTriggered = this.goalTriggered;
      const previousGoalTimer = this.goalTimer;
      const previousExploding = this.exploding;
      const previousFinishDebris = this.finishDebris;
      const previousDamageSmoke = this.damageSmoke;
      const previousDamageFire = this.damageFire;
      const previousExplosionParticles = this.explosionParticles;
      const previousSkidMarks = this.skidMarks;
      const previousWallFlashes = this.wallFlashes;
      const previousPlayerCarColor = this.playerCarColor;
      const previousPlayerCarVariant = this.playerCarVariant;
      this.level = replay.level;
      this.gameMode = replay.playbackMode;
      this.goalTriggered = replay.goalTriggered;
      this.goalTimer = replay.goalTimer;
      this.exploding = false;
      this.finishDebris = replay.finishDebris;
      this.damageSmoke = [];
      this.damageFire = [];
      this.explosionParticles = replay.explosionParticles;
      this.skidMarks = replay.skidMarks;
      this.wallFlashes = replay.wallFlashes;
      if (typeof replay.record?.carVariant === "string") {
        this.playerCarVariant = this.getCarVariantDefinition(replay.record.carVariant).id;
      }
      if (isValidHexColor(replay.record?.carColor)) {
        this.playerCarColor = replay.record.carColor;
      }
      ...
      ctx.translate(-this.width * 0.5, -this.height * 0.5);
      this.renderWorld(ctx, { x: replay.camera.x, y: replay.camera.y }, now, {
        x: frame.x,
        y: frame.y,
        angle: frame.angle,
      });
      ...
      this.goalTriggered = previousGoalTriggered;
      this.goalTimer = previousGoalTimer;
      this.exploding = previousExploding;
      this.finishDebris = previousFinishDebris;
      this.damageSmoke = previousDamageSmoke;
      this.damageFire = previousDamageFire;
      this.explosionParticles = previousExplosionParticles;
      this.skidMarks = previousSkidMarks;
      this.wallFlashes = previousWallFlashes;
      this.playerCarColor = previousPlayerCarColor;
      this.playerCarVariant = previousPlayerCarVariant;

      ctx.save();
      ctx.textAlign = "center";
      ctx.fillStyle = "#edf7ff";
      ctx.font = "700 30px Consolas, monospace";
      ctx.fillText(replay.label, this.width * 0.5, 40);
      ctx.fillStyle = "#ffd76a";
      ctx.font = "700 28px Consolas, monospace";
      ctx.fillText(formatTime(replay.elapsedTime || 0), this.width * 0.5, 72);
      ctx.fillStyle = "rgba(211, 233, 247, 0.88)";
      ctx.font = "15px Consolas, monospace";
      const infoText = `${replay.record.playerName || "Player"}  Record ${formatTime(replay.record.time)}`;
      ctx.fillText(fitTextToWidth(ctx, infoText, this.width - 180), this.width * 0.5, 96);
      ctx.fillStyle = replay.paused ? "#ffd76a" : "rgba(154, 255, 210, 0.92)";
      ctx.font = "14px Consolas, monospace";
      const statusText = replay.paused
        ? (replay.ended
          ? "Replay ended. Tap anywhere to watch again. Use left/right to step frames."
          : "Paused. Tap anywhere to resume. Use left/right or the arrows above to step.")
        : "Tap anywhere to pause.";
      ctx.fillText(fitTextToWidth(ctx, statusText, this.width - 180), this.width * 0.5, this.height - 28);
      ctx.restore();

      this.renderMenuButtons(ctx, [this.getLeaderboardReplayBackButton(), ...this.getLeaderboardReplayStepButtons()]);
    }
    ...
    renderOnlineScreen(ctx, now) {
      ...
      const buttons = this.getOnlineMenuButtons();
      this.renderMenuButtons(ctx, buttons);
      this.renderOnlineRacingBetaBadge(ctx, buttons.find((button) => button.id === "online_racing"));
    }

    renderOnlineRacingBetaBadge(ctx, button) {
      if (!button) {
        return;
      }
      const hover = Number.isFinite(button.hover) ? button.hover : 0;
      const badgeW = Math.max(48, Math.round(button.w * 0.2));
      const badgeH = Math.max(18, Math.round(button.h * 0.34));
      const badgeX = button.x + button.w - badgeW * 0.5 - 14;
      const badgeY = button.y + badgeH * 0.5 + 10;
      ctx.save();
      ctx.translate(badgeX, badgeY);
      ctx.scale(1 + hover * 0.05, 1 + hover * 0.05);
      ctx.shadowColor = `rgba(255, 214, 110, ${0.2 + hover * 0.28})`;
      ctx.shadowBlur = 8 + hover * 12;
      ctx.fillStyle = "rgba(116, 78, 18, 0.96)";
      ctx.strokeStyle = "rgba(255, 225, 142, 0.72)";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.roundRect(-badgeW * 0.5, -badgeH * 0.5, badgeW, badgeH, badgeH * 0.5);
      ctx.fill();
      ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.fillStyle = "#fff2bd";
      ctx.font = `700 ${Math.max(10, Math.round(badgeH * 0.56))}px Consolas, monospace`;
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText("BETA", 0, 1);
      ctx.restore();
    }

    renderOnlineRacingHeader(ctx, title, subtitle = "") {
      ctx.save();
      ctx.textAlign = "center";
      ctx.fillStyle = "#edf7ff";
      ctx.font = "700 52px Consolas, monospace";
      ctx.fillText(title, this.width * 0.5, Math.max(70, this.height * 0.16));
      if (subtitle) {
        ctx.fillStyle = "rgba(213, 237, 255, 0.78)";
        ctx.font = "15px Consolas, monospace";
        ctx.fillText(fitTextToWidth(ctx, subtitle, this.width - 44), this.width * 0.5, Math.max(102, this.height * 0.16 + 30));
      }
      ctx.restore();
    }

    renderOnlineRacingScreen(ctx, now) {
      this.renderOnlineBackdrop(ctx, now);
      this.renderOnlineRacingHeader(ctx, "Online Racing", "Beta frame only - race hosting and matchmaking are coming later.");
      this.renderOnlineBackButton(ctx);
      this.renderMenuButtons(ctx, this.getOnlineRacingMenuButtons());
    }

    renderOnlineRacingJoinScreen(ctx, now) {
      this.renderOnlineBackdrop(ctx, now);
      this.renderOnlineRacingHeader(ctx, "Join Match", "Placeholder lobbies for the future online racing flow.");
      this.renderOnlineBackButton(ctx);

      for (const tab of this.getOnlineRacingJoinTabs()) {
        const selected = tab.id === `online_racing_tab_${this.onlineRacingSelectedTab}`;
        this.renderCreateLevelButton(ctx, tab, selected ? 1 : tab.hover, selected ? "tab_active" : "tab");
      }

      const cards = this.getOnlineRacingMatchCards();
      for (const card of cards) {
        this.renderOnlineRacingMatchCard(ctx, card, this.onlineRacingMatchHovers[card.index] || 0);
      }
      if (cards.length === 0) {
        ctx.save();
        ctx.textAlign = "center";
        ...
        ctx.font = "18px Consolas, monospace";
        ctx.fillText(fitTextToWidth(ctx, "No public races available yet.", this.width - 48), this.width * 0.5, this.height * 0.5);
        ctx.restore();
      }
      ...
      ctx.fillStyle = "rgba(213, 237, 255, 0.62)";
      ctx.font = "13px Consolas, monospace";
      const note = this.onlineRacingSelectedTab === "private"
        ? "Private races will ask for the host code later. These rows are not joinable yet."
        : "These public races are mock rows for layout only.";
      ctx.fillText(fitTextToWidth(ctx, note, this.width - 48), this.width * 0.5, Math.min(this.height - 42, this.height * 0.82));
      ctx.restore();
    }

    renderOnlineRacingMatchCard(ctx, card, hover = 0) {
      ctx.save();
      const centerX = card.x + card.w * 0.5;
      const centerY = card.y + card.h * 0.5;
      ctx.translate(centerX, centerY);
      ctx.scale(1 + hover * 0.025, 1 + hover * 0.025);
      ctx.shadowColor = `rgba(118, 220, 255, ${0.12 + hover * 0.24})`;
      ctx.shadowBlur = 10 + hover * 18;
      const fill = ctx.createLinearGradient(-card.w * 0.5, -card.h * 0.5, card.w * 0.5, card.h * 0.5);
      fill.addColorStop(0, card.private ? "rgba(48, 33, 58, 0.94)" : "rgba(12, 43, 57, 0.94)");
      fill.addColorStop(1, card.private ? "rgba(72, 45, 75, 0.9)" : "rgba(19, 76, 80, 0.9)");
      ctx.fillStyle = fill;
      ctx.strokeStyle = card.private ? `rgba(229, 185, 255, ${0.32 + hover * 0.3})` : `rgba(157, 238, 255, ${0.32 + hover * 0.34})`;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(-card.w * 0.5, -card.h * 0.5, card.w, card.h, 16);
      ctx.fill();
      ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.textAlign = "left";
      ...
      ctx.font = "700 21px Consolas, monospace";
      ctx.fillText(fitTextToWidth(ctx, card.name, card.w - 160), -card.w * 0.5 + 24, -10);
      ctx.fillStyle = "rgba(216, 237, 245, 0.74)";
      ctx.font = "14px Consolas, monospace";
      ctx.fillText(fitTextToWidth(ctx, card.detail, card.w - 170), -card.w * 0.5 + 24, 17);
      if (card.private) {
        ctx.textAlign = "right";
        ctx.fillStyle = "#ffd98e";
        ctx.font = "700 13px Consolas, monospace";
        ctx.fillText("CODE", card.w * 0.5 - 24, 1);
      }
      ctx.restore();
    }

    renderOnlineRacingHostScreen(ctx, now) {
      this.renderOnlineBackdrop(ctx, now);
      this.renderOnlineBackButton(ctx);
      this.renderOnlineRacingHeader(ctx, "Host Race");
    }

    renderOnlineRacingAccountScreen(ctx, now) {
      this.renderOnlineBackdrop(ctx, now);
      this.renderOnlineRacingHeader(ctx, "Account", "Profile controls will connect here later.");
      this.renderOnlineBackButton(ctx);
      this.renderOnlineRacingAccountCar(ctx);
      this.renderMenuButtons(ctx, this.getOnlineRacingAccountButtons());
    }

    renderOnlineRacingAccountCar(ctx) {
      const centerX = this.width * 0.5;
      const centerY = Math.round(this.height * 0.34);
      const cardW = Math.min(300, this.width - 64);
      const cardH = Math.max(132, Math.round(this.height * 0.18));
      ctx.save();
      ctx.shadowColor = "rgba(112, 218, 255, 0.18)";
      ctx.shadowBlur = 20;
      const fill = ctx.createLinearGradient(centerX - cardW * 0.5, centerY - cardH * 0.5, centerX + cardW * 0.5, centerY + cardH * 0.5);
      fill.addColorStop(0, "rgba(9, 28, 42, 0.94)");
      fill.addColorStop(1, "rgba(16, 50, 65, 0.9)");
      ctx.fillStyle = fill;
      ctx.strokeStyle = "rgba(146, 225, 255, 0.34)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(centerX - cardW * 0.5, centerY - cardH * 0.5, cardW, cardH, 22);
      ctx.fill();
      ctx.stroke();
      ctx.translate(centerX, centerY + 12);
      ctx.rotate(-Math.PI * 0.5);
      ctx.scale(1.55, 1.55);
      this.drawCarShape(ctx, this.getPlayerCarPalette(this.playerCarColor), this.playerCarVariant);
      ctx.restore();
    }
    ...
    renderCommunityLevelsScreen(ctx, now) {
      ...
      for (const card of cards) {
        ...
        ctx.fillText(fitTextToWidth(ctx, `By ${card.displayCreator || card.creator}`, card.w * 0.28), card.w * 0.5 - 22, -card.h * 0.06);
        ctx.fillStyle = "rgba(214, 234, 255, 0.76)";
        ctx.font = `${card.smallFontSize}px Consolas, monospace`;
        ctx.fillText(
          fitTextToWidth(ctx, `${card.playCountValue} ${card.playCountValue === 1 ? "View" : "Views"}`, card.w * 0.22),
          card.w * 0.5 - 22,
          card.h * 0.24
        );
        ctx.restore();
    ...
    renderMyLevelsScreen(ctx, now) {
      ...
      ctx.fillStyle = "rgba(213, 237, 255, 0.78)";
      ctx.font = "15px Consolas, monospace";
      ctx.fillText(fitTextToWidth(ctx, "Click any of your levels to open its page. If a level is declined, click its status to read the admin note.", this.width - 44), this.width * 0.5, Math.max(102, this.height * 0.15 + 30));
      ctx.restore();
    ...
    renderMyLevelDetailScreen(ctx, now) {
      this.renderOnlineBackdrop(ctx, now);
      this.renderOnlineBackButton(ctx);
      ...
      const layout = this.getMyLevelDetailLayout(level);
      const statusLabel = getCommunitySubmissionStatusLabel(level.submissionStatus);
      const modifiedText = formatModifiedTimestamp(level.updatedAt || level.createdAt);
      const bestTimeText = Number.isFinite(level.myBestTime) ? formatTime(level.myBestTime) : "--:--.---";

      ctx.save();
      ctx.textAlign = "center";
      ctx.fillStyle = "#f5fbff";
      ctx.font = `700 ${Math.max(24, Math.round(40 * layout.scale))}px Consolas, monospace`;
      ctx.fillText(fitTextToWidth(ctx, level.name, this.width - 44), this.width * 0.5, layout.titleY);

      ctx.fillStyle = "rgba(202, 226, 240, 0.84)";
      ctx.font = `${Math.max(11, Math.round(15 * layout.scale))}px Consolas, monospace`;
      ctx.fillText(fitTextToWidth(ctx, modifiedText, this.width - 44), this.width * 0.5, layout.subtitleY);

      ctx.fillStyle = "rgba(6, 18, 29, 0.82)";
      ctx.strokeStyle = "rgba(154, 255, 210, 0.25)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(layout.panelX, layout.panelY, layout.panelW, layout.panelH, Math.max(12, Math.round(16 * layout.scale)));
      ctx.fill();
      ctx.stroke();

      ctx.textAlign = "left";
      ctx.fillStyle = "rgba(192, 220, 236, 0.72)";
      ctx.font = `700 ${Math.max(11, Math.round(14 * layout.scale))}px Consolas, monospace`;
      ctx.fillText("Status", layout.panelX + layout.innerPad, layout.headerY);
      ctx.textAlign = "right";
      ctx.fillText("Time", layout.panelX + layout.panelW - layout.innerPad, layout.headerY);

      ctx.textAlign = "left";
      ctx.fillStyle = "#9affd2";
      ctx.font = `700 ${Math.max(16, Math.round(22 * layout.scale))}px Consolas, monospace`;
      ctx.fillText(statusLabel, layout.panelX + layout.innerPad, layout.statusValueY);
      ctx.textAlign = "right";
      ctx.fillStyle = "#ffd76a";
      ctx.fillText(bestTimeText, layout.panelX + layout.panelW - layout.innerPad, layout.statusValueY);

      ctx.textAlign = "left";
      ctx.fillStyle = "rgba(226, 240, 250, 0.9)";
      ctx.font = `${Math.max(11, Math.round(14 * layout.scale))}px Consolas, monospace`;
      const noteLines = normalizeCommunitySubmissionStatus(level.submissionStatus || "private") === "declined" && level.reviewMessage
        ? wrapTextToLines(ctx, `Admin note: ${level.reviewMessage}`, layout.panelW - layout.innerPad * 2, 3)
        : [];
      if (this.myLevelDetailStatusText) {
        ctx.fillStyle = this.myLevelDetailStatusTone === "error" ? "#ffb8a8" : this.myLevelDetailStatusTone === "ok" ? "#9affd2" : "rgba(223, 248, 255, 0.9)";
        ctx.fillText(
          fitTextToWidth(ctx, this.myLevelDetailStatusText, layout.panelW - layout.innerPad * 2),
          layout.panelX + layout.innerPad,
          layout.messageY
        );
      }
      if (noteLines.length > 0) {
        noteLines.forEach((line, index) => {
          ctx.fillStyle = "rgba(255, 215, 215, 0.88)";
          ctx.fillText(
            line,
            layout.panelX + layout.innerPad,
            layout.noteY + index * Math.max(15, Math.round(18 * layout.scale))
          );
        });
      }
      ctx.restore();

      this.renderMenuButtons(ctx, [layout.playButton, layout.actionButton, layout.editButton, layout.deleteButton]);
      this.renderMyLevelActionPromptOverlay(ctx);
    }
    ...
    renderCreateLevelScreen(ctx, now) {
      ...
      const layout = this.getCreateLevelLayout();
      if (layout.mode === "setup") {
        this.renderCreateLevelSetupScreen(ctx, layout);
      } else {
        this.renderCreateLevelEditorScreen(ctx, layout);
      }
    }

    renderCreateLevelButton(ctx, button, hoverAmount = 0, accent = "blue", disabled = false) {
      const hover = typeof hoverAmount === "number" ? clamp(hoverAmount, 0, 1) : hoverAmount ? 1 : 0;
      ctx.save();
      ctx.translate(button.x + button.w * 0.5, button.y + button.h * 0.5);
      ctx.scale(1 + hover * 0.04, 1 + hover * 0.04);
      ctx.shadowColor = disabled
        ? "rgba(0, 0, 0, 0)"
        : accent === "tab_active"
          ? `rgba(134, 219, 255, ${0.38 + hover * 0.2})`
        : accent === "green"
          ? `rgba(126, 255, 168, ${0.18 + hover * 0.18})`
          : accent === "gold"
            ? `rgba(255, 215, 116, ${0.18 + hover * 0.18})`
            : accent === "red"
              ? `rgba(255, 138, 138, ${0.18 + hover * 0.18})`
              : accent === "tab"
                ? `rgba(134, 219, 255, ${0.18 + hover * 0.16})`
              : `rgba(134, 219, 255, ${0.18 + hover * 0.18})`;
      ctx.shadowBlur = disabled ? 0 : accent === "tab_active" ? 16 + hover * 10 : 10 + hover * 10;
      const fill = ctx.createLinearGradient(-button.w * 0.5, -button.h * 0.5, button.w * 0.5, button.h * 0.5);
      if (disabled) {
        fill.addColorStop(0, "rgba(28, 33, 41, 0.92)");
        fill.addColorStop(1, "rgba(18, 22, 29, 0.94)");
      } else if (accent === "tab_active") {
        fill.addColorStop(0, "rgba(12, 36, 52, 0.97)");
        fill.addColorStop(1, "rgba(19, 62, 88, 0.94)");
      } else if (accent === "green") {
        fill.addColorStop(0, "rgba(20, 74, 42, 0.96)");
        fill.addColorStop(1, "rgba(30, 110, 56, 0.92)");
      } else if (accent === "gold") {
        fill.addColorStop(0, "rgba(82, 58, 18, 0.96)");
        fill.addColorStop(1, "rgba(142, 98, 28, 0.92)");
      } else if (accent === "red") {
        fill.addColorStop(0, "rgba(84, 28, 33, 0.96)");
        fill.addColorStop(1, "rgba(128, 45, 50, 0.92)");
      } else if (accent === "tab") {
        fill.addColorStop(0, "rgba(10, 31, 45, 0.97)");
        fill.addColorStop(1, "rgba(16, 53, 76, 0.94)");
      } else {
        fill.addColorStop(0, "rgba(10, 32, 48, 0.96)");
        fill.addColorStop(1, "rgba(16, 58, 84, 0.92)");
      }
      ctx.fillStyle = fill;
      ctx.strokeStyle = disabled
        ? "rgba(84, 94, 106, 0.42)"
        : accent === "tab_active"
          ? "rgba(175, 235, 255, 0.94)"
        : accent === "tab"
          ? "rgba(183, 232, 255, 0.38)"
          : "rgba(183, 232, 255, 0.38)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(-button.w * 0.5, -button.h * 0.5, button.w, button.h, Math.max(10, Math.round((button.h || 40) * 0.24)));
      ctx.fill();
      if (!disabled && (accent === "tab" || accent === "tab_active")) {
        const shine = ctx.createLinearGradient(0, -button.h * 0.5, 0, button.h * 0.12);
        shine.addColorStop(0, accent === "tab_active" ? "rgba(255, 255, 255, 0.18)" : "rgba(255, 255, 255, 0.12)");
        shine.addColorStop(1, "rgba(255, 255, 255, 0)");
        ctx.fillStyle = shine;
        ctx.beginPath();
        ctx.roundRect(-button.w * 0.5, -button.h * 0.5, button.w, button.h * 0.62, Math.max(10, Math.round((button.h || 40) * 0.24)));
        ctx.fill();
        ctx.fillStyle = fill;
      }
      ctx.stroke();
      if (!disabled && accent === "tab_active") {
        ctx.strokeStyle = "rgba(146, 235, 255, 0.9)";
        ctx.shadowColor = "rgba(126, 221, 255, 0.65)";
        ctx.shadowBlur = 14 + hover * 10;
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.roundRect(
          -button.w * 0.5 - 3,
          ...
          button.w + 6,
          button.h + 6,
          Math.max(12, Math.round((button.h || 40) * 0.28))
        );
        ctx.stroke();
      }
      ctx.shadowBlur = 0;
      ctx.fillStyle = disabled ? "#6d7884" : accent === "tab_active" ? "#f8fdff" : accent === "tab" ? "#edf8ff" : "#edf8ff";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.font = `700 ${button.fontSize || 16}px Consolas, monospace`;
      ctx.fillText(fitTextToWidth(ctx, button.label, button.w - 14), 0, Number.isFinite(button.textOffsetY) ? button.textOffsetY : 0);
      ctx.restore();
    }

    renderCreateLevelSetupScreen(ctx, layout) {
      const editor = this.createLevelEditor;
      ctx.save();
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(layout.panel.x, layout.panel.y, layout.panel.w, layout.panel.h, layout.panel.radius);
      ctx.fill();
      ctx.stroke();
      ctx.textAlign = "center";
      ctx.fillStyle = "#f2fbff";
      ctx.font = `700 ${Math.max(24, Math.round(layout.scale * 38))}px Consolas, monospace`;
      ctx.fillText("Create a Level", this.width * 0.5, layout.titleY);
      ctx.fillStyle = "#9bd0e7";
      ctx.font = `${Math.max(14, Math.round(layout.scale * 18))}px Consolas, monospace`;
      ctx.fillText("Name your level, then press Done to begin building.", this.width * 0.5, layout.copyY);
      ctx.fillStyle = "rgba(226, 238, 248, 0.88)";
      ctx.font = `${Math.max(12, Math.round(layout.scale * 15))}px Consolas, monospace`;
      ctx.fillText("Choose how to start", this.width * 0.5, layout.sourceButtons[0].y - Math.max(10, Math.round(12 * layout.scale)));
      if (editor?.setupSource === "import") {
        ctx.fillStyle = "rgba(198, 223, 238, 0.82)";
        ctx.font = `${Math.max(11, Math.round(layout.scale * 13))}px Consolas, monospace`;
        ctx.fillText("Paste raw map rows and Drifty will convert them into the editor.", this.width * 0.5, layout.importInput.y - Math.max(10, Math.round(12 * layout.scale)));
      }
      ctx.restore();
      for (const button of layout.sourceButtons) {
        const selected = (button.id === "setup_source_blank" && editor?.setupSource !== "import")
          || (button.id === "setup_source_import" && editor?.setupSource === "import");
        this.renderCreateLevelButton(ctx, button, this.getCreateLevelButtonHover(button.id, selected), "blue");
        if (selected) {
          ctx.save();
          ctx.strokeStyle = "rgba(146, 235, 255, 0.9)";
          ctx.shadowColor = "rgba(126, 221, 255, 0.65)";
          ...
          ctx.lineWidth = 2.5;
          ctx.beginPath();
          ctx.roundRect(button.x - 3, button.y - 3, button.w + 6, button.h + 6, Math.max(12, Math.round(button.h * 0.28)));
          ctx.stroke();
          ctx.restore();
        }
      }
      for (const button of layout.buttons) {
        this.renderCreateLevelButton(ctx, button, this.getCreateLevelButtonHover(button.id));
      }
      if (editor?.messageText) {
        ctx.save();
        const messageW = Math.min(Math.round(500 * layout.scale), this.width - 36);
        const messageH = Math.max(38, Math.round(44 * layout.scale));
        const messageX = this.width * 0.5 - messageW * 0.5;
        const messageY = layout.panel.y + layout.panel.h - layout.buttons[0].h - messageH - Math.max(32, Math.round(40 * layout.scale));
        ctx.fillStyle = editor.messageTone === "error" ? "rgba(96, 24, 28, 0.95)" : "rgba(10, 24, 36, 0.94)";
        ctx.strokeStyle = editor.messageTone === "error" ? "rgba(255, 178, 178, 0.44)" : "rgba(154, 219, 248, 0.32)";
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.roundRect(messageX, messageY, messageW, messageH, 14);
        ctx.fill();
        ctx.stroke();
        ctx.fillStyle = "#f3fbff";
        ctx.textAlign = "center";
        ctx.font = `700 ${Math.max(12, Math.round(layout.scale * 14))}px Consolas, monospace`;
        ctx.fillText(fitTextToWidth(ctx, editor.messageText, messageW - 20), this.width * 0.5, messageY + messageH * 0.62);
        ctx.restore();
      }
    }

    getCreateLevelTileColors(tile, tileX, tileY) {
      if (tile === "S") {
        return { front: "#2fab88", top: "#72e3c1", side: "#1d6b53" };
      }
      if (tile === "G") {
        return { front: mixHexColors("#4f3716", "#f5ce69", 0.42), top: "#e0b34f", side: "#7d571d" };
      }
      if (tile === "#") {
        return { front: "#334854", top: "#506875", side: "#1e2d36" };
      }
      ...
      if (tile === "I") {
        const base = (tileX + tileY) % 2 === 0 ? "#17374d" : "#1f4b67";
        return { front: base, top: mixHexColors(base, "#d7f1ff", 0.3), side: mixHexColors(base, "#08131a", 0.35) };
      }
      if (tile === CREATE_LEVEL_VOID_TILE) {
        return { front: "rgba(0,0,0,0)", top: "rgba(0,0,0,0)", side: "rgba(0,0,0,0)" };
      }
      const base = (tileX + tileY) % 2 === 0 ? "#111c25" : "#17242d";
      return { front: base, top: mixHexColors(base, "#cde8f7", 0.15), side: mixHexColors(base, "#04080c", 0.4) };
    }

    renderCreateLevelTile(ctx, tile, x, y, size, tileX = 0, tileY = 0) {
      if (tile === CREATE_LEVEL_VOID_TILE) {
        ctx.strokeStyle = "rgba(120, 142, 162, 0.34)";
        ctx.setLineDash([6, 5]);
      ...
      const colors = this.getCreateLevelTileColors(tile, tileX, tileY);
      const edge = Math.max(2, Math.round(size * 0.16));
      ...
      if (tile === "." || tile === "I") {
        ...
        if (tile === "I") {
          ...
          ctx.fillRect(x + size * 0.13, y + size * 0.16, size * 0.59, Math.max(2, size * 0.11));
          ctx.fillRect(x + size * 0.25, y + size * 0.44, size * 0.38, Math.max(2, size * 0.08));
          ctx.fillRect(x + size * 0.19, y + size * 0.69, size * 0.47, Math.max(2, size * 0.09));
          ctx.restore();
        }
        return;
      }
      ...
      if (tile === "S") {
        const startSize = size * 0.7;
        const startX = x + (size - startSize) * 0.5;
        const startY = y + (size - startSize) * 0.5;
        ctx.fillStyle = colors.front;
        ...
        ctx.strokeRect(startX + inset, startY + inset, startSize - inset * 2, startSize - inset * 2);
        return;
      }
      ...
      if (tile === "P") {
        ...
        ctx.fillRect(x + size * 0.19, y + size * 0.19, size * 0.62, size * 0.62);
        ctx.restore();
        return;
      }
    ...
    renderCreateLevelTilePreview(ctx, tile, x, y, w, h, tileX = 0, tileY = 0) {
      const inset = Math.max(6, Math.round(Math.min(w, h) * 0.18));
      const size = Math.max(8, Math.min(w - inset * 2, h - inset * 2));
      const drawX = x + Math.max(inset, (w - size) * 0.5);
      const drawY = y + Math.max(inset, (h - size) * 0.5);
      this.renderCreateLevelTile(ctx, tile, drawX, drawY, size, tileX, tileY);
    }

    renderCreateLevelEditorScreen(ctx, layout) {
      const editor = this.createLevelEditor;
      const quickButtons = layout.quickButtons;
      const activeQuickPublish = quickButtons.find((button) => button.id === "editor_publish");
      ctx.save();
      const topGradient = ctx.createLinearGradient(0, layout.topBar.y, 0, layout.topBar.y + layout.topBar.h);
      topGradient.addColorStop(0, "rgba(8, 18, 28, 0.96)");
      topGradient.addColorStop(1, "rgba(4, 10, 16, 0.92)");
      ctx.fillStyle = topGradient;
      ctx.strokeStyle = "rgba(134, 219, 255, 0.24)";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.roundRect(layout.topBar.x, layout.topBar.y, layout.topBar.w, layout.topBar.h, 16);
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = "#edf7ff";
      ctx.textAlign = "left";
      const createTitle = normalizeCommunityLevelName(editor?.name) || (this.isEditingMyLevel() ? "Edit Level" : "Create a Level");
      const titleFontSize = Math.max(18, Math.round(layout.scale * 22));
      const showStaticTitle = editor.publishPanelOpen || editor.menuOpen || editor.exitConfirmOpen || editor.deleteConfirmOpen;
      if (showStaticTitle) {
        ctx.font = `700 ${titleFontSize}px Consolas, monospace`;
        ctx.fillText(fitTextToWidth(ctx, createTitle, layout.editorTitleMaxW), layout.editorTitleX, layout.topBar.y + layout.topBar.h * 0.62);
      }
      ctx.restore();

      ctx.save();
      ctx.fillStyle = "rgba(6, 14, 22, 0.94)";
      ctx.strokeStyle = "rgba(148, 221, 255, 0.22)";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.roundRect(layout.viewport.x, layout.viewport.y, layout.viewport.w, layout.viewport.h, 16);
      ctx.fill();
      ctx.stroke();
      ctx.beginPath();
      ...
      const floorPatternOffsetX = Math.floor(Number.isFinite(editor.floorPatternOffsetX) ? editor.floorPatternOffsetX : 0);
      const floorPatternOffsetY = Math.floor(Number.isFinite(editor.floorPatternOffsetY) ? editor.floorPatternOffsetY : 0);
      for (let y = 0; y < editor.height; y += 1) {
        for (let x = 0; x < editor.width; x += 1) {
          const tile = editor.rows[y]?.[x] ?? CREATE_LEVEL_VOID_TILE;
          const screen = this.createLevelWorldToScreen(x * TILE_SIZE, y * TILE_SIZE, layout.viewport, editor);
          const tileSize = TILE_SIZE * editor.zoom;
          if (screen.x + tileSize < layout.viewport.x || screen.x > layout.viewport.x + layout.viewport.w || screen.y + tileSize < layout.viewport.y || screen.y > layout.viewport.y + layout.viewport.h) {
            continue;
          }
          this.renderCreateLevelTile(ctx, tile, screen.x, screen.y, tileSize, x + floorPatternOffsetX, y + floorPatternOffsetY);
        }
      }
      ...
      ctx.shadowColor = "rgba(238, 248, 255, 0.34)";
      ctx.shadowBlur = 14;
      ctx.lineWidth = Math.max(2, editor.zoom * 1.2);
      ctx.strokeRect(topLeft.x, topLeft.y, bottomRight.x - topLeft.x, bottomRight.y - topLeft.y);
      ctx.shadowBlur = 0;
      const activeResizeEdge = this.createLevelPointerGesture
        ? (this.createLevelPointerGesture.mode === "resize_edge" ? this.createLevelPointerGesture.edge : null)
        : this.createLevelTouchGesture?.mode
          ? (this.createLevelTouchGesture.mode === "resize_edge" ? this.createLevelTouchGesture.edge : null)
          : this.hoveredCreateLevelResizeEdge;
      if (activeResizeEdge) {
        const hotspot = this.getCreateLevelResizeEdgeHotspots(layout, editor).find((entry) => entry.edge === activeResizeEdge);
        if (hotspot) {
          ctx.save();
          ctx.strokeStyle = "rgba(120, 224, 255, 0.96)";
          ctx.shadowColor = "rgba(120, 224, 255, 0.52)";
          ctx.shadowBlur = 16;
          ctx.lineWidth = Math.max(3, editor.zoom * 1.6);
          ctx.beginPath();
          ctx.moveTo(hotspot.x1, hotspot.y1);
          ctx.lineTo(hotspot.x2, hotspot.y2);
          ctx.stroke();
          ctx.restore();
        }
      }
      ...
      if (startTile) {
        const center = this.createLevelWorldToScreen((startTile.x + 0.5) * TILE_SIZE, (startTile.y + 0.5) * TILE_SIZE, layout.viewport, editor);
        const ringRadius = Math.max(16, TILE_SIZE * editor.zoom * 0.42);
        const dotRadius = Math.max(7, ringRadius * 0.14);
        const dotX = center.x + Math.cos(editor.startAngle) * ringRadius;
        const dotY = center.y + Math.sin(editor.startAngle) * ringRadius;
        ctx.strokeStyle = "rgba(110, 216, 255, 0.78)";
        ctx.lineWidth = Math.max(2, editor.zoom);
        ctx.beginPath();
      ...
      const undoDisabled = this.isCreateLevelEditorLocked(editor) || editor.undoStack.length <= 0;
      const redoDisabled = this.isCreateLevelEditorLocked(editor) || editor.redoStack.length <= 0;
      const saveDisabled = !this.createLevelEditorHasUnsavedChanges(editor);
      for (const button of [layout.saveButton, ...layout.historyButtons, ...layout.quickButtons, ...layout.sideButtons]) {
        const accent = button.id === "editor_publish"
          ? (editor.status === "published" ? "red" : "gold")
          : button.id === "editor_play"
            ? "green"
            : "blue";
        const disabled = button.id === "editor_save"
          ? saveDisabled
          : button.id === "editor_undo"
          ? undoDisabled
          : button.id === "editor_redo"
            ? redoDisabled
            : false;
        this.renderCreateLevelButton(ctx, button, this.getCreateLevelButtonHover(button.id), accent, disabled);
      }
      ...
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.roundRect(layout.bottomPanel.x, layout.bottomPanel.y, layout.bottomPanel.w, layout.bottomPanel.h, 18);
      ctx.fill();
      ctx.stroke();
      const bottomUiScale = layout.scale * (layout.bottomPanelScale || 1);
      if (layout.bottomPanelCollapsed) {
        ctx.fillStyle = "rgba(234, 245, 255, 0.88)";
        ctx.textAlign = "left";
        ctx.font = `700 ${Math.max(12, Math.round(16 * bottomUiScale))}px Consolas, monospace`;
        ctx.fillText("Tools", layout.bottomPanel.x + Math.max(14, Math.round(18 * bottomUiScale)), layout.bottomPanel.y + layout.bottomPanel.h * 0.62);
      } else {
        ctx.fillStyle = "#edf7ff";
        ctx.textAlign = "left";
        ctx.font = `700 ${Math.max(12, Math.round(18 * bottomUiScale))}px Consolas, monospace`;
        ctx.fillText("Mouse", layout.mouseModePanel.x, layout.mouseModePanel.y - Math.max(6, Math.round(8 * bottomUiScale)));
        ctx.fillText("Palette", layout.palettePanel.x, layout.palettePanel.y - Math.max(6, Math.round(8 * bottomUiScale)));
        ctx.fillText("Build Size", layout.sizePanel.x, layout.sizePanel.y - Math.max(6, Math.round(8 * bottomUiScale)));
      }
      ctx.restore();
      this.renderCreateLevelButton(ctx, layout.bottomToggleButton, this.getCreateLevelButtonHover(layout.bottomToggleButton.id), "blue");

      if (!layout.bottomPanelCollapsed) {
        for (const button of [...layout.mouseModeButtons, ...layout.sizeControls.buttons]) {
          this.renderCreateLevelButton(ctx, button, this.getCreateLevelButtonHover(button.id), "blue");
        }

        for (const button of layout.tabs) {
          const tabSelected = editor.activeTab === button.id;
          this.renderCreateLevelButton(ctx, button, this.getCreateLevelButtonHover(button.id, tabSelected), "blue");
          if (tabSelected) {
            ctx.save();
            ctx.strokeStyle = "rgba(146, 235, 255, 0.9)";
            ctx.shadowColor = "rgba(126, 221, 255, 0.65)";
            ...
            ctx.roundRect(
              ...
              button.w + 6,
              button.h + 6,
              Math.max(12, Math.round(button.h * 0.28))
            );
            ctx.stroke();
            ctx.restore();
          }
        ...
        for (const button of layout.mouseModeButtons) {
          const selected = (button.id === "editor_mode_move" && editor.mouseMode !== "drag") || (button.id === "editor_mode_drag" && editor.mouseMode === "drag");
          if (selected) {
            ctx.save();
            ctx.strokeStyle = "rgba(146, 235, 255, 0.9)";
            ctx.shadowColor = "rgba(126, 221, 255, 0.65)";
        ...
        for (const card of this.getCreateLevelPaletteCards(layout)) {
          const selected = this.hoveredCreateLevelPaletteIndex === card.index || this.createLevelEditor.selectedPaletteId === card.id;
          ctx.save();
          ctx.fillStyle = selected ? "rgba(18, 54, 76, 0.98)" : "rgba(10, 22, 34, 0.92)";
          ctx.strokeStyle = selected ? "rgba(168, 232, 255, 0.82)" : "rgba(111, 178, 204, 0.22)";
          ctx.lineWidth = selected ? 2 : 1.5;
          ctx.beginPath();
          ctx.roundRect(card.x, card.y, card.w, card.h, 14);
          ctx.fill();
          ctx.stroke();
          this.renderCreateLevelTilePreview(ctx, card.tile, card.x + 8, card.y + 10, card.w - 16, Math.max(34, card.h - 44), 0, 0);
          ctx.fillStyle = "#edf7ff";
          ctx.textAlign = "center";
          ctx.font = `700 ${Math.max(10, Math.round(bottomUiScale * 13))}px Consolas, monospace`;
          ctx.fillText(fitTextToWidth(ctx, card.label, card.w - 10), card.x + card.w * 0.5, card.y + card.h - 12);
          ctx.restore();
        }

        ctx.save();
        ctx.fillStyle = "rgba(226, 238, 248, 0.8)";
        ctx.textAlign = "center";
        ctx.font = `${Math.max(10, Math.round(bottomUiScale * 13))}px Consolas, monospace`;
        const sizeLabelGap = Math.max(6, Math.round(9 * bottomUiScale));
        const sizeLabelBaselineOffset = Math.max(4, Math.round(5 * bottomUiScale));
        ctx.textAlign = "right";
        ctx.fillText("X", layout.sizeControls.xInput.x - sizeLabelGap, layout.sizeControls.xInput.y + layout.sizeControls.xInput.h * 0.5 + sizeLabelBaselineOffset);
        ctx.fillText("Y", layout.sizeControls.yInput.x - sizeLabelGap, layout.sizeControls.yInput.y + layout.sizeControls.yInput.h * 0.5 + sizeLabelBaselineOffset);
        ctx.font = `${Math.max(9, Math.round(bottomUiScale * 12))}px Consolas, monospace`;
        const hintText = editor.mouseMode === "drag"
          ? "Tap to place. Drag to paint. Scroll or pinch to zoom."
          : "Tap to place. Drag to move. Scroll or pinch to zoom.";
        ctx.fillText(hintText, layout.bottomPanel.x + layout.bottomPanel.w * 0.5, layout.bottomPanel.y + layout.bottomPanel.h - Math.max(10, Math.round(12 * bottomUiScale)));
        ctx.restore();
      }

      this.renderCreateLevelButton(
        ctx,
        activeQuickPublish,
        this.getCreateLevelButtonHover(activeQuickPublish.id),
        editor.status === "published" ? "red" : "gold"
      );

      if (editor.messageText) {
        ctx.save();
        const messageW = Math.min(Math.round(520 * layout.scale), this.width - 24);
        const messageH = Math.max(38, Math.round(46 * layout.scale));
        const messageX = this.width * 0.5 - messageW * 0.5;
        const messageY = layout.topBar.y + layout.topBar.h + 8;
        ctx.fillStyle = editor.messageTone === "error" ? "rgba(96, 24, 28, 0.95)" : editor.messageTone === "ok" ? "rgba(20, 72, 44, 0.95)" : "rgba(10, 24, 36, 0.94)";
        ctx.strokeStyle = editor.messageTone === "error" ? "rgba(255, 178, 178, 0.44)" : editor.messageTone === "ok" ? "rgba(172, 255, 202, 0.42)" : "rgba(154, 219, 248, 0.32)";
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.roundRect(messageX, messageY, messageW, messageH, 14);
        ctx.fill();
      ...
      if (editor.menuOpen) {
        this.renderCreateLevelEditorMenuOverlay(ctx, layout);
      }
      if (editor.publishPanelOpen) {
        this.renderCreateLevelPublishOverlay(ctx, layout);
      }
      if (editor.exitConfirmOpen || editor.deleteConfirmOpen) {
        this.renderCreateLevelConfirmOverlay(
          ctx,
          layout,
          editor.deleteConfirmOpen ? "Delete this level?" : "Are you sure you want to exit without saving?"
        );
      }
    }

    renderCreateLevelEditorMenuOverlay(ctx, layout) {
      const overlay = layout.menuOverlay;
      ctx.save();
      ctx.fillStyle = "rgba(2, 6, 10, 0.72)";
      ctx.fillRect(0, 0, this.width, this.height);
      ctx.fillStyle = "rgba(5, 12, 18, 0.96)";
      ctx.strokeStyle = "rgba(150, 224, 255, 0.34)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(overlay.panel.x, overlay.panel.y, overlay.panel.w, overlay.panel.h, overlay.panel.radius);
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = "#edf8ff";
      ctx.textAlign = "center";
      ctx.font = `700 ${Math.max(22, Math.round(layout.scale * 30))}px Consolas, monospace`;
      ctx.fillText("Menu", this.width * 0.5, overlay.panel.y + Math.round(42 * layout.scale));
      ctx.restore();
      for (const button of overlay.buttons) {
        const accent = button.id === "editor_menu_exit" ? "red" : "blue";
        this.renderCreateLevelButton(ctx, button, this.getCreateLevelButtonHover(button.id), accent);
      }
    }

    renderCreateLevelPublishOverlay(ctx, layout) {
      const editor = this.createLevelEditor;
      const overlay = layout.publishOverlay;
      ctx.save();
      ctx.fillStyle = "rgba(2, 6, 10, 0.72)";
      ctx.fillRect(0, 0, this.width, this.height);
      ctx.fillStyle = "rgba(5, 12, 18, 0.96)";
      ctx.strokeStyle = "rgba(255, 220, 148, 0.36)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(overlay.panel.x, overlay.panel.y, overlay.panel.w, overlay.panel.h, overlay.panel.radius);
      ctx.fill();
      ...
      ctx.font = `700 ${Math.max(22, Math.round(layout.scale * 30))}px Consolas, monospace`;
      ctx.fillText("Publish Level", this.width * 0.5, overlay.panel.y + Math.round(42 * layout.scale));
      ctx.fillStyle = "#9cd4e8";
      ctx.font = `${Math.max(13, Math.round(layout.scale * 16))}px Consolas, monospace`;
      const timeText = Number.isFinite(editor.validatedTime)
        ? `Best validation time: ${formatTime(editor.validatedTime)}`
        : "Beat the level once to publish it.";
      ctx.fillText(timeText, this.width * 0.5, overlay.testButton.y + overlay.testButton.h + Math.round(28 * layout.scale));
      ctx.restore();
      this.renderCreateLevelButton(ctx, overlay.testButton, this.getCreateLevelButtonHover(overlay.testButton.id), Number.isFinite(editor.validatedTime) ? "green" : "blue");
      if (Number.isFinite(editor.validatedTime)) {
        ctx.save();
        ctx.strokeStyle = "rgba(164, 255, 196, 0.86)";
        ctx.shadowColor = "rgba(126, 255, 168, 0.52)";
        ctx.shadowBlur = 14;
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.roundRect(
          overlay.testButton.x - 4,
          overlay.testButton.y - 4,
          overlay.testButton.w + 8,
          overlay.testButton.h + 8,
          Math.max(12, Math.round((overlay.testButton.h + 8) * 0.24))
        );
        ctx.stroke();
        ctx.restore();
      }
      this.renderCreateLevelButton(
        ...
        this.getCreateLevelButtonHover(overlay.actionButton.id),
        editor.status === "pending" || editor.status === "published" ? "red" : "gold"
      );
      this.renderCreateLevelButton(ctx, overlay.backButton, this.getCreateLevelButtonHover(overlay.backButton.id));
    }

    renderCreateLevelConfirmOverlay(ctx, layout, title) {
      const overlay = layout.confirmOverlay;
      const confirmLabel = this.createLevelEditor?.deleteConfirmOpen ? "Delete" : "Exit";
      ctx.save();
      ctx.fillStyle = "rgba(2, 6, 10, 0.74)";
      ctx.fillRect(0, 0, this.width, this.height);
      ctx.fillStyle = "rgba(5, 12, 18, 0.96)";
      ctx.strokeStyle = "rgba(255, 188, 188, 0.34)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(overlay.panel.x, overlay.panel.y, overlay.panel.w, overlay.panel.h, overlay.panel.radius);
      ctx.fill();
      ...
      const lines = wrapTextToLines(ctx, title, overlay.panel.w - Math.round(60 * layout.scale), 2);
      lines.forEach((line, index) => {
        ctx.fillText(line, this.width * 0.5, overlay.panel.y + Math.round(64 * layout.scale) + index * Math.max(22, Math.round(28 * layout.scale)));
      });
      ctx.restore();
      this.renderCreateLevelButton(ctx, overlay.buttons[0], this.getCreateLevelButtonHover(overlay.buttons[0].id));
      this.renderCreateLevelButton(
        ctx,
        { ...overlay.buttons[1], label: confirmLabel },
        this.getCreateLevelButtonHover(overlay.buttons[1].id),
        "red"
      );
    }
    ...
    renderMyLevelActionPromptOverlay(ctx) {
      if (!this.myLevelActionPrompt) {
        return;
      }
      const layout = this.getMyLevelActionPromptLayout();
      ...
      const title = this.myLevelActionPrompt.mode === "unpublish"
        ? "Are you sure you want to unpublish?"
        : this.myLevelActionPrompt.mode === "delete"
          ? "Delete this level?"
        : "Are you sure you want to publish?";
      const detail = this.myLevelActionPrompt.mode === "unpublish"
        ? "The level will be removed from Community Levels and moved back to Private."
        : this.myLevelActionPrompt.mode === "delete"
          ? "Enter your account password to permanently delete this level."
        : "The level will be sent for review and marked Pending.";
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(-panelW * 0.5, -panelH * 0.5, panelW, panelH, Math.max(14, Math.round(22 * scale)));
      ctx.fill();
      ctx.stroke();

      ctx.textAlign = "center";
      ctx.fillStyle = "#f2fbff";
      ctx.font = `700 ${Math.max(18, Math.round(29 * scale))}px Consolas, monospace`;
      const titleLines = wrapTextToLines(ctx, title, panelW - Math.round(56 * scale), 2);
      titleLines.forEach((line, index) => {
        ctx.fillText(line, 0, -Math.round(54 * scale) + index * Math.max(22, Math.round(28 * scale)));
      });

      ctx.fillStyle = "#90c9de";
      ctx.font = `${Math.max(12, Math.round(17 * scale))}px Consolas, monospace`;
      const detailLines = wrapTextToLines(ctx, detail, panelW - Math.round(56 * scale), 3);
      detailLines.forEach((line, index) => {
        ctx.fillText(line, 0, Math.round(10 * scale) + index * Math.max(16, Math.round(21 * scale)));
      });
      if (this.myLevelActionPrompt.mode === "delete" && this.myLevelActionPrompt.statusText) {
        ctx.fillStyle = this.myLevelActionPrompt.statusTone === "error" ? "#ffb8a8" : this.myLevelActionPrompt.statusTone === "ok" ? "#9affd2" : "#dff8ff";
        ctx.font = `${Math.max(11, Math.round(14 * scale))}px Consolas, monospace`;
        ctx.fillText(
          fitTextToWidth(ctx, this.myLevelActionPrompt.statusText, panelW - Math.round(56 * scale)),
          0,
          Math.round(80 * scale)
        );
      }
      ...
      this.renderMenuButtons(ctx, layout.buttons);
    }
    ...
    renderCreatePublishPromptOverlay(ctx) {
      ...
      const lines = wrapTextToLines(ctx, "Beat the level once to publish it. Try to set your best time first.", panelW - Math.round(56 * overlayScale), 3);
      lines.forEach((line, index) => {
      ...
      for (const button of this.getCreatePublishPromptButtons()) {
        ...
        this.renderPolishedMenuButton(ctx, {
          ...button,
          x: button.x - this.width * 0.5,
          y: button.y - this.height * 0.5,
        }, {
          hover,
          hoverScale: 0.06,
          accent: button.id === "continue" ? "gold" : "blue",
        });
      }
      ctx.restore();
    }
    ...
    renderWorld(ctx, camera, now, carState) {
      ...
      if (!this.exploding) {
        ...
        this.renderActiveGhostCar(ctx, camera);
    ...
    renderActiveGhostCar(ctx, camera) {
      const replay = this.activeGhostReplay;
      if (!replay || this.ghostMode === "none" || this.isTwoPlayerDrivingMode() || this.isCustomLevelMode()) {
        return;
      }
      const frame = this.getGhostReplayFrameAtElapsed(replay, this.levelTimer);
      if (!frame || frame.levelIndex !== this.levelIndex) {
        return;
      }
      const record = replay.record || {};
      const variant = this.getCarVariantDefinition(record.carVariant).id;
      const color = isValidHexColor(record.carColor) ? record.carColor : "#8fdcff";
      const screen = this.worldToScreen(frame.x, frame.y, camera);

      ctx.save();
      ctx.globalAlpha = 0.42;
      ctx.translate(screen.x, screen.y);
      ctx.rotate(frame.angle);
      ctx.shadowColor = "rgba(151, 225, 255, 0.7)";
      ctx.shadowBlur = 18;
      this.drawCarShape(ctx, this.getPlayerCarPalette(color), variant);
      ctx.restore();
      ...
      ctx.font = "700 13px Consolas, monospace";
      ctx.lineWidth = 4;
      ctx.strokeStyle = "rgba(2, 8, 14, 0.72)";
      ctx.fillStyle = replay.label === "World Record" ? "#ffd76a" : "#9affd2";
      ctx.strokeText(replay.label, screen.x, screen.y - 38);
      ctx.fillText(replay.label, screen.x, screen.y - 38);
      ctx.restore();
    }
    ...
    renderSignOutConfirmOverlay(ctx) {
      ...
      for (const button of this.getSignOutConfirmButtons()) {
        ...
        this.renderPolishedMenuButton(ctx, {
          ...
          hover,
          hoverScale: 0.05,
          accent: button.id === "signout" ? "red" : "blue",
        });
      }
      ctx.restore();
    }
    ...
    renderPolishedMenuButton(ctx, button, options = {}) {
      const hover = Number.isFinite(options.hover) ? options.hover : (Number.isFinite(button.hover) ? button.hover : 0);
      const enabled = options.enabled ?? (button.enabled !== false);
      const accent = options.accent || button.accent || "blue";
      const label = options.label ?? button.label ?? "";
      const fontSize = options.fontSize || button.fontSize || 24;
      const radius = options.radius || button.radius || 14;
      const scale = 1 + hover * (options.hoverScale ?? 0.08);
      const centerX = button.x + button.w * 0.5;
      const centerY = button.y + button.h * 0.5;
      const palette = accent === "green"
        ? {
            shadow: `rgba(133, 255, 194, ${0.16 + hover * 0.4})`,
            fillStart: `rgba(14, 61, 43, ${0.95 - hover * 0.08})`,
            fillEnd: `rgba(26, 112, 72, ${0.92 + hover * 0.03})`,
            stroke: `rgba(182, 255, 212, ${0.32 + hover * 0.42})`,
            gloss: `rgba(198, 255, 224, ${0.12 + hover * 0.12})`,
            text: hover > 0.01 ? "#f4fff8" : "#d8f3e6",
          }
        : accent === "gold"
          ? {
              shadow: `rgba(255, 214, 107, ${0.16 + hover * 0.42})`,
              fillStart: `rgba(74, 53, 16, ${0.96 - hover * 0.08})`,
              fillEnd: `rgba(138, 99, 24, ${0.93 + hover * 0.03})`,
              stroke: `rgba(255, 225, 142, ${0.34 + hover * 0.44})`,
              gloss: `rgba(255, 240, 190, ${0.12 + hover * 0.12})`,
              text: hover > 0.01 ? "#fff4cd" : "#f7e6ae",
            }
          : accent === "red"
            ? {
                shadow: `rgba(255, 141, 122, ${0.18 + hover * 0.42})`,
                fillStart: `rgba(84, 28, 28, ${0.96 - hover * 0.08})`,
                fillEnd: `rgba(132, 44, 44, ${0.93 + hover * 0.03})`,
                stroke: `rgba(255, 170, 150, ${0.34 + hover * 0.44})`,
                gloss: `rgba(255, 215, 210, ${0.12 + hover * 0.12})`,
                text: hover > 0.01 ? "#fff0eb" : "#ffe0d6",
              }
            : accent === "dark"
              ? {
                  shadow: `rgba(124, 136, 154, ${0.12 + hover * 0.22})`,
                  fillStart: `rgba(8, 10, 14, ${0.96 - hover * 0.06})`,
                  fillEnd: `rgba(18, 22, 30, ${0.94 + hover * 0.02})`,
                  stroke: `rgba(100, 112, 129, ${0.34 + hover * 0.26})`,
                  gloss: `rgba(220, 228, 240, ${0.06 + hover * 0.08})`,
                  text: hover > 0.01 ? "#e3e9f2" : "#b9c3d0",
                }
              : {
                  shadow: `rgba(128, 214, 255, ${0.22 + hover * 0.4})`,
                  fillStart: `rgba(10, 33, 49, ${0.92 - hover * 0.12})`,
                  fillEnd: `rgba(16, 57, 78, ${0.9 + hover * 0.08})`,
                  stroke: `rgba(152, 223, 255, ${0.28 + hover * 0.55})`,
                  gloss: `rgba(180, 235, 255, ${0.16 + hover * 0.32})`,
                  text: hover > 0.01 ? "#f4fdff" : "#d6eaf3",
                };

      ctx.save();
      ctx.translate(centerX, centerY);
      ctx.scale(scale, scale);
      ctx.shadowColor = enabled ? palette.shadow : "rgba(0, 0, 0, 0.12)";
      ctx.shadowBlur = enabled ? 8 + hover * 28 : 0;
      ...
      ctx.fillStyle = fill;
      ctx.strokeStyle = enabled ? palette.stroke : "rgba(82, 97, 109, 0.42)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.roundRect(-button.w * 0.5, -button.h * 0.5, button.w, button.h, radius);
      ctx.fill();
      ctx.stroke();
      ...
      if (label) {
        ctx.shadowBlur = 0;
        ctx.fillStyle = enabled ? palette.text : "#7e8a94";
        ctx.font = `700 ${fontSize}px Consolas, monospace`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(fitTextToWidth(ctx, label, button.w - 20), 0, Number.isFinite(button.textOffsetY) ? button.textOffsetY : Math.round(fontSize * 0.12));
      }
      ctx.restore();
    }
    ...
    renderMenuButtons(ctx, buttons) {
      ...
      ctx.textAlign = "center";
      for (const button of buttons) {
        this.renderPolishedMenuButton(ctx, button);
      }
      ctx.restore();
    }
    ...
    renderLevelSelectCards(ctx) {
      ...
      const cardDifficultyFontSize = Math.max(10, Math.round(13 * scale));
      ...
      for (const card of groups.flatMap((group) => group.cards)) {
        ...
        const levelTextY = -card.h * 0.5 + cardAccentHeight + Math.max(4, Math.round(6 * scale));
        const difficultyLabelGapY = Math.max(10, Math.round(12 * scale));
        const difficultyDotsGapY = Math.max(9, Math.round(11 * scale));
        const difficultyToBestGapY = Math.max(18, Math.round(20 * scale));
        const difficultyDotRadius = Math.max(3, Math.round(4 * scale));
        const difficultyDotGap = Math.max(8, Math.round(10 * scale));
        const difficultyDotsWidth = difficultyDotRadius * 2 * card.difficultyDots + difficultyDotGap * Math.max(0, card.difficultyDots - 1);
        const difficultyLabelFont = `700 ${cardDifficultyFontSize}px Consolas, monospace`;
        const stackedDifficultyLabelY = levelTextY + difficultyLabelGapY;
        const stackedDifficultyDotsY = stackedDifficultyLabelY + difficultyDotsGapY;
        const inlineDifficultyY = levelTextY + Math.max(15, Math.round(18 * scale));
        const bestTextBottomLimitY = card.h * 0.5 - Math.max(10, Math.round(12 * scale));
        const stackedBestTextTargetY = stackedDifficultyDotsY + difficultyToBestGapY;
        const inlineDifficultyGap = Math.max(10, Math.round(12 * scale));
        const minInlineLabelWidth = 24;
        ctx.font = difficultyLabelFont;
        const inlineDifficultyLabel = fitTextToWidth(
          ctx,
          card.difficultyLabel,
          Math.max(minInlineLabelWidth, cardTextWidth - difficultyDotsWidth - inlineDifficultyGap)
        );
        const inlineDifficultyWidth = difficultyDotsWidth + inlineDifficultyGap + ctx.measureText(inlineDifficultyLabel).width;
        const useInlineDifficulty = stackedBestTextTargetY > bestTextBottomLimitY &&
          inlineDifficultyWidth <= cardTextWidth &&
          cardTextWidth >= difficultyDotsWidth + inlineDifficultyGap + minInlineLabelWidth;
        const bestTextY = useInlineDifficulty
          ? Math.min(bestTextBottomLimitY, inlineDifficultyY + Math.max(19, Math.round(21 * scale)))
          : Math.min(bestTextBottomLimitY, stackedBestTextTargetY);
        const difficultyDotColor = blackoutSelected
          ? (card.unlocked ? mixHexColors(card.difficultyColor, "#d6dfeb", 0.42) : mixHexColors(card.difficultyColor, "#535c67", 0.72))
          : (card.unlocked ? card.difficultyColor : mixHexColors(card.difficultyColor, "#4a5560", 0.6));
        const difficultyLabelColor = blackoutSelected
          ? (card.unlocked ? "#b1bccb" : "#5d6670")
          : (card.unlocked ? mixHexColors(card.difficultyColor, "#eef8ff", 0.58) : "#66707a");

        ctx.fillStyle = titleColor;
        ctx.font = `700 ${fittedLevelFontSize}px Consolas, monospace`;
        ctx.fillText(levelText, 0, levelTextY);

        if (useInlineDifficulty) {
          ctx.save();
          ctx.textAlign = "left";
          ctx.textBaseline = "middle";
          ctx.fillStyle = difficultyDotColor;
          ctx.strokeStyle = hexToRgba(mixHexColors(difficultyDotColor, "#ffffff", 0.22), blackoutSelected ? 0.22 : 0.34);
          ctx.lineWidth = 1.5;
          let difficultyDotX = -inlineDifficultyWidth * 0.5 + difficultyDotRadius;
          for (let dotIndex = 0; dotIndex < card.difficultyDots; dotIndex += 1) {
            ctx.beginPath();
            ctx.arc(difficultyDotX, inlineDifficultyY, difficultyDotRadius, 0, Math.PI * 2);
            ctx.fill();
            ctx.stroke();
            difficultyDotX += difficultyDotRadius * 2 + difficultyDotGap;
          }

          ctx.fillStyle = difficultyLabelColor;
          ctx.font = difficultyLabelFont;
          ctx.fillText(
            inlineDifficultyLabel,
            -inlineDifficultyWidth * 0.5 + difficultyDotsWidth + inlineDifficultyGap,
            inlineDifficultyY
          );
          ctx.restore();
        } else {
          ...
          ctx.strokeStyle = hexToRgba(mixHexColors(difficultyDotColor, "#ffffff", 0.22), blackoutSelected ? 0.22 : 0.34);
          ctx.lineWidth = 1.5;
          let difficultyDotX = -difficultyDotsWidth * 0.5 + difficultyDotRadius;
          for (let dotIndex = 0; dotIndex < card.difficultyDots; dotIndex += 1) {
            ctx.beginPath();
            ctx.arc(difficultyDotX, stackedDifficultyDotsY, difficultyDotRadius, 0, Math.PI * 2);
            ctx.fill();
            ctx.stroke();
            difficultyDotX += difficultyDotRadius * 2 + difficultyDotGap;
          }
          ...
          ctx.fillStyle = difficultyLabelColor;
          ctx.font = difficultyLabelFont;
          ctx.fillText(card.difficultyLabel, 0, stackedDifficultyLabelY);
        }
        ...
        ctx.fillText(bestText, 0, Math.max(bestTextY, Math.max(cardBestY, Math.round(fittedLevelFontSize * 0.9))));
        ctx.shadowBlur = 0;
    ...
    renderCustomizationButtons(ctx) {
      ...
      for (const button of buttons) {
        ...
        this.renderPolishedMenuButton(ctx, button, {
          hover,
          hoverScale: 0.06,
          accent: button.id === "done" ? "green" : "blue",
        });
      }
      ctx.restore();
    }
    ...
    renderHUDToggleButton(ctx, button) {
      ...
      this.renderPolishedMenuButton(ctx, { ...button, label: "", radius: 12 }, { hover, hoverScale: 0.05 });
      ctx.save();
      ctx.translate(button.x + button.w * 0.5, button.y + button.h * 0.5);
      ctx.scale(1 + hover * 0.05, 1 + hover * 0.05);
    ...
    renderCampaignCompletionOverlay(ctx) {
      ...
      for (const button of this.getCampaignCompleteButtons()) {
        ...
        this.renderPolishedMenuButton(ctx, {
          ...
          hover,
          hoverScale: 0.06,
          accent: button.id === "next" || button.id === "publish_complete"
            ? "gold"
            : button.id === "replay"
              ? "green"
              : "blue",
    ...
    renderHomeConfirmOverlay(ctx) {
      ...
      for (const button of this.getHomeConfirmButtons()) {
        ...
        this.renderPolishedMenuButton(ctx, {
          ...
          hover,
          hoverScale: 0.05,
          accent: button.id === "leave" ? "red" : "blue",
        });
      }
      ctx.restore();
    }
    ...
    renderPublishConfirmOverlay(ctx) {
      ...
      for (const button of this.getPublishConfirmButtons()) {
        ...
        this.renderPolishedMenuButton(ctx, {
          ...
          hover,
          hoverScale: 0.05,
          accent: button.id === "publish" ? "green" : "blue",
        });
      }
      ctx.restore();
    }
    ...
    renderUI(ctx, now) {
      ...
      if (!(this.gameMode !== "tag" && this.completed && this.levelCompletionSummary)) {
        const exitButton = this.getGameExitButton();
        this.renderPolishedMenuButton(ctx, { ...exitButton, label: this.isGameBackButtonMode() ? "Back" : "Home", radius: 12 }, {
          hover: this.gameExitHover,
          hoverScale: 0.06,
          accent: "blue",
        });
        ...
        if (!this.isTwoPlayerDrivingMode()) {
          ...
          this.renderPolishedMenuButton(ctx, { ...restartButton, label: "Restart", radius: 12 }, {
            hover: this.gameRestartHover,
            hoverScale: 0.06,
            accent: "green",
          });
        }
