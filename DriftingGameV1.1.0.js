(() => {
  ...
  class DriftGame {
    constructor() {
      ...
      this.createLevelSizeXInput.className = "drift-username-input";
      ...
      this.createLevelSizeYInput.className = "drift-username-input";
      ...
      this.createLevelSizeXInput.addEventListener("input", (event) => {
        ...
        const numeric = this.sanitizeCreateLevelSizeValue(event.target.value);
        if (event.target.value !== numeric.raw) {
          event.target.value = numeric.raw;
        }
      });
    ...
    async fetchOnlineLeaderboardRows() {
      ...
      if (!response.ok && response.status === 400) {
        response = await fetch(this.getOnlineLeaderboardFetchUrlWithoutReplay(true), {
          method: "GET",
          headers: this.getOnlineLeaderboardHeaders(false),
        });
      }
      ...
      if (!response.ok && response.status === 400) {
        response = await fetch(this.getOnlineLeaderboardFetchUrlWithoutReplay(false), {
          method: "GET",
          headers: this.getOnlineLeaderboardHeaders(false),
        });
      }
    ...
    createCreateLevelEditorState({ name = "", mapRows = null, hiddenMapRows = null, startAngle = 0, submissionStatus = "private", validatedTime = null } = {}) {
    ...
    setCreateLevelEditorTile(editor, gridX, gridY, nextTile) {
      ...
      if (editor.storedRows?.[gridY]?.[gridX] != null) {
        editor.storedRows[gridY][gridX] = nextTile;
      }
      return true;
    }
    ...
    resizeCreateLevelEditor(editor, nextWidth, nextHeight) {
      ...
      editor.storedRows = this.normalizeCreateLevelHiddenRows(editor.storedRows, editor.rows);
      const storedWidth = Math.max(editor.storedRows[0]?.length || 0, width);
      const storedHeight = Math.max(editor.storedRows.length || 0, height);
      ...
      for (let y = 0; y < editor.height; y += 1) {
        for (let x = 0; x < editor.width; x += 1) {
          if (editor.rows[y]?.[x] != null && editor.storedRows[y]?.[x] != null) {
            editor.storedRows[y][x] = editor.rows[y][x];
          }
        }
      }
      const nextRows = Array.from({ length: height }, (_, y) => (
        Array.from({ length: width }, (_, x) => editor.storedRows?.[y]?.[x] ?? ".")
      ));
      editor.width = width;
    ...
    buildCreateLevelDefinitionFromEditor(editor = this.createLevelEditor) {
      ...
      for (let x = 0; x < editor.width; x += 1) {
        ...
        if (!(top === "#" || top === "P" || top === "G") || !(bottom === "#" || bottom === "P" || bottom === "G")) {
      ...
      for (let y = 0; y < editor.height; y += 1) {
        ...
        if (!(left === "#" || left === "P" || left === "G") || !(right === "#" || right === "P" || right === "G")) {
          return { error: "The border must be fully sealed with wall blocks." };
        }
      }
      const definition = {
    ...
    getMyLevelDetailLayout(level = this.getActiveMyLevel()) {
      ...
      const panelH = Math.max(190, Math.round(236 * scale));
      ...
      const buttonsY = panelY + panelH - innerPad - buttonH;
    ...
    getMyLevelActionPromptButtons() {
      ...
      const confirmLabel = this.myLevelActionPrompt?.mode === "unpublish" ? "Unpublish" : "Publish";
      return [
        {
          id: "cancel",
          label: "Cancel",
    ...
    getCreateLevelLayout() {
      ...
      if (editor.mode === "setup") {
        ...
        const panelH = Math.min(Math.max(220, Math.round(280 * scale)), this.height - margin * 2);
        ...
        const gap = Math.max(12, Math.round(18 * scale));
        const buttonsY = panelY + panelH - buttonH - Math.max(18, Math.round(24 * scale));
      ...
      const bottomPanelH = compactEditorLayout
        ? Math.max(286, Math.round(344 * scale))
        : Math.max(170, Math.round(206 * scale));
      const paletteTabsH = Math.max(34, Math.round(40 * scale));
      const paletteViewportH = Math.max(92, Math.round(108 * scale));
      const topY = margin;
      const viewport = {
      ...
      const maxEditorNameInputW = Math.max(
        compactEditorLayout ? 112 : 180,
        quickPlayX - quickGap - editorNameInputX - titleGap - quickButtonW
      );
      ...
      const saveButtonX = editorNameInputX + editorNameInputW + titleGap;
      ...
      const sizePanelW = compactEditorLayout
        ? bottomPanel.w - Math.max(24, Math.round(28 * scale))
        : Math.max(182, Math.round(206 * scale));
      const bottomInnerPad = Math.max(12, Math.round(14 * scale));
      const panelColumnGap = compactEditorLayout ? Math.max(14, Math.round(16 * scale)) : Math.max(18, Math.round(22 * scale));
      const palettePanel = compactEditorLayout
        ? {
            x: bottomPanel.x + bottomInnerPad,
            y: bottomPanel.y + bottomInnerPad,
            w: bottomPanel.w - bottomInnerPad * 2,
            h: paletteTabsH + Math.max(16, Math.round(18 * scale)) + paletteViewportH,
          }
        : {
            x: bottomPanel.x + Math.max(14, Math.round(16 * scale)),
            y: bottomPanel.y + Math.max(12, Math.round(14 * scale)),
            w: bottomPanel.w - sizePanelW - Math.max(34, Math.round(40 * scale)),
            h: bottomPanel.h - Math.max(24, Math.round(28 * scale)),
          };
      const sizePanel = compactEditorLayout
        ? {
            x: palettePanel.x,
      ...
      const tabGap = quickGap;
      const tabW = compactEditorLayout
        ? Math.max(74, Math.floor((palettePanel.w - tabGap * 2) / 3))
        : Math.max(92, Math.round(110 * scale));
      const paletteViewport = {
        x: palettePanel.x,
        y: palettePanel.y + paletteTabsH + Math.max(16, Math.round(18 * scale)),
        w: palettePanel.w,
        h: paletteViewportH,
      };
      const sizeInputW = Math.max(54, Math.round(64 * scale));
      const sizeInputH = Math.max(34, Math.round(38 * scale));
      const sizeArrowW = sizeInputW;
      const sizeArrowH = Math.max(22, Math.round(24 * scale));
      const sizeGap = Math.max(12, Math.round(18 * scale));
      const sizeStartX = sizePanel.x + sizePanel.w * 0.5 - (sizeInputW * 2 + sizeGap) * 0.5;
      const sizeMidY = compactEditorLayout
        ? sizePanel.y + Math.max(44, Math.round(52 * scale))
        : sizePanel.y + Math.round(88 * scale);
      ...
      const menuButtonsTop = menuPanelY + Math.max(72, Math.round(86 * scale));
      const menuButtonsBottom = menuPanelY + menuPanelH - Math.max(24, Math.round(28 * scale));
      const menuButtonCount = 3;
      const menuButtonH = Math.max(
        Math.max(48, Math.round(58 * scale)),
        Math.floor((menuButtonsBottom - menuButtonsTop - overlayGap * Math.max(0, menuButtonCount - 1)) / menuButtonCount)
      );
      const menuButtonGap = Math.max(
        Math.max(8, Math.round(10 * scale)),
        Math.floor((menuButtonsBottom - menuButtonsTop - menuButtonH * menuButtonCount) / Math.max(1, menuButtonCount - 1))
      );
      ...
      return {
        ...
        sizePanel,
        tabs: [
          { id: "walls", label: "Walls", x: palettePanel.x, y: palettePanel.y, w: tabW, h: paletteTabsH },
          { id: "floors", label: "Floors", x: palettePanel.x + tabW + tabGap, y: palettePanel.y, w: tabW, h: paletteTabsH },
          { id: "erase", label: "Erase", x: palettePanel.x + (tabW + tabGap) * 2, y: palettePanel.y, w: tabW, h: paletteTabsH },
        ],
        sideButtons: [
        ...
        saveButton: {
          ...
          label: "Save",
          x: saveButtonX,
          y: compactEditorLayout ? topBar.y + compactPad : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5,
          w: quickButtonW,
          h: quickButtonH,
        ...
        quickButtons: [
          { id: "editor_play", label: "Play", x: quickPlayX, y: compactEditorLayout ? topBar.y + compactPad : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: quickButtonW, h: quickButtonH, fontSize: Math.max(13, Math.round(18 * scale)) },
          { id: "editor_publish", label: editor.status === "published" ? "Delete" : "Publish", x: quickPublishX, y: compactEditorLayout ? topBar.y + compactPad : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: quickButtonW, h: quickButtonH, fontSize: Math.max(13, Math.round(18 * scale)) },
          { id: "editor_menu", label: "Menu", x: quickRightX, y: compactEditorLayout ? topBar.y + compactPad : topBar.y + topBar.h * 0.5 - quickButtonH * 0.5, w: quickButtonW, h: quickButtonH, fontSize: Math.max(14, Math.round(18 * scale)) },
        ],
        sizeControls: {
          ...
          buttons: [
            { id: "editor_width_up", label: "▲", x: sizeStartX, y: sizeMidY - sizeArrowH - 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(12, Math.round(18 * scale)) },
            { id: "editor_width_down", label: "▼", x: sizeStartX, y: sizeMidY + sizeInputH + 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(12, Math.round(18 * scale)) },
            { id: "editor_height_up", label: "▲", x: sizeStartX + sizeInputW + sizeGap, y: sizeMidY - sizeArrowH - 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(12, Math.round(18 * scale)) },
            { id: "editor_height_down", label: "▼", x: sizeStartX + sizeInputW + sizeGap, y: sizeMidY + sizeInputH + 4, w: sizeArrowW, h: sizeArrowH, fontSize: Math.max(12, Math.round(18 * scale)) },
          ],
        },
        menuOverlay: {
          ...
          buttons: Array.from({ length: menuButtonCount }, (_, index) => ({
            id: ["editor_menu_back", "editor_menu_save_exit", "editor_menu_exit"][index],
            label: ["Back", "Save and Exit", "Exit"][index],
    ...
    getCreateLevelPaletteCards(layout = this.getCreateLevelLayout()) {
      ...
      const cardW = Math.max(84, Math.round(98 * layout.scale));
      const gap = Math.max(10, Math.round(12 * layout.scale));
    ...
    setPointerPosition(x, y, inside = true) {
      ...
      } else if (this.currentScreen === "online" || this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level" || this.currentScreen === "my_levels" || this.currentScreen === "my_level_detail") {
        this.refreshOnlineHover();
    ...
    onCanvasPointerUp() {
      if (this.currentScreen === "create_level" && this.createLevelPointerGesture) {
        this.createLevelSuppressNextClick = Boolean(this.createLevelPointerGesture.moved);
        this.createLevelPointerGesture = null;
    ...
    setCreateLevelAngleFromPoint(screenX, screenY) {
      ...
      if (!dial || !this.createLevelEditor) {
        return false;
      }
    ...
    getCreateLevelHoverTarget(x, y) {
      ...
      if (layout.mode === "setup") {
        const hoveredButton = layout.buttons.find((button) => this.isPointInsideRect(x, y, button));
    ...
    refreshCreateLevelHover() {
      ...
      this.canvas.style.cursor = target ? (target.type === "viewport" ? "crosshair" : "pointer") : "default";
    }
    ...
    async publishCreateLevelEditor() {
      ...
      if (this.createLevelEditorHasPendingChangesSinceValidation(editor) || !Number.isFinite(editor.validatedTime)) {
        this.setCreateLevelEditorMessage("Beat the current version first to unlock publishing.", "error");
        return;
      }
    ...
    refreshOnlineHover() {
      if (!["online", "community_levels", "community_detail", "create_level", "my_levels", "my_level_detail"].includes(this.currentScreen) || !this.mouse.inside) {
        this.hoveredOnlineButton = null;
        this.hoveredCommunityLevel = null;
        this.hoveredCreatePublishPromptButton = null;
        this.hoveredMyLevelActionPromptButton = null;
      ...
      const buttons = this.currentScreen === "online"
        ...
        : this.currentScreen === "my_level_detail"
          ? [this.getOnlineBackButton(), ...(() => {
              ...
              return [layout.playButton, layout.actionButton, layout.editButton];
            })()]
    ...
    openLeaderboardReplay(target) {
      const record = this.normalizeWorldRecordEntry(target?.record);
      if (!target || !Array.isArray(record.replayFrames) || record.replayFrames.length === 0) {
        return;
      }
      this.beginScreenWipe(() => {
    ...
    openCreateLevel(returnScreen = "online", options = {}) {
      this.beginScreenWipe(() => {
        ...
        if (editLevel) {
          ...
          this.customLevelStatusText = "Editing your level. Test changes below, then publish when you are ready.";
          this.customLevelStatusTone = "neutral";
        } else {
          this.createLevelEditor = this.createCreateLevelEditorState();
    ...
    fixedUpdate(dt) {
      ...
      if (this.currentScreen === "online" || this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level" || this.currentScreen === "my_levels" || this.currentScreen === "my_level_detail") {
    ...
    renderMyLevelDetailScreen(ctx, now) {
      ...
      this.renderMenuButtons(ctx, [layout.playButton, layout.actionButton, layout.editButton]);
    ...
    renderCreateLevelButton(ctx, button, hoverActive = false, accent = "blue", disabled = false) {
      const hover = hoverActive ? 1 : 0;
      ctx.save();
    ...
    renderCreateLevelSetupScreen(ctx, layout) {
      ...
      for (const button of layout.buttons) {
        this.renderCreateLevelButton(ctx, button, this.hoveredCreateLevelButton === button.id);
      }
    }
    ...
    renderCreateLevelEditorScreen(ctx, layout) {
      ...
      for (let y = 0; y < editor.height; y += 1) {
        for (let x = 0; x < editor.width; x += 1) {
          ...
          this.renderCreateLevelTile(ctx, tile, screen.x, screen.y, tileSize, x, y);
        }
      }
      ...
      for (const button of [layout.saveButton, ...layout.quickButtons, ...layout.sideButtons, ...layout.sizeControls.buttons]) {
        const accent = button.id === "editor_publish"
          ? (editor.status === "published" ? "red" : "gold")
          : button.id === "editor_play"
            ? "green"
            : "blue";
        this.renderCreateLevelButton(ctx, button, this.hoveredCreateLevelButton === button.id, accent);
      }
      ...
      ctx.fillStyle = "#edf7ff";
      ctx.textAlign = "left";
      ctx.font = `700 ${Math.max(14, Math.round(layout.scale * 18))}px Consolas, monospace`;
      ctx.fillText("Palette", layout.palettePanel.x, layout.palettePanel.y - Math.max(6, Math.round(8 * layout.scale)));
      ctx.fillText("Build Size", layout.sizePanel.x, layout.sizePanel.y - Math.max(6, Math.round(8 * layout.scale)));
      ctx.restore();

      for (const button of layout.tabs) {
        const tabSelected = editor.activeTab === button.id;
        this.renderCreateLevelButton(ctx, button, this.hoveredCreateLevelButton === button.id || tabSelected, "blue");
        if (tabSelected) {
          ctx.save();
          ctx.strokeStyle = "rgba(146, 235, 255, 0.9)";
          ctx.shadowColor = "rgba(126, 221, 255, 0.65)";
      ...
      for (const card of this.getCreateLevelPaletteCards(layout)) {
        ...
        ctx.fill();
        ctx.stroke();
        this.renderCreateLevelTilePreview(ctx, card.tile, card.x + 8, card.y + 10, card.w - 16, Math.max(40, card.h - 48), 0, 0);
        ctx.fillStyle = "#edf7ff";
        ctx.textAlign = "center";
        ctx.font = `700 ${Math.max(11, Math.round(layout.scale * 13))}px Consolas, monospace`;
      ...
      ctx.fillText("X", layout.sizeControls.xInput.x + layout.sizeControls.xInput.w * 0.5, layout.sizeControls.xInput.y - 10);
      ctx.fillText("Y", layout.sizeControls.yInput.x + layout.sizeControls.yInput.w * 0.5, layout.sizeControls.yInput.y - 10);
      ctx.font = `${Math.max(10, Math.round(layout.scale * 12))}px Consolas, monospace`;
      ctx.fillText("Tap to place. Drag to move. Scroll or pinch to zoom.", layout.bottomPanel.x + layout.bottomPanel.w * 0.5, layout.bottomPanel.y + layout.bottomPanel.h - 12);
      ctx.restore();

      const canQuickPublish = editor.status === "published"
        ? true
        : !this.createLevelEditorHasPendingChangesSinceValidation(editor) && Number.isFinite(editor.validatedTime);
      this.renderCreateLevelButton(
        ctx,
        activeQuickPublish,
        this.hoveredCreateLevelButton === activeQuickPublish.id,
        editor.status === "published" ? "red" : "gold",
        !canQuickPublish && editor.status !== "published"
      );
    ...
    renderCreateLevelPublishOverlay(ctx, layout) {
      ...
      const canPublish = !this.createLevelEditorHasPendingChangesSinceValidation(editor) && Number.isFinite(editor.validatedTime);
      ctx.save();
      ctx.fillStyle = "rgba(2, 6, 10, 0.72)";
      ctx.fillRect(0, 0, this.width, this.height);
      ctx.fillStyle = "rgba(5, 12, 18, 0.96)";
      ...
      const timeText = Number.isFinite(editor.validatedTime)
        ...
        : "Beat the level once to unlock publishing.";
      ctx.fillText(timeText, this.width * 0.5, overlay.testButton.y + overlay.testButton.h + Math.round(28 * layout.scale));
      ctx.restore();
      this.renderCreateLevelButton(ctx, overlay.testButton, this.hoveredCreateLevelButton === overlay.testButton.id, Number.isFinite(editor.validatedTime) ? "green" : "blue");
      if (Number.isFinite(editor.validatedTime)) {
        ctx.save();
      ...
      this.renderCreateLevelButton(
        ...
        this.hoveredCreateLevelButton === overlay.actionButton.id,
        editor.status === "pending" || editor.status === "published" ? "red" : "gold",
        editor.status !== "pending" && editor.status !== "published" && !canPublish
      );
      this.renderCreateLevelButton(ctx, overlay.backButton, this.hoveredCreateLevelButton === overlay.backButton.id);
    }
    ...
    renderCreateLevelConfirmOverlay(ctx, layout, title) {
      ...
      ctx.restore();
      this.renderCreateLevelButton(ctx, overlay.buttons[0], this.hoveredCreateLevelButton === overlay.buttons[0].id);
      this.renderCreateLevelButton(
        ctx,
        { ...overlay.buttons[1], label: confirmLabel },
        this.hoveredCreateLevelButton === overlay.buttons[1].id,
        "red"
      );
    }
    ...
    renderMyLevelActionPromptOverlay(ctx) {
      ...
      const panelH = Math.min(Math.round(228 * scale), this.height - 24);
      ...
      const detail = this.myLevelActionPrompt.mode === "unpublish"
        ...
        : "The level will be sent in for review and its status will change to Pending.";
      ...
      this.renderMenuButtons(ctx, this.getMyLevelActionPromptButtons());
    }
    ...
    renderLevelSelectCards(ctx) {
      ...
      for (const card of groups.flatMap((group) => group.cards)) {
        ...
        const difficultyLabelY = levelTextY + Math.max(10, Math.round(12 * scale));
        const difficultyDotsY = difficultyLabelY + Math.max(9, Math.round(11 * scale));
        const bestTextY = Math.min(card.h * 0.5 - Math.max(10, Math.round(12 * scale)), difficultyDotsY + Math.max(18, Math.round(20 * scale)));
        ...
        for (let dotIndex = 0; dotIndex < card.difficultyDots; dotIndex += 1) {
          ctx.beginPath();
          ctx.arc(difficultyDotX, difficultyDotsY, difficultyDotRadius, 0, Math.PI * 2);
          ctx.fill();
          ctx.stroke();
        ...
        ctx.font = `700 ${cardDifficultyFontSize}px Consolas, monospace`;
        ctx.fillText(card.difficultyLabel, 0, difficultyLabelY);
