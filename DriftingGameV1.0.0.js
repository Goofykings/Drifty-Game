(() => {
  ...
  function hasRealLeaderboardName(value) {
    ...
    return Boolean(normalized) && normalized !== "Player";
  }
  ...
  class DriftGame {
    constructor() {
      ...
      this.twoPlayerColorInputs.red.setAttribute("aria-label", "Red car color");
      this.twoPlayerColorInputs.blue.setAttribute("aria-label", "Blue car color");
      ...
      this.usernameInput.addEventListener("input", (event) => {
        ...
        this.playerUsername = normalized || "Player";
        this.leaderboardNameUpdateStatusText = "";
        this.leaderboardNameUpdateStatusTone = "neutral";
    ...
    canShowLeaderboardNameUpdateButton() {
      return this.isOnlineLeaderboardConfigured() &&
        !this.leaderboardNameUpdateInFlight &&
        this.playerUsername !== "Player" &&
        this.hasUnnamedOwnedLeaderboardRecords();
    }
    ...
    isOwnedUnnamedLeaderboardRecord(record, ownedKeys = new Set(this.ownedUnnamedLeaderboardRecordKeys)) {
      ...
      if (!ownedKeys.has(recordKey) || !onlineRow || hasRealLeaderboardName(onlineRow.entry.playerName)) {
        return false;
      }
      if (!Number.isInteger(record.levelIndex) || record.levelIndex < 0 || record.levelIndex >= LEVELS.length) {
        return false;
      }
      if (record.scope === this.getVersionedLevelScope("level_normal", record.levelIndex)) {
        return timesMatchForOwnership(this.levelBestTimes[record.levelIndex], onlineRow.entry.time);
      }
      if (record.scope === this.getVersionedLevelScope("level_perfect", record.levelIndex)) {
        return this.levelCleanFinishes[record.levelIndex] &&
          timesMatchForOwnership(this.levelBestTimes[record.levelIndex], onlineRow.entry.time);
      }
      return false;
    }
    ...
    getTitleUsernameLayout() {
      ...
      const panelH = Math.max(82, Math.round((showUpdateButton ? 148 : 106) * scale));
      ...
      const inputY = Math.round(showUpdateButton
        ? panelY + panelH - inputPad - buttonH - buttonGap - inputH
        : panelY + panelH - inputH - inputPad);
    ...
    getLeaderboardButtons() {
      const scale = this.getMenuScale(1);
      const margin = Math.max(12, Math.round(28 * scale));
      const width = Math.min(Math.round(240 * scale), this.width - margin * 2);
      const height = Math.max(42, Math.round(58 * scale));
      return this.leaderboardButtons.map((button) => ({
        ...button,
        x: Math.round(this.width * 0.5 - width * 0.5),
        y: Math.round(this.height - height - margin),
        w: width,
        h: height,
        fontSize: Math.max(14, Math.round(22 * scale)),
        radius: Math.max(10, Math.round(14 * scale)),
    ...
    getCommunityLevelCards() {
      ...
      const startY = Math.max(128, Math.round(this.height * 0.24));
      const x = Math.round(this.width * 0.5 - cardW * 0.5);
      return this.communityLevels.map((level, index) => ({
        ...level,
        index,
        x,
    ...
    setPointerPosition(x, y, inside = true) {
      ...
      } else if (this.currentScreen === "online" || this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level") {
    ...
    handleCanvasTapAt(x, y) {
      ...
      if (this.currentScreen === "title") {
        if (this.hoveredTitleUsernameButton === "update_leaderboard_name") {
      ...
      if (this.currentScreen === "online") {
        if (this.hoveredOnlineButton === "back") {
          ...
          this.openCreateLevel();
        } else if (this.hoveredOnlineButton === "leaderboard") {
          this.openLeaderboard();
        }
        return;
      }
      if (this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level") {
      ...
      if (this.currentScreen === "game") {
        ...
        if (this.gameMode !== "tag" && this.hoveredGameButton === "restart") {
    ...
    refreshTitleHover() {
      ...
      this.hoveredTitleOnlineButton = false;
      this.hoveredTitleUsernameButton = hoveredUsernameButton ? "update_leaderboard_name" : null;
      this.hoveredTitleButton = hoveredUsernameButton ? null : (hovered ? hovered.id : null);
      this.canvas.style.cursor = hovered || hoveredUsernameButton ? "pointer" : "default";
    }

    refreshOnlineHover() {
      if (!["online", "community_levels", "community_detail", "create_level"].includes(this.currentScreen) || !this.mouse.inside) {
        this.hoveredOnlineButton = null;
        this.hoveredCommunityLevel = null;
        this.hoveredCreatePublishPromptButton = null;
        this.canvas.style.cursor = "default";
      ...
      const hoveredCard = this.currentScreen === "community_levels"
        ? this.getCommunityLevelCards().find(
            (card) =>
              this.mouse.x >= card.x &&
    ...
    refreshGameHover() {
      ...
      const hoveredMenu =
        ...
        this.mouse.y >= button.y &&
        this.mouse.y <= button.y + button.h;
      const restartButton = this.gameMode === "tag" ? null : this.getGameRestartButton();
    ...
    parseCustomLevelText(text) {
      ...
      for (const row of rows) {
        for (const tile of row) {
          if (!"#.ISGP".includes(tile)) {
            return { error: `Invalid tile "${tile}". Use # . I S G or P.` };
          }
          if (tile === "S") {
            startCount += 1;
          } else if (tile === "G") {
    ...
    async finishCommunityLevelPublish() {
      ...
      if (this.isOnlineCommunityConfigured()) {
        ...
        this.customLevelStatusTone = "neutral";
        try {
          await this.submitCommunityLevelForReview(this.pendingCommunityLevel);
          this.publishLevelName = "";
          this.publishLevelText = "";
      ...
      this.localCommunityLevels = [
        ...this.localCommunityLevels,
        {
          ...
          startAngle: Number.isFinite(this.pendingCommunityLevel.startAngle) ? this.pendingCommunityLevel.startAngle : 0,
      ...
      this.publishLevelNameInput.value = "";
      this.publishLevelInput.value = "";
      this.customLevelStatusText = "Published to Community Levels.";
      this.customLevelStatusTone = "ok";
      this.pendingCommunityLevel = null;
      this.savePersistentProgress();
      this.openCommunityLevels();
    ...
    recordCommunityLevelTime(index, time) {
      const level = this.communityLevels[index];
      ...
      this.communityLevels[index] = {
        ...level,
        leaderboard: updatedLeaderboard,
      };
      if (level.remoteId) {
        this.onlineCommunityLevels = this.onlineCommunityLevels.map((entry) => (
          entry.remoteId === level.remoteId
            ? { ...entry, leaderboard: updatedLeaderboard }
            : entry
        ));
      } else if (level.local) {
        this.localCommunityLevels = this.localCommunityLevels.map((entry) => (
          entry === level ? { ...entry, leaderboard: updatedLeaderboard } : entry
        ));
      }
      if (level.remoteId) {
    ...
    captureOwnedUnnamedLeaderboardRecordKeysFromLoadedProgress() {
      ...
      for (const record of this.getTrackedOnlineLeaderboardRecords()) {
        ...
        if (
          Number.isInteger(record.levelIndex) &&
          record.scope === this.getVersionedLevelScope("level_normal", record.levelIndex) &&
          timesMatchForOwnership(this.levelBestTimes[record.levelIndex], record.entry.time)
        ) {
          keys.add(this.getOnlineLeaderboardRecordKey(record.scope, record.levelIndex));
        } else if (
          Number.isInteger(record.levelIndex) &&
          record.scope === this.getVersionedLevelScope("level_perfect", record.levelIndex) &&
          this.levelCleanFinishes[record.levelIndex] &&
          timesMatchForOwnership(this.levelBestTimes[record.levelIndex], record.entry.time)
        ) {
          keys.add(this.getOnlineLeaderboardRecordKey(record.scope, record.levelIndex));
        }
    ...
    finishCurrentLevel() {
      ...
      if (this.isCustomLevelMode()) {
        ...
        if (this.gameMode === "publish_test" && this.pendingCommunityLevel) {
          ...
          const result = this.recordCommunityLevelTime(this.selectedCommunityLevelIndex, this.levelTimer);
    ...
    fixedUpdate(dt) {
      ...
      if (this.currentScreen === "online" || this.currentScreen === "community_levels" || this.currentScreen === "community_detail" || this.currentScreen === "create_level") {
    ...
    updateUsernameInputVisibility() {
      if (!this.usernameInput || this.currentScreen !== "title") {
    ...
    renderTwoPlayerMenuScreen(ctx, now) {
      ...
      ctx.font = `${metrics.subtitleSize}px Consolas, monospace`;
      ctx.fillText(
        fitTextToWidth(ctx, "Play tag or customize both cars.", this.width - 28),
        this.width * 0.5,
        metrics.subtitleY
      );
    ...
    renderLeaderboardScreen(ctx, now) {
      ...
      const renderColumn = (panelX, title, titleColor, timeColor, borderColor, fillStyle, shimmerStyle, records) => {
        ...
        for (let index = 0; index < LEVELS.length; index += 1) {
          ...
          if (hasRecord) {
            ...
            ctx.textAlign = "right";
            const displayName = hasRealLeaderboardName(record.playerName) ? record.playerName : "";
      ...
      speedrunRecords.forEach((entry, index) => {
        ...
        if (hasRecord) {
          ...
          ctx.font = "700 10px Consolas, monospace";
          ctx.textAlign = "right";
          const displayName = hasRealLeaderboardName(entry.record.playerName) ? entry.record.playerName : "";
          ctx.fillText(fitTextToWidth(ctx, displayName, 58), speedrunSectionW - 56, speedrunSectionH * 0.64);
          ctx.textAlign = "left";
        }
    ...
    renderOnlineScreen(ctx, now) {
      ...
      ctx.fillText(fitTextToWidth(ctx, "Browse community tracks or build something new.", this.width - 40), this.width * 0.5, Math.max(108, this.height * 0.18 + 34));
      ctx.restore();
    ...
    renderCommunityLevelsScreen(ctx, now) {
      ...
      for (const card of cards) {
        ...
        ctx.fillText(fitTextToWidth(ctx, `By ${card.creator}`, card.w * 0.28), card.w * 0.5 - 22, 0);
        ctx.restore();
      }
      ctx.restore();
    }
    ...
    renderCommunityLevelDetailScreen(ctx, now) {
      ...
      ctx.fillText(fitTextToWidth(ctx, `Made by ${level.creator}`, this.width - 48), this.width * 0.5, Math.max(98, this.height * 0.14 + Math.round(30 * scale)));
    ...
    renderCreateLevelScreen(ctx, now) {
      ...
      const instructions = [
        ...
        "3. Use # as walls, . as open floor, I for ice, S for start, G for the goal, and P for bouncy walls.",
        "4. Go back to the Online tab, click Test Your Level, and paste in the file you created.",
    ...
    renderTwoPlayerCustomizationScreen(ctx, now) {
      ...
      ctx.fillText(fitTextToWidth(ctx, "Tune the red and blue cars before starting tag.", this.width - margin * 2), this.width * 0.5, subtitleY);
      ctx.restore();
    ...
    renderTagUI(ctx) {
      ...
      ctx.fillText(
        fitTextToWidth(ctx, "Red: WASD   Blue: Arrows   Space rematch", this.width - 60),
    ...
    renderCampaignCompletionOverlay(ctx) {
      ...
      ctx.font = `${Math.max(12, Math.round(18 * overlayScale))}px Consolas, monospace`;
      ctx.fillText(
        fitTextToWidth(ctx, summary.speedrun ? (summary.blackoutSpeedrun ? "Blackout full run summary" : "Full run summary") : this.level.name, panelW - Math.round(56 * overlayScale)),
        0,
    ...
    renderUI(ctx, now) {
      ...
      if (!(this.gameMode !== "tag" && this.completed && this.levelCompletionSummary)) {
        ...
        ctx.textAlign = "center";
        ctx.fillText("Home", 0, Math.round((exitButton.fontSize || 18) * 0.35));
        ctx.restore();
      ...
      ctx.fillText(`LEVEL ${this.levelIndex + 1}`, panelX + panelPad, panelY + Math.round(25 * scale));
      ...
      if (!this.completed) {
        ...
        ctx.fillText(fitTextToWidth(ctx, "Drive into the glowing finish cube", maxTextWidth), panelX + panelPad, panelY + panelH - Math.round(15 * scale));
      } else {
      ...
      const completedHint = this.gameMode === "publish_test"
        ? "Enter / Space publish   R retry   Esc home"
        : this.isCustomLevelMode()
          ? "Enter / Space retry   Esc home"
          : "Enter / Space next   Esc home";
      ctx.fillText(
        fitTextToWidth(ctx, this.completed ? completedHint : "WASD / Arrows drive   R restart", this.width - 28),
