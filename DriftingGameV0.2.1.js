  ...
  class DriftGame {
    constructor() {
      ...
      this.titleButtons = [
        ...
        { id: "leaderboard", label: "Leaderboard", hover: 0 },
      ];
      this.titleTrail = [];
    ...
    getOnlineLeaderboardScopeFromLevelRecordArray(records) {
    ...
    getOnlineLeaderboardScopeFromSpeedrunKey(targetKey) {
      if (targetKey === "speedrunWorldRecord") {
        return "speedrun_normal";
      }
      if (targetKey === "perfectSpeedrunWorldRecord") {
        return "speedrun_perfect";
      }
      if (targetKey === "blackoutSpeedrunWorldRecord") {
        return "speedrun_blackout";
      }
      return null;
    }

    getOnlineLeaderboardRecordKey(scope, levelIndex = null) {
    ...
    getTrackedOnlineLeaderboardRecords() {
      ...
      for (const group of levelScopes) {
        for (let index = 0; index < LEVELS.length; index += 1) {
          ...
          if (entry.time != null) {
            tracked.push({ scope: group.scope, levelIndex: index, entry });
          }
        }
      }
      const globalScopes = [
        { scope: "speedrun_normal", entry: this.speedrunWorldRecord },
        { scope: "speedrun_perfect", entry: this.perfectSpeedrunWorldRecord },
        { scope: "speedrun_blackout", entry: this.blackoutSpeedrunWorldRecord },
      ];
    ...
    getOnlineLeaderboardFetchUrl() {
      const params = new URLSearchParams({
        select: "record_key,scope,level_index,time_ms,car_variant,car_color,deaths,updated_at",
    ...
    applyOnlineLeaderboardEntry(scope, levelIndex, entry) {
      ...
      if (scope === "level_normal" && Number.isInteger(levelIndex) && levelIndex >= 0 && levelIndex < this.worldRecords.length) {
      ...
      if (scope === "level_perfect" && Number.isInteger(levelIndex) && levelIndex >= 0 && levelIndex < this.perfectWorldRecords.length) {
      ...
      if (scope === "level_blackout" && Number.isInteger(levelIndex) && levelIndex >= 0 && levelIndex < this.blackoutWorldRecords.length) {
    ...
    async fetchOnlineLeaderboardRows() {
      const response = await fetch(this.getOnlineLeaderboardFetchUrl(), {
        method: "GET",
    ...
    async submitOnlineLeaderboardRecord(scope, levelIndex, entry) {
      const normalized = this.normalizeWorldRecordEntry(entry);
      if (!this.isOnlineLeaderboardConfigured() || normalized.time == null) {
        return null;
      }

      const response = await fetch(this.getOnlineLeaderboardSubmitUrl(), {
        method: "POST",
        headers: this.getOnlineLeaderboardHeaders(true),
        body: JSON.stringify({
          p_record_key: this.getOnlineLeaderboardRecordKey(scope, levelIndex),
          p_scope: scope,
    ...
    queueOnlineLeaderboardRecordSubmission(scope, levelIndex, entry) {
      if (!this.isOnlineLeaderboardConfigured()) {
        return;
      }
      this.onlineLeaderboardSubmissionQueue = this.onlineLeaderboardSubmissionQueue
        .catch(() => undefined)
        .then(() => this.submitOnlineLeaderboardRecord(scope, levelIndex, entry))
        .catch(() => {
    ...
    getLeaderboardButtons() {
      const width = Math.min(240, this.width * 0.24);
      const height = 58;
      const x = this.width * 0.5 - width * 0.5;
      const y = this.height - height - 28;
    ...
    getGameRestartButton() {
      ...
      return {
        id: "restart",
        x: exitButton.x + exitButton.w + 12,
        y: exitButton.y,
        w: 118,
        h: exitButton.h,
      };
    ...
    getCampaignHUDLayout() {
      ...
      const statLines = this.isSpeedrunMode()
        ? [
    ...
    getLevelSelectBlackoutButton() {
      ...
      return {
        ...
        label: "Blackout",
        enabled: this.highestVisitedLevel >= LEVELS.length - 1,
        x: layout.startX + layout.width + layout.gap,
        y: layout.y,
        w: layout.width,
        h: layout.height,
      };
    ...
    getLevelSelectModeButtonLayout() {
      ...
      const gap = 18;
      const width = Math.min(248, (this.width - 120 - gap) * 0.5);
      return {
        width,
        height: metrics.speedrunHeight,
        gap,
        startX: this.width * 0.5 - (width * 2 + gap) * 0.5,
    ...
    handleCanvasTapAt(x, y) {
      ...
      if (this.currentScreen === "title") {
        ...
        } else if (this.hoveredTitleButton === "leaderboard") {
      ...
      if (this.currentScreen === "game") {
        ...
        if (this.hoveredGameButton === "restart") {
          const canManualRestart = this.gameMode === "tag"
            ? !this.tagMatchFinished && this.tagCars.every((player) => player.fireSequenceTimer <= 0 && !player.exploding)
            : !this.completed && !this.goalTriggered && this.fireSequenceTimer <= 0 && !this.exploding;
          if (canManualRestart) {
            this.resetLevel();
          }
          return;
    ...
    resetTouchJoystick() {
      ...
      this.touchJoystick.baseX = 0;
      this.touchJoystick.baseY = 0;
      this.touchJoystick.knobX = 0;
      this.touchJoystick.knobY = 0;
      this.touchJoystick.inputX = 0;
      this.touchJoystick.inputY = 0;
      this.touchJoystick.steer = 0;
      this.touchJoystick.throttle = 0;
      this.touchJoystick.reverseLatched = false;
    }

    startTouchJoystick(touchId, x, y) {
      this.touchJoystick.active = true;
      this.touchJoystick.touchId = touchId;
      this.touchJoystick.baseX = x;
      this.touchJoystick.baseY = y;
      this.touchJoystick.knobX = x;
      this.touchJoystick.knobY = y;
      this.touchJoystick.inputX = 0;
      this.touchJoystick.inputY = 0;
      this.touchJoystick.steer = 0;
      this.touchJoystick.throttle = 0;
    ...
    updateTouchJoystick(x, y) {
      if (!this.touchJoystick.active) {
        return;
      }
      const radius = 64;
      const deadZone = 0.14;
      const deltaX = x - this.touchJoystick.baseX;
      const deltaY = y - this.touchJoystick.baseY;
      ...
      this.touchJoystick.knobX = this.touchJoystick.baseX + offsetX;
      this.touchJoystick.knobY = this.touchJoystick.baseY + offsetY;
      this.touchJoystick.inputX = directionX * remappedMagnitude;
      this.touchJoystick.inputY = directionY * remappedMagnitude;
    }

    getTouchJoystickDriveBasis() {
      ...
      return {
        ...
        reverseThreshold: this.touchJoystick.reverseLatched ? reverseHoldThreshold : reverseEnterThreshold,
      };
    }

    getTouchJoystickTargetMarker() {
      if (!this.touchJoystick.active || !this.canUseTouchJoystick()) {
        return null;
      }
      const inputX = this.touchJoystick.inputX;
      const inputY = this.touchJoystick.inputY;
      ...
      return {
        x: this.car.x + direction.x * TOUCH_JOYSTICK_TARGET_DISTANCE,
        y: this.car.y + direction.y * TOUCH_JOYSTICK_TARGET_DISTANCE,
    ...
    getSinglePlayerControlState() {
      const targetMarker = this.getTouchJoystickTargetMarker();
      if (targetMarker) {
        const basis = this.getTouchJoystickDriveBasis();
        const toMarkerX = targetMarker.x - this.car.x;
        const toMarkerY = targetMarker.y - this.car.y;
        ...
        this.touchJoystick.reverseLatched = reversing;
        const controlAngle = reversing ? this.car.angle + Math.PI : this.car.angle;
    ...
    isTouchPointOnGameUI(x, y) {
      ...
      const restartButton = this.getGameRestartButton();
      if (this.isPointInsideRect(x, y, restartButton)) {
        return true;
      }
      const hudToggleButton = this.getHUDToggleButton();
    ...
    onCanvasTouchStart(event) {
      ...
      if (!this.touchJoystick.active && this.canUseTouchJoystick() && !this.isTouchPointOnGameUI(point.x, point.y)) {
        this.startTouchJoystick(touch.identifier, point.x, point.y);
      }
      ...
      if (this.touchJoystick.active) {
        for (const touch of event.touches) {
          if (touch.identifier === this.touchJoystick.touchId) {
            const point = this.getCanvasPointFromClient(touch.clientX, touch.clientY);
            this.updateTouchJoystick(point.x, point.y);
            break;
          }
        }
      }
    ...
    isBlackoutGameplayMode(mode = this.gameMode) {
      return mode === "blackout" || mode === "blackout_speedrun";
    }
    ...
    recordWorldRecord(index, time, carVariant = this.playerCarVariant, carColor = this.playerCarColor, records = this.worldRecords) {
      ...
      this.savePersistentProgress();
      const scope = this.getOnlineLeaderboardScopeFromLevelRecordArray(records);
      if (scope) {
    ...
    finishCurrentLevel() {
      ...
      if (this.isSpeedrunMode()) {
        ...
        if (isBlackoutSpeedrun) {
          ...
          const newNormalSpeedrunRecord = previousSpeedrunWorldRecord == null || this.totalTimer < previousSpeedrunWorldRecord;
        ...
        const newNormalWorldRecord = previousNormalWorldRecord == null || this.levelTimer < previousNormalWorldRecord;
      ...
      for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
          ...
          for (const sample of samples) {
            const sampleStrength = this.getPointRevealStrength(
    ...
    renderLeaderboardScreen(ctx, now) {
      ...
      const panelsTop = Math.round(this.height * 0.22);
      ...
      renderColumn(
        ...
        this.blackoutWorldRecords
      );

      const speedrunRecords = [
        {
          title: "Normal",
          record: this.normalizeWorldRecordEntry(this.speedrunWorldRecord),
          timeColor: "#cfe0ee",
        ...
        {
          title: "Perfect",
          record: this.normalizeWorldRecordEntry(this.perfectSpeedrunWorldRecord),
          timeColor: "#ffd76a",
        ...
        {
          title: "Blackout",
          record: this.normalizeWorldRecordEntry(this.blackoutSpeedrunWorldRecord),
          timeColor: "#b3bed0",
      ...
      speedrunRecords.forEach((entry, index) => {
        ...
        renderPreview(entry.record, speedrunSectionW - 34, speedrunSectionH * 0.64, 0.78, "rgba(120, 132, 143, 0.5)");
        ctx.restore();
      });
      ctx.restore();
      ...
      if (environmentAlpha > 0.001) {
        ...
        for (let y = 0; y < this.level.height; y += 1) {
          for (let x = 0; x < this.level.width; x += 1) {
            ...
            const visibility = isBlackoutMode ? this.getBlackoutTileVisibility(blackoutVisibility, x, y) : 1;
            if (visibility <= 0.015) {
              continue;
            }
    ...
    renderTouchJoystickMarker(ctx, camera, state = null) {
      const marker = this.getTouchJoystickTargetMarker();
      if (!marker) {
        return;
      }
    ...
    renderTouchJoystick(ctx) {
      if (!this.touchJoystick.active || this.currentScreen !== "game" || this.gameMode === "tag" || this.homeConfirmOpen || this.completed || this.goalTriggered || this.fireSequenceTimer > 0 || this.exploding) {
        return;
      }

      const baseX = this.touchJoystick.baseX;
      const baseY = this.touchJoystick.baseY;
      const knobX = this.touchJoystick.knobX;
      const knobY = this.touchJoystick.knobY;
      const basis = this.getTouchJoystickDriveBasis();
      const forwardAngle = Math.atan2(basis.forwardY, basis.forwardX);
      const reverseAngle = forwardAngle + Math.PI;
      const reverseEnterHalfAngle = Math.acos(clamp(-basis.reverseEnterThreshold, 0, 1));
      const reverseHoldHalfAngle = Math.acos(clamp(-basis.reverseHoldThreshold, 0, 1));
      const reverseHalfAngle = Math.acos(clamp(-basis.reverseThreshold, 0, 1));
      ...
      outerGlow.addColorStop(0, "rgba(185, 223, 255, 0.24)");
      outerGlow.addColorStop(0.55, "rgba(112, 155, 194, 0.12)");
      ...
      ctx.fillStyle = "rgba(255, 128, 128, 0.1)";
      ctx.beginPath();
      ctx.arc(0, 0, outerRadius, Math.PI - reverseHoldHalfAngle, Math.PI + reverseHoldHalfAngle);
      ctx.arc(0, 0, innerRadius, Math.PI + reverseHoldHalfAngle, Math.PI - reverseHoldHalfAngle, true);
      ctx.closePath();
      ctx.fill();

      ctx.fillStyle = "rgba(120, 220, 164, 0.18)";
      ctx.beginPath();
      ctx.arc(0, 0, outerRadius, reverseHalfAngle, Math.PI * 2 - reverseHalfAngle);
      ctx.arc(0, 0, innerRadius, Math.PI * 2 - reverseHalfAngle, reverseHalfAngle, true);
      ctx.closePath();
      ctx.fill();

      ctx.fillStyle = "rgba(255, 128, 128, 0.22)";
      ctx.beginPath();
      ctx.arc(0, 0, outerRadius, Math.PI - reverseEnterHalfAngle, Math.PI + reverseEnterHalfAngle);
      ctx.arc(0, 0, innerRadius, Math.PI + reverseEnterHalfAngle, Math.PI - reverseEnterHalfAngle, true);
      ctx.closePath();
      ctx.fill();
      ctx.restore();

      ctx.strokeStyle = "rgba(194, 228, 255, 0.28)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(baseX, baseY, outerRadius, 0, Math.PI * 2);
      ctx.stroke();

      ctx.strokeStyle = "rgba(255, 145, 145, 0.26)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(baseX, baseY, outerRadius - 1, reverseAngle - reverseHalfAngle, reverseAngle + reverseHalfAngle);
      ctx.stroke();

      ctx.strokeStyle = "rgba(255, 145, 145, 0.12)";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.arc(baseX, baseY, outerRadius - 8, reverseAngle - reverseHoldHalfAngle, reverseAngle + reverseHoldHalfAngle);
      ctx.stroke();

      ctx.strokeStyle = "rgba(132, 226, 173, 0.24)";
      ctx.beginPath();
      ctx.arc(baseX, baseY, outerRadius - 1, forwardAngle + reverseHalfAngle, forwardAngle - reverseHalfAngle + Math.PI * 2);
      ctx.stroke();

      ctx.strokeStyle = "rgba(194, 228, 255, 0.12)";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ...
      ctx.fillStyle = "rgba(207, 235, 255, 0.28)";
      ctx.strokeStyle = "rgba(232, 246, 255, 0.44)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ...
      ctx.strokeStyle = "rgba(232, 246, 255, 0.26)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(baseX, baseY);
      ctx.lineTo(baseX + basis.forwardX * 26, baseY + basis.forwardY * 26);
      ctx.stroke();
      ctx.restore();
    }
    ...
    renderCampaignCompletionOverlay(ctx) {
      ...
      ctx.font = "18px Consolas, monospace";
      ctx.fillText(summary.speedrun ? (summary.blackoutSpeedrun ? "Blackout full run summary" : "Full run summary") : this.level.name, 0, -58);
